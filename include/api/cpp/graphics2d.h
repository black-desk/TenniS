...
#define __INNER_PRODUCT_Am_Bn(m, n)                                            \
  (A.data(m, 0) * B.data(0, n) + A.data(m, 1) * B.data(1, n)                   \
   + A.data(m, 2) * B.data(2, n))
    return Trans2D<T>(
...
#define __INNER_PRODUCT_Mn_p(n)                                                \
  (M.data(n, 0) * p.data(0) + M.data(n, 1) * p.data(1)                         \
   + M.data(n, 2) * p.data(2))
    return Vec3D<T>(
...
#undef __INNER_PRODUCT_Mn_p
  ...
  {
    /**
     * \brief get inverse transformation matrix, perform p(x', y') = M dot p(x,
     * y) \tparam T dtype \param M origin matrix \return the inverse matrix
     */
    template <typename T>
    Trans2D<T> inverse(const Trans2D<T> &M) {
      ...
      return Trans2D<T>(
        -M [4] * recip_t3t1_t0t4,
        M [1] * recip_t3t1_t0t4,
        -(M [1] * M [5] - M [4] * M [2]) * recip_t3t1_t0t4,
        M [3] * recip_t3t1_t0t4,
        -M [0] * recip_t3t1_t0t4,
        -(M [3] * M [2] - M [0] * M [5]) * recip_t3t1_t0t4,
        T(0),
        T(0),
        T(1));
    }
