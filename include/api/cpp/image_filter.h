  ...
  namespace api
  {
    ...
    class ImageFilter {
      public:
        ...
        ImageFilter(const ts_Device *device) :
          self(ts_new_ImageFilter(device)) {
          TS_API_AUTO_CHECK(m_impl != nullptr);
        }

        void clear() { TS_API_AUTO_CHECK(ts_ImageFilter_clear(m_impl.get())); }
        ...
        void sub_mean(const std::vector<float> &mean) {
          TS_API_AUTO_CHECK(ts_ImageFilter_sub_mean(
            m_impl.get(), mean.data(), int(mean.size())));
        }

        void div_std(const std::vector<float> &std) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_div_std(m_impl.get(), std.data(), int(std.size())));
        }
        ...
        void center_crop(int width, int height) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_center_crop(m_impl.get(), width, height));
        }

        void center_crop(int width) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_center_crop(m_impl.get(), width, width));
        }

        void channel_swap(const std::vector<int> &shuffle) {
          TS_API_AUTO_CHECK(ts_ImageFilter_channel_swap(
            m_impl.get(), shuffle.data(), int(shuffle.size())));
        }
        ...
        void letterbox(int width, int height, float outer_value = 0) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_letterbox(m_impl.get(), width, height, outer_value));
        }
        ...
        void divide(int width, int height, float padding_value = 0) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_divided(m_impl.get(), width, height, padding_value));
        }

        void resize(int width, int height, ts_ResizeMethod method) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_resize_v2(m_impl.get(), width, height, method));
        }

        void resize(int width, ts_ResizeMethod method) {
          TS_API_AUTO_CHECK(
            ts_ImageFilter_resize_scalar_v2(m_impl.get(), width, method));
        }

        void letterbox(
          int width, int height, float outer_value, ts_ResizeMethod method) {
          TS_API_AUTO_CHECK(ts_ImageFilter_letterbox_v2(
            m_impl.get(), width, height, outer_value, method));
        }
        ...
        void letterbox(
          int width, int height, float outer_value, ResizeMethod method) {
        ...
        Tensor run(const Tensor &tensor) { return run(tensor.get_raw()); }
        ...
        void force_gray(const std::vector<float> &scale) {
          TS_API_AUTO_CHECK(ts_ImageFilter_force_gray_v2(
            m_impl.get(), scale.data(), int(scale.size())));
        }

        void force_bgr2gray() { force_gray({ 0.114f, 0.587f, 0.299f }); }

        void force_rgb2gray() { force_gray({ 0.299f, 0.587f, 0.114f }); }
      ...
      private:
        ...
        static shared_raw pack(raw *ptr) {
          return shared_raw(ptr, ts_free_ImageFilter);
        }
