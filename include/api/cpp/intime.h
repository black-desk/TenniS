  ...
  namespace api
  {
    namespace intime
    {
      ...
      inline Tensor transpose(
        const Tensor &x, const std::vector<int32_t> &permute) {
        auto y = ts_intime_transpose(
          x.get_raw(), permute.data(), int32_t(permute.size()));
      ...
      inline Tensor gather(
        const Tensor &x, const Tensor &indices, int32_t axis) {
      ...
      inline Tensor concat(const std::vector<Tensor> &x, int32_t dim) {
        ...
        for (auto &input: x) { inputs.emplace_back(input.get_raw()); }
      ...
      inline Tensor pad(
        const Tensor &x, const Tensor &padding, float padding_value = 0) {
      ...
      inline Tensor pad(
        ...
        auto padding_tensor = tensor::build(
          INT32, { int(padding.size()), 2 }, &padding [0].first);
      ...
      inline Tensor resize2d(
        const Tensor &x,
        const Tensor &size,
        ResizeMethod  method = ResizeMethod::BILINEAR) {
        auto y = ts_intime_resize2d(
          x.get_raw(), size.get_raw(), int32_t(method));
        TS_API_AUTO_CHECK(y != nullptr);
        return Tensor::NewRef(y);
      }
      ...
      inline Tensor affine_on_sample2d(
        ...
        auto y = ts_intime_affine_on_sample2d(
          x.get_raw(), size.get_raw(), affine.get_raw(), dim, int32_t(method));
        TS_API_AUTO_CHECK(y != nullptr);
        return Tensor::NewRef(y);
      }
      ...
      inline Tensor matmul(
        const Tensor &A, const Tensor &B, bool transpose = false) {
