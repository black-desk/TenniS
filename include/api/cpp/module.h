  ...
  namespace api
  {
    ...
    class Module {
      public:
        ...
        static Module Load(
          const std::string &path, SerializationFormat format = TS_BINARY) {
          Module loaded(
            ts_Module_Load(path.c_str(), ts_SerializationFormat(format)));
          TS_API_AUTO_CHECK(loaded.m_impl != nullptr);
          return std::move(loaded);
        }

        static Module Load(
          StreamReader &stream, SerializationFormat format = TS_BINARY) {
          Module loaded(ts_Module_LoadFromStream(
            &stream, StreamReader::C, ts_SerializationFormat(format)));
          TS_API_AUTO_CHECK(loaded.m_impl != nullptr);
          return std::move(loaded);
        }

        static Module Fusion(
          const Module &in,
          int32_t       in_out_slot,
          const Module &out,
          int32_t       out_in_slot) {
          Module fusion(ts_Module_Fusion(
            in.get_raw(), in_out_slot, out.get_raw(), out_in_slot));
      ...
      private:
        ...
        static shared_raw pack(raw *ptr) {
          return shared_raw(ptr, ts_free_Module);
        }
