  ...
  namespace api
  {
    ...
    class NewAndFree {
      public:
        ...
        static void Free(const void *obj) {
          delete reinterpret_cast<const T *>(obj);
        }
    };
    ...
    class OperatorParams {
      public:
        ...
        Tensor get(const std::string &param) const {
          return get(param.c_str());
        }
      private:
        const ts_OperatorParams *m_params;
    };

    inline std::vector<Tensor> borrowed_tensors(
      int32_t argc, ts_Tensor **argv) {
    ...
    class Operator {
      public:
        ...
        virtual void init(
          const OperatorParams &params, ts_OperatorContext *context) = 0;

        virtual std::vector<std::pair<DTYPE, Shape>> infer(
          const std::vector<Tensor> &args, ts_OperatorContext *context) = 0;

        virtual std::vector<Tensor> run(
          const std::vector<Tensor> &args, ts_OperatorContext *context) = 0;
        ...
        static ts_Tensor *Infer(
          ...
          for (auto &pair: proto) {
            ...
            for (auto dim: pair.second) { data.emplace_back(dim); }
          }
          auto count         = int32_t(data.size());
          auto packed_tensor = ts_new_Tensor(&count, 1, TS_INT32, data.data());
          // TS_API_AUTO_CHECK(packed_tensor != nullptr); // check in tennis
          // side
          return packed_tensor;
        }

        static ts_Tensor *Run(
          void               *op,
          int32_t             argc,
          ts_Tensor         **argv,
          ts_OperatorContext *context) {
          ...
          auto packed_raw = ts_Tensor_pack(
            cfields.data(), int32_t(cfields.size()));
          // TS_API_AUTO_CHECK(packed_raw != nullptr);    // check in tennis
          // side
          return packed_raw;
        }
    };
