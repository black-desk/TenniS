...
namespace ts
{
  namespace api
  {
    ...
    class Tensor {
      public:
        ...
        Tensor(DTYPE dtype, const Shape &shape, const void *data = nullptr) :
          self(ts_new_Tensor(
            shape.data(), int32_t(shape.size()), ts_DTYPE(dtype), data)) {
        ...
        int size(int i) const {
          if (i < 0 || i >= ts_Tensor_shape_size(m_impl.get()))
            throw Exception("index out of range");
          return ts_Tensor_shape(m_impl.get()) [i];
        }

        int size(size_t i) const {
          if (i >= size_t(ts_Tensor_shape_size(m_impl.get())))
            throw Exception("index out of range");
          return ts_Tensor_shape(m_impl.get()) [i];
        }

        int dims() const { return ts_Tensor_shape_size(m_impl.get()); }

        DTYPE dtype() const { return DTYPE(ts_Tensor_dtype(m_impl.get())); }

        const void *data() const { return ts_Tensor_data(m_impl.get()); }

        void *data() { return ts_Tensor_data(m_impl.get()); }
        ...
        void sync_cpu() { TS_API_AUTO_CHECK(ts_Tensor_sync_cpu(m_impl.get())); }

        Tensor view(InFlow in_flow) const { return view(ts_InFlow(in_flow)); }
        ...
        int32_t count() const {
          ...
          return std::accumulate(
            shape, shape + shape_len, 1, std::multiplies<int32_t>());
        }

        Tensor reshape(const Shape &shape) const {
          auto casted_raw = ts_Tensor_reshape(
            m_impl.get(), shape.data(), int32_t(shape.size()));
          TS_API_AUTO_CHECK(casted_raw != nullptr);
          return Tensor(casted_raw);
        }
        ...
        bool packed() const { return bool(ts_Tensor_packed(m_impl.get())); }

        std::vector<Tensor> unpack() const {
          auto                count = fields_count();
          std::vector<Tensor> fields;
          for (int i = 0; i < count; ++i) { fields.emplace_back(field(i)); }
        ...
        static Tensor Pack(const std::vector<Tensor> &fields) {
          ...
          for (auto &field: fields) { cfields.emplace_back(field.get_raw()); }
      ...
      private:
        ...
        static shared_raw pack(raw *ptr) {
          return shared_raw(ptr, ts_free_Tensor);
        }
    ...
    class tensor_builder {
      public:
        static Tensor build(const T &value) { return build(&value, 1); }
        ...
        static Tensor build(
          const std::vector<T> &value, const std::vector<int> &shape) {
          return build(value.data(), value.size(), shape);
        }

        static Tensor build(
          const T *data, size_t count, const std::vector<int> &shape) {
          auto shape_count = std::accumulate(
            shape.begin(), shape.end(), 1, std::multiplies<int32_t>());
          if (int32_t(count) != shape_count)
            throw Exception("Shape count mismatch.");
    ...
    namespace tensor
    {
      ...
      inline Tensor from(const char (&value) [_size]) {
      ...
      template <typename T>
      inline Tensor from(const T value) {
        return tensor_builder<T>::build(value);
      }

      template <typename T>
      inline Tensor from(const std::initializer_list<T> &value) {
        return tensor_builder<T>::build(value);
      }

      template <typename T>
      inline Tensor from(const std::vector<T> &value) {
        return tensor_builder<T>::build(value);
      }
      ...
      inline int to_int(const Tensor &value) {
        ...
        cpu_value.sync_cpu();
        if (cpu_value.dtype() != TS_INT32)
          cpu_value = cast(TS_INT32, cpu_value);
      ...
      inline unsigned int to_uint(const Tensor &value) {
        auto cpu_value = value;
        cpu_value.sync_cpu();
        if (cpu_value.dtype() != TS_UINT32)
          cpu_value = cast(TS_UINT32, cpu_value);
      ...
      inline float to_float(const Tensor &value) {
        auto cpu_value = value;
        cpu_value.sync_cpu();
        if (cpu_value.dtype() != TS_FLOAT32)
          cpu_value = cast(TS_FLOAT32, cpu_value);
      ...
      inline double to_double(const Tensor &value) {
        auto cpu_value = value;
        cpu_value.sync_cpu();
        if (cpu_value.dtype() != TS_FLOAT64)
          cpu_value = cast(TS_FLOAT64, cpu_value);
      ...
      template <typename T>
      inline Tensor build(
        DTYPE dtype, const Shape &shape, const std::vector<T> &value) {
        return cast(dtype, tensor_builder<T>::build(value, shape));
      }
      ...
      inline Tensor build(DTYPE dtype, const Shape &shape, const T *data) {
        ...
        return cast(
          dtype, tensor_builder<T>::build(data, size_t(count), shape));
      }
