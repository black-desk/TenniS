    ...
    class Workbench {
      public:
        ...
        void input(const std::string &name, const ts_Tensor *tensor) {
          TS_API_AUTO_CHECK(
            ts_Workbench_input_by_name(m_impl.get(), name.c_str(), tensor));
        }
        ...
        void run() { TS_API_AUTO_CHECK(ts_Workbench_run(m_impl.get())); }
        ...
        void output(const std::string &name, ts_Tensor *tensor) {
          TS_API_AUTO_CHECK(
            ts_Workbench_output_by_name(m_impl.get(), name.c_str(), tensor));
        }
        ...
        void set_computing_thread_number(int number) {
          TS_API_AUTO_CHECK(
            ts_Workbench_set_computing_thread_number(m_impl.get(), number));
        }

        void bind_filter(int slot, const ts_ImageFilter *filter) {
          TS_API_AUTO_CHECK(
            ts_Workbench_bind_filter(m_impl.get(), slot, filter));
        }
        ...
        void bind_filter(
          const std::string &name, const ts_ImageFilter *filter) {
          TS_API_AUTO_CHECK(ts_Workbench_bind_filter_by_name(
            m_impl.get(), name.c_str(), filter));
        }
        ...
        void setup(const Program &program) {
          TS_API_AUTO_CHECK(
            ts_Workbench_setup(m_impl.get(), program.get_raw()));
        }
        ...
        Program compile(const ts_Module *module) {
          auto program = Program::NewRef(
            ts_Workbench_compile(m_impl.get(), module));
        ...
        void run_hook(const std::vector<std::string> &node_names) {
          ...
          TS_API_AUTO_CHECK(ts_Workbench_run_hook(
            m_impl.get(), c_node_names.data(), int32_t(c_node_names.size())));
        }
        ...
        static Workbench Load(
          ...
          Workbench loaded(ts_Workbench_Load_v2(
        ...
        Program compile(const Module &module, const std::string &options) {
          auto program = Program::NewRef(ts_Workbench_compile_v2(
            m_impl.get(), module.get_raw(), options.c_str()));
          TS_API_AUTO_CHECK(program != nullptr);
          return std::move(program);
        }
      ...
      private:
        ...
        static shared_raw pack(raw *ptr) {
          return shared_raw(ptr, ts_free_Workbench);
        }
