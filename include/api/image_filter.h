...
extern "C" {
   ...
   * @return false if failed
   */
  TENNIS_C_API ts_bool ts_ImageFilter_sub_mean(
    ts_ImageFilter *filter, const float *mean, int32_t len);
   ...
   * @return false if failed
   */
  TENNIS_C_API ts_bool
  ts_ImageFilter_div_std(ts_ImageFilter *filter, const float *std, int32_t len);
   ...
   */
  TENNIS_C_API ts_bool
  ts_ImageFilter_resize(ts_ImageFilter *filter, int32_t width, int32_t height);
   ...
   */
  TENNIS_C_API ts_bool
  ts_ImageFilter_resize_scalar(ts_ImageFilter *filter, int32_t width);
   ...
   * @return false if failed
   * @note if given image is smaller than [width, height], this filter will zero
   * pad image to [width, height]
   */
  TENNIS_C_API ts_bool ts_ImageFilter_center_crop(
    ts_ImageFilter *filter, int32_t width, int32_t height);
   ...
   * @note call ts_ImageFilter_channel_swap(filter, {2, 1, 0}, 3) means do
   * BGR2RGB method
   */
  TENNIS_C_API ts_bool ts_ImageFilter_channel_swap(
    ts_ImageFilter *filter, const int32_t *shuffle, int32_t len);
  ...
  /**
   * Add filter to stream: equal scale image' long edge to given [width,
   * height], fill outer image area with outer_value
   * @param filter the return value of ts_new_ImageFilter
   ...
   * @note using TS_RESIZE_BILINEAR by default
   */
  TENNIS_C_API ts_bool ts_ImageFilter_letterbox(
    ts_ImageFilter *filter, int32_t width, int32_t height, float outer_value);
   ...
   * @return false if failed
   * @note Ex: input [800, 600] image, divided [32, 32], will pad image to [800,
   * 608]
   */
  TENNIS_C_API ts_bool ts_ImageFilter_divided(
    ts_ImageFilter *filter, int32_t width, int32_t height, float padding_value);
   ...
   */
  TENNIS_C_API ts_Tensor *ts_ImageFilter_run(
    ts_ImageFilter *filter, const ts_Tensor *tensor);
   ...
   * 608]
   */
  TENNIS_C_API ts_bool ts_ImageFilter_letterbox_v2(
    ts_ImageFilter *filter,
    int32_t         width,
    int32_t         height,
    float           outer_value,
   ...
   * @return false if failed
   */
  TENNIS_C_API ts_bool ts_ImageFilter_resize_v2(
    ts_ImageFilter *filter,
    int32_t         width,
    int32_t         height,
    ts_ResizeMethod method);
   ...
   * @return false if failed
   */
  TENNIS_C_API ts_bool ts_ImageFilter_resize_scalar_v2(
    ts_ImageFilter *filter, int32_t width, ts_ResizeMethod method);

  /**
   * Add filter to stream: force image to color mode, may copy each gary channel
   * to output channels
   * @param filter the return value of ts_new_ImageFilter
   * @return false if failed
   */
  ...
  /**
   * Add filter to stream: force image to gray mode, use [0.114, 0.587, 0.299]
   * converting BGR mode.
   * @return false if failed
   */
   ...
   * @return false if failed
   * @note if scale is NULL or len is 0, this function same as
   * ts_ImageFilter_force_gray
   * @note use [0.114, 0.587, 0.299] converting BGR mode.
   * @note use [0.299, 0.587, 0.114] converting RGB mode.
   */
  TENNIS_C_API ts_bool ts_ImageFilter_force_gray_v2(
    ts_ImageFilter *filter, const float *scale, int32_t len);
   ...
   */
  TENNIS_C_API ts_bool
  ts_ImageFilter_norm_image(ts_ImageFilter *filter, float epsilon);
