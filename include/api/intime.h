...
extern "C" {
   ...
   * @note call ts_Workbench_setup_context to fix Exception "Must bind Workbench
   * before run"
   * @note output tensor should on device setting in setup context ts_Workbench
   */
  TENNIS_C_API ts_Tensor *ts_intime_transpose(
   ...
   * @note output tensor should on device setting in setup context ts_Workbench
   */
  TENNIS_C_API ts_Tensor *ts_intime_gather(
   ...
   * @note output tensor should on device setting in setup context ts_Workbench
   */
  TENNIS_C_API ts_Tensor *ts_intime_concat(
   ...
   */
  TENNIS_C_API ts_Tensor *ts_intime_softmax(
    const ts_Tensor *x, int32_t dim, ts_bool smooth);
   ...
   */
  TENNIS_C_API ts_Tensor *ts_intime_pad(
   ...
   * before run"
   */
  TENNIS_C_API ts_Tensor *ts_intime_resize2d(
   ...
   * @param dst_ptr tensor data pointer, must match description. it would be
   * dst_desc.data() if pointer is nullptr
   * @param dst_shift memory will copy to dst_ptr + dst_shift
   * @param src_desc tensor description, tell API device information
   * @param src_ptr tensor data pointer, must match description. it would be
   * dst_desc.data() if pointer is nullptr
   ...
   * @note output tensor should on device setting in setup context ts_Workbench
   */
  TENNIS_C_API ts_Tensor *ts_intime_matmul(
