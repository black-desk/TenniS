...
extern "C" {
   ...
   * @param filename
   * @param format @sa ts_SerializationFormat, only support TS_BINARY in this
   * version.
   * @return New reference. Return NULL if failed.
   * @note call @see ts_free_Module to free ts_Module
   */
  TENNIS_C_API ts_Module *ts_Module_Load(
    const char *filename, ts_SerializationFormat format);
   ...
   * @note call @see ts_free_Module to free ts_Module
   */
  TENNIS_C_API ts_Module *ts_Module_LoadFromStream(
   ...
   * @return New reference. Return NULL if failed.
   * @note call @see ts_free_Module to free ts_Module
   * @note new module's input and output is sorted all `in` and `out` modules
   * left input and output nodes
   */
  TENNIS_C_API ts_Module *ts_Module_Fusion(
    const ts_Module *in,
    int32_t          in_out_slot,
    const ts_Module *out,
    int32_t          out_in_slot);
