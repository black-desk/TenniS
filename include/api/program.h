...
extern "C" {
   ...
   * @note Call ts_Workbench_setup_context before compile, or call
   * ts_Workbench_compile instead
   * @note call @see ts_free_Program to free ts_Program
   */
  TENNIS_C_API ts_Program *ts_Program_Compile(
    const ts_Module *module, const ts_Device *device);

  /**
   * Free program.
   * @param module the return value of ts_Program_Compile or
   * ts_Workbench_compile Happen nothing if failed.
   */
   ...
   */
  TENNIS_C_API ts_Program *ts_Program_Compile_v2(
   ...
   */
  TENNIS_C_API ts_bool ts_Program_set_operator_param(
    ts_Program      *program,
