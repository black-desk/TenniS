...
extern "C" {
   ...
   * @param data initialization data on cpu, must have same data type as dtype.
   * passing NULL for no initialization.
   * @return new reference, NULL if failed.
   * @note @sa ts_free_Tensor to free ts_Tensor
   */
  TENNIS_C_API ts_Tensor *ts_new_Tensor(
   ...
   */
  TENNIS_C_API ts_Tensor *ts_Tensor_reshape(
   ...
   * @return new reference, NULL if failed.
   * @note @sa ts_free_Tensor to free ts_Tensor
   * @note call ts_Workbench_setup_context to fix exception "Empty
   * context:<ts::Workbench>"
   */
  TENNIS_C_API ts_Tensor *ts_new_Tensor_in_flow(
    ts_InFlow      in_flow,
    const int32_t *shape,
    int32_t        shape_len,
    ts_DTYPE       dtype,
   ...
   * context:<ts::Workbench>"
   */
  TENNIS_C_API ts_Tensor *ts_Tensor_view_in_flow(
    ts_Tensor *tensor, ts_InFlow in_flow);
   ...
   */
  TENNIS_C_API ts_Tensor *ts_Tensor_slice_v2(
    ts_Tensor *tensor, int32_t beg, int32_t end);
  ...
  ts_Tensor_save(const char *path, const ts_Tensor *tensor);
