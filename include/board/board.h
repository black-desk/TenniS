...
namespace ts
{
  ...
  class TS_DEBUG_API Statistics {
    ...
    public:
      void append(const Datum datum) { this->m_data.push_back(datum); }
      ...
      Datum sum() const {
        return std::accumulate(
          this->m_data.begin(), this->m_data.end(), Datum(0));
      }

      size_t count() const { return this->m_data.size(); }
  ...
  class TS_DEBUG_API Board {
    ...
    private:
      ...
      using iterator =
        typename std::unordered_map<key_type, value_type>::iterator;
      using const_iterator =
        typename std::unordered_map<key_type, value_type>::const_iterator;
    public:
      void append(const key_type &name, const Datum datum) {
        this->m_board [name].append(datum);
      }

      void clear(const key_type &name) { this->query(name).clear(); }

      Datum max(const key_type &name) const { return this->query(name).max(); }

      Datum min(const key_type &name) const { return this->query(name).min(); }

      Datum avg(const key_type &name) const { return this->query(name).avg(); }

      Datum sum(const key_type &name) const { return this->query(name).sum(); }

      size_t count(const key_type &name) const {
        return this->query(name).count();
      }

      void clean() { this->m_board.clear(); }

      iterator begin() { return this->m_board.begin(); }

      iterator end() { return this->m_board.end(); }

      const_iterator begin() const { return this->m_board.begin(); }

      const_iterator end() const { return this->m_board.end(); }

      iterator find(const key_type &name) { return this->m_board.find(name); }
