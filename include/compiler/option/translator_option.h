...
namespace ts
{
  ...
  class TranslatorV2Option {
    public:
      ...
      virtual Module::shared translate(
        const ComputingDevice &device, Module::shared module) const = 0;
  };
...
#define TS_REGISTER_TRANSLATOR_OPTION(option)                                  \
  ...
  {                                                                            \
    ...
    TS_STATIC_ACTION(                                                          \
      ts::RegisterTranslateOption, &(ts_serial_name(_ts_translator_option)));  \
  }
...
#define TS_REGISTER_TRANSLATOR_V2_OPTION(option)                               \
  ...
  {                                                                            \
    static option ts_serial_name(_ts_translator_option);                       \
    TS_STATIC_ACTION(                                                          \
      ts::RegisterTranslateV2Option,                                           \
      &(ts_serial_name(_ts_translator_option)));                               \
  }
