...
namespace ts
{
  ...
  // This device and id may used in tensor view and sync, means do-not change
  // the memory device
  ...
  class TS_DEBUG_API Device {
    public:
      ...
      const std::string repr() const {
        return m_type.std() + ":" + std::to_string(m_id);
      }
    ...
    public:
      ...
      Device(self &&other) { *this = std::move(other); }
...
#undef MOVE_MEMBER
  ...
  class TS_DEBUG_API DeviceMismatchException : public Exception {
    public:
      ...
      explicit DeviceMismatchException(
        const Device &needed, const Device &given);

      static std::string DeviceMismatchMessage(
        const Device &needed, const Device &given);
    ...
    public:
      ...
      DeviceMismatchException(self &&other) { *this = std::move(other); }
...
#undef MOVE_MEMBER
  ...
  class TS_DEBUG_API MemoryDevice : public Device {
    public:
      ...
      MemoryDevice(self &&other) { *this = std::move(other); }
  ...
  class TS_DEBUG_API ComputingDevice : public Device {
    public:
      ...
      ComputingDevice(self &&other) { *this = std::move(other); }
...
namespace std
{
  ...
  struct hash<ts::Device> {
      std::size_t operator()(const ts::Device &key) const {
        ...
        return hash<ts::DeviceType>()(key.type()) ^ hash<int>()(key.id());
      }
  };
