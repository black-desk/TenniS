...
namespace ts
{
  ...
  template <>
  struct dtype<FLOAT32> {
      using declare = float;
  };
  template <>
  struct dtype<FLOAT64> {
  ...
  template <>
  struct dtype<CHAR16> {
      using declare = char16_t;
  };
  template <>
  struct dtype<CHAR32> {
      using declare = char32_t;
  };
  template <>
  struct dtype<BOOLEAN> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q0> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q1> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q2> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q3> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q4> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q5> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q6> {
      using declare = uint8_t;
  };
  template <>
  struct dtype<SINK8Q7> {
      using declare = uint8_t;
  };

  template <typename T>
  struct dtypeid {
      static const DTYPE id = VOID;
  };
  ...
  struct dtypeid<int8_t> {
      static const DTYPE id = INT8;
  };
  template <>
  struct dtypeid<uint8_t> {
      static const DTYPE id = UINT8;
  };
  template <>
  struct dtypeid<int16_t> {
      static const DTYPE id = INT16;
  };
  template <>
  struct dtypeid<uint16_t> {
      static const DTYPE id = UINT16;
  };
  template <>
  struct dtypeid<int32_t> {
      static const DTYPE id = INT32;
  };
  template <>
  struct dtypeid<uint32_t> {
      static const DTYPE id = UINT32;
  };
  template <>
  struct dtypeid<int64_t> {
      static const DTYPE id = INT64;
  };
  template <>
  struct dtypeid<uint64_t> {
      static const DTYPE id = UINT64;
  };
  template <>
  struct dtypeid<float> {
      static const DTYPE id = FLOAT32;
  };
  template <>
  struct dtypeid<double> {
      static const DTYPE id = FLOAT64;
  };
  template <>
  struct dtypeid<void *> {
      static const DTYPE id = PTR;
  };
  template <>
  struct dtypeid<char> {
      static const DTYPE id = CHAR8;
  };
}  // namespace ts
