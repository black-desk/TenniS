  ...
  class ieee754_float {
    public:
      ...
      template <
        ...
        size_t _T_EXPONENT,
        size_t _T_FRACTION>
      explicit ieee754_float(
        const ieee754_float<_T_W, _T_SIGN, _T_EXPONENT, _T_FRACTION> &other) {
        // for float 0
        if ((other.code() & ~other.mask_sign().code) == 0) {
          this->m_bits.code = typename bitset::type(
            (other.value_sign() << (W - 1)));
          return;
        }
        ...
        constexpr auto max_exponent = (decltype(origin_exponent)(1) << exponent)
                                    - 2;
        constexpr auto max_fraction = (decltype(part_fraction)(1) << fraction)
                                    - 1;
        ...
        auto part_exponent = (origin_exponent << fraction)
                           & this->mask_exponent().code;
      ...
      const typename bitset::type &code() const { return m_bits.code; }
...
#define DECLARE_OPERATOR(type)                                                 \
  operator type() const { return type(double(*this)); }
...
#undef DECLARE_OPERATOR
      ...
      friend self operator+(const self &lhs, const self &rhs) {
        return self(
          typename upper_float<_W>::type(lhs) +
          typename upper_float<_W>::type(rhs));
      }
    ...
    private:
      ...
      int64_t value_exponent() const {
        return int64_t(
                 (this->m_bits.code & this->mask_exponent().code) >> fraction)
             - int64_t(bias);
      }
      ...
      static constexpr bitset mask_exponent() TS_NOEXCEPT {
        return bitset(
          ((typename bitset::type(0x01) << exponent) - 1) << fraction);
      }
  ...
  template <>
  struct dtype<FLOAT16> {
      using declare = float16;
  };
  template <>
  struct dtypeid<float16> {
      static const DTYPE id = FLOAT16;
  };
}  // namespace ts
