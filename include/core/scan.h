...
namespace ts
{
  ...
  class IterationStep : public IterationBase<Iteration::STEP> {
    public:
      ...
      explicit IterationStep(times_t times, step_t step) :
        supper(times), step(step) {}
  ...
  class IterationIters : public IterationBase<Iteration::ITERS> {
    public:
      ...
      explicit IterationIters(
        times_t times, const std::vector<Iteration *> &iters) :
        supper(times), iters(iters) {}

      explicit IterationIters(times_t times, std::vector<Iteration *> &&iters) :
        supper(times), iters(std::move(iters)) {}

      std::vector<Iteration *> iters;  // those pointers should allocate by new
  };

  inline IterationStep *new_iteration_step() { return new IterationStep(); }

  inline IterationIters *new_iteration_iters() { return new IterationIters(); }
  ...
  inline Iteration *new_iteration(
    Iteration::times_t times, Iteration::step_t step) {
  ...
  inline Iteration *new_iteration(
    Iteration::times_t times, const std::vector<Iteration *> &iters) {
    return new IterationIters(times, iters);
  }

  inline Iteration *new_iteration(
    Iteration::times_t times, std::vector<Iteration *> &&iters) {
  ...
  inline Iteration *clone_iteration(const Iteration *iteration) {
    ...
    if (iteration->type == Iteration::ITERS) {
      ...
      std::unique_ptr<IterationIters, decltype(&delete_iteration)>
        iteration_clone(new_iteration_iters(), &delete_iteration);
    ...
    public:
      ...
      class Group {
        public:
          ...
          static void try_insert_group_rape(
            std::vector<Iteration *> &group, Arg0 &&arg0) {
            group.push_back(try_rape(std::forward<Arg0>(arg0)));
          };

          template <typename Arg0, typename... Args>
          static void try_insert_group_rape(
            std::vector<Iteration *> &group, Arg0 &&arg0, Args &&...args) {
            group.push_back(try_rape(std::forward<Arg0>(arg0)));
            try_insert_group_rape(group, std::forward<Args>(args)...);
          };

          template <typename... Args>
          static std::vector<Iteration *> try_return_group_rape(
            Args &&...args) {
          ...
          template <typename... Args>
          Group(Args &&...args) :
            m_proto_group(try_return_group_rape(std::forward<Args>(args)...)) {}
      ...
      IterationDescriptor(times_t times, step_t step) :
        m_proto(new_iteration(times, step)) {}

      IterationDescriptor(
        times_t times, const IterationDescriptor &descriptor) :
      ...
      IterationDescriptor(times_t times, const Group &group) :
        m_proto(new_iteration(times, try_rape(group))) {}

      IterationDescriptor(times_t times, Group &&group) :
        m_proto(new_iteration(times, try_rape(std::move(group)))) {}

      IterationDescriptor(const IterationDescriptor &descriptor) :
        m_proto(try_rape(descriptor)) {}

      IterationDescriptor(IterationDescriptor &&descriptor) :
        m_proto(try_rape(std::move(descriptor))) {}
    ...
    public:
      ...
      class Status {
        public:
          ...
          explicit Status(const Iteration *proto) :
            proto(proto), count(proto->times) {}
          ...
          Iteration::step_t step() const {
            return reinterpret_cast<const IterationStep *>(proto)->step;
          }

          const std::vector<Iteration *> iters() const {
            return reinterpret_cast<const IterationIters *>(proto)->iters;
          }
      };
