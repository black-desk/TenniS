...
#define TS_DEFINE_NUMBER_MAX(_T, value)                                        \
  template <>                                                                  \
  _T number<_T>::Max() {                                                       \
    return (value);                                                            \
  }
#define TS_DEFINE_NUMBER_MIN(_T, value)                                        \
  template <>                                                                  \
  _T number<_T>::Min() {                                                       \
    return (value);                                                            \
  }
...
#define TS_DEFINE_FIXED_NUMBER_UNSIGNED_CONVERTER(_T)                          \
  ...
  class number_converter<_COMMON_T, _T> {                                      \
    public:                                                                    \
      ...
      static _FIXED_T ToFixed(_COMMON_T value, int _Q) {                       \
        return _FIXED_T(                                                       \
          number_converter<_COMMON_T, _SIGNED_FIXED_T>::ToFixed(value, _Q));   \
      }                                                                        \
      static _COMMON_T ToCommon(_FIXED_T value, int _Q) {                      \
        return number_converter<_COMMON_T, _SIGNED_FIXED_T>::ToCommon(         \
          _SIGNED_FIXED_T(value), _Q);                                         \
      }                                                                        \
  };
...
#define TS_DECLARE_CONSTRUCTOR(_COMMON_T)                                      \
  fixed_point_number(_COMMON_T value) :                                        \
    supper(_T(number_converter<decltype(value), S>::ToFixed(value, _Q))) {}
...
#define TS_DECLARE_OPERATOR(_COMMON_T)                                         \
  operator _COMMON_T() const {                                                 \
    return number_converter<_COMMON_T, S>::ToCommon(S(this->value), _Q);       \
  }
...
#undef TS_DECLARE_OPERATOR
  ...
  sink<_T, _Q> operator-(const sink<_T, _Q> &lhs, const sink<_T, _Q> &rhs) {
    ...
    return sink<_T, _Q>::from_fixed(
      _T(fixed > max ? max : (fixed < min ? min : fixed)));
#endif
  }
  ...
  sink<_T, _Q> operator/(const sink<_T, _Q> &lhs, const sink<_T, _Q> &rhs) {
    using _S   = typename _signed<_T>::T;
    using _DS  = typename _double_width<_S>::T;
    auto fixed = rhs.value == 0 ?
                   sink<_T, _Q>::Max().value :
                   (((_DS)lhs.value) << ((_Q) + (_Q) - (_Q))) / rhs.value;
#ifndef TS_SINK_CHECKING_OVERFLOW
    return sink<_T, _Q>::from_fixed(_T(fixed));
#else
    auto max = top_number(number<_S>::Max());
  ...
  template <typename _T, int _Q>
  bool operator<(const sink<_T, _Q> &lhs, const sink<_T, _Q> &rhs) {
