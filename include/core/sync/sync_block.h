...
namespace ts
{
  ...
  class TS_DEBUG_API SyncBlock {
    public:
      ...
      using sync_handler = std::function<_VALUE(
        const _VALUE &from_value, const _KEY &from_key, const _KEY &to_key)>;
    ...
    private:
      void set(const _KEY &key, const _VALUE &value) {
        ...
        if (key == m_default_key) {
          ...
          auto pair_it_flag = m_param->m_sync_values.insert(
          ...
          auto pair_it_flag  = m_param->m_sync_values.insert(
            std::make_pair(default_key, default_value));
          m_default_value = &(pair_it_flag.first->second);
        }
      }
      ...
      void clear() {
        ...
        m_param->m_sync_values.clear();
        auto pair_it_flag = m_param->m_sync_values.insert(
          std::make_pair(m_default_key, kept_value));
        m_default_value = &(pair_it_flag.first->second);
      }
      ...
      void clear(const _KEY &key) {
        auto _write = this->lock_write();
        if (key == m_default_key)
          TS_LOG_ERROR << "Can not clear default key=" << key << eject;
    ...
    public:
      _VALUE &sync(const _KEY &key) {
        {
          ...
          if (key == m_default_key) return *m_default_value;
          auto it = m_param->m_sync_values.find(key);
          if (it != m_param->m_sync_values.end()) { return it->second; }
        }
        {
          auto _write = this->lock_write();
      ...
      const _KEY &key() const { return m_default_key; }
      ...
      void foreach(
        const std::function<void(const _KEY &key, const _VALUE &value)>
          &handler) const {
        auto _read = this->lock_read();
    ...
    private:
      ...
      _VALUE &sync_insert(const _KEY &key) {
        ...
        auto   pair_it = m_param->m_sync_values.insert(
          std::make_pair(key, value));
      ...
      std::unique_ptr<unique_read_lock> lock_read() const {
        if (m_mutex) {
          return std::unique_ptr<unique_read_lock>(
            new unique_read_lock(*m_mutex));
        } else {
      ...
      std::unique_ptr<unique_write_lock> lock_write() const {
        if (m_mutex) {
          return std::unique_ptr<unique_write_lock>(
            new unique_write_lock(*m_mutex));
        } else {
      ...
      class Param {
        public:
          ...
          Param(const sync_handler &handler) : m_hanlder(handler) {}
    ...
    public:
      SyncBlock(self &&other) { *this = std::move(other); }
