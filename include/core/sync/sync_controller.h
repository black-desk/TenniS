...
namespace ts
{
  ...
  class TS_DEBUG_API SyncDeviceMemoryController : public SyncMemoryController {
    public:
      ...
      SyncDeviceMemoryController(const MemoryDevice &device) :
        m_device(device) {}
  ...
  class TS_DEBUG_API HypeSyncMemoryController :
    ...
    public std::enable_shared_from_this<
      HypeSyncMemoryController<_MemoryController>> {
    public:
    ...
    private:
      HypeSyncMemoryController(
        const MemoryDevice &device, bool need_lock = false) :
        SyncDeviceMemoryController(device),
        m_sync_controllers(
          device,
          std::make_shared<BaseMemoryController>(device),
          sync_controller_handler,
          need_lock),
    ...
    public:
      ...
      SyncMemory::Block::sync_handler sync_handler() {
        ...
        return [=](
                 const typename SyncMemory::Block::value_t &from_memory,
        ...
        m_sync_controllers.foreach(
          [&](
            ...
            if (comma) oss << ", ";
              ...
              comma = true;
            oss << "\"" << device << "\": \""
                << memory_size_string(controller->summary()) << "\"";
          });
        oss << "}";
        return oss.str();
      }
    private:
      using SyncControllerBlock =
        SyncBlock<MemoryDevice, std::shared_ptr<BaseMemoryController>>;
  ...
  using DynamicSyncMemoryController =
    HypeSyncMemoryController<DynamicMemoryController>;
}  // namespace ts
