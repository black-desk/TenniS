...
namespace ts
{
  class TS_DEBUG_API SyncMemory {
    public:
      ...
      static Block::value_t dynamic_sync_handler(
        const Block::value_t &from_memory,
      ...
      SyncMemory(const Memory &memory, bool lock, Block::sync_handler handler) {
        m_sync_memory = std::make_shared<Block>(
          memory.device(), memory, handler, lock);
      }
      ...
      SyncMemory(bool lock = false) : SyncMemory(0, lock) {}
       ...
       */
      SyncMemory(
        const HardMemory::shared &hard, size_t size, size_t shift = 0) :
        SyncMemory(Memory(hard, size, shift)) {}

      SyncMemory(
        const HardMemory::shared &hard, size_t size, size_t shift, bool lock) :
      ...
      SyncMemory(
        HardMemory::shared &&hard, size_t size, size_t shift, bool lock) :
       ...
       * @param shift shift from start pointer
       */
      SyncMemory(const HardMemory::shared &hard) : SyncMemory(Memory(hard)) {}
       ...
       * @param other other object
       */
      SyncMemory(self &&other) TS_NOEXCEPT { *this = std::move(other); }
...
#undef MOVE_MEMBER
       ...
       */
      Memory weak_memory() const { return m_sync_memory->value().weak(); }
      ...
      void broadcast() { m_sync_memory->broadcast(); }
    private:
      SyncMemory(std::shared_ptr<Block> sync_memory) :
        m_sync_memory(std::move(sync_memory)) {}
