    ...
    public:
      class TS_DEBUG_API Prototype {
        public:
          ...
          Prototype(DTYPE dtype, const Shape &sizes) :
            m_dtype(dtype), m_sizes(sizes) {}

          Prototype(DTYPE dtype, Shape &&sizes) :
            m_dtype(dtype), m_sizes(sizes) {}
        ...
        public:
          ...
          Prototype(self &&other) { *this = std::move(other); }
...
#undef MOVE_MEMBER
      ...
      Tensor(
        MemoryController::shared controller,
        DTYPE                    dtype,
      ...
      Tensor(
        MemoryController::shared controller,
        const Prototype         &proto);  // allocate memory from controller
       ...
       * @param proto
       * Notice: must have context:RuntimeContext, optional have
       * context:DeviceContext RuntimeContext give flow memory controller
      ...
      Tensor(InFlow in_flow, DTYPE dtype, const Shape &shape) :
        self(in_flow, Prototype(dtype, shape)) {}

      /**
       *
       * @param in_flow HOST or DEVICE given in DeviceContext
       * @param proto
       * @param device if in_flow=InFlow::HOST, the device will discard, or
       * device means using this device Notice: must have
       * context:RuntimeContext, optional have context:DeviceContext
       * RuntimeContext give flow memory controller
       * DeviceContext must given with in_flow=InFlow::DEVICE,
       * if in_flow=InFlow::HOST, use CPU:0 memory device.
       */
      Tensor(
        InFlow in_flow, const Prototype &proto, const MemoryDevice &device);
      ...
      Tensor(
        ...
        self(in_flow, Prototype(dtype, shape), device) {}
  ...
  class TS_DEBUG_API TensorPrototype : public Tensor::Prototype {
    public:
      ...
      TensorPrototype(DTYPE dtype, Shape &&sizes) :
        supper(dtype, std::move(sizes)) {}
      ...
      TensorPrototype &operator=(const supper &other) {
        supper::operator=(other);
        return *this;
      };

      TensorPrototype &operator=(supper &&other) {
    ...
    public:
      ...
      TensorPrototype(self &&other) { *this = std::move(other); }
...
#undef MOVE_MEMBER
  ...
  TS_DEBUG_API std::ostream &operator<<(
    std::ostream &out, const Tensor::Prototype &proto);

  TS_DEBUG_API std::ostream &operator<<(
    std::ostream &out, const TensorPrototype &proto);

  TS_DEBUG_API bool operator==(
    const Tensor::Prototype &lhs, const Tensor::Prototype &rhs);

  TS_DEBUG_API bool operator==(
    const TensorPrototype &lhs, const TensorPrototype &rhs);

  TS_DEBUG_API bool operator==(
    const Tensor::Prototype &lhs, const TensorPrototype &rhs);

  TS_DEBUG_API bool operator==(
    const TensorPrototype &lhs, const Tensor::Prototype &rhs);
