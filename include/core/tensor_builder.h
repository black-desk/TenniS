...
namespace ts
{
  ...
  inline Tensor tensor_builder<bool>::build(const std::vector<bool> &value) {
    ...
    for (int i = 0; i < t.count(); ++i) { out [i] = type(value [i] ? 1 : 0); }
    return t;
  }
  ...
  inline Tensor tensor_builder<bool>::build(const bool *data, size_t count) {
    ...
    using type = ts::dtype<ts::BOOLEAN>::declare;
    auto out   = t.data<type>();
    for (int i = 0; i < t.count(); ++i) { out [i] = type(data [i] ? 1 : 0); }
    return t;
  }
  ...
  namespace tensor
  {
    ...
    template <typename T>
    Tensor from(const T value) {
      return tensor_builder<T>::build(value);
    }

    template <typename T>
    Tensor from(const std::initializer_list<T> &value) {
      return tensor_builder<T>::build(value);
    }

    template <typename T>
    Tensor from(const std::vector<T> &value) {
      return tensor_builder<T>::build(value);
    }

    template <typename T, size_t N, typename S>
    Tensor from(const otl::vector<T, N, S> &value) {
      return tensor_builder<T>::build(value);
    }
    ...
    template <typename T>
    inline Tensor build(
      DTYPE dtype, const Shape &shape, const std::initializer_list<T> &value) {
