...
namespace ts
{
  ...
  std::function<void(const T *)> default_deleter() {
    return [](const T *object) { delete object; };
  }
  ...
  class TS_DEBUG_API Counter {
    public:
      ...
      Counter(Object *object, int count) : object(object), use_count(count) {}
      ...
      Counter(self &&other) { *this == std::move(other); }
...
#undef MOVE_MEMBER
  ...
  class TS_DEBUG_API Smart {
    public:
      ...
      Smart(const self &other) { *this = other; }
      ...
      Smart &operator=(const self &other) {
        ...
        if (m_counted && m_mode == SMART) { m_counted->use_count++; }
        return *this;
      }

      ~Smart() { dispose(); }

      Smart(self &&other) { this->swap(other); }
      ...
      self weak() const { return self(MANUALLY, m_counted); }
      ...
      operator bool() const { return m_counted != nullptr; }
    private:
      Smart(Object *object, SmartMode mode = SMART) :
        m_mode(mode),
        m_counted(new CountedObject(object, mode == SMART ? 1 : 0)) {}
