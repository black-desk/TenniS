...
namespace ts
{
  namespace frontend
  {
    ...
    TS_DEBUG_API Node pad(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &padding,
      float               padding_value = 0);

    TS_DEBUG_API Node pad(
      const std::string             &name,
      const NodeOrTensor            &x,
      const std::vector<DimPadding> &padding,
      float                          padding_value = 0);

    TS_DEBUG_API Node resize2d(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &size,
    ...
    TS_DEBUG_API Node add(
      const std::string  &name,
      const NodeOrTensor &lhs,
      const NodeOrTensor &rhs);

    TS_DEBUG_API Node sub(
      const std::string  &name,
      const NodeOrTensor &lhs,
      const NodeOrTensor &rhs);

    TS_DEBUG_API Node mul(
      const std::string  &name,
      const NodeOrTensor &lhs,
      const NodeOrTensor &rhs);

    TS_DEBUG_API Node div(
      const std::string  &name,
      const NodeOrTensor &lhs,
      const NodeOrTensor &rhs);

    TS_DEBUG_API Node transpose(
      const std::string          &name,
      const NodeOrTensor         &x,
    ...
    TS_DEBUG_API Node gather(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &indices,
      int32_t             axis);

    TS_DEBUG_API Node concat(
      const std::string &name, const std::vector<NodeOrTensor> &x, int32_t dim);

    TS_DEBUG_API Node softmax(
      const std::string  &name,
      const NodeOrTensor &x,
      int32_t             dim,
      bool                smooth = true);
    ...
    TS_DEBUG_API Node
    cast(const std::string &name, const NodeOrTensor &x, DTYPE dtype);

    TS_DEBUG_API Node affine_sample2d(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &size,
    ...
    TS_DEBUG_API Node affine_on_sample2d(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &size,
      const NodeOrTensor &affine,
    ...
    TS_DEBUG_API Node matmul(
      const std::string  &name,
      const NodeOrTensor &A,
      const NodeOrTensor &B,
      bool                transpose = false);

    TS_DEBUG_API Node broadcast(
      const std::string  &name,
      const NodeOrTensor &x,
      const NodeOrTensor &shape);
