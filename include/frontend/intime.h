...
namespace ts
{
  ...
  {
    /**
     * @note: the returned tensor is using borrowed memory from bench, use clone
     * or free, before bench finalization
     */
    TS_DEBUG_API Tensor run(
      Workbench                 &bench,
      const Bubble              &bubble,
     ...
     */
    TS_DEBUG_API Tensor
    run(const Bubble &bubble, const std::vector<Tensor> &inputs);

    TS_DEBUG_API Tensor resize2d(
      const Tensor    &x,
      const Tensor    &size,
    ...
    TS_DEBUG_API Tensor
    transpose(const Tensor &x, const std::vector<int32_t> &permute);

    TS_DEBUG_API Tensor sigmoid(const Tensor &x);

    TS_DEBUG_API Tensor
    gather(const Tensor &x, const Tensor &indices, int32_t axis);

    TS_DEBUG_API Tensor
    gather(const Tensor &x, const std::vector<int32_t> &indices, int32_t axis);

    TS_DEBUG_API Tensor concat(const std::vector<Tensor> &x, int32_t dim);

    TS_DEBUG_API Tensor
    softmax(const Tensor &x, int32_t dim, bool smooth = true);

    TS_DEBUG_API Tensor
    pad(const Tensor &x, const Tensor &padding, float padding_value = 0);

    TS_DEBUG_API Tensor cast(const Tensor &x, DTYPE dtype);

    TS_DEBUG_API Tensor affine_sample2d(
      const Tensor    &x,
      const Tensor    &size,
      const Tensor    &affine,
      int32_t          dim         = -1,
    ...
    TS_DEBUG_API Tensor affine_on_sample2d(
      const Tensor    &x,
      const Tensor    &size,
      const Tensor    &affine,
      int32_t          dim  = -1,
    ...
    TS_DEBUG_API int64_t memcpy(
      ...
      void         *dst_data,
      int64_t       dst_shift,
      const Tensor &src_desc,
      const void   *src_data,
      int64_t       src_shift,
      int64_t       size);

    TS_DEBUG_API Tensor
    matmul(const Tensor &A, const Tensor &B, bool transpose = false);

    TS_DEBUG_API Tensor broadcast(const Tensor &x, const Tensor &shape);

    TS_DEBUG_API Tensor
    broadcast(const Tensor &x, const std::vector<int32_t> &shape);
  }  // namespace intime
}  // namespace ts
