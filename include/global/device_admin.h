...
namespace ts
{
  ...
  class TS_DEBUG_API DeviceAdmin {
    public:
       ...
       * @param handle Pointer to the pointer of handle, ready to be initialized
       * of finalized, which controlled by `action`
       * @param device_id admin device id
       * @param action @sa DeviceAction
       */
      void DeviceAdminFunction(
        DeviceHandle **handle, int device_id, Action action);
      ...
      static void Register(
        ...
        const function   &device_admin) TS_NOEXCEPT;
