...
namespace ts
{
  class TS_DEBUG_API OutOfMemoryException : public Exception {
    public:
      explicit OutOfMemoryException(
        const MemoryDevice &device, size_t failed_size) :
        Exception(OutOfMemoryMessage(device, failed_size)),
        m_device(device),
        m_failed_size(failed_size) {}

      static std::string OutOfMemoryMessage(
        const MemoryDevice &device, size_t failed_size) {
        std::ostringstream oss;
        oss << "No enough memory on " << device << ", " << failed_size
            << "B needed.";
        return oss.str();
      }
      ...
      size_t failed_size() const { return m_failed_size; }
    private:
  ...
  class TS_DEBUG_API HardAllocator {
    public:
       ...
       * @return allocator
       * @note supporting called by threads without calling @sa
       * RegisterDeviceAllocator or @sa RegisterAllocator
       * @note the query device should be memory device, you may call @sa
       * QueryMemoryDevice to get memory device by compute device
       */
      ...
      static void Register(
        const DeviceType &device_type, const function &allocator) TS_NOEXCEPT;
      ...
      static void RegisterV2(
        ...
        const _malloc    &_new,
        const _free      &_delete) TS_NOEXCEPT;
       ...
       * @note only can be called before running
       */
      static void RegisterV3(
        const DeviceType &device_type,
        const _malloc    &_new,
        const _free      &_delete,
        const _realloc   &_reset) TS_NOEXCEPT;

      static function Bind(
        const _malloc &_new, const _free &_delete, const _realloc &_reset);
