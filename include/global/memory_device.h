...
namespace ts
{
  class TS_DEBUG_API NoMemoryDeviceException : public Exception {
    public:
      explicit NoMemoryDeviceException(const DeviceType &device_type) :
        Exception(NoMemoryDeviceMessage(device_type)),
        m_no_memory_device_type(device_type) {}

      static std::string NoMemoryDeviceMessage(const DeviceType &device_type) {
        std::ostringstream oss;
        oss << "Compute device " << device_type
            << " has no memory device registered. Please call "
               "RegisterMemoryDevice firstly.";
        return oss.str();
      }
  ...
  class TS_DEBUG_API ComputingMemory {
    public:
       ...
       * @param compute_device_type the compute device, such as CPU, GPU,
       * CUBLAS, CUDNN, etc.
       * @param memory_device_type the memory device, such as CPU and GPU.
       */
      static void Register(
        const DeviceType &compute_device_type,
       ...
       */
      static std::set<std::pair<std::string, std::string>> AllItems()
        TS_NOEXCEPT;
  };
}  // namespace ts
