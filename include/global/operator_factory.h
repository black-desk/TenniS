...
namespace ts
{
  ...
  class TS_DEBUG_API OperatorNotFoundException : public Exception {
    public:
      explicit OperatorNotFoundException(
        const DeviceType &device_type, const std::string &operator_name) :
        Exception(OperatorNotFoundMessage(device_type, operator_name)),
        m_device_type(device_type),
        m_operator_name(operator_name) {}

      static std::string OperatorNotFoundMessage(
        const DeviceType &device_type, const std::string &operator_name) {
        std::ostringstream oss;
        oss << "No operator \"" << operator_name << "\" registered on device \""
            << device_type << "\".";
        return oss.str();
      }

      const DeviceType &device_type() const { return m_device_type; }

      const std::string &operator_name() const { return m_operator_name; }
    private:
  ...
  class TS_DEBUG_API OperatorCreator {
    public:
      ...
      using CreatorFucMap =
        std::map<std::pair<DeviceType, std::string>, OperatorCreator::function>;
      ...
      static Operator::shared CreateNoException(
        const DeviceType  &device_type,
        const std::string &operator_name,
       ...
       */
      static std::set<std::pair<std::string, std::string>> AllKeys()
        TS_NOEXCEPT;
  };
}  // namespace ts
...
#define TS_REGISTER_OPERATOR(CLASS_NAME, DEVICE_TYPE, OP_NAME)                 \
  namespace                                                                    \
  {                                                                            \
    static ts::Operator::shared _ts_concat_name(CLASS_NAME, _CREATOR)() {      \
      return std::make_shared<CLASS_NAME>();                                   \
    }                                                                          \
    ts::StaticAction ts_serial_name(_ts_static_action_)(                       \
      ts::OperatorCreator::Register,                                           \
      DEVICE_TYPE,                                                             \
      OP_NAME,                                                                 \
      _ts_concat_name(CLASS_NAME, _CREATOR));                                  \
  }
