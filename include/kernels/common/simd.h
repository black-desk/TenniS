...
namespace ts
{
  ...
  inline T sum(const simd<T, 4> &value, int index) {
    ...
    value.store(a);
    T sum = 0;
    for (int i = 0; i < index && i < 4; i++) { sum += a [i]; }
    return sum;
  }
  ...
  class simd<float, 4> : public simd_base<float, 4> {
    public:
      ...
      simd(base a, base b, base c, base d) :
        value(_simd_f32x4_set(a, b, c, d)) {}

      void store(base *p) const { _simd_f32x4_store(p, value); }
  };

  inline simd<float, 4> operator+(
    const simd<float, 4> &lhs, const simd<float, 4> &rhs) {
  ...
  inline simd<float, 4> max_float32x4(
    const simd<float, 4> &lhs, const simd<float, 4> &rhs) {
    return _simd_f32x4_max(lhs.value, rhs.value);
  }

  inline simd<float, 4> min_float32x4(
    const simd<float, 4> &lhs, const simd<float, 4> &rhs) {
    return _simd_f32x4_min(lhs.value, rhs.value);
  }

  inline void transposex4x4(
    simd<float, 4> &q0,
    simd<float, 4> &q1,
    simd<float, 4> &q2,
    simd<float, 4> &q3) {
  ...
  inline simd<float, 4> fmadd(
    const simd<float, 4> &q0,
    const simd<float, 4> &q1,
    const simd<float, 4> &q2) {
  ...
  // Note:The index value specifies the lowest vector element to extract from
  // the first source register, and consecutive elements are extracted from the
  // first, then second, source registers until the destination vector is
  // filled.
  inline simd<float, 4> concat(
    const simd<float, 4> &q0, const simd<float, 4> &q1, const int index) {
    if (index >= 0 && index <= 3)
  ...
  class simd<float, 8> : public simd_base<float, 8> {
    public:
      ...
      simd(base a, base b, base c, base d, base e, base f, base g, base h) :
        value(_simd_f32x4x2_set(a, b, c, d, e, f, g, h)) {}
  ...
  inline simd<float, 8> operator+(
    const simd<float, 8> &lhs, const simd<float, 8> &rhs) {
  ...
  inline simd<float, 8> fmadd(
    const simd<float, 8> &q0,
    const simd<float, 8> &q1,
    const simd<float, 8> &q2) {
  ...
  class simd<float, 12> : public simd_base<float, 12> {
    public:
      ...
      simd(const float32x4 &q0, const float32x4 &q1, const float32x4 &q2) :
        value(_simd_concat(q0.value, q1.value, q2.value)) {}
      ...
      void store(base *p, int index) {
        if (index >= 0 && index <= 2) _simd_f32x4x3_store(p, value, index);
      }
  };
  ...
  class simd<int32_t, 4> : public simd_base<int32_t, 4> {
    public:
      ...
      simd(base a, base b, base c, base d) :
        value(_simd_int32x4_set(a, b, c, d)) {}

      void store(base *p) const { _simd_int32x4_store(p, value); }
  };

  inline simd<int32_t, 4> operator+(
    const simd<int32_t, 4> &lhs, const simd<int32_t, 4> &rhs) {
  ...
  inline simd<int32_t, 8> operator+(
    const simd<int32_t, 8> &lhs, const simd<int32_t, 8> &rhs) {
