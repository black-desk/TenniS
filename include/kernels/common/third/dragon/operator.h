...
namespace ts
{
  namespace dragon
  {
    ...
    class OperatorBase {
      public:
        explicit OperatorBase(const OperatorDef &def, Workspace *ws) :
          m_def(def), m_ws(ws) {
          m_type = m_def->op();
        }
        ...
        void bind_outputs(const std::vector<ts::Tensor> &outputs) {
          m_outputs = std::vector<dragon::Tensor>(
        ...
        size_t InputSize() const { return m_inputs.size(); }

        size_t OutputSize() const { return m_outputs.size(); }

        void bind_outputs(int N) { m_outputs.resize(N); }
        ...
        std::string DTypeHelper(
          const Tensor &tensor, const std::set<std::string> &dtypes) const {
          std::stringstream ss;
        ...
        std::string DTypeHelper(
          const std::string &dtype, const std::set<std::string> &dtypes) const {
          std::stringstream ss;
        ...
        void clean() {
          m_inputs.clear();
          m_outputs.clear();
          for (auto &tmp: m_temp) { tmp->dispose(); }
        }
      private:
        OperatorDef         m_def;
        Workspace          *m_ws;
