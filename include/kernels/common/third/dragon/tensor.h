...
namespace ts
{
  namespace dragon
  {
    ...
    inline std::vector<T> cast_tensor(const std::vector<F> &vec) {
      ...
      std::transform(vec.begin(), vec.end(), to_vec.begin(), [](const F &a) {
        return T(a);
      });
      ...
      public:
        ...
        self *Reshape(DTYPE dtype, const std::vector<int64_t> &shape) {
          ...
          auto count    = std::accumulate(
            ts_shape.begin(), ts_shape.end(), 1, std::multiplies<int>());
        ...
        template <typename T>
        bool IsType() const {
          return m_tst.dtype() == dtypeid<T>::id;
        }
    ...
    class TensorObject {
      public:
        ...
        TensorObject(const ts::Tensor &tensor) : m_tst(tensor) {
        ...
        void *mutable_data_ptr() {
          if (TypeMeta::Id<Context>() == TypeMeta::Id<CPUContext>()) {
            ...
            return mutable_view(
              ctx::of<Workbench>::ref().device().memory_device);
          }
          m_tst.broadcast();
          return m_tst.data();
        }
        ...
        void newdata(
          ...
          m_tst = ts::Tensor(
            flow, dtype, cast_tensor<int32_t>(m_shape), MemoryDevice(CPU));
        }
        ...
        void newdata(DTYPE dtype) const { newdata(dtype, MemoryDevice(CPU)); }
    ...
    class Tensor {
      public:
        ...
        Tensor(const ts::Tensor &tst) :
          m_obj(std::make_shared<TensorObject>(tst)) {}
        ...
        template <typename T>
        bool IsType() const {
          return m_obj->IsType<T>();
        }

        int64_t count() const { return m_obj->count(); }
        ...
        void dispose() { m_obj = std::make_shared<TensorObject>(); }
      private:
