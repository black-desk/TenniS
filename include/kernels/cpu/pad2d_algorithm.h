  ...
  namespace cpu
  {
    ...
    class TS_DEBUG_API PadAlgorithm {
      public:
        ...
        static void pad_nchw_nhwc(
          const Tensor                          &x,
          const std::vector<std::array<int, 2>> &padding,
          float                                  padding_value,
          Tensor                                &out);

        // NOTE:pad2d_Superseded and cut2d_Superseded only support NCHW pad on
        // [H,W] now. pad2d don't support negative pad,cut2d support negative
        // pad only.
        static void pad2d(
          const Tensor             &x,
