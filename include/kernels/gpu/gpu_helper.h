...
namespace ts
{
  namespace gpu
  {
    ...
    struct GpuHypeShape {
        ...
        int32_t *weights = nullptr;  // each weight, [weight_i]_{i=0}^{dims},
                                     // and weight_i = \sum_{j=i}^{dims} dim_i
    };

    // std::pair<SyncMemory, GpuHypeShape> MakeGPUHypeShape(const MemoryDevice
    // &device, const Shape &shape);
