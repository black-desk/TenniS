      ...
      class TS_DEBUG_API math {
        public:
          static void check(const Tensor &tensor) {
            if (tensor.device().type() != CUBLAS)
              throw DeviceMismatchException(Device(CUBLAS), tensor.device());
          }

          static bool dot(
            cublasHandle_t hadnle, const int N, const T *x, const T *y, T *z);
          ...
          static bool asum(cublasHandle_t hadnle, int N, const T *x, T *out);
      };
    }  // namespace cublas
  }    // namespace gpu
}  // namespace ts
