      ...
      public:
        static void check(const Tensor &tensor) {
          if (tensor.device().type() != GPU)
            throw DeviceMismatchException(Device(GPU), tensor.device());
        }

        // static bool dot(const int N, const T *x, int incx, const T *y, int
        // incy, T * z);
        ...
        static bool asum(int N, const T *x, T *out);

        static bool sum(int N, const T *x, T *out);
    };
  }  // namespace gpu
}  // namespace ts
