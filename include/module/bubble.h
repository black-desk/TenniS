...
namespace ts
{
  class TS_DEBUG_API Bubble : public Serializable {
    public:
      ...
      explicit Bubble(const std::string &op);

      explicit Bubble(const std::string &op, const std::string &name);

      explicit Bubble(const std::string &op, const Shape &shape);

      explicit Bubble(
        const std::string &op, const std::string &name, const Shape &shape);

      explicit Bubble(const std::string &op, int output_count);

      explicit Bubble(
        const std::string &op, const std::string &name, int output_count);
      ...
      int get_int(const std::string &param) const {
        return tensor::to_int(get(param));
      }

      unsigned int get_uint(const std::string &param) const {
        return tensor::to_uint(get(param));
      }

      float get_float(const std::string &param) const {
        return tensor::to_float(get(param));
      }

      double get_double(const std::string &param) const {
        return tensor::to_double(get(param));
      }

      std::string get_string(const std::string &param) const {
        return tensor::to_string(get(param));
      }

      bool get_bool(const std::string &param) const {
        return tensor::to_bool(get(param));
      }
    ...
    public:
      ...
      static const char * const Const;  // Mean never change in graph, including
                                        // weights, saving in static memory
      static const char
        * const Variable;  // Mean may change in graph, saving some state, do no
                           // use in this version
