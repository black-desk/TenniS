...
namespace ts
{
   ...
   *  Notice: The c is pack(c:1, c:2) node, and the unpack method's first
   * parameter must be pack node
  ...
  class TS_DEBUG_API Node {
    public:
      ...
      //     if (value_ptr == nullptr) throw NullPointerException("Getting
      //     reference from null pointer"); return *value_ptr;
      // }
      //
      // template<typename T>
      ...
      static void Link(const Node &node, const std::vector<Node> &inputs) {
        ...
        for (auto &input: inputs)
          raw_inputs.emplace_back(LinkedBubble::weak(input));
      ...
      Bubble &bubble() {
        auto raw_ptr = m_ptr.lock();
        if (! raw_ptr)
          throw NullPointerException("Getting expired node's bubble");
      ...
      Bubble *operator->() { return &bubble(); }

      const Bubble *operator->() const { return &bubble(); }
  ...
  // template<typename T>
  // T *Node::ptr() {
  //     TS_LOG_ERROR << "Using not recommended API, please use \"Node::bubble\"
  //     instead."; auto raw_ptr = m_ptr.lock(); if (!raw_ptr) return nullptr;
  //     return raw_ptr->ptr();
  // }
  ...
  class TS_DEBUG_API Graph : public SetupContext<Graph> {
    public:
      ...
      // Node make(Args &&...args) {
      //     TS_LOG_ERROR << "Using not recommended API, please use
      //     \"Graph::make\" instead."; auto node =
      //     std::make_shared<LinkedBubble>(std::forward<Args>(args)...);
  ...
  TS_DEBUG_API std::ostream &plot_graph(
