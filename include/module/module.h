...
namespace ts
{
  class TS_DEBUG_API Module {
    public:
      ...
      static Module::shared Load(
        StreamReader &stream, SerializationFormat format = BINARY);
      static Module::shared Load(
        const std::string &filename, SerializationFormat format = BINARY);
      ...
      static std::vector<std::pair<Node, int>> list_reference_nodes(
      ...
      class Route {
        public:
          Route(
            int32_t in, int32_t in_out_slot, int32_t out, int32_t out_in_slot) :
      ...
      static shared Fusion(
        const std::vector<shared> &submodules,
    ...
    private:
       ...
       */
      static std::vector<Node> graph_walker(
