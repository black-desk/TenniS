  ...
  namespace v2
  {
    ...
    class Node {
      public:
        ...
        Node() : m_raw(std::make_shared<LinkedBubble>()) {}
        ...
        explicit Node(LinkedBubble *raw) :
          m_raw(raw, [](const LinkedBubble *) {}) {}

        void link(const std::vector<Node> &inputs) {
          LinkedBubble::vector raw_inputs;
          raw_inputs.reserve(inputs.size());
          for (auto &i: inputs) { raw_inputs.emplace_back(i.m_raw); }
        ...
        std::string str() const { return m_raw->str(); }

        std::string repr() const { return m_raw->repr(); }

        Bubble &bubble() { return m_raw->value(); }

        const Bubble &bubble() const { return m_raw->value(); }
