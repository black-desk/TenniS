...
namespace ts
{
  TS_DEBUG_API TensorPrototype
  infer(Node &node, std::unordered_map<Node, TensorPrototype> &cache);

  TS_DEBUG_API std::vector<TensorPrototype> infer(
    std::vector<Node> &nodes, std::unordered_map<Node, TensorPrototype> &cache);
