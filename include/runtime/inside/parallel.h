...
namespace ts
{
  ...
  inline void parallel_run(
    const std::function<void(int, int, int)> &range_solver,
    int                                       begin,
    int                                       end,
    bool                                      joinable = true) {
    auto parallel_gun = ts::try_parallel(end - begin);
    if (parallel_gun) {
      auto parallel_ranges = ts::split_bins(
        begin, end, int(parallel_gun->size()));
      ...
      if (joinable) { parallel_gun->join(); }
    } else {
      range_solver(0, begin, end);
    }
  }

  inline void parallel_range(
    const std::function<void(int, const Range &)> &range_solver,
    int                                            begin,
    int                                            end,
    bool                                           joinable = true) {
    auto parallel_gun = ts::try_parallel(end - begin);
 ...
 * ```
 * , but in parallel
 * @note remeber use TS_PARALLEL_SYNC after every parallel task should sync of
 * finish
 */
#  define TS_PARALLEL_FOR_BEGIN(                                               \
    var_loop_value, var_loop_begin, var_loop_end, ...)                         \
    {                                                                          \
      int  __ts_parallel_begin = int(var_loop_begin);                          \
      int  __ts_parallel_end   = int(var_loop_end);                            \
      auto __ts_parallel_solver =                                              \
        [&, ##__VA_ARGS__](                                                    \
          const int __parallel_id, int begin, int end) -> void { \
        int var_loop_value = begin; \
        for (; var_loop_value < end; ++var_loop_value) {
/**
 * @note TS_PARALLEL_FOR_END can parse an bool value, mean the parallel tasks if
 * is joinable
...
#  define TS_PARALLEL_FOR_END()                                                \
    ...
    ;                                                                          \
    ts::parallel_run(                                                          \
      __ts_parallel_solver, __ts_parallel_begin, __ts_parallel_end);           \
    }
 ...
 * finish
 */
#  define TS_PARALLEL_RANGE_BEGIN(                                             \
    var_range_value, var_range_begin, var_range_end, ...)                      \
    {                                                                          \
      int  __ts_parallel_begin = int(var_range_begin);                         \
      int  __ts_parallel_end   = int(var_range_end);                           \
      auto __ts_parallel_solver =                                              \
        [&, ##__VA_ARGS__](                                                    \
          const int        __parallel_id,                                      \
          const ts::Range &__ts_parallel_range) -> void { \
 ...
 * @see TS_PARALLEL_FOR_BEGIN
 */
#  define TS_PARALLEL_SYNC ts::parallel_sync();

#  define TS_PARALLEL_SIZE ts::parallel_size()
