...
namespace ts
{
  class TS_DEBUG_API Thread {
    public:
      ...
      /**
       * @brief run Asynchronous build and run task, first calls the task, then
       * calls the after_task.
      ...
      void run(
        ...
        const after_task_type &after_task = nullptr);
    ...
    private:
      ...
      std::condition_variable
                        task_cond;   ///< condition to tell if fire finished
      ...
      after_task_type after_task =
        nullptr;  ///< side function call after `bullet` called
  ...
  class TS_DEBUG_API ThreadPool : public SetupContext<ThreadPool> {
    ...
    private:
      ...
      std::condition_variable
        running_core_cond;           ///< active when cartridge pushed in chest
