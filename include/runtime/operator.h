...
namespace ts
{
  ...
  class TS_DEBUG_API Operator {
    public:
       ...
       * @note operator's output count must be 1, this function get each field
       * proto
       */
      virtual int infer(
        Stack &stack, std::vector<Tensor::Prototype> &output) = 0;
      ...
      void field(
        const std::string &param, FieldAttr attr, const Tensor &default_value);
   ...
   * @note all input argument would be pop after running
   */
  TS_DEBUG_API int InferOperator(
    Operator::shared                op,
    Stack                          &stack,
    int                             nargs,
