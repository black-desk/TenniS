  ...
  class TS_DEBUG_API Stack {
    public:
      ...
      explicit Stack(
        ...
        const SyncMemoryController::shared &controller);
      ...
      Tensor *clone(int i, const MemoryDevice &device) {
        return this->clone_push(*this->index(i), device);
      }
       ...
       * @return stack[i]
       * @note if i >= 0, then i is bottom_up_index; else if i < 0, the i is
       * top_down_index
       */
      Tensor *index(int i);
      ...
      std::deque<Tensor>::iterator begin() { return m_stack.begin() + m_base; }

      std::deque<Tensor>::const_iterator end() const { return m_stack.end(); }

      std::deque<Tensor>::iterator end() { return m_stack.end(); }
    private:
      ...
      mutable HardConverter::function m_converter =
        nullptr;  ///< convert memory in stack
  };
}  // namespace ts
