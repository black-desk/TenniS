...
namespace ts
{
   ...
   */
  TS_DEBUG_API int edit_distance(
   ...
   */
  TS_DEBUG_API std::vector<std::pair<int, int>> split_bins(
    int first, int second, int bins);
   ...
   * @note Example input(0, 10, 3) returns [(0, 4), (4, 8), (8, 10)]
   */
  TS_DEBUG_API std::vector<std::pair<size_t, size_t>> lsplit_bins(
    size_t first, size_t second, size_t bins);

  TS_DEBUG_API std::string to_string(
   ...
   * @see strftime
   */
  TS_DEBUG_API std::string now_time(
    const std::string &format = "%Y-%m-%d %H:%M:%S");

  TS_DEBUG_API std::vector<std::string> Split(
    const std::string &str, const std::string &sep, size_t _size = 1);

  TS_DEBUG_API std::string Join(
