...
namespace ts
{
  class TS_DEBUG_API NoLiteContextException : public Exception {
    public:
      ...
      explicit NoLiteContextException(
        const std::string &name, const std::thread::id &id);
    private:
    ...
    namespace lite
    {
      ...
      class TS_DEBUG_API bind {
        public:
          ...
          explicit bind(T *ctx) : m_pre_ctx(of<T>::get()) { of<T>::set(ctx); }

          explicit bind(T &ctx_ref) : bind(&ctx_ref) {}

          ~bind() { of<T>::set(m_pre_ctx); }
  ...
  class TS_DEBUG_API SetupContext {
    public:
      ...
      void setup_context() { ctx::of<T>::set(static_cast<T *>(this)); }
  };
