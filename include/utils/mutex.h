...
namespace ts
{
  class TS_DEBUG_API rwmutex {
    public:
      ...
      void lock_read() {
        ...
        m_cond_read.wait(
          _locker, [this]() -> bool { return m_write_count == 0; });
        ++m_read_count;
      }
      ...
      void lock_write() {
        std::unique_lock<std::mutex> _locker(m_mutex_counter);
        ++m_write_count;
        m_cond_write.wait(_locker, [this]() -> bool {
  ...
  class TS_DEBUG_API unique_write_lock {
    public:
      ...
      ~unique_write_lock() {
        if (m_ptr_rw_lockable) m_ptr_rw_lockable->release_write();
      }
  ...
  class TS_DEBUG_API unique_read_lock {
    public:
      ...
      ~unique_read_lock() {
        if (m_ptr_rw_lockable) m_ptr_rw_lockable->release_read();
      }
