...
namespace ts
{
  namespace otl
  {
    ...
    namespace sso
    {
      class OutOfRangeException : Exception {
        public:
          ...
          static std::string Message(size_t N, const std::string &std_string) {
            ...
            oss << "Can not convert \"" << std_string << "\" ("
                << std_string.length() << ") to "
                << "otl::sso::string<" << N << ">";
            return oss.str();
          }
      };
      ...
      class string {
        ...
        public:
          ...
          string(const std::string &std_string) : self() {
            if (std_string.length() > CAPACITY) {
              TS_LOG_ERROR << OutOfRangeException::Message(N, std_string)
                           << eject;
            }
            std::strcpy(m_buf, std_string.c_str());
          }

          string(const char *c_str) : self() {
          ...
          string(std::nullptr_t) : self() {}

          const char *c_str() const { return m_buf; }

          size_t length() const { return std::strlen(m_buf); }
    ...
    class VectorOutOfRangeException : Exception {
      public:
        VectorOutOfRangeException(size_t N, int i) : Exception(Message(N, i)) {}

        static std::string Message(size_t N, int i) {
          std::ostringstream oss;
          oss << "Index " << i << " out of range of "
              << "otl::vector<" << N << ">";
          return oss.str();
        }
    };
    ...
    class vector {
      ...
      public:
        ...
        // fill
        explicit vector(size_type n) : m_size(S(n)) {}

        vector(size_type n, const value_type &val) : m_size(S(n)) {
          for (auto &slot: *this) slot = val;
        }

        vector(int n, const value_type &val) : m_size(S(n)) {
          for (auto &slot: *this) slot = val;
        }
        ...
        vector(std::initializer_list<value_type> il) {
          if (il.size() > CAPACITY) {
            TS_LOG_ERROR << VectorOutOfRangeException::Message(N, il.size())
                         << eject;
          }
          size_type n = 0;
...
#undef OTL_VECTOR_SUPPORT_INDEX
      ...
      private:
        ...
        void _erase(size_type i, size_type len) {
          ...
          std::memmove(
            m_buf + i, m_buf + i + len, sizeof(T) * (this->size() - i - len));
          m_size -= len;
        }

        void _insert(size_type i, const T &val) {
          if (i + 1 > this->capacity()) {
            TS_LOG_ERROR << VectorOutOfRangeException::Message(N, int(N))
                         << eject;
          }
          std::memmove(
            m_buf + i + 1, m_buf + i, sizeof(T) * (this->size() - i));
          m_buf [i] = val;
          m_size += 1;
        }
        ...
        void _insert(size_type i, const vector<T, _N, _S> &vec) {
          ...
          std::memmove(
            m_buf + i + vec.size(), m_buf + i, sizeof(T) * (this->size() - i));
    ...
    inline std::ostream &operator<<(
      std::ostream &out, const vector<T, N, S> &vec) {
    ...
    inline bool operator==(
      const vector<T, N1, S1> &lhs, const vector<T, N2, S2> &rhs) {
      return lhs.size() == rhs.size()
          && std::memcmp(lhs.data(), rhs.data(), sizeof(T) * lhs.size()) == 0;
    }
