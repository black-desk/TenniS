...
ts_ImageFilter *ts_new_ImageFilter(const ts_Device *device) {
  ...
  std::unique_ptr<ts_ImageFilter> image_filter(
    device ? new ts_ImageFilter(ComputingDevice(device->type, device->id)) :
             new ts_ImageFilter());
...
ts_bool ts_ImageFilter_sub_mean(
...
ts_bool ts_ImageFilter_div_std(
  ts_ImageFilter *filter, const float *std, int32_t len) {
  TRY_HEAD
  if (! filter) throw Exception("NullPointerException: @param: 1");
...
ts_bool ts_ImageFilter_center_crop(
  ts_ImageFilter *filter, int32_t width, int32_t height) {
  TRY_HEAD
  if (! filter) throw Exception("NullPointerException: @param: 1");
...
ts_bool ts_ImageFilter_channel_swap(
...
ts_Tensor *ts_ImageFilter_run(ts_ImageFilter *filter, const ts_Tensor *tensor) {
  ...
  std::unique_ptr<ts_Tensor> dolly(new ts_Tensor((*filter)->run(**tensor)));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}

ts_bool ts_ImageFilter_letterbox(
  ts_ImageFilter *filter, int32_t width, int32_t height, float outer_value) {
  TRY_HEAD
  if (! filter) throw Exception("NullPointerException: @param: 1");
  (*filter)->letterbox(
    width, height, outer_value, ImageFilter::ResizeMethod::BILINEAR);
  RETURN_OR_CATCH(ts_true, ts_false)
}

ts_bool ts_ImageFilter_divided(
...
ts_bool ts_ImageFilter_resize_v2(
...
ts_bool ts_ImageFilter_resize_scalar_v2(
...
ts_bool ts_ImageFilter_letterbox_v2(
  ts_ImageFilter *filter,
  int32_t         width,
  int32_t         height,
  float           outer_value,
  ...
  if (! filter) throw Exception("NullPointerException: @param: 1");
  (*filter)->letterbox(
    width, height, outer_value, ImageFilter::ResizeMethod(int32_t(method)));
  RETURN_OR_CATCH(ts_true, ts_false)
}
...
ts_bool ts_ImageFilter_force_gray_v2(
...
ts_Module *ts_ImageFilter_module(const ts_ImageFilter *filter) {
  TRY_HEAD
  if (! filter) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Module> module(new ts_Module((*filter)->module()));
