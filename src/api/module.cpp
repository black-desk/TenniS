...
ts_Module *ts_Module_Load(const char *filename, ts_SerializationFormat format) {
  ...
  std::unique_ptr<ts_Module> module(
    new ts_Module(Module::Load(filename, Module::SerializationFormat(format))));
...
ts_Module *ts_Module_LoadFromStream(
  void *obj, ts_stream_read *reader, ts_SerializationFormat format) {
  TRY_HEAD
...
ts_Module *ts_Module_Fusion(
  ...
  std::unique_ptr<ts_Module> module(new ts_Module(Module::Fusion(
    { (*in).pointer, (*out).pointer },
    { { 0, in_out_slot, 1, out_in_slot } })));
  RETURN_OR_CATCH(module.release(), nullptr)
}
