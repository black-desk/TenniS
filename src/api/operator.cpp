  ...
  public:
    ...
    APIPluginOperator(
      ...
      if (obj == nullptr) {
        ...
        TS_LOG_ERROR << "Call ts_new_Operator failed on " << device << " for "
                     << op << "."
                     << (message.empty() ? "" : "\nWith: " + message) << eject;
      }
    ...
    void init() override {
      ...
      if (! f_init_ex(obj, &params, &context)) {
        auto &message = api::GetLEM();
        TS_LOG_ERROR << "Call ts_Operator_init failed on " << device << " for "
                     << op << "."
                     << (message.empty() ? "" : "\nWith: " + message) << eject;
      }
    }
    ...
    std::shared_ptr<ts_Tensor> CallOperatorRun(Stack &stack) {
      ...
      ts_OperatorContext context;
      api::SetLEM("");
      std::shared_ptr<ts_Tensor> out(
        f_run(obj, int32_t(args.args.size()), args.args.data(), &context),
        ts_free_Tensor);
      if (out == nullptr) {
        auto &message = api::GetLEM();
        TS_LOG_ERROR << "Call ts_Operator_run failed (return null) on "
                     << device << " for " << op << "."
                     << (message.empty() ? "" : "\nWith: " + message) << eject;
      }
      return out;
    ...
    std::shared_ptr<ts_Tensor> CallOperatorInfer(Stack &stack) {
      ...
      api::SetLEM("");
      std::shared_ptr<ts_Tensor> out(
        f_infer(obj, int32_t(args.args.size()), args.args.data(), &context),
        ts_free_Tensor);
      if (out == nullptr) {
        auto &message = api::GetLEM();
        TS_LOG_ERROR << "Call ts_Operator_infer failed (return null) on "
                     << device << " for " << op << "."
                     << (message.empty() ? "" : "\nWith: " + message) << eject;
      }
      return out;
    ...
    int infer(Stack &stack, std::vector<Tensor::Prototype> &output) override {
      if (f_infer == nullptr) {
        ...
        try {
          ...
          TS_LOG_ERROR << "Call ts_Operator_infer failed (format error) on "
                       << device << " for " << op << "." << eject;
        }
      }
      return 1;
...
void ts_Operator_Register(
  ...
  TRY_HEAD
  if (
    f_new == nullptr || f_free == nullptr || f_init == nullptr
    || f_run == nullptr) {
  ...
  auto        creator = [=]() -> Operator::shared {
    return std::make_shared<APIPluginOperator>(
      cpp_device, cpp_op, f_new, f_free, f_init, f_infer, f_run);
  };
  OperatorCreator::Register(cpp_device, cpp_op, creator);
  TRY_TAIL
}

ts_Tensor *ts_OperatorParams_get(
  const ts_OperatorParams *dict, const char *param) {
  TRY_HEAD
...
void ts_Operator_ThrowV2(
  const char *message, const char *filename, int32_t line_number) {
  if (message == nullptr) {
    LogStream(LOG_ERROR) << "[" << filename << ":" << line_number
                         << "]: [TS API]: Unknown exception." << eject;
  } else {
    LogStream(LOG_ERROR) << "[" << filename << ":" << line_number
                         << "]: [TS API]: " << message << eject;
  }
}

void ts_Operator_RegisterEx(
  const char          *device,
  const char          *op,
  ts_new_Operator     *f_new,
  ts_free_Operator    *f_free,
  ...
  auto        creator = [=]() -> Operator::shared {
    return std::make_shared<APIPluginOperator>(
      cpp_device, cpp_op, f_new, f_free, nullptr, f_infer, f_run, f_init);
  };
  OperatorCreator::Register(cpp_device, cpp_op, creator);
  TRY_TAIL
}
