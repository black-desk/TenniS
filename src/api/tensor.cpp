...
ts_Tensor *ts_new_Tensor(
  const int32_t *shape, int32_t shape_len, ts_DTYPE dtype, const void *data) {
  TRY_HEAD
  ...
  switch (dtype) {
    default: TS_LOG_ERROR << "Not support dtype: " << dtype << eject; break;
#define DECLARE_TENSOR_BUILD(api_dtype, type)                                  \
  case api_dtype:                                                              \
    **tensor = tensor::build(                                                  \
      DTYPE(dtype),                                                            \
      Shape(shape, shape + shape_len),                                         \
...
#undef DECLARE_TENSOR_BUILD
    ...
    {
      Tensor tensor_string = tensor::build(
        INT8,
        Shape(shape, shape + shape_len),
...
ts_Tensor *ts_Tensor_cast(ts_Tensor *tensor, ts_DTYPE dtype) {
  ...
  std::unique_ptr<ts_Tensor> dolly(
    new ts_Tensor(tensor::cast(DTYPE(dtype), **tensor)));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}

ts_Tensor *ts_Tensor_reshape(
  ts_Tensor *tensor, const int32_t *shape, int32_t shape_len) {
  TRY_HEAD
  if (! tensor) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Tensor> dolly(
    new ts_Tensor((*tensor)->reshape(Shape(shape, shape + shape_len))));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}
...
ts_Tensor *ts_Tensor_view_in_flow(ts_Tensor *tensor, ts_InFlow in_flow) {
  ...
  if (! tensor) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Tensor> dolly(
    new ts_Tensor((*tensor)->view(Tensor::InFlow(in_flow))));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}

ts_Tensor *ts_new_Tensor_in_flow(
  ts_InFlow      in_flow,
  const int32_t *shape,
  int32_t        shape_len,
  ts_DTYPE       dtype,
  ...
  **tensor = Tensor(
    Tensor::InFlow(in_flow), DTYPE(dtype), Shape(shape, shape + shape_len));
...
ts_Tensor *ts_Tensor_pack(ts_Tensor **fields, int32_t count) {
  ...
  for (int i = 0; i < count; ++i) {
    if (! fields [i])
      throw Exception(
        "NullPointerException: @param: fields[" + std::to_string(i) + "]");
...
ts_Tensor *ts_Tensor_slice(ts_Tensor *tensor, int32_t i) {
  TRY_HEAD
  if (! tensor) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Tensor> dolly(new ts_Tensor((*tensor)->slice(i)));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}
...
ts_Tensor *ts_Tensor_slice_v2(ts_Tensor *tensor, int32_t beg, int32_t end) {
  TRY_HEAD
  if (! tensor) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Tensor> dolly(new ts_Tensor((*tensor)->slice(beg, end)));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}
...
ts_Tensor *ts_Tensor_load(const char *path) {
  TRY_HEAD
  if (! path) throw Exception("NullPointerException: @param: 1");
  std::unique_ptr<ts_Tensor> dolly(new ts_Tensor(tensor::load(path)));
  RETURN_OR_CATCH(dolly.release(), nullptr)
}
