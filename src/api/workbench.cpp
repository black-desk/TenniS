...
ts_Workbench *ts_Workbench_Load(
  ...
  std::unique_ptr<ts_Workbench> workbench(new ts_Workbench(Workbench::Load(
    module->pointer, ComputingDevice(device->type, device->id))));
...
ts_bool ts_Workbench_input_by_name(
...
ts_bool ts_Workbench_output(
  ts_Workbench *workbench, int32_t i, ts_Tensor *tensor) {
  TRY_HEAD
  if (! workbench) throw Exception("NullPointerException: @param: 1");
  if (! tensor) throw Exception("NullPointerException: @param: 3");
...
ts_bool ts_Workbench_output_by_name(
...
ts_bool ts_Workbench_set_computing_thread_number(
...
ts_bool ts_Workbench_bind_filter(
...
ts_bool ts_Workbench_bind_filter_by_name(
...
ts_Workbench *ts_new_Workbench(const ts_Device *device) {
  ...
  std::unique_ptr<ts_Workbench> workbench(
    device ? new ts_Workbench(ComputingDevice(device->type, device->id)) :
...
ts_Program *ts_Workbench_compile(
  ts_Workbench *workbench, const ts_Module *module) {
  TRY_HEAD
  if (! workbench) throw Exception("NullPointerException: @param: 1");
  if (! module) throw Exception("NullPointerException: @param: 2");
  std::unique_ptr<ts_Program> program(
    new ts_Program((*workbench)->compile(module->pointer)));
...
ts_Workbench *ts_Workbench_Load_v2(
  ...
  std::unique_ptr<ts_Workbench> workbench(new ts_Workbench(Workbench::Load(
    module->pointer, ComputingDevice(device->type, device->id), options)));
  RETURN_OR_CATCH(workbench.release(), nullptr)
}

ts_Program *ts_Workbench_compile_v2(
  ...
  if (! options) throw Exception("NullPointerException: @param: 3");
  std::unique_ptr<ts_Program> program(
    new ts_Program((*workbench)->compile(module->pointer, options)));
  RETURN_OR_CATCH(program.release(), nullptr)
}

ts_bool ts_Workbench_run_hook(
  ts_Workbench *workbench, const char **node_names, int32_t len) {
  TRY_HEAD
  if (! workbench) throw Exception("NullPointerException: @param: 1");
  if (! node_names) throw Exception("NullPointerException: @param: 2");
  (*workbench)
    ->run_hook(std::vector<std::string>(node_names, node_names + len));
  RETURN_OR_CATCH(ts_true, ts_false)
}

ts_bool ts_Workbench_set_operator_param(
...
ts_bool ts_Workbench_set_cpu_mode(
  ts_Workbench *workbench, ts_CpuPowerMode mode) {
  TRY_HEAD
  if (! workbench) throw Exception("NullPointerException: @param: 1");
