...
namespace ts
{
  namespace base
  {
    ...
    void Affine_Sample2D::init() {
      ...
      TS_AUTO_CHECK(
        (m_type >= Affine_Sample2DType::LINEAR)
        && (m_type <= Affine_Sample2DType::HARD));
    }

    int Affine_Sample2D::infer(
      ...
      int dim = m_dim;
      if (dim < 0) { dim = int(input_shape.size()) + dim; }
    ...
    int Affine_Sample2D::run(Stack &stack) {
      ...
      TS_AUTO_CHECK(
        (affine_shape.size() == 2) && (affine_shape [0] == 3)
        && (affine_shape [1] == 3));
