...
namespace ts
{
  namespace base
  {
    ...
    void BroadcastV2::init() { supper::init(); }

    int BroadcastV2::infer(
    ...
    int BroadcastV2::run(Stack &stack) {
      ...
      x = x.view(memory_device)
            .reshape(x_shape);  // do sync, and set default data to given device
    ...
    bool BroadcastV2::broadcast(Shape &x, const Shape &shape) {
      if (x.size() > shape.size()) {
        TS_LOG_ERROR << "Can not broadcast " << to_string(x) << " to "
