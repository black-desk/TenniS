...
namespace ts
{
  namespace base
  {
    ...
    Concat::Concat() { field(name::dim, REQUIRED); }
    ...
    static void throw_error_message(
      const std::string &title, Stack &stack, int dim) {
      ...
      oss << "}";
      TS_LOG_ERROR << title << "Can not concat " << oss.str()
                   << " at dim=" << dim << ts::eject;
    }
    ...
    int Concat::infer(Stack &stack, std::vector<Tensor::Prototype> &output) {
      ...
      // TS_AUTO_CHECK(stack.index(0)->dtype() == FLOAT32 ||
      // stack.index(0)->dtype() == FLOAT64);
      ...
      if (fixed_dim < 0 || fixed_dim >= output_dims) {
        TS_LOG_ERROR << "Concat dim must in [-" << output_dims << ", "
                     << output_dims << ")" << eject;
      }
      ...
      for (size_t i = 1; i < input_num; i++) {
        ...
        if (shape.size() != num_dims) { throw_error_message("", stack, m_dim); }

        for (int j = 0; j < shape.size(); j++) {
          if (j == fixed_dim) continue;
