  ...
  namespace base
  {
    ...
    void Conv2DQuantized::init() {
      ...
      if (dilation_tensor.empty()) {
        TS_LOG_ERROR << this->op() << " must set " << name::dilation << " or "
                     << name::typo::dialations << eject;
      }
      ...
      TS_AUTO_CHECK(dilation_tensor.has_shape({
        4,
      }));
      ...
      for (size_t i = 0; i < 4; ++i)
        m_dilation4 [i] = dilation_tensor.data<int32_t>(i);
      ...
      if (m_format == FORMAT_NCHW) {
        ...
        if (m_dilation4 [0] != 1 || m_dilation4 [1] != 1) {
          TS_LOG_ERROR << this->op() << " do not support dialations: "
                       << to_string(m_dilation4) << eject;
        }
        ...
        if (m_dilation4 [0] != 1 || m_dilation4 [3] != 1) {
          TS_LOG_ERROR << this->op() << " do not support dialations: "
                       << to_string(m_dilation4) << eject;
        }
      }
      ...
      for (int i = 0; i < dequantize_scale_tensor.count(); i++) {
        m_dequantize_scales [i] = tensor::cast(FLOAT32, dequantize_scale_tensor)
                                    .data<float>() [i];
      }
    }

    int Conv2DQuantized::infer(
      ...
      if (m_format == FORMAT_NCHW) {
        out_proto = Tensor::Prototype(
          FLOAT32, { x_tensor.size(0), w_tensor.size(0), y.height, y.width });
      } else if (m_format == FORMAT_NHWC) {
        out_proto = Tensor::Prototype(
          FLOAT32, { x_tensor.size(0), y.height, y.width, w_tensor.size(0) });
      }
