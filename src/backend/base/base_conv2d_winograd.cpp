...
namespace ts
{
  namespace base
  {
    Conv2DWinograd::Conv2DWinograd() {
      ...
      field(
        name::kernel_winograd_transformed, OPTIONAL, tensor::from<bool>(false));
    }
    ...
    void Conv2DWinograd::init() {
      ...
      if (has(name::kernel_winograd_transformed)) {
        m_kernel_transformed = tensor::to_bool(
          get(name::kernel_winograd_transformed));
      }
    ...
    int Conv2DWinograd::infer(
      ...
      Size2D     x;
      // winograd conv2d only suppot ksize == 3x3 && stride == 1 && dialations
      // == 1
      Size2D     ksize(3, 3);
      Stride2D   stride(1, 1);
    ...
    int Conv2DWinograd::run(ts::Stack &stack) {
      ...
      if (! m_kernel_transformed || m_k_transformed.empty()) {
        ...
        KernelCommonFunc<float>::winograd_mode_select_on_arm(
          x_tensor.sizes(), kernel_tensor.size(0), winograd_mode);
        ...
        kernel_transformed_shape = {
          kernel_shape [0], kernel_shape [1], in_tile_height, in_tile_width
        };
        m_k_transformed = Tensor(
          Tensor::InFlow::HOST,
          kernel_tensor.dtype(),
          kernel_transformed_shape);
