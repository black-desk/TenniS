...
namespace ts
{
  namespace base
  {
    ...
    bool FusedBatchNorm::check_inputs(Stack &stack) const {
      ...
      if (
        ! (m_dim < x.dims()) || ! (mean.has_shape({ x.size(m_dim) }))
        || ! (variance.has_shape({ x.size(m_dim) }))
        || ! (scale.has_shape({ x.size(m_dim) }))
        || ! (bias.has_shape({ x.size(m_dim) }))
        || ! (x.dtype() == mean.dtype()) || ! (x.dtype() == variance.dtype())
        || ! (x.dtype() == scale.dtype()) || ! (x.dtype() == bias.dtype())) {
        TS_LOG_ERROR << "FusedBatchNorm failed: x=" << x.proto()
                     << ", mean=" << mean.proto()
                     << ", variance=" << mean.proto()
                     << ", scale=" << mean.proto() << ", bias=" << mean.proto()
                     << ", dim=" << m_dim << "." << eject;
      }
