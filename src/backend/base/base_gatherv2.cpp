...
namespace ts
{
  namespace base
  {
    ...
    void GatherV2::init() { supper::init(); }

    static Tensor::Prototype infer_gather(
      const Tensor &x, const Tensor &indices) {
      ...
      TS_AUTO_CHECK(
        indices_shape [indices_shape.size() - 1] <= input_shape.size());
      output_shape.insert(
        output_shape.end(),
    ...
    int GatherV2::run(Stack &stack) {
      TS_AUTO_CHECK(stack.size() == 2);
      // auto indices =  stack[1].view(memory_device);//tensor::cast(INT32,
      // stack[1]); auto output_proto = infer_gather(stack[0], indices);
