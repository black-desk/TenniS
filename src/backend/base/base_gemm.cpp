...
namespace ts
{
  namespace base
  {
    ...
    static void infer_gemm(
      ...
      bool               transB,
      int               &K,
      Tensor::Prototype &out,
      Shape             &adjusted_C_shape) {
      if (A.dims() != 2)
        TS_LOG_ERROR << "A.dims() expect equal 2, got " << A.dims() << eject;
      if (B.dims() != 2)
        TS_LOG_ERROR << "A.dims() expect equal 2, got " << B.dims() << eject;
      if (C.dims() > 2)
        TS_LOG_ERROR << "A.dims() expect less 2, got " << C.dims() << eject;
      ...
      if (
        (C_shape [0] != 1 && C_shape [0] != M)
        || (C_shape [1] != 1 && C_shape [1] != N)) {
        TS_LOG_ERROR << "Can not broadcast " << to_string(C.sizes()) << " to "
                     << to_string(out.sizes()) << eject;
      }
