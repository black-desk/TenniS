...
namespace ts
{
  namespace base
  {
    ...
    int GlobalPooling2D::infer(
      ...
      if (m_format == FORMAT_NCHW) {
        ...
        output [0] = Tensor::Prototype(
          x_tensor.dtype(), { x_tensor.size(0), x_tensor.size(1), 1, 1 });
      } else if (m_format == FORMAT_NHWC) {
        output.resize(1);
        output [0] = Tensor::Prototype(
          x_tensor.dtype(), { x_tensor.size(0), 1, 1, x_tensor.size(3) });
      }
    ...
    int GlobalPooling2D::run(Stack &stack) {
      ...
      pooling2d(
        x, m_type, padding, Padding2DType::BLACK, ksize, stride, m_format, out);
