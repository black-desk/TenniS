...
namespace ts
{
  namespace base
  {
    Pooling2DV2::Pooling2DV2() {
      ...
      field(
        name::padding_type, OPTIONAL, tensor::from(int(Padding2DType::BLACK)));
    ...
    void Pooling2DV2::init() {
      ...
      m_padding_type = static_cast<Padding2DType>(
        tensor::to_int(get(name::padding_type)));
      ...
      if (format == name::NCHW) {
        ...
        TS_LOG_ERROR << this->op() << " do not support format: " << format
                     << eject;
      }
    }

    int Pooling2DV2::infer(
      ...
      TS_AUTO_CHECK(ksize_tensor.has_shape({
        4,
      }));
      TS_AUTO_CHECK(stride_tensor.has_shape({
        4,
      }));

      std::valarray<int> m_padding4x2(8);
      for (size_t i = 0; i < 8; ++i)
        m_padding4x2 [i] = padding_tensor.data<int32_t>(i);
      std::valarray<int> m_ksize4(4);
      for (size_t i = 0; i < 4; ++i)
        m_ksize4 [i] = ksize_tensor.data<int32_t>(i);
      std::valarray<int> m_stride4(4);
      for (size_t i = 0; i < 4; ++i)
        m_stride4 [i] = stride_tensor.data<int32_t>(i);

      // only support native conv2d
      if (m_format == FORMAT_NCHW) {
        if (
          m_padding4x2 [0] != 0 || m_padding4x2 [1] != 0
          || m_padding4x2 [2] != 0 || m_padding4x2 [3] != 0) {
          TS_LOG_ERROR << this->op()
                       << " do not support padding: " << to_string(m_padding4x2)
                       << eject;
        }
        if (m_ksize4 [0] != 1 || m_ksize4 [1] != 1) {
          TS_LOG_ERROR << this->op()
                       << " do not support ksize: " << to_string(m_ksize4)
                       << eject;
        }
        if (m_stride4 [0] != 1 || m_stride4 [1] != 1) {
          TS_LOG_ERROR << this->op()
                       << " do not support stride: " << to_string(m_stride4)
                       << eject;
        }
      } else if (m_format == FORMAT_NHWC) {
        if (
          m_padding4x2 [0] != 0 || m_padding4x2 [1] != 0
          || m_padding4x2 [6] != 0 || m_padding4x2 [7] != 0) {
          TS_LOG_ERROR << this->op()
                       << " do not support padding: " << to_string(m_padding4x2)
                       << eject;
        }
        if (m_ksize4 [0] != 1 || m_ksize4 [3] != 1) {
          TS_LOG_ERROR << this->op()
                       << " do not support ksize: " << to_string(m_ksize4)
                       << eject;
        }
        if (m_stride4 [0] != 1 || m_stride4 [3] != 1) {
          TS_LOG_ERROR << this->op()
                       << " do not support stride: " << to_string(m_stride4)
                       << eject;
        }
    ...
    int Pooling2DV2::run(Stack &stack) {
      ...
      if (m_format == FORMAT_NCHW) {
        ksize = Size2D(
          ksize_tensor.data<int32_t>() [2], ksize_tensor.data<int32_t>() [3]);
        padding = Padding2D(
          padding_tensor.data<int32_t>() [4],
          padding_tensor.data<int32_t>() [5],
          padding_tensor.data<int32_t>() [6],
          padding_tensor.data<int32_t>() [7]);
        stride = Stride2D(
          stride_tensor.data<int32_t>() [2], stride_tensor.data<int32_t>() [3]);
      } else if (m_format == FORMAT_NHWC) {
        ksize = Size2D(
          ksize_tensor.data<int32_t>() [1], ksize_tensor.data<int32_t>() [2]);
        padding = Padding2D(
          padding_tensor.data<int32_t>() [2],
          padding_tensor.data<int32_t>() [3],
          padding_tensor.data<int32_t>() [4],
          padding_tensor.data<int32_t>() [5]);
        stride = Stride2D(
          stride_tensor.data<int32_t>() [1], stride_tensor.data<int32_t>() [2]);
      }

      pooling2d(
        x, m_type, padding, m_padding_type, ksize, stride, m_format, out);
