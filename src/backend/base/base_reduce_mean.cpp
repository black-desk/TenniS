...
namespace ts
{
  namespace base
  {
    ...
    void ReduceMean::init() {
      ...
      for (size_t i = 0; i < dims_count; ++i) { m_dims [i] = dims_data [i]; }
    ...
    static std::vector<int> checkout(
      Stack &stack, std::vector<int> dims, bool keep_dim, Shape &output) {
      TS_AUTO_CHECK(stack.size() == 1);
      Shape input    = stack [0].sizes();
      auto  has_dims = int(input.size());

      bool continue_flag = true;
      if (dims.size() > 1) { continue_flag = check_dims_continue(dims); }
      ...
      for (int i = 0; i < fixed_dims.size(); ++i) {
        ...
        if (fixed_dims [i] < 0 || fixed_dims [i] >= has_dims) {
          TS_LOG_ERROR << "Reduce dim must in [-" << has_dims << ", "
                       << has_dims << ")" << eject;
        }
      }
      ...
      if (keep_dim) {
        ...
        std::copy(
          std::next(input.begin(), i), input.end(), std::back_inserter(output));
      }

      return fixed_dims;
    }

    int ReduceMean::infer(
