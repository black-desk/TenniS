...
namespace ts
{
  namespace base
  {
    Reshape::Reshape() { field(name::shape, REQUIRED); }
    ...
    void Reshape::init() {
      ...
      for (size_t i = 0; i < count; ++i) {
        ...
        if (m_broadcast_dim >= 0)
          TS_LOG_ERROR << "Can not reshape tensor to " << to_string(m_shape)
                       << eject;
    ...
    Shape Reshape::newshape(const Tensor &x) {
      ...
      for (size_t i = 0; i < shape.size(); ++i) {
        if (shape [i] == 0) {
          if (i >= x.dims()) {
            TS_LOG_ERROR << "Can not reshape " << to_string(x.sizes()) << " to "
                         << to_string(m_shape) << eject;
          }
          shape [i] = x.size(i);
        }
      }
