...
namespace ts
{
  namespace base
  {
    ...
    int find_resized_height_dim(
      const int *shape_data, const int *size_data, int n) {
      ...
      if (
        resized_height_dim < 0 || resized_height_dim >= n - 1
    ...
    static Tensor::Prototype check_outputs(
      const Stack &stack, int &dim, bool &ready) {
      ...
      auto resized_height_dim = find_resized_height_dim(
        shape_data, size_data, n);
      if (resized_height_dim < 0) {
        TS_LOG_ERROR << "Can not resize " << to_string(x.sizes()) << " to "
                     << to_string(Shape(size_data, size_data + n)) << eject;
      }
      dim = resized_height_dim;

      Shape resized_shape = x.sizes();

      ready = resized_shape [resized_height_dim]
             == size_data [resized_height_dim]
           && resized_shape [resized_height_dim + 1]
                == size_data [resized_height_dim + 1];
