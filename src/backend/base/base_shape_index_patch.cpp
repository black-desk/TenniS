...
namespace ts
{
  namespace base
  {
    ...
    void ShapeIndexPatch::init() {
      ...
      TS_AUTO_CHECK(
        tensor_origin_patch.has_shape(2) && tensor_origin.has_shape(2));
    ...
    int ShapeIndexPatch::infer(
      ...
      int x_patch_h = int(
        m_origin_patch.height * x_shape [2] / float(m_origin.height) + 0.5f);
      int x_patch_w = int(
        m_origin_patch.width * x_shape [3] / float(m_origin.width) + 0.5f);

      std::vector<int> out_shape = {
        x_shape [0], x_shape [1], x_patch_h, landmarkx2 / 2, x_patch_w
      };
