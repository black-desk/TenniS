...
namespace ts
{
  namespace base
  {
    ...
    static int bit_ones(int32_t n) { return bit_ones(uint32_t(n)); }
     ...
     * @param in [out] reshape x to in's shape, than do slice by [begin, end,
     * stride]
    ...
    static bool infer_output(
      ...
      if (begin.size() != end.size() || begin.size() != stride.size())
        return false;
      ...
      if (ellipsis_ones) {
        ...
        in.insert(
          ...
          std::accumulate(
            x.begin() + left_count,
            x.begin() + left_count + ellipsis_count,
            1,
            std::multiplies<int>()));
        ...
        ellipsis_shape = Shape(
          x.begin() + left_count, x.begin() + left_count + ellipsis_count);
      } else {
        in = x;
      }
      ...
      if (slice_size < in.size()) {
        ...
        auto final_size = std::accumulate(
          in.begin() + slice_size, in.end(), 1, std::multiplies<int>());
      ...
      // shrink output, and expand ellipsis_shape
      {
        for (int i = int(slice_size) - 1; i >= 0; --i) {
          if (! ellipsis_shape.empty() && (i == ellipsis_index)) {
            ...
            out.insert(
              out.begin() + i, ellipsis_shape.begin(), ellipsis_shape.end());
            continue;
          }
    ...
    static std::string slice_string(
      ...
      auto               slice_size = std::min(
        std::min(begin.size(), end.size()), stride.size());
      oss << "[";
      for (size_t i = 0; i < slice_size; ++i) {
        if (i) oss << ", ";
        if (ellipsis_mask & (1 << i)) {
          ...
          std::string begin_content  = (begin_mask & (1 << i)) ?
                                         "" :
                                         std::to_string(begin [i]);
          std::string end_content    = (end_mask & (1 << i)) ?
                                         "" :
                                         std::to_string(end [i]);
          std::string stride_content = stride [i] == 1 ?
                                         "" :
                                         std::to_string(stride [i]);
    ...
    void SliceV3::load_params(const Stack &stack) {
      ...
      if (stack.size() > 3) {
        ...
        for (int32_t i = 0; i < slice_size; ++i) { axes [i] = i; }
      }
      ...
      for (int32_t i = 0; i < slice_size; ++i) {
        ...
        if (axis < -dims || axis >= dims) {
          TS_LOG_ERROR << "axes must in [" << -dims << ", " << dims
                       << "), but got " << to_string(axes) << eject;
        }
        if (axis < 0) axis += dims;
        if (axis <= pre_axis) {
          TS_LOG_ERROR << "axes must in increasing rank, bug got "
                       << to_string(axes) << eject;
        }
        pre_axis = axis;
