...
namespace ts
{
  namespace base
  {
    ...
    void SpaceToBatch4D::CaculateOutputSize(
      ...
      output_shape [2] = (input_shape [2] + padding_top + padding_bottom)
                       / block_height;
      output_shape [3] = (input_shape [3] + padding_left + padding_right)
                       / block_width;
    ...
    void SpaceToBatch4D::init() {
      ...
      TS_AUTO_CHECK(block_shape_tensor.has_shape({
        2,
      }));
    ...
    int SpaceToBatch4D::infer(
      ...
      TS_AUTO_CHECK(
        (((input_shape [2] + m_padding [0] + m_padding [1]) % m_block_shape [0])
         == 0)
