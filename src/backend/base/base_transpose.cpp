...
namespace ts
{
  namespace base
  {
    Transpose::Transpose() { field(name::permute, OPTIONAL); }
    ...
    void Transpose::init() {
      ...
      if (has(name::permute)) {
        ...
        if (std::find(flag.begin(), flag.end(), false) != flag.end()) {
          TS_LOG_ERROR << "Can not transpose to " << to_string(m_permute)
                       << eject;
        }
      }
    }
    ...
    std::vector<int> Transpose::get_permute(const Tensor &x) {
      if (! m_permute.empty()) {
        if (x.dims() > m_permute.size()) {
          TS_LOG_ERROR << "Can not transpose " << x.proto()
                       << " with permute=" << to_string(m_permute) << eject;
        }
        if (x.dims() < m_permute.size()) {
          // It's OK now!
        }
      ...
      for (size_t i = 0; i < permute.size(); ++i) { permute [i] = int(i); }
      if (permute.size() >= 2) {
    ...
    int Transpose::infer(
      ts::Stack &stack, std::vector<ts::Tensor::Prototype> &output) {
      ...
      output [0] = Tensor::Prototype(
        x.dtype(), permuted_shape(x.sizes(), get_permute(x)));
    ...
    int Transpose::run(ts::Stack &stack) {
      ...
      Tensor::Prototype output_proto(
        x.dtype(), permuted_shape(x.sizes(), permute));
