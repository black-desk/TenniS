...
namespace ts
{
  ...
  bool ElementWiseReduce::reduce(
    ...
    if (! broadcast) {
      TS_LOG_ERROR << "[" << op->op() << ":" << op->name()
                   << "] Can not reduce shape: " << to_string(_lhs_shape)
                   << " vs. " << to_string(_rhs_shape) << eject;
    }
  ...
  void ElementWiseReduce::init() { supper::init(); }

  int ElementWiseReduce::infer(
    ...
    if (lhs.dtype() != rhs.dtype()) {
      TS_LOG_ERROR << "[" << this->op() << ":" << this->name()
                   << "] Can not reduce mismatch type: "
                   << type_str(lhs.dtype()) << " vs. " << type_str(rhs.dtype())
                   << eject;
    }
  ...
  int ElementWiseReduce::run(Stack &stack) {
    ...
    lhs = lhs.view(memory_device)
            .reshape(
              lhs_shape);  // do sync, and set default data to given device
  ...
  bool ElementWiseReduce::is_bias(
    Shape &lhs_shape, Shape &rhs_shape, int &dim) {
    auto count = std::accumulate(
      rhs_shape.begin(), rhs_shape.end(), 1, std::multiplies<int>());
  ...
  void ElementWiseReduce::reduce_with_bias_cross(
    const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
    (void)(dim);
    this->reduce_with_broadcast(lhs, rhs, out);
  }

  void ElementWiseReduce::reduce_with_scalar_cross(
    const Tensor &lhs, const Tensor &rhs, Tensor &out) {
    this->reduce_with_broadcast(lhs, rhs, out);
  }
    ...
    for (size_t i = 0; i < out.size();) {
      ...
      if (j - i > 1) {
        auto a = std::accumulate(
          lhs.begin() + i, lhs.begin() + j, 1, std::multiplies<int32_t>());
        auto b = std::accumulate(
          rhs.begin() + i, rhs.begin() + j, 1, std::multiplies<int32_t>());
        auto c = std::accumulate(
          out.begin() + i, out.begin() + j, 1, std::multiplies<int32_t>());
