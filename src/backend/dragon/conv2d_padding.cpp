...
namespace ts
{
  namespace dragon
  {
    ...
    void Conv2DPadding::init() {
      ...
      auto padding_method_str = tensor::to_string(
        this->get(name::padding_method));
      ...
      if (padding_method_str == name::SAME) {
        ...
        TS_LOG_ERROR << "Not supported padding_method=" << padding_method_str
                     << eject;
      }
      ...
      if (m_format == Conv2DFormat::FORMAT_NCHW) {
        TS_AUTO_CHECK(
          static_padding.data<int32_t>() [0] == 0
...
#define DEFINE_SAME_PADDING(A, B)                                              \
  ...
  B = padding_needed - A
    ...
    static inline _out_pad forward_same(
      ...
      int                          stride,
      int                          dilation,
      Conv2DPadding::PaddingMethod padding) {
      _out_pad out;
      ...
      if (padding != Conv2DPadding::PaddingMethod::SAME_UPPER) {
    ...
    static inline std::array<_out_pad, 2> conv2d_forward_same(
      const Size2D                &x,
      ...
      out [0] = forward_same(
        x.height, ksize.height, stride.height, dilation.height, padding_method);
      out [1] = forward_same(
        x.width, ksize.width, stride.width, dilation.width, padding_method);
      return out;
    }
    ...
    static inline Padding2D dynamic_padding_same(
      ...
      auto dynamic_output_size = conv2d_forward_same(
        input_size, static_padding, ksize, stride, dilation, padding_method);
    ...
    int Conv2DPadding::run(Stack &stack) {
      ...
      if (m_padding_method == PaddingMethod::SAME_UPPER) {
        dynamic_padding = dynamic_padding_same(
          x, m_static_padding, ksize, m_stride, m_dilation, m_padding_method);
        ...
        dynamic_padding = dynamic_padding_valid(
          x, m_static_padding, ksize, m_stride, m_dilation);
      } else {
