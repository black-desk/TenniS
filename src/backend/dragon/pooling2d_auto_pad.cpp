...
namespace ts
{
  namespace dragon
  {
    ...
    static inline int forward_valid(
      int x, int ksize, int pad_l, int pad_r, int stride, bool ceil_mode) {
      int y;
      if (ceil_mode) {
    ...
    static inline Padding2D dynamic_padding_valid(
      ...
      Size2D expected_output_size = pooling2d_forward_valid(
        input_size, ksize, static_padding, stride, ceil_mode);
    ...
    struct _out_pad {
        ...
        int out;    // output size
        int pad_l;  // final padding (including static padding and dynamic
                    // padding)
        int pad_r;  // final padding (including static padding and dynamic
                    // padding)
    };
...
#define DEFINE_SAME_PADDING(A, B)                                              \
  ...
  B = padding_needed - A
     ...
     * @return
     */
    static inline _out_pad forward_same(
      int                           x,
      int                           ksize,
      int                           stride,
      Pooling2DAutoPad::AutoPadType padding,
      bool                          ceil_mode) {
      _out_pad out;
      ...
      if (padding == Pooling2DAutoPad::AutoPadType::SAME_UPPER) {
        DEFINE_SAME_PADDING(pad_l, pad_r);
      } else {
        DEFINE_SAME_PADDING(pad_r, pad_l);
      }
    ...
    static inline std::array<_out_pad, 2> pooling2d_forward_same(
      const Size2D                 &x,
      const KSize2D                &ksize,
      const Stride2D               &stride,
      Pooling2DAutoPad::AutoPadType padding,
      ...
      out [0] = forward_same(
        x.height, ksize.height, stride.height, padding, ceil_mode);
      out [1] = forward_same(
        x.width, ksize.width, stride.width, padding, ceil_mode);
      return out;
    }

    static inline Padding2D dynamic_padding_same(
      const Size2D                 &input_size,
      const Padding2D              &static_padding,
      const KSize2D                &ksize,
      const Stride2D               &stride,
      ...
      auto dynamic_output_size = pooling2d_forward_same(
        input_size, ksize, stride, padding, ceil_mode);

      Size2D expected_output_size(
        dynamic_output_size [0].out, dynamic_output_size [1].out);
      Size2D expected_input_size = pooling2d_backward(
        expected_output_size, static_padding, ksize, stride);
      ...
      dynamic_padding.bottom = static_padding.top + static_padding.bottom
                             + (padding_height - dynamic_output_size [0].pad_l);
      dynamic_padding.right = static_padding.left + static_padding.right
                            + (padding_width - dynamic_output_size [1].pad_l);
      ...
      switch (this->auto_pad) {
        case AutoPadType::NOTSET:
          dynamic_padding = dynamic_padding_valid(
            input_size, static_padding, ksize, stride, ceil_mode);
          break;
        ...
        case AutoPadType::SAME_UPPER:
          dynamic_padding = dynamic_padding_same(
            input_size,
            static_padding,
            ksize,
            stride,
            AutoPadType::SAME_UPPER,
            ceil_mode);
          break;
