...
namespace ts
{
  namespace mxnet
  {
    ...
    static Size2D valid_pooling2d_forward(
      const Size2D    &x,
    ...
    int Pooling2dPadding::run(Stack &stack) {
      ...
      auto ksize_tensor = tensor::cast(
        INT32, stack.index(1)->view(MemoryDevice(CPU)));
      auto stride_tensor = tensor::cast(
        INT32, stack.index(2)->view(MemoryDevice(CPU)));
      ...
      if (this->valid) {
        ...
        /* Heavy version
        Size2D expected_output_size = valid_pooling2d_forward(input_size,
...
TS_REGISTER_FP16_OPERATOR(
  Pooling2dPadding, GPU, name::layer::mx_pooling2d_padding())
