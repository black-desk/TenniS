  ...
  namespace onnx
  {
    ...
    Pooling2DAutoPad::Pooling2DAutoPad() {
      ...
      this->field(
        name::padding,
        OPTIONAL,
        tensor::build(INT32, { 4, 2 }, { 0, 0, 0, 0, 0, 0, 0, 0 }));
    }
    ...
    int Pooling2DAutoPad::infer(
      Stack &stack, std::vector<Tensor::Prototype> &output) {
      output.resize(1);
      output [0] = Tensor::Prototype(INT32, { 4, 2 });
      return 1;
    }

    static inline Size2D pooling2d_forward_notset(
      const Size2D    &x,
      const Padding2D &padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      Size2D y;
      y.height = int(std::floor(
        (x.height + padding.top + padding.bottom - ksize.height)
          / (float)stride.height
        + 1));
      y.width  = int(std::floor(
        (x.width + padding.left + padding.right - ksize.width)
          / (float)stride.width
        + 1));
      return y;
    }

    static inline Padding2D dynamic_padding_notset(
      const Size2D    &input_size,
      const Padding2D &static_padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      ...
      /*
      Size2D expected_output_size = pooling2d_forward_notset(input_size,
      static_padding, ksize, stride); Size2D expected_input_size =
      pooling2d_backward(expected_output_size, static_padding, ksize, stride);
      ...
      expected_input_size.height - input_size.height; dynamic_padding.right =
      static_padding.right + expected_input_size.width - input_size.width;
       */

      dynamic_padding.top    = static_padding.top;
      dynamic_padding.left   = static_padding.left;
      dynamic_padding.bottom = static_padding.bottom
                             - (input_size.height + static_padding.top
                                + static_padding.bottom - ksize.height)
                                 % stride.height;
      dynamic_padding.right = static_padding.right
                            - (input_size.width + static_padding.left
                               + static_padding.right - ksize.width)
                                % stride.width;
    ...
    static inline int forward_valid(
      int input_spatial_shape,
      int kernel_spatial_shape,
      int strides_spatial_shape) {
      return int(std::ceil(
        (input_spatial_shape - kernel_spatial_shape + 1)
        / float(strides_spatial_shape)));
    }

    static inline Size2D pooling2d_forward_valid(
      const Size2D    &x,
      const Padding2D &padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      Size2D y;
      y.height = forward_valid(
        x.height + padding.top + padding.bottom, ksize.height, stride.height);
      y.width = forward_valid(
        x.width + padding.left + padding.right, ksize.width, stride.width);
      return y;
    }

    static inline Padding2D dynamic_padding_valid(
      const Size2D    &input_size,
      const Padding2D &static_padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      ...
      Size2D expected_output_size = pooling2d_forward_valid(
        input_size, static_padding, ksize, stride);
      Size2D expected_input_size = pooling2d_backward(
        expected_output_size, static_padding, ksize, stride);
      dynamic_padding.top    = static_padding.top;
      dynamic_padding.left   = static_padding.left;
      dynamic_padding.bottom = static_padding.bottom
                             + expected_input_size.height - input_size.height;
      dynamic_padding.right = static_padding.right + expected_input_size.width
                            - input_size.width;
    ...
    static inline int forward_same(
      int input_spatial_shape,
      int kernel_spatial_shape,
      int strides_spatial_shape) {
    ...
    static inline Size2D pooling2d_forward_same(
      const Size2D    &x,
      const Padding2D &padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      Size2D y;
      y.height = forward_same(
        x.height + padding.top + padding.bottom, ksize.height, stride.height);
      y.width = forward_same(
        x.width + padding.left + padding.right, ksize.width, stride.width);
      return y;
    }

    static inline Padding2D dynamic_padding_same_upper(
      const Size2D    &input_size,
      const Padding2D &static_padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
      ...
      Size2D expected_output_size = pooling2d_forward_same(
        input_size, static_padding, ksize, stride);
      Size2D expected_input_size = pooling2d_backward(
        expected_output_size, static_padding, ksize, stride);
      ...
      dynamic_padding.bottom = static_padding.bottom
                             + (padding_height - half_padding_height);
      dynamic_padding.right = static_padding.right
                            + (padding_width - half_padding_width);

      return dynamic_padding;
    }

    static inline Padding2D dynamic_padding_same_lower(
      const Size2D    &input_size,
      const Padding2D &static_padding,
      const KSize2D   &ksize,
      const Stride2D  &stride) {
    ...
    int Pooling2DAutoPad::run(Stack &stack) {
      ...
      // get output
      auto &dynamic_padding_tensor = *stack.push(
        INT32, { 4, 2 }, MemoryDevice(CPU));
      ...
      switch (this->auto_pad) {
        case AutoPadType::NOTSET:
          dynamic_padding = dynamic_padding_notset(
            input_size, static_padding, ksize, stride);
          break;
        case AutoPadType::VALID:
          dynamic_padding = dynamic_padding_valid(
            input_size, static_padding, ksize, stride);
          break;
        case AutoPadType::SAME_UPPER:
          dynamic_padding = dynamic_padding_same_upper(
            input_size, static_padding, ksize, stride);
          break;
        case AutoPadType::SAME_LOWER:
          dynamic_padding = dynamic_padding_same_lower(
            input_size, static_padding, ksize, stride);
          break;
      }
...
TS_REGISTER_OPERATOR(
  Pooling2DAutoPad, CPU, name::layer::onnx_pooling2d_padding())
