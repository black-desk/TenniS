...
namespace ts
{
  namespace tf
  {
    ...
    static inline Size2D conv2d_forward_valid(
      const Size2D     &x,
    ...
    static inline Padding2D dynamic_padding_valid(
      ...
      Size2D expected_output_size = conv2d_forward_valid(
        input_size, static_padding, ksize, stride, dilation);
      Size2D expected_input_size = conv2d_backward(
        expected_output_size, static_padding, ksize, stride, dilation);
    ...
    // SAME: output_spatial_shape[i] = ceil(input_spatial_shape[i] /
    ...
    static inline Size2D conv2d_forward_same(
      const Size2D     &x,
      const Padding2D  &padding,
      const KSize2D    &ksize,
      const Stride2D   &stride,
    ...
    static inline Padding2D dynamic_padding_same(
      ...
      Size2D expected_output_size = conv2d_forward_same(
        input_size, static_padding, ksize, stride, dilation);
      Size2D expected_input_size = conv2d_backward(
        expected_output_size, static_padding, ksize, stride, dilation);
    ...
    int Conv2DPadding::run(Stack &stack) {
      ...
      if (w_tensor.size(1) != x_tensor.size(1)) {
        TS_LOG_ERROR << "Conv2D failed: x=" << x_tensor.proto()
                     << ", w=" << w_tensor.proto()
                     << ", format=" << tensor::to_string(get(name::format))
                     << "." << eject;
      }
