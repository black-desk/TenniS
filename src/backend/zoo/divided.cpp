...
namespace ts
{
  namespace zoo
  {
    ...
    void Divided::init() {
      ...
      m_size.resize(count);
      for (int i = 0; i < count; ++i) { m_size [i] = size_data [i]; }
    ...
    static inline void divided_shape(
      Shape &x, const std::vector<int32_t> &size) {
      if (size.size() > x.size()) {
        TS_LOG_ERROR << "Can not divided shape " << to_string(x) << " to "
                     << to_string(size) << eject;
      }
