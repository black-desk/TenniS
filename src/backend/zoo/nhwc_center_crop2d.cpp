...
namespace ts
{
  namespace zoo
  {
    ...
    NHWCCenterCrop2D::NHWCCenterCrop2D() { field(name::size, REQUIRED); }
    ...
    void NHWCCenterCrop2D::init() {
      ...
      m_pad_op = OperatorCreator::Create(
        context.computing_device.type(), name::layer::pad(), false);

      TS_CHECK_NQ(m_pad_op, nullptr)
        << "Can not find operator: " << name::layer::pad();
    ...
    int NHWCCenterCrop2D::infer(
      ...
      output [0] = Tensor::Prototype(
        x.dtype(), { x.size(0), m_size.height, m_size.width, x.size(3) });
    ...
    int NHWCCenterCrop2D::run(Stack &stack) {
      ...
      auto x_padding_tensor = tensor::build(
        INT32,
        { 4, 2 },
        {
          0,
...
TS_REGISTER_OPERATOR(
  NHWCCenterCrop2D, ts::CPU, ts::name::layer::nhwc_center_crop2d());
