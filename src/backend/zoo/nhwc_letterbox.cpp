...
namespace ts
{
  namespace zoo
  {
    ...
    void NHWCLetterBox::init() {
      ...
      m_sample_op->set(
        name::outer_value, tensor::clone(FLOAT32, get(name::outer_value)));
    ...
    static inline Size2D infer_size(
      const Size2D &x, const std::vector<int> &size) {
      if (size.size() == 2) {
    ...
    int NHWCLetterBox::infer(
      ...
      output [0] = Tensor::Prototype(
        x.dtype(), { -1, y_size.height, y_size.width, -1 });
