...
namespace ts
{
  namespace zoo
  {
    ...
    void NHWCScaleResize2D::init() {
      ...
      m_resize2d_op = OperatorCreator::Create(
        context.computing_device.type(), name::layer::resize2d(), false);

      TS_CHECK_NQ(m_resize2d_op, nullptr)
...
TS_REGISTER_OPERATOR(
  NHWCScaleResize2D, ts::CPU, ts::name::layer::nhwc_scale_resize2d());
