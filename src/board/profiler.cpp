...
namespace ts
{
  ...
  Board<float> &Profiler::board() { return m_board; }

  const Board<float> &Profiler::board() const { return m_board; }
  ...
  void Profiler::log(std::ostream &out) const {
    std::map<Board<float>::key_type, Board<float>::value_type> sorted_board(
      this->board().begin(), this->board().end());
    out << "============= "
        << "Profiler(" << to_string(this) << ")"
        << " timer"
        << " =============" << std::endl;
    for (auto &key_value: sorted_board) {
      out << "[" << key_value.first
          << "]: avg spent = " << key_value.second.avg() << "ms" << std::endl;
    }
  }
  ...
  bool profiler_on() { return ctx::ptr<Profiler>() != nullptr; }
