...
namespace ts
{
  void ts::ArgParser::add(
    const std::vector<std::string> &arg,
    const std::vector<std::string> &neg_arg,
    bool                            default_value) {
    if (arg.empty()) { TS_LOG_ERROR << "param@1 can not be empty." << eject; }
  ...
  bool ArgParser::get(const std::string &arg) const {
    ...
    if (it != m_arg_value.end()) { return it->second; }
    return false;
  }
