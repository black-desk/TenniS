...
namespace ts
{
  ...
  std::vector<Instruction::shared> Compiler::convert_operator_instruction(
    const Node &node) {
    auto &bubble = node.bubble();

    // step 1: check inner InstructionCreator
    auto icreator = InstructionCreator::Query(bubble.op());
    if (icreator != nullptr) { return icreator(node); }
    auto creator = OperatorCreator::Query(
      m_computing_device.type(), bubble.op(), false);

    if (creator == nullptr)
      TS_LOG_ERROR << "Not supported operator " << bubble.op() << eject;
    std::string description = bubble.op()
                            + "(in=" + std::to_string(node.inputs().size())
                            + ", out=" + std::to_string(1) + ")";
    auto op = creator();

    for (auto &param: bubble.params()) { op->set(param.first, param.second); }
    try {
      op->init();
    } catch (const Exception &e) {
      TS_LOG_ERROR << "While initializing " << bubble.op() << ":"
                   << bubble.name() << " got Exception: " << e.what() << eject;
    }
    std::vector<Instruction::shared> instructions;
    auto op_inst = std::make_shared<OperatorInstruction>(
      op, int(node.inputs().size()), int(bubble.output_count()), description);
  ...
  InstructionBlock Compiler::compile(
    const std::vector<Node> &raw_inputs,
    const std::vector<Node> &raw_outputs,
    ...
    // std::endl; plot_graph(std::cout, outputs);
    Graph            temp_graph;
    ...
    // transform graph, make other graph from different framework to TS
    // framework
    ...
    auto push_data_sagment = [&](Node node) -> int {
      ...
      if (bubble.has(name::device)) {
        block.data_segment.push_back(
          DeviceTensor(value, tensor::to_string(bubble.get(name::device))));
      } else {
    ...
    auto simulator_push = [&](Node node) {
      ...
      if (bubble.op() != Bubble::Parameter) { ++unsolved_node_count; }
    ...
    while (unsolved_node_count) {
      ...
      if (it != working_nodes.end()) {
        if (it->second < i) {
          block.instructions.push_back(
            instruction::Stack::push(int(it->second)));
          simulator_pop();
          continue;
        }
      }
      ...
      if (bubble.op() == Bubble::Const) {
        ...
        block.instructions.push_back(
          std::make_shared<DataSegmentInstruction>(data_index));
        simulator_pop();
        continue;
      }
      ...
      if (bubble.op() == Bubble::Variable) {
        TS_LOG_ERROR << "Not support " << Bubble::Variable << " in this version"
                     << eject;
      }
      ...
      // case3: check if this node will be compute later, if true, then swap
      // it's son to top
      ...
      for (auto inst_it = operator_instructions.rbegin();
           inst_it != operator_instructions.rend();
           ++inst_it) {
    ...
    if (! satisfied) {
      ...
      for (auto &node: inputs)
        working_input_nodes.insert(
          std::make_pair(node, working_input_nodes.size()));
      block.instructions.push_back(
        instruction::Stack::erase(0, -int(simulator.size())));
      for (auto it = simulator.rbegin(); it != simulator.rend(); ++it) {
        block.instructions.push_back(
          instruction::Stack::push(int(working_input_nodes [*it])));
      }
      simulator.clear();
    ...
    // inplace operator
    // 是不是可以检测operator，如果是inplace操作，就把push换成clone。或者不支持inplace操作，最简单了。
