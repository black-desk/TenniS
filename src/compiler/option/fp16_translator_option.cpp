...
bool ts::Fp16TranslatorOption::translate(
  const ts::ComputingDevice &device,
  const ts::Node             node,
  ...
  auto fp16_op_creator = Fp16OperatorCreator::Query(
    device.type(), node.bubble().op(), false);

  // status 0: current node is output but stream is fp16
  if (output_flag && fp16_op_creator != nullptr) {
    auto cast_fp32_node = bubble::op(
      node.bubble().name() + "_cast_fp32", name::layer::cast(), { node });
  ...
  for (auto input: inputs) {
    ...
    auto input_creator = Fp16OperatorCreator::Query(
      device.type(), input.bubble().op(), false);
    ...
    // status: current stream is not fp16 but op support fp16 and to_fp16 is
    // true
    if (! stream_is_fp16 && fp16_op_creator != nullptr && const_to_fp16) {
      auto cast_fp16_node = bubble::op(
        input.bubble().name() + "_cast_fp16", name::layer::cast(), { input });
    ...
    // status: current stream is fp16 but op doesn't support fp16
      ...
      auto cast_fp32_node = bubble::op(
        input.bubble().name() + "_cast_fp32", name::layer::cast(), { input });
      Tensor dtype = tensor::from<int>(DTYPE::FLOAT32);
      cast_fp32_node.bubble().set(name::dtype, dtype);
