...
static bool has_defined_op(
  const ts::ComputingDevice &device, const std::string &op) {
  auto creator = ts::OperatorCreator::Query(device.type(), op, true);
  return creator != nullptr;
}

bool ts::PackTranslatorOption::translate(
  ...
  if (
    op_name != name::layer::conv2d() && op_name != name::layer::conv2d_v2()
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu::math<TYPE, TYPE>::matrix_transpose(                                   \
      A_tensor.data<TYPE>(), transposed.data<TYPE>(), shape [0], shape [1]);   \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(FLOAT32, float);
#undef DECLARE_COMPUTE_RUN
        default: {
          TS_LOG_ERROR << "Pack translator not support data type(" << dtype
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu::math<TYPE, TYPE>::matrix_transpose(                                   \
      B_tensor.data<TYPE>(), transposed.data<TYPE>(), shape [0], shape [1]);   \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(FLOAT32, float);
#undef DECLARE_COMPUTE_RUN
    ...
    auto inner_prod_node = bubble::op(
      name + "_inner_prod",
      name::layer::inner_prod(),
      { A_trans_node, B_trans_node });

    auto alpha_node = bubble::data(
      name + "_alpha", node.bubble().get(name::alpha));
    auto alpha_mul_node = bubble::op(
      name + "_alpha_mul", name::layer::mul(), { alpha_node, inner_prod_node });

    auto beta_node = bubble::data(
      name + "_beta", node.bubble().get(name::beta));
    auto beta_mul_node = bubble::op(
      name + "_beta_mul", name::layer::mul(), { beta_node, C_node });

    if (ts::near(beta, float(0))) {
      if (ts::near(alpha, float(1))) translated_node = inner_prod_node;
      else
        translated_node = alpha_mul_node;
    } else {
      if (ts::near(alpha, float(1))) {
        auto add_node = bubble::op(
          name + "_add",
          name::layer::add(),
          { inner_prod_node, beta_mul_node });
        translated_node = add_node;
      } else {
        auto add_node = bubble::op(
          name + "_add", name::layer::add(), { alpha_mul_node, beta_mul_node });
        translated_node = add_node;
      }
    }
  ...
  if (
    op_name == name::layer::conv2d() || op_name == name::layer::inner_prod()) {
  ...
  if (parser.get("--winograd")) {
    if (
      op_name == name::layer::conv2d() || op_name == name::layer::conv2d_v2()) {
      Tensor stride_tensor = tensor::cast(
        INT32, node.bubble().get(name::stride));
      Stride2D stride_size(
        stride_tensor.data<int>() [2], stride_tensor.data<int>() [3]);
      Tensor dilation_tensor = tensor::cast(
        INT32, node.bubble().get(name::dilation));
      Dilation2D dilation_size(
        dilation_tensor.data<int>() [2], dilation_tensor.data<int>() [3]);
      bool winograd_flag = false;
      winograd_flag      = KernelCommonFunc<float>::winograd_check(
        kernel_shape, stride_size, dilation_size);
      if (winograd_flag) {
        Node::Link(translated_node, node.inputs());
        return true;
      }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu::math<TYPE, TYPE>::pack8_A(                                            \
      kernel_size_height,                                                      \
      kernel_size_width,                                                       \
      kernel_tensor.data<TYPE>(),                                              \
      kernel_size_width,                                                       \
      kernel_packed.data<TYPE>());                                             \
    break;                                                                     \
  }
      DECLARE_COMPUTE_RUN(FLOAT32, float);
#undef DECLARE_COMPUTE_RUN
      default: {
        TS_LOG_ERROR << "Pack translator not support data type(" << kernel_type
                     << "): " << type_str(kernel_type) << eject;
        break;
      }
    }
  } else {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    if (need_transpose) {                                                      \
      ...
      cpu::math<TYPE, TYPE>::matrix_transpose(                                 \
        kernel_tensor.data<TYPE>(),                                            \
        transposed.data<TYPE>(),                                               \
        kernel_size_height,                                                    \
        kernel_size_width);                                                    \
      cpu::math<TYPE, TYPE>::pack8_B(                                          \
        kernel_size_width,                                                     \
        kernel_size_height,                                                    \
        transposed.data<TYPE>(),                                               \
        kernel_size_height,                                                    \
        packed_transposed.data<TYPE>());                                       \
