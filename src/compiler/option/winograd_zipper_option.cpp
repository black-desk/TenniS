...
namespace ts
{
  bool Conv2dZipperOption::zip(
    const ComputingDevice &device, Node node, Node &zipped_node) const {
    ...
    if (format != name::NCHW) return false;
    ...
    for (size_t i = 0; i < 4; ++i) stride4 [i] = stride_tensor.data<int32_t>(i);
