...
namespace ts
{
  Translator::Translator(const ComputingDevice &device) : m_device(device) {}

  static Node translate_node(
    const Node                            &node,
    ...
    if (ready_it != ready_map.end()) { return ready_it->second; }

    auto translated_node = node;
    for (auto option: options) {
      if (option->translate(
            device, node, translated_node, params, output_flag)) {
        break;
      }
    }
    ...
    for (auto &input: input_nodes) {
      auto translated_input = translate_node(
        input, ready_map, device, options, params, false);
  ...
  Module::shared Translator::translate(const Module::shared &module) const {
    ...
    // std::endl; plot_graph(std::cout, module->outputs());
    ...
    for (auto &option: m_options) { options.push_back(option); }
    ...
    for (auto &node: output_nodes) {
      auto translated_node = translate_node(
        node, ready_map, m_device, options, m_params, true);
      traslated_nodes.emplace_back(translated_node);
    }

    // std::cout << "+++++++++++++++++ translated graph ++++++++++++++++++++++"
    // << std::endl; plot_graph(std::cout, traslated_nodes);
  ...
  Translator::~Translator() {
    for (auto &option: m_options) { delete option; }
    m_options.clear();
  }
}  // namespace ts