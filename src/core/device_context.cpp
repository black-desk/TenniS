...
namespace ts
{
  DeviceContext::~DeviceContext() { this->finalize(); }
  ...
  void DeviceContext::initialize(ComputingDevice computing_device) {
    ...
    if (m_device_admin != nullptr) {
      m_device_admin(
        &this->handle, computing_device.id(), DeviceAdmin::INITIALIZATION);
    }
  }
  ...
  void DeviceContext::finalize() {
    if (m_device_admin != nullptr && this->handle != nullptr) {
      m_device_admin(
        &this->handle, computing_device.id(), DeviceAdmin::FINALIZATION);
      this->handle = nullptr;
    }
  }
  ...
  void DeviceContext::active() {
    if (m_device_admin != nullptr && this->handle != nullptr) {
      m_device_admin(
        &this->handle, computing_device.id(), DeviceAdmin::ACTIVATION);
    }
  }
  ...
  void DeviceContext::synchronize() {
    if (m_device_admin != nullptr && this->handle != nullptr) {
      m_device_admin(
        &this->handle, computing_device.id(), DeviceAdmin::SYNCHRONIZE);
    }
  }
