...
namespace ts
{
  HardMemory::HardMemory(const MemoryDevice &device) : m_device(device) {
  ...
  HardMemory::HardMemory(
    const MemoryDevice &device, const HardAllocator::function &allocator) :
  ...
  HardMemory::HardMemory(
    ...
    const HardAllocator::function &allocator,
    size_t                         size) :
  ...
  void HardMemory::expect(size_t size) {
    if (! m_allocator)
      TS_LOG_ERROR("Borrowed memory can not be expected.") << eject;
    if (size > m_capacity) {
  ...
  void HardMemory::shrink(size_t size) {
    if (! m_allocator)
      TS_LOG_ERROR("Borrowed memory can not be shrunk.") << eject;
    if (size < m_capacity) {
      m_data     = m_allocator(m_device.id(), size, m_data, m_capacity);
      m_capacity = size;
    }
  ...
  void HardMemory::resize(size_t size) {
    if (! m_allocator)
      TS_LOG_ERROR("Borrowed memory can not be resized.") << eject;
    if (size != m_capacity) {
  ...
  HardMemory::HardMemory(self &&other) TS_NOEXCEPT { *this = std::move(other); }
