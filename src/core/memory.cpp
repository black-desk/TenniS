...
namespace ts
{
  ...
  Memory::Memory(HardMemory::shared &&hard, size_t size, size_t shift) :
    m_hard(std::move(hard)),
    m_size(size),
    m_shift(shift),
    m_usage(default_usage()) {}

  Memory::Memory(const MemoryDevice &device, size_t size) :
    m_hard(new HardMemory(device, size)),
    m_size(size),
    m_shift(0),
    m_usage(default_usage()) {}

  Memory::Memory(size_t size) :
    m_hard(new HardMemory(MemoryDevice(), size)),
    m_size(size),
    m_shift(0),
    m_usage(default_usage()) {}
  ...
  Memory::Memory(Memory::self &&other) TS_NOEXCEPT { *this = std::move(other); }
...
#undef MOVE_MEMBER
  ...
  long Memory::use_count() const { return m_usage.use_count(); }

  const MemoryDevice &Memory::device() const { return this->m_hard->device(); }
  ...
  void memcpy(Memory &dst, const Memory &src, size_t size) {
    ...
    HardConverter::function converter = HardConverter::Query(
      dst.device().type(), src.device().type());
    TS_AUTO_CHECK(converter != nullptr);
    converter(
      dst.device().id(), dst.data(), src.device().id(), src.data(), size);
  }
  ...
  size_t memcpy(
    ...
    HardConverter::function converter = HardConverter::Query(
      dst_device.type(), src_device.type());
    TS_AUTO_CHECK(converter != nullptr);
  ...
  void memset(
    ...
    const Device &src_device,
    size_t        src_size) {
    HardConverter::function cross_device_converter = HardConverter::Query(
      dst_device.type(), src_device.type());
    TS_AUTO_CHECK(cross_device_converter != nullptr);
    HardConverter::function in_device_converter = (dst_device == src_device) ?
                                                    cross_device_converter :
                                                    HardConverter::Query(
                                                      dst_device.type(),
                                                      dst_device.type());
    ...
    cross_device_converter(
      ...
      src_device.id(),
      src_ptr,
      std::min(dst_size, src_size));
    size_t copy_anchor = src_size;
    while (copy_anchor <= size_t(dst_size >> 1)) {
      in_device_converter(
        dst_device.id(),
        reinterpret_cast<char *>(dst_ptr) + copy_anchor,
        dst_device.id(),
        dst_ptr,
        copy_anchor);
      copy_anchor <<= 1;
