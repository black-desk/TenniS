...
namespace ts
{
  struct EmptyMemoryKeeper {
    public:
      static void *Allocator(
        int id, size_t new_size, void *mem, size_t mem_size) {
        ...
        if (new_mem == nullptr)
          throw OutOfMemoryException(MemoryDevice(CPU, id), new_size);
        return new_mem;
      }

      EmptyMemoryKeeper() {
        Tensor::Prototype proto(VOID, Shape());
        auto              memory = std::make_shared<HardMemory>(
          CPU, Allocator, proto.type_bytes() * proto.count());
      ...
      Smart<SyncMemory> &get() { return *object; }
    private:
  ...
  Tensor::Tensor(
    MemoryController::shared controller, DTYPE dtype, const Shape &_shape) :
    Tensor(controller, Prototype(dtype, _shape)) {}

  Tensor::Tensor(
    SyncMemoryController::shared controller, DTYPE dtype, const Shape &_shape) :
    Tensor(controller, Prototype(dtype, _shape)) {}
  ...
  Tensor::Tensor(
    MemoryController::shared controller, const Tensor::Prototype &proto) :
    m_memory(
      is_empty(proto) ?
        empty_memory() :
        Smart<TensorMemory>(controller->alloc(
          static_cast<size_t>(proto.count() * proto.type_bytes())))),
    m_proto(proto) {}

  Tensor::Tensor(
    SyncMemoryController::shared controller, const Tensor::Prototype &proto) :
    m_memory(
      is_empty(proto) ? empty_memory() :
                        controller->alloc(static_cast<size_t>(
                          proto.count() * proto.type_bytes()))),
    m_proto(proto) {}

  Tensor::Tensor(
    SyncMemoryController::shared controller,
    const Tensor::Prototype     &proto,
    const MemoryDevice          &device) :
    m_memory(
      is_empty(proto) ?
        empty_memory(device) :
        controller->alloc(
          device, static_cast<size_t>(proto.count() * proto.type_bytes()))),
    m_proto(proto) {}
  ...
  Tensor::Tensor(const Tensor::Prototype &proto) :
    m_memory(
      is_empty(proto) ? empty_memory() :
                        Smart<TensorMemory>(static_cast<size_t>(
                          proto.count() * proto.type_bytes()))),
    m_proto(proto) {}

  Tensor::Tensor(const Memory &memory, const Tensor::Prototype &proto) :
    m_memory(memory), m_proto(proto) {}

  Tensor::Tensor(const SyncMemory &memory, const Tensor::Prototype &proto) :
    m_memory(memory), m_proto(proto) {}

  Tensor::Tensor(
    const Smart<TensorMemory> &memory, const Tensor::Prototype &proto) :
    m_memory(memory), m_proto(proto) {}

  Tensor Tensor::clone() const {
    std::shared_ptr<MemoryController> controller =
      std::make_shared<DynamicMemoryController>(this->device());
  ...
  Tensor Tensor::clone(MemoryController::shared controller) const {
    ...
    for (auto &value: fields) {
      ...
      memcpy(
        dst, src, size_t(value.m_proto.count() * value.m_proto.type_bytes()));
      value = dolly;
    }
    Tensor dolly;
    dolly.pack(fields);
  ...
  Tensor Tensor::clone(
    SyncMemoryController::shared controller, const MemoryDevice &device) const {
    auto fields = this->unpack();
    for (auto &value: fields) {
  ...
  Tensor::shared Tensor::clone_shared(
    MemoryController::shared controller) const {
  ...
  Tensor::shared Tensor::clone_shared(
    SyncMemoryController::shared controller) const {
    return std::make_shared<Tensor>(this->clone(std::move(controller)));
  }
  ...
  Tensor::Tensor() : Tensor(VOID, Shape()) {}

  bool Tensor::empty() const { return dtype() == VOID && dims() == 0; }
  ...
  Tensor Tensor::reshape(const Shape &shape) const {
    ...
    for (size_t i = 0; i < fixed_shape.size(); ++i) {
      if (fixed_shape [i] < 0) {
        if (fixed_index >= 0) {
          TS_LOG_ERROR << "Can not reshape " << to_string(this->sizes())
                       << " to " << to_string(shape) << eject;
        }
        fixed_shape [i] = -1;
    ...
    if (fixed_index >= 0) {
      auto up = std::accumulate(
        this->sizes().begin(), this->sizes().end(), 1, std::multiplies<int>());
      auto down = std::accumulate(
        fixed_shape.begin(), fixed_shape.end(), 1, std::multiplies<int>());
      fixed_shape [fixed_index] = up / -down;
    }

    Prototype proto(this->dtype(), fixed_shape);
    if (proto.count() != this->count()) {
      TS_LOG_ERROR << "Can not reshape " << to_string(this->sizes()) << " to "
                   << to_string(shape) << eject;
    }
    Tensor t  = *this;
  ...
  Tensor Tensor::field(size_t offset) const {
    if (offset == 0) { return Tensor(m_memory, m_proto); }
    if (offset - 1 >= m_fields.size()) {
      TS_LOG_ERROR << "Tensor offset output range error. Access index "
                   << offset << " in range(" << fields_count() << ")" << eject;
    }
  ...
  size_t Tensor::fields_count() const { return 1 + m_fields.size(); }

  bool Tensor::packed() const { return ! m_fields.empty(); }
  ...
  static size_t serialize_prototype_memory(
    StreamWriter            &stream,
    const Tensor::Prototype &proto,
    const Memory            &memory) {
    ...
    // 1.2 write size
    writen_size += binio::write<uint32_t>(
      stream, uint32_t(proto.sizes().size()));
    ...
    // 2. write memory
    writen_size += binio::write<char>(
      stream,
      cpu_memory.data<char>(),
      size_t(proto.count()) * proto.type_bytes());
    return writen_size;
  }

  static size_t externalize_prototype_memory(
    StreamReader &stream, Tensor::Prototype &proto, Memory &memory) {
    std::unique_ptr<MemoryController> may_controller(
      new DynamicMemoryController(memory.device()));
  ...
  size_t Tensor::serialize(StreamWriter &stream) const {
    size_t writen_size = 0;
    writen_size += binio::write<uint32_t>(
      stream, uint32_t(this->fields_count()));
    for (auto &tensor: this->unpack()) {
      auto cpu_memory = tensor.m_memory->view(MemoryDevice(CPU)).weak_memory();
      writen_size += serialize_prototype_memory(
        stream, tensor.m_proto, cpu_memory);
    }
    return writen_size;
  }
  ...
  size_t Tensor::externalize(StreamReader &stream) {
    ...
    for (auto &tensor: fields) {
      Memory cpu_memory;
      read_size += externalize_prototype_memory(
        stream, tensor.m_proto, cpu_memory);
  ...
  bool Tensor::has_empty_shape() const { return this->sizes().empty(); }
  ...
  static inline std::vector<int> flatten_shape(const Tensor &x, int m_dim) {
    ...
    if (need_size < x_size) {
      ...
      shape.back() = std::accumulate(
        size.begin() + m_dim, size.end(), 1, std::multiplies<int>());
...
#define FAIL_ARG(i)  (arg##i >= 0 && this_shape [i] != arg##i)
  ...
  bool Tensor::has_shape(int arg0, int arg1, int arg2, int arg3) const {
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(4) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3));
  }

  bool Tensor::has_shape(
    int arg0, int arg1, int arg2, int arg3, int arg4) const {
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(5) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4));
  }

  bool Tensor::has_shape(
    int arg0, int arg1, int arg2, int arg3, int arg4, int arg5) const {
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(6) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4) || FAIL_ARG(5));
  }
  ...
  bool Tensor::has_shape(
    ...
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(7) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4) || FAIL_ARG(5) || FAIL_ARG(6));
  }
  ...
  bool Tensor::has_shape(
    ...
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(8) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4) || FAIL_ARG(5) || FAIL_ARG(6) || FAIL_ARG(7));
  }
  ...
  bool Tensor::has_shape(
    ...
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(9) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4) || FAIL_ARG(5) || FAIL_ARG(6) || FAIL_ARG(7)
      || FAIL_ARG(8));
  }
  ...
  bool Tensor::has_shape(
    ...
    auto &this_shape = this->sizes();
    return ! (
      FAIL_SIZE(10) || FAIL_ARG(0) || FAIL_ARG(1) || FAIL_ARG(2) || FAIL_ARG(3)
      || FAIL_ARG(4) || FAIL_ARG(5) || FAIL_ARG(6) || FAIL_ARG(7) || FAIL_ARG(8)
      || FAIL_ARG(9));
  }

  Memory Tensor::weak_memory() const { return m_memory->weak_memory(); }

  Tensor::Tensor(
    Tensor::InFlow           in_flow,
    const Tensor::Prototype &proto,
    const MemoryDevice      &device) {
    switch (in_flow) {
      case InFlow::HOST: {
        ...
        if (flow) {
          *this = Tensor(flow, proto, MemoryDevice(CPU));
          ...
          throw Exception(
            std::string("Not flow binding in context: <")
            + typeid(RuntimeContext).name() + ">");
        }
        break;
      }
      case InFlow::DEVICE: {
  ...
  Tensor Tensor::slice(int i) {
    ...
    auto  width = std::accumulate(
      sizes.begin() + 1, sizes.end(), 1, std::multiplies<int>());
  ...
  Tensor Tensor::slice(int beg, int end) {
    ...
    slice_shape [0]   = batch;
    Memory slice_memory(
      this->device(), this->data<char>() + beg * width, batch * width);
    return Tensor(slice_memory, Tensor::Prototype(this->dtype(), slice_shape));
  }
...
#undef FAIL_SIZE
  ...
  TensorPrototype::supper TensorPrototype::field(size_t offset) const {
    if (offset == 0) { return self(dtype(), sizes()); }
    if (offset - 1 >= m_fields.size()) {
      TS_LOG_ERROR << "Tensor offset output range error. Access index "
                   << offset << " in range(" << fields_count() << ")" << eject;
    }
  ...
  void TensorPrototype::field(
    size_t offset, const TensorPrototype::supper &value) {
    if (offset == 0) {
  ...
  void TensorPrototype::pack(
    const std::vector<TensorPrototype::supper> &fields) {
    if (fields.empty()) {
      *this = self();
      return;
    }
  ...
  size_t TensorPrototype::fields_count() const { return 1 + m_fields.size(); }

  bool TensorPrototype::packed() const { return ! m_fields.empty(); }
  ...
  TensorPrototype::TensorPrototype(
    const std::vector<Tensor::Prototype> &fields) :
    supper() {
    this->pack(fields);
  }
  ...
  void TensorPrototype::field(
    int offset, const TensorPrototype::supper &value) {
    return field(
      size_t(offset >= 0 ? offset : int(fields_count()) + offset), value);
  }
  ...
  bool operator==(const Tensor::Prototype &lhs, const TensorPrototype &rhs) {
    return rhs.fields_count() == 1 && lhs.dtype() == rhs.dtype()
        && lhs.sizes() == rhs.sizes();
  }

  bool operator==(const TensorPrototype &lhs, const Tensor::Prototype &rhs) {
    return lhs.fields_count() == 1 && lhs.dtype() == rhs.dtype()
        && lhs.sizes() == rhs.sizes();
  }
}  // namespace ts