...
namespace ts
{
  namespace tensor
  {
    ...
    class type_cast_template {
      public:
        static void cast(
          typename dtype<DTYPE_DST>::declare       *dst,
          const typename dtype<DTYPE_SRC>::declare *src,
          size_t                                    size) {
          size_t i = 0;
          for (; i + 4 <= size; i += 4) {
            *dst = static_cast<typename dtype<DTYPE_DST>::declare>(*src);
            ++dst;
            ++src;
            *dst = static_cast<typename dtype<DTYPE_DST>::declare>(*src);
            ++dst;
    ...
    class type_cast_template<SAME_DTYPE, SAME_DTYPE> {
      public:
        static void cast(
          typename dtype<SAME_DTYPE>::declare       *dst,
          const typename dtype<SAME_DTYPE>::declare *src,
          size_t                                     size) {
          std::memcpy(
            dst, src, size * sizeof(typename dtype<SAME_DTYPE>::declare));
        };
    };

    template <DTYPE DTYPE_SRC>
    static void type_cast_to(
      void                                     *dst,
      DTYPE                                     dst_dtype,
      const typename dtype<DTYPE_SRC>::declare *src,
      size_t                                    size) {
      switch (dst_dtype) {
        default:
          TS_LOG_ERROR << "Can not convert dtype " << type_str(DTYPE_SRC)
                       << " to " << type_str(dst_dtype) << eject;
          break;
#define __CASE_TYPE_CALL_TYPE_CAST(__type__)                                   \
  case __type__:                                                               \
    type_cast_template<__type__, DTYPE_SRC>::cast(                             \
      reinterpret_cast<typename dtype<__type__>::declare *>(dst), src, size);  \
    break;
...
#undef __CASE_TYPE_CALL_TYPE_CAST
    ...
    static void type_cast_to_from(
      ...
      switch (src_dtype) {
        default:
          TS_LOG_ERROR << "Can not convert dtype " << type_str(src_dtype)
                       << " to " << type_str(dst_dtype) << eject;
          break;
#define __CASE_TYPE_CALL_TYPE_CAST_TO(__type__)                                \
  case __type__:                                                               \
    type_cast_to<__type__>(                                                    \
      dst,                                                                     \
      dst_dtype,                                                               \
      reinterpret_cast<const dtype<__type__>::declare *>(src),                 \
      size);                                                                   \
    break;
...
#undef __CASE_TYPE_CALL_TYPE_CAST_TO
    ...
    Tensor cast(DTYPE dtype, const Tensor &value) {
      ...
      auto cpu_controller = std::make_shared<DynamicMemoryController>(
        MemoryDevice(CPU));
      ...
      if (
        unsupported_types.find(dtype) != unsupported_types.end()
        || unsupported_types.find(cpu_value.dtype())
             != unsupported_types.end()) {
        TS_LOG_ERROR << "Can not convert dtype " << type_str(cpu_value.dtype())
                     << " to " << type_str(dtype) << eject;
      }
    ...
    int to_int(const Tensor &value) {
      ...
      if (value.count() == 0)
        TS_LOG_ERROR("Can not convert empty tensor to int") << eject;
    ...
    unsigned int to_uint(const Tensor &value) {
      if (value.dtype() == CHAR8) {
        try {
          return (
            unsigned int)(std::strtoul(to_string(value).c_str(), nullptr, 10));
        } catch (const Exception &) {}
      }
      if (value.count() == 0)
        TS_LOG_ERROR("Can not convert empty tensor to int") << eject;
    ...
    bool to_bool(const Tensor &value) {
      ...
      if (value.count() == 0)
        TS_LOG_ERROR("Can not convert empty tensor to bool") << eject;
...
#undef __CASE_TYPE_SUPPORTED
    ...
    void save(const std::string &filename, const Tensor &tensor) {
      ...
      if (! stream.is_open())
        TS_LOG_ERROR << "Can not access " << filename << eject;
