...
namespace ts
{
  bool AESFileStreamReader::is_open() const { return m_stream.is_open(); }
  ...
  size_t AESFileStreamReader::read(void *buffer, size_t size) {
    ...
    if (m_master_datalen - m_master_offset > 0) {
      memcpy(
        buffer, m_master + m_master_offset, m_master_datalen - m_master_offset);
    ...
    while (buffer_offset < size) {
      ...
      if (iseof) { m_master_datalen -= (int)(m_master [AES_BLOCKLEN - 1]); }
  ...
  void AESFileStreamReader::close() { m_stream.close(); }

  AESFileStreamReader::~AESFileStreamReader() { close(); }

  ////////////////////////////////////////////////////////////
  bool AESFileStreamWriter::is_open() const { return m_stream.is_open(); }
  ...
  size_t AESFileStreamWriter::write(const void *buffer, size_t size) {
    ...
    while (nwrite < size) {
      if (AES_BLOCKLEN - m_master_datalen >= size - nwrite) {
        memcpy(
          m_master + m_master_datalen,
          reinterpret_cast<const char *>(buffer) + nwrite,
          size - nwrite);
  ...
  AESFileStreamWriter::~AESFileStreamWriter() { close(); }
