...
namespace ts
{
  ...
  class EncryptedFileStreamReader::Implement {
    public:
      Implement(const std::string &path, const std::string &key) :
        stream(path, key) {}
  ...
  void EncryptedFileStreamReader::close() { m_impl->stream.close(); }
  ...
  void EncryptedFileStreamWriter::close() { m_impl->stream.close(); }
