...
namespace ts
{
  namespace desc
  {
    ...
    Bubble add() { return Bubble(name::layer::add(), name::layer::add()); }

    Bubble sub() { return Bubble(name::layer::sub(), name::layer::sub()); }

    Bubble mul() { return Bubble(name::layer::mul(), name::layer::mul()); }

    Bubble div() { return Bubble(name::layer::div(), name::layer::div()); }
    ...
    Bubble affine_sample2d(int32_t dim, float outer_value, ResizeType type) {
      Bubble bubble(
        name::layer::affine_sample2d(), name::layer::affine_sample2d());
      bubble.set(name::dim, tensor::from<int32_t>(dim));
    ...
    Bubble broadcast() { return Bubble("broadcast", "broadcast"); }
  }  // namespace desc
}  // namespace ts
