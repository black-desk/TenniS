...
namespace ts
{
  namespace frontend
  {
    NodeOrTensor::NodeOrTensor(const Node &node) : m_node(node) {}
    ...
    NodeOrTensor::NodeOrTensor(
      const Tensor &tensor, const MemoryDevice &device) :
      m_node(bubble::data("", tensor, device.type())) {}

    NodeOrTensor::operator Node() const { return m_node; }
    ...
    Node concat(
      const std::string               &name,
      const std::vector<NodeOrTensor> &x,
      int32_t                          dim) {
    ...
    Node affine_sample2d(
      ...
      float               outer_value,
      desc::ResizeType    type) {
      return symbol::affine_sample2d(
        name, x, size, affine, dim, outer_value, type);
    }
