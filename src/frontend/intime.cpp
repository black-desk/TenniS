...
namespace ts
{
  namespace intime
  {
    Tensor run(
      ...
      if (fields_count == 1) { return stack [0]; }
      std::vector<Tensor> fields(fields_count);
      for (int i = 0; i < fields_count; ++i) { fields [i] = stack [i]; }
      Tensor output;
      output.pack(fields);
    ...
    Tensor resize2d(
      const Tensor &x, const Tensor &size, desc::ResizeType type) {
    ...
    Tensor sigmoid(const Tensor &x) { return run(desc::sigmoid(), { x }); }
    ...
    Tensor gather(
      const Tensor &x, const std::vector<int32_t> &indices, int32_t axis) {
    ...
    Tensor affine_sample2d(
      ...
      desc::ResizeType type) {
      return run(
        desc::affine_sample2d(dim, outer_value, type), { x, size, affine });
    }
    ...
    int64_t memcpy(
      ...
      auto copied = memcpy(
        reinterpret_cast<char *>(dst_data) + dst_shift,
        dst_desc.device(),
        size_t(size),
        reinterpret_cast<const char *>(src_data) + src_shift,
        src_desc.device(),
        size_t(size));
