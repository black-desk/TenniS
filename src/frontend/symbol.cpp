...
namespace ts
{
  namespace symbol
  {
    ...
    Node concat(
      const std::string &name, const std::vector<Node> &x, int32_t dim) {
    ...
    Node softmax(
      const std::string &name, const Node &x, int32_t dim, bool smooth) {
    ...
    Node affine_sample2d(
      ...
      float              outer_value,
      desc::ResizeType   type) {
      Node node = bubble::bubble(
        desc::affine_sample2d(dim, outer_value, type), name);
    ...
    Node matmul(
      const std::string &name, const Node &A, const Node &B, bool transpose) {
