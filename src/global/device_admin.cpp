...
namespace ts
{
  ...
  DeviceAdmin::function DeviceAdmin::Query(const DeviceType &device_type)
    TS_NOEXCEPT {
  ...
  void DeviceAdmin::Register(
    const DeviceType &device_type, const function &device_admin) TS_NOEXCEPT {
    auto &map_device_type_admin         = MapDeviceTypeAdmin();
