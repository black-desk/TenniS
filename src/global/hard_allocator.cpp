...
namespace ts
{
  ...
  HardAllocator::function HardAllocator::Query(const DeviceType &device_type)
    TS_NOEXCEPT {
  ...
  void HardAllocator::RegisterV3(
    const DeviceType              &device_type,
    const HardAllocator::_malloc  &_new,
  ...
  void HardAllocator::RegisterV2(
    const DeviceType             &device_type,
    const HardAllocator::_malloc &_new,
    const HardAllocator::_free   &_delete) TS_NOEXCEPT {
    auto _converter = HardConverter::Query(device_type, device_type);
    auto _reset =
      [_new, _delete, _converter](
        int id, size_t new_size, void *mem, size_t mem_size) -> void * {
  ...
  HardAllocator::function HardAllocator::Bind(
    const HardAllocator::_malloc  &_new,
    const HardAllocator::_free    &_delete,
    const HardAllocator::_realloc &_reset) {
    return [_new, _delete, _reset](
             int id, size_t new_size, void *mem, size_t mem_size) -> void * {
      if (new_size == 0) {
        _delete(id, mem);
        return nullptr;
  ...
  std::set<std::string> HardAllocator::AllKeys() TS_NOEXCEPT {
    ...
    for (auto &key_value: map_key_values) {
      keys.insert(key_value.first.std());
    }
    return keys;
  }
}  // namespace ts
