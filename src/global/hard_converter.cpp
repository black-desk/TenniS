...
namespace ts
{
  static std::map<DeviceType, std::map<DeviceType, HardConverter::function>>
    &MapDstSrcConverter() {
    static std::map<DeviceType, std::map<DeviceType, HardConverter::function>>
      map_dst_src_converter;
    return map_dst_src_converter;
  };

  HardConverter::function HardConverter::Query(
    DeviceType dst_device_type, DeviceType src_device_type) TS_NOEXCEPT {
  ...
  void HardConverter::Register(
    ...
    if (dst_src_converter != map_dst_src_converter.end()) {
      ...
      map_dst_src_converter.insert(
        std::make_pair(dst_device_type, std::move(map_src_converter)));
    }
  }
  ...
  std::set<std::pair<std::string, std::string>> HardConverter::AllKeys()
    TS_NOEXCEPT {
    auto &map_dst_src_converter = MapDstSrcConverter();
