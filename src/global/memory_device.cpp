...
namespace ts
{
  ...
  std::set<std::pair<std::string, std::string>> ComputingMemory::AllItems()
    TS_NOEXCEPT {
    auto &map_memory_device = MapMemoryDevice();
    std::set<std::pair<std::string, std::string>> items;
    for (auto &pair_compute_memory: map_memory_device) {
      items.insert(std::make_pair(
        pair_compute_memory.first.std(), pair_compute_memory.second.std()));
    }
    return items;
  }
}  // namespace ts