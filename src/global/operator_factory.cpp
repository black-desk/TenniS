...
namespace ts
{
  ...
  OperatorCreator::function OperatorCreator::Query(
    ...
    if (name_creator != map_name_creator.end()) { return name_creator->second; }
    return OperatorCreator::function(nullptr);
  }

  void OperatorCreator::Register(
    const DeviceType                &device_type,
  ...
  void OperatorCreator::flush(
    const OperatorCreator::CreatorFucMap &creator_map) {
    auto &map_name_creator = MapNameCreator();
  ...
  static OperatorCreator::function TalentQuery(
    const DeviceType  &device_type,
    const std::string &operator_name,
    bool               strict) {
    ...
    if (creator == nullptr) {
      // step 2.x: try find operator on memory device, if computing device
      // failed
  ...
  Operator::shared OperatorCreator::CreateNoException(
    const DeviceType  &device_type,
    const std::string &operator_name,
  ...
  Operator::shared OperatorCreator::Create(
    const DeviceType  &device_type,
    const std::string &operator_name,
    bool               strict) {
    auto op = CreateNoException(device_type, operator_name, strict);
    if (op == nullptr)
      throw OperatorNotFoundException(device_type, operator_name);
    return op;
  }

  std::set<std::pair<std::string, std::string>> OperatorCreator::AllKeys()
    TS_NOEXCEPT {
    auto &map_name_creator = MapNameCreator();
