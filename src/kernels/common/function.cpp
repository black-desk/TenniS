...
namespace ts
{
  ...
  template <typename T>
  void KernelCommonFunc<T>::in_out_pad_and_fix_size(
    const Tensor    &input,
    ...
    // input_padded_heigh = std::max(input_padded_heigh, (out_padded_height - 1)
    // * stride.h + (ksize.height - 1) * dilation.h + 1);
    input_padded_height = std::max(
      input_padded_height,
      (out_padded_height - 1) * stride.height + ksize.height);
    input_padded_width = std::max(
      input_padded_width, (out_padded_width - 1) * stride.width + ksize.width);
    // input_padded_height = std::max(input_padded_height, out_padded_height +
    // 2); input_padded_width = std::max(input_padded_width, out_padded_width +
    // 2);

    Padding2D fixed_input_pad;
    // fixed_input_pad.top = (input_padded_height - input_height) >> 1;
    fixed_input_pad.top    = padding.top;
    fixed_input_pad.bottom = input_padded_height - input_height
                           - fixed_input_pad.top;
    // fixed_input_pad.left = (input_padded_width - input_width) >> 1;
    fixed_input_pad.left  = padding.left;
    fixed_input_pad.right = input_padded_width - input_width
                          - fixed_input_pad.left;

    bool in_need_pad = input_padded_height != input_height
                    || input_padded_width != input_width;
    bool out_need_pad = out_padded_height != out_height
                     || out_padded_width != out_width;
    if (in_need_pad) {
      Tensor padded_input(
        Tensor::InFlow::HOST,
        input.dtype(),
        { num, input_channel, input_padded_height, input_padded_width });
      std::array<int, 2> input_pad_h = { fixed_input_pad.top,
                                         fixed_input_pad.bottom };
      std::array<int, 2> input_pad_w = { fixed_input_pad.left,
                                         fixed_input_pad.right };
      cpu::PadAlgorithm<T>::pad2d(
        input, input_pad_h, input_pad_w, (float)padding_value, padded_input);
    ...
    if (out_need_pad) {
      ...
      cpu::PadAlgorithm<T>::pad2d(
        out, out_pad_h, out_pad_w, (float)padding_value, padded_out);
  ...
  template <typename T>
  bool KernelCommonFunc<T>::winograd_check(
    const Shape &ksize, const Stride2D &stride, const Dilation2D &dilation) {
    ...
    if (
      kernel_height == 3 && kernel_width == 3 && stride.width == 1
      && stride.height == 1 && dilation.height == 1 && dilation.width == 1) {
      if (input_channel >= 32 && out_channel >= 32) { return true; }
    }
    return false;
  }

  template <typename T>
  bool KernelCommonFunc<T>::winograd_mode_select(
  ...
  template <typename T>
  bool KernelCommonFunc<T>::winograd_mode_select_on_arm(
    const Shape        &input_shape,
    const int           out_channels,
    WinogradConv2DMode &winograd_model) {
    int input_channels = input_shape [1];
