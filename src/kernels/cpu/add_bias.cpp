...
{
  template <typename T>
  void cpu_add_bias_compute_run(
    const Tensor &x, const Tensor &b, int dim, Tensor &out) {
    const Shape &shape     = x.sizes();
    int          pre_dims  = 1;
    int          back_dims = 1;
    for (int i = 0; i < dim; i++) { pre_dims *= shape [i]; }

    for (int i = dim + 1; i < shape.size(); i++) { back_dims *= shape [i]; }
    ...
    for (int i = 0; i < pre_dims; i++) {
      for (int k = 0; k < shape [dim]; k++) {
        ...
        for (int m = 0; m < back_dims; m++) { pdst [offset + m] += pbias [k]; }
      }
    }
  }

  template <>
  void cpu_add_bias_compute_run<float>(
    const Tensor &x, const Tensor &b, int dim, Tensor &out) {
    const Shape &shape     = x.sizes();
    int          pre_dims  = 1;
    int          back_dims = 1;
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_add_bias_compute_run<TYPE>(x, b, dim, out);                            \
    break;                                                                     \
  }
