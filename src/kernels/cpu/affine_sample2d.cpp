...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void affine_sample2d_linear(
      const Tensor         *x,
      Tensor               *y,
      int                   src_height,
      int                   src_width,
...
#  endif
      ...
      for (int n_y_d = 0; n_y_d < dst_height; n_y_d++) {
        for (int n_x_d = 0; n_x_d < dst_width; n_x_d++) {
          ...
          auto         location = transform<float>(
            rz00, rz01, rz02, rz10, rz11, rz12, rz20, rz21, rz22, cur);

          double lf_x_s = location.x;
          double lf_y_s = location.y;

          auto inner = lf_x_s >= 0 && lf_x_s < src_width - 1 && lf_y_s >= 0
                    && lf_y_s < src_height - 1;
              ...
              clamp<T, double>(
                ((1 - lf_weight_y) * (1 - lf_weight_x)
                 ...
                 + lf_weight_y * lf_weight_x
                     * src_im
                       [((n_y_s + 1) * src_width + n_x_s + 1) * channels + c]));
    ...
    template <typename T>
    static void affine_sample2d_nearest(
      const Tensor         *x,
      Tensor               *y,
      int                   src_height,
      int                   src_width,
...
#  endif
      ...
      for (int n_y_d = 0; n_y_d < dst_height; n_y_d++) {
        for (int n_x_d = 0; n_x_d < dst_width; n_x_d++) {
          ...
          auto inner = n_x_s >= 0 && n_x_s < src_width - 1 && n_y_s >= 0
                    && n_y_s < src_height - 1;
    ...
    template <typename T>
    static void affine_sample2d_hard(
      const Tensor         *x,
      Tensor               *y,
      int                   src_height,
      int                   src_width,
...
#  endif
    ...
    template <typename T>
    static void affine_sample2d_cubic(
      const Tensor         *x,
      Tensor               *y,
      int                   x_height,
      int                   x_width,
...
#  endif
      ...
      for (int m = 0; m < y_height; m++) {
        ...
        for (int n = 0; n < y_width; n++) {
          ...
          auto outter = sy < 1 || sy >= x_height - 3 || sx < 1
                     || sx >= x_width - 3;
          ...
          if (outter && outer_mode == base::AffineOuterMode::VALUE) {
            ...
            coeffsY [2] = ((A + 2) * (1 - fy) - (A + 3)) * (1 - fy) * (1 - fy)
                        + 1;
            ...
            for (int k = 0; k < channels; k++) {
              pdst [m * dstrows + n * channels + k] = clamp<T, double>(
                ((psrc [(sy - 1) * srcrows + (sx - 1) * channels + k]
                    * coeffsX [0] * coeffsY [0]
                  + psrc [(sy)*srcrows + (sx - 1) * channels + k] * coeffsX [0]
                      * coeffsY [1]
                  + psrc [(sy + 1) * srcrows + (sx - 1) * channels + k]
                      * coeffsX [0] * coeffsY [2]
                  + psrc [(sy + 2) * srcrows + (sx - 1) * channels + k]
                      * coeffsX [0] * coeffsY [3]
                  +

                  psrc [(sy - 1) * srcrows + (sx)*channels + k] * coeffsX [1]
                    * coeffsY [0]
                  + psrc [(sy)*srcrows + (sx)*channels + k] * coeffsX [1]
                      * coeffsY [1]
                  + psrc [(sy + 1) * srcrows + (sx)*channels + k] * coeffsX [1]
                      * coeffsY [2]
                  + psrc [(sy + 2) * srcrows + (sx)*channels + k] * coeffsX [1]
                      * coeffsY [3]
                  +

                  psrc [(sy - 1) * srcrows + (sx + 1) * channels + k]
                    * coeffsX [2] * coeffsY [0]
                  + psrc [(sy)*srcrows + (sx + 1) * channels + k] * coeffsX [2]
                      * coeffsY [1]
                  + psrc [(sy + 1) * srcrows + (sx + 1) * channels + k]
                      * coeffsX [2] * coeffsY [2]
                  + psrc [(sy + 2) * srcrows + (sx + 1) * channels + k]
                  ...
                  psrc [(sy - 1) * srcrows + (sx + 2) * channels + k]
                    * coeffsX [3] * coeffsY [0]
                  + psrc [(sy)*srcrows + (sx + 2) * channels + k] * coeffsX [3]
                      * coeffsY [1]
                  + psrc [(sy + 1) * srcrows + (sx + 2) * channels + k]
                      * coeffsX [3] * coeffsY [2]
                  + psrc [(sy + 2) * srcrows + (sx + 2) * channels + k]
                      * coeffsX [3] * coeffsY [3])));
            }
          }
        }
    ...
    template <typename T>
    static void batch_affine_sample2d(
      int                   number,
      const Tensor         *x,
      Tensor               *y,
      int                   x_height,
    ...
    void Affine_Sample2D::affine_sample_run(
      const Tensor         &x,
      float                 rz00,
      float                 rz01,
      float                 rz02,
      ...
      for (int k = 0; k < dim; k++) { number *= output_shape [k]; }
