...
namespace ts
{
  namespace cpu
  {
    namespace arm
    {
      ...
      template <typename T>
      void Conv2d3x3V2<T>::conv2d_3x3_s1(
        const Tensor    &x,
      ...
      template <>
      void Conv2d3x3V2<float>::conv2d_3x3_s1(
        const Tensor    &x,
        const Padding2D &padding,
        float            padding_value,
        const Tensor    &w,
...
#  pragma omp parallel for num_threads(openmp_threads())
          ...
          for (int m = 0; m < out_channel_bound; ++m) {
            ...
            for (int c = 0; c < input_channel; ++c) {
              ...
              // k02_x4(kernel_at_0 + 6); float32x4 k10_x4(kernel_at_1),
              // k11_x4(kernel_at_1 + 3), k12_x4(kernel_at_1 + 6);
