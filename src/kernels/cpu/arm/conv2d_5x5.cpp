...
namespace ts
{
  namespace cpu
  {
    namespace arm
    {
      ...
      template <typename T>
      void Conv2d5x5<T>::conv2d_5x5_s1(
        const Tensor    &x,
      ...
      //  4channel*1height*4width rather than 2c2h4w due to i don't know how to
      //  optimize pipeline when i use neon instruction
      template <>
      void Conv2d5x5<float>::conv2d_5x5_s1(
        const Tensor    &x,
        const Padding2D &padding,
        float            padding_value,
        const Tensor    &w,
...
#  pragma omp parallel for num_threads(openmp_threads())
          ...
          for (int m = 0; m < out_channel_bound; ++m) {
            ...
            for (int c = 0; c < input_channel; ++c) {
              const float *kernel_at = kernel_ptr + mm * input_channel * 25
                                     + c * 25;
              ...
              for (int h = 0; h < out_height; ++h) {
                for (int w = 0; w + 3 < out_width; w += 4) {
                  ...
                  for (int i = 0; i < 5; ++i) {
                    ...
                    float32x4 k00_x4(kernel_cur_0),
                      k00_x4_sup(kernel_cur_0 + 3);
                    float32x4 k10_x4(kernel_cur_1),
                      k10_x4_sup(kernel_cur_1 + 3);
                    float32x4 k20_x4(kernel_cur_2),
                      k20_x4_sup(kernel_cur_2 + 3);
                    float32x4 k30_x4(kernel_cur_3),
                      k30_x4_sup(kernel_cur_3 + 3);
...
#  pragma omp parallel for num_threads(openmp_threads())
          ...
          for (int m = remain_out_channel; m < out_channel; ++m) {
            ...
            for (int c = 0; c < input_channel; ++c) {
              const float *kernel_at = kernel_ptr + m * input_channel * 25
                                     + c * 25;
              const float *kernel_at_0 = kernel_at;
      ...
      template <typename T>
      void Conv2d5x5<T>::conv2d_5x5_s2(
        const Tensor    &x,
        const Padding2D &padding,
        float            padding_value,
        const Tensor    &w,
      ...
      template <>
      void Conv2d5x5<float>::conv2d_5x5_s2(
        const Tensor    &x,
        const Padding2D &padding,
        float            padding_value,
        const Tensor    &w,
              ...
              for (int h = 0; h < out_height; ++h) {
                for (int w = 0; w + 3 < out_width; w += 4) {
                  ...
                  for (int i = 0; i < 5; ++i) {
                    ...
                    float32x4x2 i0246_1357_sup = incx4x2_load(
                      input_cur_0 + 8, 2);
                    // float32x4 i00_x4 = i0246_1357[0];
                    // float32x4 i01_x4 = i0246_1357[1];
                    float32x4 i02_x4 = concat(
                      i0246_1357 [0], i0246_1357_sup [0], 1);
                    float32x4 i03_x4 = concat(
                      i0246_1357 [1], i0246_1357_sup [1], 1);
                    float32x4 i04_x4 = concat(
                      i0246_1357 [0], i0246_1357_sup [0], 2);
