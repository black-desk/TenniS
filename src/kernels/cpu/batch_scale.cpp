...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_batch_scale_compute_run(
      const Tensor &x,
      const Tensor &scale,
      const Tensor &bias,
      int           dim,
    ...
    template <>
    void cpu_batch_scale_compute_run<float>(
      const Tensor &x,
      const Tensor &scale,
      const Tensor &bias,
      int           dim,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_batch_scale_compute_run<TYPE>(x, mean, variance, dim, out);            \
    break;                                                                     \
  }
