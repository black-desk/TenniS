...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_batchtospace4d_compute_run(
      const Tensor &x,
      const int     crop_top,
      ...
      // int output_size = output_number * output_channels * output_height *
      // output_width; int output_number_step = output_channels * output_height
      // * output_width; int output_channels_step = output_height *
      // output_width; int output_height_step = output_width; int
      // output_width_step = 1;
      ...
      int at_output_i =
        0;  // n * output_number_step + c * output_channels_step;
        ...
        for (int c = 0; c < output_channels; ++c) {
          ...
          for (int h = 0; h < output_height; ++h) {
            ...
            for (int w = 0; w < output_width; ++w) {
              const int in = (pre_compute + (w + crop_left) % block_width)
                             * output_number
                           + n;
    ...
    void BatchToSpace4D::batchtospace4d_run(
      const Tensor &x,
      const int     crop_top,
      const int     crop_bottom,
      const int     crop_left,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_batchtospace4d_compute_run<TYPE>(                                      \
      x,                                                                       \
      crop_top,                                                                \
      crop_bottom,                                                             \
      crop_left,                                                               \
