...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static inline void cpu_broadcast_with_bias(
      const Tensor &x, Tensor &out, int dim) {
      auto px   = x.data<T>();
      auto pout = out.data<T>();
...
#define DECLARE_COMPUTE_RUN(WIDTH, TYPE)                                       \
  case WIDTH: {                                                                \
    cpu_broadcast_compute_run<TYPE>(x, out);                                   \
    break;                                                                     \
  }
...
#define DECLARE_COMPUTE_RUN(WIDTH, TYPE)                                       \
  case WIDTH: {                                                                \
    cpu_broadcast_with_bias<TYPE>(x, out, dim);                                \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(1, uint8_t)
        DECLARE_COMPUTE_RUN(2, uint16_t)
...
#define DECLARE_COMPUTE_RUN(WIDTH, TYPE)                                       \
  case WIDTH: {                                                                \
    cpu_broadcast_with_scalar<TYPE>(x, out);                                   \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(1, uint8_t)
        DECLARE_COMPUTE_RUN(2, uint16_t)
