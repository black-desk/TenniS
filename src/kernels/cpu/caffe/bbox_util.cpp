...
#define INFO     LOG_INFO
    ...
    class counting_iterator {
      public:
        ...
        counting_iterator operator++(int) { return self(m_val++); }
        ...
        T operator-(const self &it) const { return this->m_val - it.m_val; }
      private:
        T m_val;
    };
    ...
    bool SortBBoxDescend(
      const NormalizedBBox &bbox1, const NormalizedBBox &bbox2) {
      return bbox1.score() > bbox2.score();
    }

    template <typename T>
    bool SortScorePairAscend(
      const pair<float, T> &pair1, const pair<float, T> &pair2) {
      return pair1.first < pair2.first;
    }

    // Explicit initialization.
    template bool SortScorePairAscend(
      const pair<float, int> &pair1, const pair<float, int> &pair2);

    template bool SortScorePairAscend(
      const pair<float, pair<int, int> > &pair1,
      const pair<float, pair<int, int> > &pair2);

    template <typename T>
    bool SortScorePairDescend(
      const pair<float, T> &pair1, const pair<float, T> &pair2) {
      return pair1.first > pair2.first;
    }
    ...
    void IntersectBBox(
      const NormalizedBBox &bbox1,
      const NormalizedBBox &bbox2,
      NormalizedBBox       *intersect_bbox) {
      if (
        bbox2.xmin() > bbox1.xmax() || bbox2.xmax() < bbox1.xmin()
    ...
    void OutputBBox(
      const NormalizedBBox  &bbox,
      const pair<int, int>  &img_size,
      const bool             has_resize,
      const ResizeParameter &resize_param,
      ...
      if (has_resize && resize_param.resize_mode()) {
        ...
        switch (resize_param.resize_mode()) {
          ...
          case ResizeParameter::Resize_mode::FIT_SMALL_SIZE:
            ...
            break;
          default: LOG(FATAL) << "Unknown resize mode." << eject;
        }
      } else {
    ...
    bool ProjectBBox(
      ...
      if (
        bbox.xmin() >= src_bbox.xmax() || bbox.xmax() <= src_bbox.xmin()
    ...
    void ExtrapolateBBox(
      ...
      if (
        height_scale > 0 && width_scale > 0
        && param.resize_mode()
             == ResizeParameter::Resize_mode::FIT_SMALL_SIZE) {
    ...
    Dtype JaccardOverlap(const Dtype *bbox1, const Dtype *bbox2) {
      if (
        bbox2 [0] > bbox1 [2] || bbox2 [2] < bbox1 [0] || bbox2 [1] > bbox1 [3]
        || bbox2 [3] < bbox1 [1]) {
        return Dtype(0.);
      } else {
    ...
    bool MeetEmitConstraint(
      ...
      if (emit_type == EmitConstraint::EmitType::CENTER) {
        ...
        if (
          x_center >= src_bbox.xmin() && x_center <= src_bbox.xmax()
    ...
    void EncodeBBox(
      const NormalizedBBox &prior_bbox,
    ...
    void DecodeBBox(
      ...
      if (code_type == PriorBoxParameter::CodeType::CORNER) {
        ...
        if (variance_encoded_in_target) {
          ...
          decode_bbox_center_x = prior_variance [0] * bbox.xmin() * prior_width
                               + prior_center_x;
          decode_bbox_center_y = prior_variance [1] * bbox.ymin() * prior_height
                               + prior_center_y;
      ...
      if (clip_bbox) { ClipBBox(*decode_bbox, decode_bbox); }
    }
    ...
    void DecodeBBoxes(
      ...
      if (num_bboxes >= 1) { CHECK_EQ(prior_variances [0].size(), 4); }
      decode_bboxes->clear();
    ...
    void DecodeBBoxesAll(
      ...
      for (int i = 0; i < num; ++i) {
        ...
        for (int c = 0; c < num_loc_classes; ++c) {
          ...
          const vector<NormalizedBBox>
            &label_loc_preds = all_loc_preds [i].find(label)->second;
          DecodeBBoxes(
            prior_bboxes,
            prior_variances,
            code_type,
    ...
    void MatchBBox(
      ...
      if (label == -1) {
        ...
        for (int i = 0; i < num_gt; ++i) { gt_indices.push_back(i); }
      } else {
      ...
      for (int i = 0; i < num_pred; ++i) {
        if (
          ignore_cross_boundary_bbox && IsCrossBoundaryBBox(pred_bboxes [i])) {
          (*match_indices) [i] = -2;
          continue;
        }
        for (int j = 0; j < num_gt; ++j) {
          float overlap = JaccardOverlap(
            pred_bboxes [i], gt_bboxes [gt_indices [j]]);
          if (overlap > 1e-6) {
      ...
      // Bipartite matching.
      vector<int> gt_pool;
      for (int i = 0; i < num_gt; ++i) { gt_pool.push_back(i); }
      ...
      switch (match_type) {
        ...
        case MultiBoxLossParameter::MatchType::PER_PREDICTION:
          ...
          break;
        default: LOG(FATAL) << "Unknown matching type." << eject; break;
      }
    ...
    void FindMatches(
      ...
      // Get parameters.
      TS_CHECK(multibox_loss_param.has_num_classes())
        << "Must provide num_classes." << eject;
      const int num_classes = multibox_loss_param.num_classes();
      TS_CHECK_GE(num_classes, 1)
        << "num_classes should not be less than 1." << eject;
      ...
      const bool  use_prior_for_matching = multibox_loss_param
      ...
      const bool     encode_variance_in_target = multibox_loss_param
                                               .encode_variance_in_target();
      const bool ignore_cross_boundary_bbox = multibox_loss_param
    ...
    inline bool IsEligibleMining(
      const MiningType mining_type,
      const int        match_idx,
      ...
      if (mining_type == MultiBoxLossParameter::MiningType::MAX_NEGATIVE) {
        ...
        mining_type == MultiBoxLossParameter::MiningType::HARD_EXAMPLE) {
        return true;
      } else {
        return false;
      }
    ...
    template <typename Dtype>
    void MineHardExamples(
      const Blob<Dtype>                       &conf_blob,
      const vector<LabelBBox>                 &all_loc_preds,
      const map<int, vector<NormalizedBBox> > &all_gt_bboxes,
      const vector<NormalizedBBox>            &prior_bboxes,
      const vector<vector<float> >            &prior_variances,
      ...
      *num_negs      = 0;
      ...
      if (mining_type == MultiBoxLossParameter::MiningType::NONE) { return; }
      ...
      for (int i = 0; i < num; ++i) {
        ...
        for (map<int, vector<int> >::iterator it = match_indices.begin();
          ...
          if (mining_type == MultiBoxLossParameter::MiningType::MAX_NEGATIVE) {
            int num_pos = 0;
            for (int m = 0; m < match_indices [label].size(); ++m) {
              if (match_indices [label][m] > -1) { ++num_pos; }
            }
            num_sel = std::min(
              static_cast<int>(num_pos * neg_pos_ratio), num_sel);
          } else if (
            mining_type == MultiBoxLossParameter::MiningType::HARD_EXAMPLE) {
    ...
    void GetLocPredictions(
      ...
      loc_preds->clear();
      if (share_location) { CHECK_EQ(num_loc_classes, 1); }
    ...
    // Explicit initialization.
    template void GetLocPredictions(
    ...
    void EncodeLocPrediction(
      ...
      for (int i = 0; i < num; ++i) {
        for (map<int, vector<int> >::const_iterator it =
             ...
             it != all_match_indices [i].end();
             ++it) {
          const int          label       = it->first;
          const vector<int> &match_index = it->second;
          CHECK(all_loc_preds [i].find(label) != all_loc_preds [i].end());
          const vector<NormalizedBBox>
            &loc_pred = all_loc_preds [i].find(label)->second;
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            const NormalizedBBox &gt_bbox = all_gt_bboxes.find(i)
                                              ->second [gt_idx];
            ...
            if (bp_inside) {
              ...
              loc_pred_data [count * 4]     = (match_bbox.xmin() < 0
                                           || match_bbox.xmin() > 1) ?
                                                gt_encode.xmin() :
                                                loc_pred [j].xmin();
              loc_pred_data [count * 4 + 1] = (match_bbox.ymin() < 0
                                               || match_bbox.ymin() > 1) ?
                                                gt_encode.ymin() :
                                                loc_pred [j].ymin();
              loc_pred_data [count * 4 + 2] = (match_bbox.xmax() < 0
                                               || match_bbox.xmax() > 1) ?
                                                gt_encode.xmax() :
                                                loc_pred [j].xmax();
              loc_pred_data [count * 4 + 3] = (match_bbox.ymax() < 0
                                               || match_bbox.ymax() > 1) ?
                                                gt_encode.ymax() :
                                                loc_pred [j].ymax();
            } else {
    ...
    // Explicit initialization.
    template void EncodeLocPrediction(
      const vector<LabelBBox>                 &all_loc_preds,
      const map<int, vector<NormalizedBBox> > &all_gt_bboxes,
      const vector<map<int, vector<int> > >   &all_match_indices,
      const vector<NormalizedBBox>            &prior_bboxes,
    ...
    template void EncodeLocPrediction(
      ...
      const vector<vector<float> >            &prior_variances,
      const MultiBoxLossParameter             &multibox_loss_param,
      double                                  *loc_pred_data,
      double                                  *loc_gt_data);

    template <typename Dtype>
    void ComputeLocLoss(
      const Blob<Dtype>                     &loc_pred,
      const Blob<Dtype>                     &loc_gt,
      const vector<map<int, vector<int> > > &all_match_indices,
      const int                              num,
        ...
        for (map<int, vector<int> >::const_iterator it =
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            for (int k = 0; k < 4; ++k) {
              ...
              if (
                loc_loss_type
                == MultiBoxLossParameter::LocLossType::SMOOTH_L1) {
                ...
                loc_loss_type == MultiBoxLossParameter::LocLossType::L2) {
                loss += 0.5 * val * val;
              } else {
    ...
    // Explicit initialization.
    template void ComputeLocLoss(
      const Blob<float>                     &loc_pred,
    ...
    // Explicit initialization.
    template void GetConfidenceScores(
      ...
      for (int i = 0; i < num; ++i) {
        ...
        for (int p = 0; p < num_preds_per_class; ++p) {
          ...
          if (loss_type == MultiBoxLossParameter::ConfLossType::SOFTMAX) {
            ...
            loss_type == MultiBoxLossParameter::ConfLossType::LOGISTIC) {
            int target = 0;
            for (int c = 0; c < num_classes; ++c) {
              if (c == label) {
                target = 1;
      ...
      for (int i = 0; i < num; ++i) {
        ...
        for (int p = 0; p < num_preds_per_class; ++p) {
          ...
          for (map<int, vector<int> >::const_iterator it =
            ...
            if (match_index [p] > -1) {
              CHECK(all_gt_bboxes.find(i) != all_gt_bboxes.end());
              const vector<NormalizedBBox> &gt_bboxes = all_gt_bboxes.find(i)
                                                          ->second;
    ...
    void EncodeConfPrediction(
      ...
      TS_CHECK_GE(num_classes, 1)
        << "num_classes should not be less than 1." << eject;
      const int background_label_id = multibox_loss_param.background_label_id();
      const bool map_object_to_agnostic = multibox_loss_param
      ...
      if (multibox_loss_param.has_do_neg_mining()) {
        ...
        CHECK_EQ(
          do_neg_mining,
          mining_type != MultiBoxLossParameter::MiningType::NONE);
      }
            ...
            for (int j = 0; j < num_priors; ++j) {
              ...
              switch (conf_loss_type) {
                ...
                case MultiBoxLossParameter::ConfLossType::LOGISTIC:
                  ...
                  break;
                default: LOG(FATAL) << "Unknown conf loss type." << eject;
              }
              if (do_neg_mining) {
                // Copy scores for matched bboxes.
                caffe_copy<Dtype>(
                  num_classes,
                  conf_data + j * num_classes,
    ...
    // Explicite initialization.
    template void EncodeConfPrediction(
      const float                             *conf_data,
      const int                                num,
      const int                                num_priors,
      const MultiBoxLossParameter             &multibox_loss_param,
    ...
    template void EncodeConfPrediction(
      ...
      const vector<vector<int> >              &all_neg_indices,
      const map<int, vector<NormalizedBBox> > &all_gt_bboxes,
      double                                  *conf_pred_data,
      double                                  *conf_gt_data);

    template <typename Dtype>
    void GetPriorBBoxes(
      const Dtype            *prior_data,
      const int               num_priors,
    ...
    // Explicit initialization.
    template void GetDetectionResults(
    ...
    template void GetDetectionResults(
      const double                                 *det_data,
      const int                                     num_det,
      const int                                     background_label_id,
      map<int, map<int, vector<NormalizedBBox> > > *all_detections);

    void GetTopKScoreIndex(
    ...
    template void GetMaxScoreIndex(
      const float               *scores,
      const int                  num,
      const float                threshold,
      const int                  top_k,
    ...
    void ApplyNMS(
      ...
      vector<int> idx(
        counting_iterator<int>(0), counting_iterator<int>(scores.size()));
      vector<pair<float, int> > score_index_vec;
      ...
      while (score_index_vec.size() != 0) {
        ...
        for (vector<pair<float, int> >::iterator it = score_index_vec.begin();
          ...
          if (reuse_overlaps) {
            if (
              overlaps->find(best_idx) != overlaps->end()
              ...
              overlaps->find(cur_idx) != overlaps->end()
    ...
    void ApplyNMS(
      ...
      map<int, map<int, float> > overlaps;
      ApplyNMS(
        bboxes, scores, threshold, top_k, reuse_overlap, &overlaps, indices);
    }

    void ApplyNMS(const bool *overlapped, const int num, vector<int> *indices) {
      vector<int> index_vec(
        counting_iterator<int>(0), counting_iterator<int>(num));
      // Do nms.
      indices->clear();
      ...
      while (score_index_vec.size() != 0) {
        ...
        for (int k = 0; k < indices->size(); ++k) {
          if (keep) {
            const int kept_idx = (*indices) [k];
            float     overlap  = JaccardOverlap(
              bboxes + idx * 4, bboxes + kept_idx * 4);
            keep = overlap <= adaptive_threshold;
          } else {
            break;
          }
    ...
    void CumSum(const vector<pair<float, int> > &pairs, vector<int> *cumsum) {
      ...
      std::stable_sort(
        sort_pairs.begin(), sort_pairs.end(), SortScorePairDescend<int>);
    ...
    void ComputeAP(
      const vector<pair<float, int> > &tp,
      const int                        num_pos,
      const vector<pair<float, int> > &fp,
      const string                     ap_version,
      vector<float>                   *prec,
      vector<float>                   *rec,
      float                           *ap) {
      const float eps = 1e-6;
      TS_CHECK_EQ(tp.size(), fp.size())
        << "tp must have same size as fp." << eject;
      ...
      *ap = 0;
      ...
      for (int i = 0; i < num; ++i) {
        prec->push_back(
          static_cast<float>(tp_cumsum [i]) / (tp_cumsum [i] + fp_cumsum [i]));
      }
      ...
      if (ap_version == "11point") {
        ...
        for (int j = 10; j >= 0; --j) {
          for (int i = start_idx; i >= 0; --i) {
            if ((*rec) [i] < j / 10.) {
              start_idx = i;
              if (j > 0) { max_precs [j - 1] = max_precs [j]; }
              break;
            } else {
              if (max_precs [j] < (*prec) [i]) { max_precs [j] = (*prec) [i]; }
            }
          }
        }
        for (int j = 10; j >= 0; --j) { *ap += max_precs [j] / 11; }
    ...
    vector<cv::Scalar> GetColors(const int n) {
      ...
      for (int i = 0; i < n; ++i) {
        const float h = std::fmod(
          rng.uniform(0.f, 1.f) + golden_ratio_conjugate, 1.f);
    ...
    void VisualizeBBox(
      ...
      if (num_det == 0 || num_img == 0) { return; }
      // Comute FPS.
      float fps = num_img
                / (static_cast<double>(clock() - start_clock) / CLOCKS_PER_SEC);
      ...
      for (int i = 0; i < num_img; ++i) {
        ...
        cv::Size text = cv::getTextSize(
          buffer, fontface, scale, thickness, &baseline);
        cv::rectangle(
          image,
          cv::Point(0, 0),
        ...
        cv::putText(
          image,
          buffer,
          cv::Point(0, text.height + baseline / 2.),
          fontface,
          scale,
          CV_RGB(0, 0, 0),
          thickness,
        ...
        for (map<int, vector<NormalizedBBox> >::iterator it =
             ...
             it != all_detections [i].end();
             ++it) {
          int    label      = it->first;
          string label_name = "Unknown";
          if (
            label_to_display_name.find(label) != label_to_display_name.end()) {
          ...
          for (int j = 0; j < bboxes.size(); ++j) {
            ...
            cv::putText(
              image,
              buffer,
              bottom_left_pt - cv::Point(0, baseline),
              fontface,
              scale,
              CV_RGB(0, 0, 0),
              thickness,
        ...
        if (! save_file.empty()) {
          if (! cap_out.isOpened()) {
            ...
            cv::VideoWriter outputVideo(
              save_file, CV_FOURCC('D', 'I', 'V', 'X'), 30, size, true);
            cap_out = outputVideo;
          }
          cap_out.write(image);
        }
        cv::imshow("detections", image);
        if (cv::waitKey(1) == 27) { raise(SIGINT); }
      }
      start_clock = clock();
    }

    template void VisualizeBBox(
      const vector<cv::Mat>    &images,
