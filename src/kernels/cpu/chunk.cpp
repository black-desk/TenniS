...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_chunk_run(
      const Tensor &x, int dim, std::vector<Tensor> &out) {
      // auto device_type = x.device().type();
      auto device_id      = x.device().id();
      // first sync all memory to running_memory_device, then doo all inner
      // deivce copy
      auto memcpy_handler = HardConverter::Query(
        x.device().type(), x.device().type());
      ...
      for (int i = 0; i < dim; i++) { num_chunks *= output_shape_template [i]; }
      ...
      for (size_t i = 0; i < out.size(); i++) {
        ...
        for (int j = 0; j < num_chunks; j++) {
          memcpy_handler(
            ...
            device_id,
            input_data
              + (j * output_concat_axis + offset_concat_axis) * each_chunk_size,
    ...
    void Chunk::chunk(
      const Tensor &x, int chunks, int dim, std::vector<Tensor> &out) {
      class uint128_t {
        public:
          uint64_t h;
          uint64_t l;
      ...
      switch (type_bytes) {
        case 1: cpu_chunk_run<uint8_t>(x, dim, out); break;
        case 2: cpu_chunk_run<uint16_t>(x, dim, out); break;
        case 4: cpu_chunk_run<uint32_t>(x, dim, out); break;
        case 8: cpu_chunk_run<uint64_t>(x, dim, out); break;
        case 16: cpu_chunk_run<uint128_t>(x, dim, out); break;
        default: {
          auto dtype = x.dtype();
