...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_concat_run(
      const std::vector<Tensor> &x, int dim, Tensor &out) {
      ...
      auto memcpy_handler = HardConverter::Query(
        out.device().type(), out.device().type());
      ...
      for (int i = 0; i < dim; i++) { num_concats *= input_shape [i]; }
      ...
      for (size_t i = 0; i < x.size(); i++) {
        ...
        for (int j = 0; j < num_concats; j++) {
          memcpy_handler(
            device_id,
            output_data
              + (j * output_concat_axis + offset_concat_axis)
                  * input_concat_size,
            device_id,
    ...
    void Concat::concat(const std::vector<Tensor> &x, int dim, Tensor &out) {
      ...
      switch (type_bytes) {
        case 1: cpu_concat_run<uint8_t>(x, dim, out); break;
        case 2: cpu_concat_run<uint16_t>(x, dim, out); break;
        case 4: cpu_concat_run<uint32_t>(x, dim, out); break;
        case 8: cpu_concat_run<uint64_t>(x, dim, out); break;
        case 16: cpu_concat_run<uint128_t>(x, dim, out); break;
        default: {
