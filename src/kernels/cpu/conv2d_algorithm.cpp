...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void inner_transpose_temp(
      size_t m, size_t n, T *in, T *out)  // A[m][n] -> A[n][m]
    {
      for (int i = 0; i < m; i++)
        for (int j = 0; j < n; j++) out [j * m + i] = in [i * n + j];
    }

    template <typename T>
    static void inner_cut(
      const Tensor &src,
      Tensor       &dst,
      int           cut_top,
      int           cut_bottom,
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < channel; c++) {
          const T *src_at = src_data + n * src_num_offset
                          + c * src_channel_offset;
          ...
          for (int h = 0; h < out_h; h++) {
            if (out_w < 12) {
              for (int w = 0; w < out_w; w++) { dst_at [w] = cut_at [w]; }
            } else {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < channel; c++) {
          ...
          for (; y < pad_top; y++) {
            int x = 0;
            for (; x < out_w; x++) { dst_at [x] = pad_value; }
            dst_at += out_w;
          }
          ...
          for (; y < pad_top + src_shape [2]; y++) {
            int x = 0;
            for (; x < pad_left; x++) { dst_at [x] = pad_value; }
            if (src_w < 12) {
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd23_transform_kernel(
      const Tensor &kernel, Tensor &kernel_tm) {
      ...
      for (int p = 0; p < out_channel; p++) {
        for (int q = 0; q < input_channel; q++) {
          ...
          T tmp [4][3];
          for (int i = 0; i < 4; i++) {
            tmp [i][0] = k0 [0] * ktm [i][0] + k0 [1] * ktm [i][1]
                       + k0 [2] * ktm [i][2];
            tmp [i][1] = k1 [0] * ktm [i][0] + k1 [1] * ktm [i][1]
                       + k1 [2] * ktm [i][2];
            tmp [i][2] = k2 [0] * ktm [i][0] + k2 [1] * ktm [i][1]
                       + k2 [2] * ktm [i][2];
          }
          ...
          for (int j = 0; j < 4; j++) {
            ...
            for (int i = 0; i < 4; i++) {
              kernel_tm_at [j * 4 + i] = tmpp [0] * ktm [i][0]
                                       + tmpp [1] * ktm [i][1]
                                       + tmpp [2] * ktm [i][2];
            }
          }
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd23_transform_kernel_inplace(
      const Tensor &kernel, Tensor &kernel_tm) {
      auto kernel_shape  = kernel.sizes();
      int  out_channel   = kernel_shape [0];
      int  input_channel = kernel_shape [1];
...
// inner_transpose_temp<T>(4, 3, (T*)ktm, tmp_out);
          ...
          cblas::math<T>::gemm(
            ...
            0,
            kernel_tm_at);
// cblas::math<T>::gemm(blas::NoTrans, blas::NoTrans, 4, 4, 3, T(1), (const
// T*)tmp_out,ktm, 0, kernel_tm_at);
#else
          cpu::math<T, T>::gemm(
            blas::NoTrans,
            blas::NoTrans,
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd63_transform_kernel(
      const Tensor &kernel, Tensor &kernel_tm) {
      auto kernel_shape  = kernel.sizes();
      int  out_channel   = kernel_shape [0];
      int  input_channel = kernel_shape [1];
      ...
      for (int p = 0; p < out_channel; p++) {
        for (int q = 0; q < input_channel; q++) {
          ...
          for (int j = 0; j < 8; j++) {
            ...
            for (int i = 0; i < 8; i++) {
              kernel_tm_at [j * 8 + i] = tmpp [0] * ktm [i][0]
                                       + tmpp [1] * ktm [i][1]
                                       + tmpp [2] * ktm [i][2];
            }
          }
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd63_transform_kernel_inplace(
      const Tensor &kernel, Tensor &kernel_tm) {
      auto kernel_shape  = kernel.sizes();
      int  out_channel   = kernel_shape [0];
      int  input_channel = kernel_shape [1];
      ...
      const T ktm [24] = { 1,          0,          0,         -T(2) / 9,
                           -T(2) / 9,  -T(2) / 9,  -T(2) / 9, T(2) / 9,
                           -T(2) / 9,  T(1) / 90,  T(1) / 45, T(2) / 45,
                           T(1) / 90,  -T(1) / 45, T(2) / 45, T(1) / 45,
                           T(1) / 90,  T(1) / 180, T(1) / 45, -T(1) / 90,
                           T(1) / 180, 0,          0,         1 };
...
// T*)tmp_out,ktm, 0, kernel_tm_at);
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd23(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      ...
      Shape input_bordered_s = {
        num, input_channel, input_padded_h, input_padded_w
      };
      ...
      inner_pad<T>(
        ...
        input_bordered,
        0,
        input_padded_h - input_h,
        0,
        input_padded_w - input_w,
        0);
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < input_channel; c++) {
          const T *src_at = src_ptr + n * bordered_num_offset
                          + c * bordered_c_offset;
        ...
        for (int cc = 0; cc < outch; cc++) {
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            for (int qq = 0; qq < inputch; qq++) {
              int      q           = qq * 4;
              const T *input_tm_at = input_tm_ptr + n * tm_num_offset
                                   + q * tm_c_offset;
        ...
        for (int c = remain_outch; c < output_channel; c++) {
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            for (; q < input_channel; q++) {
              ...
              for (int k = 0; k < 16; k++) { sum_0 [k] += r0 [k] * k0 [k]; }
            }

            for (int k = 0; k < 16; k++) { out_0 [k] = sum_0 [k]; }
          }
        }
      }
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < output_channel; c++) {
          T *output_tm_at = out_tm_ptr + n * outtm_n_offset
                          + c * outtm_c_offset;
...
#ifdef TS_USE_SIMD
    template <>
    void Conv2dAlgorithm<float>::conv3x3_winograd23(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
...
#  endif
        for (int c = 0; c < input_channel; c++) {
          const float *src_at = src_ptr + n * bordered_num_offset
                              + c * bordered_c_offset;
...
#  endif
        for (int cc = 0; cc < outch; cc++) {
          ...
          float *out_tm_0 = out_tm_ptr + n * outtm_n_offset
                          + c * outtm_c_offset;
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            for (int qq = 0; qq < inputch; qq++) {
              int          q           = qq * 4;
              const float *input_tm_at = input_tm_ptr + n * tm_num_offset
                                       + q * tm_c_offset;
              ...
              float32x4    k010(k0 + 16), k011(k0 + 20), k012(k0 + 24),
                k013(k0 + 28);
              float32x4 k020(k0 + 32), k021(k0 + 36), k022(k0 + 40),
                k023(k0 + 44);
              float32x4 k030(k0 + 48), k031(k0 + 52), k032(k0 + 56),
                k033(k0 + 60);

              float32x4 k100(k1), k101(k1 + 4), k102(k1 + 8), k103(k1 + 12);
              float32x4 k110(k1 + 16), k111(k1 + 20), k112(k1 + 24),
                k113(k1 + 28);
              float32x4 k120(k1 + 32), k121(k1 + 36), k122(k1 + 40),
                k123(k1 + 44);
              float32x4 k130(k1 + 48), k131(k1 + 52), k132(k1 + 56),
                k133(k1 + 60);

              float32x4 k200(k2), k201(k2 + 4), k202(k2 + 8), k203(k2 + 12);
              float32x4 k210(k2 + 16), k211(k2 + 20), k212(k2 + 24),
                k213(k2 + 28);
              float32x4 k220(k2 + 32), k221(k2 + 36), k222(k2 + 40),
                k223(k2 + 44);
              float32x4 k230(k2 + 48), k231(k2 + 52), k232(k2 + 56),
                k233(k2 + 60);

              float32x4 k300(k3), k301(k3 + 4), k302(k3 + 8), k303(k3 + 12);
              float32x4 k310(k3 + 16), k311(k3 + 20), k312(k3 + 24),
                k313(k3 + 28);
              float32x4 k320(k3 + 32), k321(k3 + 36), k322(k3 + 40),
                k323(k3 + 44);
              float32x4 k330(k3 + 48), k331(k3 + 52), k332(k3 + 56),
                k333(k3 + 60);

              // sum00 += r00 * k000; sum00 += r10 * k010; sum00 += r20 * k020;
              // sum00 += r30 * k030; sum01 += r01 * k001; sum01 += r11 * k011;
              // sum01 += r21 * k021; sum01 += r31 * k031; sum02 += r02 * k002;
              // sum02 += r12 * k012; sum02 += r22 * k022; sum02 += r32 * k032;
              // sum03 += r03 * k003; sum03 += r13 * k013; sum03 += r23 * k023;
              // sum03 += r33 * k033;

              // sum10 += r00 * k100; sum10 += r10 * k110; sum10 += r20 * k120;
              // sum10 += r30 * k130; sum11 += r01 * k101; sum11 += r11 * k111;
              // sum11 += r21 * k121; sum11 += r31 * k131; sum12 += r02 * k102;
              // sum12 += r12 * k112; sum12 += r22 * k122; sum12 += r32 * k132;
              // sum13 += r03 * k103; sum13 += r13 * k113; sum13 += r23 * k123;
              // sum13 += r33 * k133;
              //
              // sum20 += r00 * k200; sum20 += r10 * k210; sum20 += r20 * k220;
              // sum20 += r30 * k230; sum21 += r01 * k201; sum21 += r11 * k211;
              // sum21 += r21 * k221; sum21 += r31 * k231; sum22 += r02 * k202;
              // sum22 += r12 * k212; sum22 += r22 * k222; sum22 += r32 * k232;
              // sum23 += r03 * k203; sum23 += r13 * k213; sum23 += r23 * k223;
              // sum23 += r33 * k233;

              // sum30 += r00 * k300; sum30 += r10 * k310; sum30 += r20 * k320;
              // sum30 += r30 * k330; sum31 += r01 * k301; sum31 += r11 * k311;
              // sum31 += r21 * k321; sum31 += r31 * k331; sum32 += r02 * k302;
              // sum32 += r12 * k312; sum32 += r22 * k322; sum32 += r32 * k332;
              // sum33 += r03 * k303; sum33 += r13 * k313; sum33 += r23 * k323;
              // sum33 += r33 * k333;

              sum00 = fmadd(r00, k000, sum00);
              sum00 = fmadd(r10, k010, sum00);
              sum00 = fmadd(r20, k020, sum00);
              sum00 = fmadd(r30, k030, sum00);
              sum01 = fmadd(r01, k001, sum01);
              sum01 = fmadd(r11, k011, sum01);
              sum01 = fmadd(r21, k021, sum01);
              sum01 = fmadd(r31, k031, sum01);
              sum02 = fmadd(r02, k002, sum02);
              sum02 = fmadd(r12, k012, sum02);
              sum02 = fmadd(r22, k022, sum02);
              sum02 = fmadd(r32, k032, sum02);
              sum03 = fmadd(r03, k003, sum03);
              sum03 = fmadd(r13, k013, sum03);
              sum03 = fmadd(r23, k023, sum03);
              sum03 = fmadd(r33, k033, sum03);

              sum10 = fmadd(r00, k100, sum10);
              sum10 = fmadd(r10, k110, sum10);
              sum10 = fmadd(r20, k120, sum10);
              sum10 = fmadd(r30, k130, sum10);
              sum11 = fmadd(r01, k101, sum11);
              sum11 = fmadd(r11, k111, sum11);
              sum11 = fmadd(r21, k121, sum11);
              sum11 = fmadd(r31, k131, sum11);
              sum12 = fmadd(r02, k102, sum12);
              sum12 = fmadd(r12, k112, sum12);
              sum12 = fmadd(r22, k122, sum12);
              sum12 = fmadd(r32, k132, sum12);
              sum13 = fmadd(r03, k103, sum13);
              sum13 = fmadd(r13, k113, sum13);
              sum13 = fmadd(r23, k123, sum13);
              sum13 = fmadd(r33, k133, sum13);

              sum20 = fmadd(r00, k200, sum20);
              sum20 = fmadd(r10, k210, sum20);
              sum20 = fmadd(r20, k220, sum20);
              sum20 = fmadd(r30, k230, sum20);
              sum21 = fmadd(r01, k201, sum21);
              sum21 = fmadd(r11, k211, sum21);
              sum21 = fmadd(r21, k221, sum21);
              sum21 = fmadd(r31, k231, sum21);
              sum22 = fmadd(r02, k202, sum22);
              sum22 = fmadd(r12, k212, sum22);
              sum22 = fmadd(r22, k222, sum22);
              sum22 = fmadd(r32, k232, sum22);
              sum23 = fmadd(r03, k203, sum23);
              sum23 = fmadd(r13, k213, sum23);
              sum23 = fmadd(r23, k223, sum23);
              sum23 = fmadd(r33, k233, sum23);

              sum30 = fmadd(r00, k300, sum30);
              sum30 = fmadd(r10, k310, sum30);
              sum30 = fmadd(r20, k320, sum30);
              sum30 = fmadd(r30, k330, sum30);
              sum31 = fmadd(r01, k301, sum31);
              sum31 = fmadd(r11, k311, sum31);
              sum31 = fmadd(r21, k321, sum31);
              sum31 = fmadd(r31, k331, sum31);
              sum32 = fmadd(r02, k302, sum32);
              sum32 = fmadd(r12, k312, sum32);
              sum32 = fmadd(r22, k322, sum32);
              sum32 = fmadd(r32, k332, sum32);
              sum33 = fmadd(r03, k303, sum33);
              sum33 = fmadd(r13, k313, sum33);
              sum33 = fmadd(r23, k323, sum33);
              sum33 = fmadd(r33, k333, sum33);
            }
            ...
            sum01.store(out_0 + 4);
            sum02.store(out_0 + 8);
            sum03.store(out_0 + 12);
            sum10.store(out_1);
            sum11.store(out_1 + 4);
            sum12.store(out_1 + 8);
            sum13.store(out_1 + 12);
            sum20.store(out_2);
            sum21.store(out_2 + 4);
            sum22.store(out_2 + 8);
            sum23.store(out_2 + 12);
            sum30.store(out_3);
            sum31.store(out_3 + 4);
            sum32.store(out_3 + 8);
            sum33.store(out_3 + 12);
          }
        }
#  ifdef TS_USE_OPENMP
#    pragma omp parallel for num_threads(openmp_threads())
...
#  endif
        for (int c = 0; c < output_channel; c++) {
          float *output_tm_at = out_tm_ptr + n * outtm_n_offset
                              + c * outtm_c_offset;
          ...
          for (int i = 0; i < col_blocks; i++) {
            ...
            for (int j = 0; j < row_blocks; j++) {
              ...
              float32x4 s0(out_tile), s1(out_tile + 4), s2(out_tile + 8),
                s3(out_tile + 12);
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd23_threadpool(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    void Conv2dAlgorithm<float>::conv3x3_winograd23_threadpool(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
        ...
        for (int n = 0; n < num; n++) {
          ...
          for (auto &bin: bins) {
            thread_pool->run([&, n, src_ptr, dst_ptr, bin](int) {
              const float *src_at = src_ptr + n * bordered_num_offset
                                  + bin.first * bordered_c_offset;
              float *dst_at = dst_ptr + n * tm_num_offset
                            + bin.first * tm_c_offset;
      ...
      for (int n = 0; n < num; n++) {
        if (thread_pool == nullptr || thread_pool->size() <= 1) {
          ...
          for (auto &bin: bins) {
            thread_pool->run(
              [&, n, input_tm_ptr, out_tm_ptr, kernel_tm_ptr, bin](int) {
                float *out_tm_0 = out_tm_ptr + n * outtm_n_offset
                                + 4 * bin.first * outtm_c_offset;

                const float *kernel_tm_0 = kernel_tm_ptr
                                         + 4 * bin.first * ktm_n_offset;
                ...
                for (int c = bin.first; c < bin.second; c++) {
                  ...
                  for (int i = 0; i < num_blocks; i++) {
                    ...
                    for (int qq = 0; qq < inputch; qq++) {
                      int          q           = qq * 4;
                      const float *input_tm_at = input_tm_ptr
                                               + n * tm_num_offset
                                               + q * tm_c_offset;
                      const float *r0 = input_tm_at + i * 16;
                      const float *r1 = r0 + tm_c_offset;
                      ...
                      const float *k2 = kernel_tm_2 + q * ktm_c_offset;
                      const float *k3 = kernel_tm_3 + q * ktm_c_offset;
                      float32x4    k000(k0), k001(k0 + 4), k002(k0 + 8),
                        k003(k0 + 12);
                      float32x4 k010(k0 + 16), k011(k0 + 20), k012(k0 + 24),
                        k013(k0 + 28);
                      float32x4 k020(k0 + 32), k021(k0 + 36), k022(k0 + 40),
                      ...
                      float32x4 k100(k1), k101(k1 + 4), k102(k1 + 8),
                        k103(k1 + 12);
                      float32x4 k110(k1 + 16), k111(k1 + 20), k112(k1 + 24),
                        k113(k1 + 28);
                      float32x4 k120(k1 + 32), k121(k1 + 36), k122(k1 + 40),
                      ...
                      float32x4 k200(k2), k201(k2 + 4), k202(k2 + 8),
                        k203(k2 + 12);
                      float32x4 k210(k2 + 16), k211(k2 + 20), k212(k2 + 24),
                        k213(k2 + 28);
                      float32x4 k220(k2 + 32), k221(k2 + 36), k222(k2 + 40),
                      ...
                      float32x4 k300(k3), k301(k3 + 4), k302(k3 + 8),
                        k303(k3 + 12);
                      float32x4 k310(k3 + 16), k311(k3 + 20), k312(k3 + 24),
                        k313(k3 + 28);
                      float32x4 k320(k3 + 32), k321(k3 + 36), k322(k3 + 40),
        ...
        if (thread_pool == nullptr || thread_pool->size() <= 1) {
          ...
          for (auto &bin: bins) {
            thread_pool->run([&, n, out_tm_ptr, out_ptr, bin](int) {
              float *output_tm_at = out_tm_ptr + n * outtm_n_offset
                                  + bin.first * outtm_c_offset;
              float *out_at = out_ptr + n * outbo_n_offset
                            + bin.first * outbo_c_offset;
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd63(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
          ...
          for (int i = 0; i < col_blocks; i++) {
            for (int j = 0; j < row_blocks; j++) {
              ...
              for (int m = 0; m < 8; m++) {
                ...
                T tmp34_a =
                  (tmp0 [6] + tmp0 [2] * T(0.25) - tmp0 [4] * T(1.25));
                T tmp34_b =
                  (tmp0 [1] * T(0.5) - tmp0 [3] * T(2.5) + tmp0 [5] * T(2));
                ...
                T tmp56_b =
                  (tmp0 [1] * T(2) - tmp0 [3] * T(2.5) + tmp0 [5] * T(0.5));
        ...
        for (int c = remain_outch; c < output_channel; c++) {
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            for (; q < input_channel; q++) {
              ...
              for (int k = 0; k < 64; k++) { sum_0 [k] += r0 [k] * k0 [k]; }
            }

            for (int k = 0; k < 64; k++) { out_0 [k] = sum_0 [k]; }
          }
        }
      }
          ...
          for (int i = 0; i < col_blocks; i++) {
            for (int j = 0; j < row_blocks; j++) {
              ...
              for (int m = 0; m < 8; m++) {
                ...
                // tmp[0][m] = w0[0] + (w0[1] + w0[2]) + (w0[3] + w0[4]) +
                // (w0[5] + w0[6]) * 32; tmp[1][m] = (w0[1] - w0[2]) + (w0[3] -
                // w0[4]) * 2 + (w0[5] - w0[6]) * 16; tmp[2][m] = (w0[1] +
                // w0[2]) + (w0[3] + w0[4]) * 4 + (w0[5] + w0[6]) * 8; tmp[3][m]
                // = (w0[1] - w0[2]) + (w0[3] - w0[4]) * 8 + (w0[5] - w0[6]) *
                // 4; tmp[4][m] = (w0[1] + w0[2]) + (w0[3] + w0[4]) * 16 +
                // (w0[5] + w0[6]) * 2; tmp[5][m] = (w0[1] - w0[2]) + (w0[3] -
                // w0[4]) * 32 + (w0[5] - w0[6]) + w0[7];
              ...
              for (int m = 0; m < 6; m++) {
                ...
                d0 [0] = tmp0 [0] + (tmp0 [1] + tmp0 [2])
                       + (tmp0 [3] + tmp0 [4]) + (tmp0 [5] + tmp0 [6]) * 32;
                d1 [0] = (tmp0 [1] - tmp0 [2]) + (tmp0 [3] - tmp0 [4]) * 2
                       + (tmp0 [5] - tmp0 [6]) * 16;
                d2 [0] = (tmp0 [1] + tmp0 [2]) + (tmp0 [3] + tmp0 [4]) * 4
                       + (tmp0 [5] + tmp0 [6]) * 8;
                d3 [0] = (tmp0 [1] - tmp0 [2]) + (tmp0 [3] - tmp0 [4]) * 8
                       + (tmp0 [5] - tmp0 [6]) * 4;
                d4 [0] = (tmp0 [1] + tmp0 [2]) + (tmp0 [3] + tmp0 [4]) * 16
                       + (tmp0 [5] + tmp0 [6]) * 2;
                d5 [0] = (tmp0 [1] - tmp0 [2]) + (tmp0 [3] - tmp0 [4]) * 32
                       + (tmp0 [5] - tmp0 [6]) + tmp0 [7];
...
#ifdef TS_USE_SIMD
    template <>
    void Conv2dAlgorithm<float>::conv3x3_winograd63(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
...
#  endif
        for (int c = 0; c < input_channel; c++) {
          ...
          // NOTE:I have no idea why i can't define these const variable out of
          // the loop, if i do this,crash on android when i use openmp and
          // neon,but it doesn't happen on x86(openmp+sse,avx)
          const float32x4 f2(2.0f), f2_5(2.5f);
          const float32x4 f5_25(5.25f), f4_25(4.25f);
          const float32x4 f1_25(1.25f), f0_5(0.5f);
...
#  endif
        for (int cc = 0; cc < outch; cc++) {
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            float32x4x2 sum00(0.f), sum01(0.f), sum02(0.f), sum03(0.f),
              sum04(0.f), sum05(0.f), sum06(0.f), sum07(0.f);
            float32x4x2 sum10(0.f), sum11(0.f), sum12(0.f), sum13(0.f),
              sum14(0.f), sum15(0.f), sum16(0.f), sum17(0.f);
            float32x4x2 sum20(0.f), sum21(0.f), sum22(0.f), sum23(0.f),
              sum24(0.f), sum25(0.f), sum26(0.f), sum27(0.f);
            float32x4x2 sum30(0.f), sum31(0.f), sum32(0.f), sum33(0.f),
              sum34(0.f), sum35(0.f), sum36(0.f), sum37(0.f);
            ...
            for (int qq = 0; qq < inputch; qq++) {
              ...
              const float *r2 = r1 + tm_c_offset;
              const float *r3 = r2 + tm_c_offset;
              float32x4x2  r00(r0), r01(r0 + 8), r02(r0 + 16), r03(r0 + 24),
                r04(r0 + 32), r05(r0 + 40), r06(r0 + 48), r07(r0 + 56);
              float32x4x2 r10(r1), r11(r1 + 8), r12(r1 + 16), r13(r1 + 24),
                r14(r1 + 32), r15(r1 + 40), r16(r1 + 48), r17(r1 + 56);
              float32x4x2 r20(r2), r21(r2 + 8), r22(r2 + 16), r23(r2 + 24),
                r24(r2 + 32), r25(r2 + 40), r26(r2 + 48), r27(r2 + 56);
              float32x4x2 r30(r3), r31(r3 + 8), r32(r3 + 16), r33(r3 + 24),
                r34(r3 + 32), r35(r3 + 40), r36(r3 + 48), r37(r3 + 56);
              ...
              const float *k0c0 = k0, *k0c1 = k0c0 + 64, *k0c2 = k0c1 + 64,
                          *k0c3 = k0c2 + 64;
              const float *k1c0 = k1, *k1c1 = k1c0 + 64, *k1c2 = k1c1 + 64,
                          *k1c3 = k1c2 + 64;
              const float *k2c0 = k2, *k2c1 = k2c0 + 64, *k2c2 = k2c1 + 64,
                          *k2c3 = k2c2 + 64;
              const float *k3c0 = k3, *k3c1 = k3c0 + 64, *k3c2 = k3c1 + 64,
                          *k3c3 = k3c2 + 64;

              float32x4x2 k000(k0c0), k001(k0c0 + 8), k002(k0c0 + 16),
                k003(k0c0 + 24), k004(k0c0 + 32), k005(k0c0 + 40),
                k006(k0c0 + 48), k007(k0c0 + 56);
              float32x4x2 k010(k0c1), k011(k0c1 + 8), k012(k0c1 + 16),
                k013(k0c1 + 24), k014(k0c1 + 32), k015(k0c1 + 40),
                k016(k0c1 + 48), k017(k0c1 + 56);
              float32x4x2 k020(k0c2), k021(k0c2 + 8), k022(k0c2 + 16),
                k023(k0c2 + 24), k024(k0c2 + 32), k025(k0c2 + 40),
                k026(k0c2 + 48), k027(k0c2 + 56);
              float32x4x2 k030(k0c3), k031(k0c3 + 8), k032(k0c3 + 16),
                k033(k0c3 + 24), k034(k0c3 + 32), k035(k0c3 + 40),
                k036(k0c3 + 48), k037(k0c3 + 56);

              float32x4x2 k100(k1c0), k101(k1c0 + 8), k102(k1c0 + 16),
                k103(k1c0 + 24), k104(k1c0 + 32), k105(k1c0 + 40),
                k106(k1c0 + 48), k107(k1c0 + 56);
              float32x4x2 k110(k1c1), k111(k1c1 + 8), k112(k1c1 + 16),
                k113(k1c1 + 24), k114(k1c1 + 32), k115(k1c1 + 40),
                k116(k1c1 + 48), k117(k1c1 + 56);
              float32x4x2 k120(k1c2), k121(k1c2 + 8), k122(k1c2 + 16),
                k123(k1c2 + 24), k124(k1c2 + 32), k125(k1c2 + 40),
                k126(k1c2 + 48), k127(k1c2 + 56);
              float32x4x2 k130(k1c3), k131(k1c3 + 8), k132(k1c3 + 16),
                k133(k1c3 + 24), k134(k1c3 + 32), k135(k1c3 + 40),
                k136(k1c3 + 48), k137(k1c3 + 56);

              float32x4x2 k200(k2c0), k201(k2c0 + 8), k202(k2c0 + 16),
                k203(k2c0 + 24), k204(k2c0 + 32), k205(k2c0 + 40),
                k206(k2c0 + 48), k207(k2c0 + 56);
              float32x4x2 k210(k2c1), k211(k2c1 + 8), k212(k2c1 + 16),
                k213(k2c1 + 24), k214(k2c1 + 32), k215(k2c1 + 40),
                k216(k2c1 + 48), k217(k2c1 + 56);
              float32x4x2 k220(k2c2), k221(k2c2 + 8), k222(k2c2 + 16),
                k223(k2c2 + 24), k224(k2c2 + 32), k225(k2c2 + 40),
                k226(k2c2 + 48), k227(k2c2 + 56);
              float32x4x2 k230(k2c3), k231(k2c3 + 8), k232(k2c3 + 16),
                k233(k2c3 + 24), k234(k2c3 + 32), k235(k2c3 + 40),
                k236(k2c3 + 48), k237(k2c3 + 56);

              float32x4x2 k300(k3c0), k301(k3c0 + 8), k302(k3c0 + 16),
                k303(k3c0 + 24), k304(k3c0 + 32), k305(k3c0 + 40),
                k306(k3c0 + 48), k307(k3c0 + 56);
              float32x4x2 k310(k3c1), k311(k3c1 + 8), k312(k3c1 + 16),
                k313(k3c1 + 24), k314(k3c1 + 32), k315(k3c1 + 40),
                k316(k3c1 + 48), k317(k3c1 + 56);
              float32x4x2 k320(k3c2), k321(k3c2 + 8), k322(k3c2 + 16),
                k323(k3c2 + 24), k324(k3c2 + 32), k325(k3c2 + 40),
                k326(k3c2 + 48), k327(k3c2 + 56);
              float32x4x2 k330(k3c3), k331(k3c3 + 8), k332(k3c3 + 16),
                k333(k3c3 + 24), k334(k3c3 + 32), k335(k3c3 + 40),
                k336(k3c3 + 48), k337(k3c3 + 56);
              ...
              sum04 = fmadd(r14, k014, sum04);
              sum04 = fmadd(r24, k024, sum04);
              sum04 = fmadd(r34, k034, sum04);
              sum05 = fmadd(r05, k005, sum05);
              sum05 = fmadd(r15, k015, sum05);
              sum05 = fmadd(r25, k025, sum05);
              sum05 = fmadd(r35, k035, sum05);
              sum06 = fmadd(r06, k006, sum06);
              sum06 = fmadd(r16, k016, sum06);
              sum06 = fmadd(r26, k026, sum06);
              sum06 = fmadd(r36, k036, sum06);
              sum07 = fmadd(r07, k007, sum07);
              sum07 = fmadd(r17, k017, sum07);
              sum07 = fmadd(r27, k027, sum07);
              sum07 = fmadd(r37, k037, sum07);
              ...
              sum14 = fmadd(r14, k114, sum14);
              sum14 = fmadd(r24, k124, sum14);
              sum14 = fmadd(r34, k134, sum14);
              sum15 = fmadd(r05, k105, sum15);
              sum15 = fmadd(r15, k115, sum15);
              sum15 = fmadd(r25, k125, sum15);
              sum15 = fmadd(r35, k135, sum15);
              sum16 = fmadd(r06, k106, sum16);
              sum16 = fmadd(r16, k116, sum16);
              sum16 = fmadd(r26, k126, sum16);
              sum16 = fmadd(r36, k136, sum16);
              sum17 = fmadd(r07, k107, sum17);
              sum17 = fmadd(r17, k117, sum17);
              sum17 = fmadd(r27, k127, sum17);
              sum17 = fmadd(r37, k137, sum17);
              ...
              sum24 = fmadd(r14, k214, sum24);
              sum24 = fmadd(r24, k224, sum24);
              sum24 = fmadd(r34, k234, sum24);
              sum25 = fmadd(r05, k205, sum25);
              sum25 = fmadd(r15, k215, sum25);
              sum25 = fmadd(r25, k225, sum25);
              sum25 = fmadd(r35, k235, sum25);
              sum26 = fmadd(r06, k206, sum26);
              sum26 = fmadd(r16, k216, sum26);
              sum26 = fmadd(r26, k226, sum26);
              sum26 = fmadd(r36, k236, sum26);
              sum27 = fmadd(r07, k207, sum27);
              sum27 = fmadd(r17, k217, sum27);
              sum27 = fmadd(r27, k227, sum27);
              sum27 = fmadd(r37, k237, sum27);
              ...
              sum34 = fmadd(r14, k314, sum34);
              sum34 = fmadd(r24, k324, sum34);
              sum34 = fmadd(r34, k334, sum34);
              sum35 = fmadd(r05, k305, sum35);
              sum35 = fmadd(r15, k315, sum35);
              sum35 = fmadd(r25, k325, sum35);
              sum35 = fmadd(r35, k335, sum35);
              sum36 = fmadd(r06, k306, sum36);
              sum36 = fmadd(r16, k316, sum36);
              sum36 = fmadd(r26, k326, sum36);
              sum36 = fmadd(r36, k336, sum36);
              sum37 = fmadd(r07, k307, sum37);
              sum37 = fmadd(r17, k317, sum37);
              sum37 = fmadd(r27, k327, sum37);
              sum37 = fmadd(r37, k337, sum37);
            }
            ...
            for (int q = remain_inputch; q < input_channel; q++) {
              ...
              const float *k2 = kernel_tm_2 + q * ktm_c_offset;
              const float *k3 = kernel_tm_3 + q * ktm_c_offset;
              float32x4x2  k000(k0), k001(k0 + 8), k002(k0 + 16), k003(k0 + 24),
                k004(k0 + 32), k005(k0 + 40), k006(k0 + 48), k007(k0 + 56);
              float32x4x2 k100(k1), k101(k1 + 8), k102(k1 + 16), k103(k1 + 24),
                k104(k1 + 32), k105(k1 + 40), k106(k1 + 48), k107(k1 + 56);
              float32x4x2 k200(k2), k201(k2 + 8), k202(k2 + 16), k203(k2 + 24),
                k204(k2 + 32), k205(k2 + 40), k206(k2 + 48), k207(k2 + 56);
              float32x4x2 k300(k3), k301(k3 + 8), k302(k3 + 16), k303(k3 + 24),
                k304(k3 + 32), k305(k3 + 40), k306(k3 + 48), k307(k3 + 56);
            ...
            sum00.store(out_0);
            sum01.store(out_0 + 8);
            sum02.store(out_0 + 16);
            sum03.store(out_0 + 24);
            sum04.store(out_0 + 32);
            sum05.store(out_0 + 40);
            sum06.store(out_0 + 48);
            sum07.store(out_0 + 56);

            sum10.store(out_1);
            sum11.store(out_1 + 8);
            sum12.store(out_1 + 16);
            sum13.store(out_1 + 24);
            sum14.store(out_1 + 32);
            sum15.store(out_1 + 40);
            sum16.store(out_1 + 48);
            sum17.store(out_1 + 56);

            sum20.store(out_2);
            sum21.store(out_2 + 8);
            sum22.store(out_2 + 16);
            sum23.store(out_2 + 24);
            sum24.store(out_2 + 32);
            sum25.store(out_2 + 40);
            sum26.store(out_2 + 48);
            sum27.store(out_2 + 56);

            sum30.store(out_3);
            sum31.store(out_3 + 8);
            sum32.store(out_3 + 16);
            sum33.store(out_3 + 24);
            sum34.store(out_3 + 32);
            sum35.store(out_3 + 40);
            sum36.store(out_3 + 48);
            sum37.store(out_3 + 56);
          }
        }
#  ifdef TS_USE_OPENMP
#    pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = remain_outch; c < output_channel; c++) {
          ...
          for (int i = 0; i < num_blocks; i++) {
            ...
            for (; q + 3 < input_channel; q += 4) {
              ...
              float32x4x2 k000(k0), k001(k0 + 8), k002(k0 + 16), k003(k0 + 24),
                k004(k0 + 32), k005(k0 + 40), k006(k0 + 48), k007(k0 + 56);
              float32x4x2 k010(k1), k011(k1 + 8), k012(k1 + 16), k013(k1 + 24),
                k014(k1 + 32), k015(k1 + 40), k016(k1 + 48), k017(k1 + 56);
              float32x4x2 k020(k2), k021(k2 + 8), k022(k2 + 16), k023(k2 + 24),
                k024(k2 + 32), k025(k2 + 40), k026(k2 + 48), k027(k2 + 56);
              float32x4x2 k030(k3), k031(k3 + 8), k032(k3 + 16), k033(k3 + 24),
                k034(k3 + 32), k035(k3 + 40), k036(k3 + 48), k037(k3 + 56);
...
#  endif
          ...
          for (int i = 0; i < col_blocks; i++) {
            for (int j = 0; j < row_blocks; j++) {
              ...
              if (((j * 6 + 6) > output_h) || ((i * 6 + 6) > output_w)) {
                ...
                l0.store(d0);
                *(d0 + 4) = *(((float *)&(l4.value)));
                *(d0 + 5) = *(((float *)&(l4.value)) + 1);
                l1.store(d1);
                *(d1 + 4) = *(((float *)&(l5.value)));
                *(d1 + 5) = *(((float *)&(l5.value)) + 1);
                l2.store(d2);
                *(d2 + 4) = *(((float *)&(l6.value)));
                *(d2 + 5) = *(((float *)&(l6.value)) + 1);
                l3.store(d3);
                *(d3 + 4) = *(((float *)&(l7.value)));
                *(d3 + 5) = *(((float *)&(l7.value)) + 1);

                r0.store(d4);
                *(d4 + 4) = *(((float *)&(r4.value)));
                *(d4 + 5) = *(((float *)&(r4.value)) + 1);
                r1.store(d5);
                *(d5 + 4) = *(((float *)&(r5.value)));
                *(d5 + 5) = *(((float *)&(r5.value)) + 1);
              }
            }
          }
        }
    ...
    // TO DO: Support threadpool on template, only support float now
    template <typename T>
    void Conv2dAlgorithm<T>::conv3x3_winograd63_threadpool(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
...
#ifdef TS_USE_SIMD
    template <>
    void Conv2dAlgorithm<float>::conv3x3_winograd63_threadpool(
      const Tensor &x, const Tensor &k_tm, Tensor &out) {
      auto input_shape = x.sizes();
      auto k_tm_shape  = k_tm.sizes();
      auto out_shape   = out.sizes();
          ...
          for (int cc = 0; cc < outch; cc++) {
            ...
            for (int i = 0; i < num_blocks; i++) {
              ...
              for (int q = remain_inputch; q < input_channel; q++) {
                ...
                const float *k2 = kernel_tm_2 + q * ktm_c_offset;
                const float *k3 = kernel_tm_3 + q * ktm_c_offset;
                float32x4x2  k000(k0), k001(k0 + 8), k002(k0 + 16),
                  k003(k0 + 24), k004(k0 + 32), k005(k0 + 40), k006(k0 + 48),
                  k007(k0 + 56);
                float32x4x2 k100(k1), k101(k1 + 8), k102(k1 + 16),
                  k103(k1 + 24), k104(k1 + 32), k105(k1 + 40), k106(k1 + 48),
                  k107(k1 + 56);
                float32x4x2 k200(k2), k201(k2 + 8), k202(k2 + 16),
                  k203(k2 + 24), k204(k2 + 32), k205(k2 + 40), k206(k2 + 48),
                  k207(k2 + 56);
                float32x4x2 k300(k3), k301(k3 + 8), k302(k3 + 16),
                  k303(k3 + 24), k304(k3 + 32), k305(k3 + 40), k306(k3 + 48),
                  k307(k3 + 56);
                ...
                for (int c = bin.first; c < bin.second; c++) {
                  ...
                  for (int i = 0; i < num_blocks; i++) {
                    ...
                    for (int qq = 0; qq < inputch; qq++) {
                      ...
                      const float *r2 = r1 + tm_c_offset;
                      const float *r3 = r2 + tm_c_offset;
                      float32x4x2  r00(r0), r01(r0 + 8), r02(r0 + 16),
                        r03(r0 + 24), r04(r0 + 32), r05(r0 + 40), r06(r0 + 48),
                        r07(r0 + 56);
                      float32x4x2 r10(r1), r11(r1 + 8), r12(r1 + 16),
                        r13(r1 + 24), r14(r1 + 32), r15(r1 + 40), r16(r1 + 48),
                        r17(r1 + 56);
                      float32x4x2 r20(r2), r21(r2 + 8), r22(r2 + 16),
                        r23(r2 + 24), r24(r2 + 32), r25(r2 + 40), r26(r2 + 48),
                        r27(r2 + 56);
                      float32x4x2 r30(r3), r31(r3 + 8), r32(r3 + 16),
                        r33(r3 + 24), r34(r3 + 32), r35(r3 + 40), r36(r3 + 48),
                        r37(r3 + 56);
                      ...
                      const float *k0c0 = k0, *k0c1 = k0c0 + 64,
                                  *k0c2 = k0c1 + 64, *k0c3 = k0c2 + 64;
                      const float *k1c0 = k1, *k1c1 = k1c0 + 64,
                                  *k1c2 = k1c1 + 64, *k1c3 = k1c2 + 64;
                      const float *k2c0 = k2, *k2c1 = k2c0 + 64,
                                  *k2c2 = k2c1 + 64, *k2c3 = k2c2 + 64;
                      const float *k3c0 = k3, *k3c1 = k3c0 + 64,
                                  *k3c2 = k3c1 + 64, *k3c3 = k3c2 + 64;
                    ...
                    for (int q = remain_inputch; q < input_channel; q++) {
                      ...
                      const float *k3 = kernel_tm_3 + q * ktm_c_offset;
                      float32x4x2  k000(k0), k001(k0 + 8), k002(k0 + 16),
                        k003(k0 + 24), k004(k0 + 32), k005(k0 + 40),
                        k006(k0 + 48), k007(k0 + 56);
                      float32x4x2 k100(k1), k101(k1 + 8), k102(k1 + 16),
                        k103(k1 + 24), k104(k1 + 32), k105(k1 + 40),
                        k106(k1 + 48), k107(k1 + 56);
                      float32x4x2 k200(k2), k201(k2 + 8), k202(k2 + 16),
                        k203(k2 + 24), k204(k2 + 32), k205(k2 + 40),
                        k206(k2 + 48), k207(k2 + 56);
                      float32x4x2 k300(k3), k301(k3 + 8), k302(k3 + 16),
                        k303(k3 + 24), k304(k3 + 32), k305(k3 + 40),
                        k306(k3 + 48), k307(k3 + 56);
...
#endif
    template <typename T>
    void Conv2dAlgorithm<T>::conv2d_3x3_sse(
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int outc_index = 0; outc_index < out_channel; outc_index++) {
          T *out_at = poutput + n * out_num_offset
                    + outc_index * out_channel_offset;

          for (int inc_index = 0; inc_index < input_channel; inc_index++) {
            T *out_ptr_0 = out_at;
            T *out_ptr_1 = out_ptr_0 + out_width;

            const T *input_cur = pinput + n * input_num_offset
                               + inc_index * input_channel_offset;

            const T *kernel_cur = pweight + outc_index * input_channel * 9
                                + inc_index * 9;
    ...
    template <>
    void Conv2dAlgorithm<float>::conv2d_3x3_sse(
      const Tensor &x, const Tensor &w, Tensor &out) {
      auto x_shape              = x.sizes();
      auto out_shape            = out.sizes();
      int  number               = x_shape [0];
...
#  pragma omp parallel for num_threads(openmp_threads())
#endif
        for (int outc_index = 0; outc_index < out_channel; outc_index++) {
          float *out_at = poutput + n * out_num_offset
                        + outc_index * out_channel_offset;
          ...
          for (int inc_index = 0; inc_index < input_channel; inc_index++) {
            ...
            const float *input_cur = pinput + n * input_num_offset
                                   + inc_index * input_channel_offset;

            const float *kernel_cur = pweight + outc_index * input_channel * 9
                                    + inc_index * 9;
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::conv2d_3x3_sse_inplace(
      const Tensor &x, const Tensor &w, Tensor &out) {
      auto x_shape              = x.sizes();
      auto out_shape            = out.sizes();
      int  number               = x_shape [0];
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    void Conv2dAlgorithm<float>::conv2d_3x3_sse_inplace(
      const Tensor &x, const Tensor &w, Tensor &out) {
      auto x_shape              = x.sizes();
      auto out_shape            = out.sizes();
      int  number               = x_shape [0];
          ...
          for (int inc_index = 0; inc_index < input_channel; inc_index++) {
            ...
            for (; i + 1 < out_height; i += 2) {
              ...
              for (; remain > 0; remain--) {
                ...
                float32x4 sum_00_x4(float(0));
                float32x4 sum_01_x4(float(0));
                float32x4 sum_10_x4(float(0));
                float32x4 sum_11_x4(float(0));
                float32x4 sum_20_x4(float(0));
                float32x4 sum_21_x4(float(0));
                float32x4 sum_30_x4(float(0));
                float32x4 sum_31_x4(float(0));
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::kernel_pack4x4(
      const Tensor &kernel, Tensor &kernel_packed) {
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int n = remain; n < kernel_num; n++) {
        const T *k0               = pkernel + n * num_offset;
        T       *kernel_packed_at = pkernel_packed + n * num_offset;
        for (int i = 0; i < num_offset; i++) { *kernel_packed_at++ = *k0++; }
      }
    }

    template <typename T>
    void Conv2dAlgorithm<T>::col_pack4x4(
      const T *col_tensor, int col_h, int col_w, T *col_packed) {
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    void Conv2dAlgorithm<float>::col_pack4x4(
      const float *col_tensor, int col_h, int col_w, float *col_packed) {
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::gemm_pack4x4(
      int      M,
      int      N,
      int      K,
      const T *kernel_packed,
      const T *col_packed,
      T       *out) {}

    template <>
    void Conv2dAlgorithm<float>::gemm_pack4x4(
      int          M,
      int          N,
      int          K,
      const float *kernel_packed,
      const float *col_packed,
      float       *out) {
      ...
      for (int mm = 0; mm < out_loop; mm++) {
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            ...
            float32x4 k1 = broadcast2float32x4(
              kernel_at + 1);  //[k10,k10,k10,k10]
            float32x4 k2 = broadcast2float32x4(
              kernel_at + 2);  //[k20,k20,k20,k20]
            float32x4 k3 = broadcast2float32x4(
              kernel_at + 3);  //[k30,k30,k30,k30]
          ...
          c1.store(output_row1);
          c2.store(output_row2);
          c3.store(output_row3);
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int m = remain; m < M; m++) {
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            float32x4 k0 = broadcast2float32x4(kernel_at);  //[k00,k00,k00,k00]
            float32x4 k1 = broadcast2float32x4(
              kernel_at + 1);  //[k01,k01,k01,k01]
            float32x4 k2 = broadcast2float32x4(
              kernel_at + 2);  //[k02,k02,k02,k02]
            float32x4 k3 = broadcast2float32x4(
              kernel_at + 3);  //[k03,k03,k03,k03]
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::kernel_pack8x8(
      const Tensor &kernel, Tensor &kernel_packed) {
      auto     shape          = kernel.sizes();
      int      kernel_num     = shape [0];
      int      kernel_channel = shape [1];
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::col_pack8x8(
      const T *col_tensor, int col_h, int col_w, T *col_packed) {
      const T *pcol        = col_tensor;
      T       *pcol_packed = col_packed;
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    void Conv2dAlgorithm<float>::col_pack8x8(
      const float *col_tensor, int col_h, int col_w, float *col_packed) {
      const float *pcol        = col_tensor;
      float       *pcol_packed = col_packed;
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void Conv2dAlgorithm<T>::gemm_pack8x8(
      int      M,
      int      N,
      int      K,
      const T *kernel_packed,
    ...
    template <>
    void Conv2dAlgorithm<float>::gemm_pack8x8(
      int          M,
      int          N,
      int          K,
      const float *kernel_packed,
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int mm = 0; mm < out_loop; mm++) {
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            //=====================pack_gemm k==0=====================
            float32x4x2 k0 = broadcast2float32x4x2(
              kernel_at);  //[k00,k00,k00,k00,k00,k00,k00,k00]
            float32x4x2 k1 = broadcast2float32x4x2(
              kernel_at + 1);  //[k10,k10,k10,k10,k10,k10,k10,k10]
            float32x4x2 k2 = broadcast2float32x4x2(
              kernel_at + 2);  //[k20,k20,k20,k20,k20,k20,k20,k20]
            float32x4x2 k3 = broadcast2float32x4x2(
              kernel_at + 3);  //[k30,k30,k30,k30,k30,k30,k30,k30]
            ...
            k0 = broadcast2float32x4x2(
              kernel_at + 4);  //[k40,k40,k40,k40,k40,k40,k40,k40]
            k1 = broadcast2float32x4x2(
              kernel_at + 5);  //[k50,k50,k50,k50,k50,k50,k50,k50]
            k2 = broadcast2float32x4x2(
              kernel_at + 6);  //[k60,k60,k60,k60,k60,k60,k60,k60]
            k3 = broadcast2float32x4x2(
              kernel_at + 7);  //[k70,k70,k70,k70,k70,k70,k70,k70]
            ...
            //=====================pack_gemm k==1=====================
            k0 = broadcast2float32x4x2(
              kernel_at + 8);  //[k01,k01,k01,k01,k01,k01,k01,k01]
            k1 = broadcast2float32x4x2(
              kernel_at + 9);  //[k11,k11,k11,k11,k11,k11,k11,k11]
            k2 = broadcast2float32x4x2(
              kernel_at + 10);  //[k21,k21,k21,k21,k21,k21,k21,k21]
            k3 = broadcast2float32x4x2(
              kernel_at + 11);  //[k31,k31,k31,k31,k31,k31,k31,k31]
            ...
            k0 = broadcast2float32x4x2(
              kernel_at + 12);  //[k41,k41,k41,k41,k41,k41,k41,k41]
            k1 = broadcast2float32x4x2(
              kernel_at + 13);  //[k51,k51,k51,k51,k51,k51,k51,k51]
            k2 = broadcast2float32x4x2(
              kernel_at + 14);  //[k61,k61,k61,k61,k61,k61,k61,k61]
            k3 = broadcast2float32x4x2(
              kernel_at + 15);  //[k71,k71,k71,k71,k71,k71,k71,k71]
            ...
            //=====================pack_gemm k==2=====================
            k0 = broadcast2float32x4x2(
              kernel_at + 16);  //[k02,k02,k02,k02,k02,k02,k02,k02]
            k1 = broadcast2float32x4x2(
              kernel_at + 17);  //[k12,k12,k12,k12,k12,k12,k12,k12]
            k2 = broadcast2float32x4x2(
              kernel_at + 18);  //[k22,k21,k21,k21,k21,k21,k21,k21]
            k3 = broadcast2float32x4x2(
              kernel_at + 19);  //[k32,k32,k32,k32,k32,k32,k32,k32]
            ...
            k0 = broadcast2float32x4x2(
              kernel_at + 20);  //[k42,k42,k42,k42,k42,k42,k42,k42]
            k1 = broadcast2float32x4x2(
              kernel_at + 21);  //[k52,k52,k52,k52,k52,k52,k52,k52]
            k2 = broadcast2float32x4x2(
              kernel_at + 22);  //[k62,k62,k62,k62,k62,k62,k62,k62]
            k3 = broadcast2float32x4x2(
              kernel_at + 23);  //[k72,k72,k72,k72,k72,k72,k72,k72]
            ...
            //=====================pack_gemm k==3=====================
            k0 = broadcast2float32x4x2(
              kernel_at + 24);  //[k03,k03,k03,k03,k03,k03,k03,k03]
            k1 = broadcast2float32x4x2(
              kernel_at + 25);  //[k13,k13,k13,k13,k13,k13,k13,k13]
            k2 = broadcast2float32x4x2(
              kernel_at + 26);  //[k23,k23,k23,k23,k23,k23,k23,k23]
            k3 = broadcast2float32x4x2(
              kernel_at + 27);  //[k33,k33,k33,k33,k33,k33,k33,k33]
            ...
            k0 = broadcast2float32x4x2(
              kernel_at + 28);  //[k43,k43,k43,k43,k43,k43,k43,k43]
            k1 = broadcast2float32x4x2(
              kernel_at + 29);  //[k53,k53,k53,k53,k53,k53,k53,k53]
            k2 = broadcast2float32x4x2(
              kernel_at + 30);  //[k63,k63,k63,k63,k63,k63,k63,k63]
            k3 = broadcast2float32x4x2(
              kernel_at + 31);  //[k73,k73,k73,k73,k73,k73,k73,k73]
          ...
          c2.store(output_row2);
          c3.store(output_row3);
          c4.store(output_row4);
          c5.store(output_row5);
          c6.store(output_row6);
          c7.store(output_row7);
        ...
        for (int n = n_remain; n < N; n++) {
          const float *kernel_at = kernel_store;
          const float *col_at    = pcol_packed + n * kernel_num_offset;
          float32x4x2  sum_col0(0.f), sum_col1(0.f), sum_col2(0.f),
            sum_col3(0.f);
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            ...
            float32x4x2 a0 = broadcast2float32x4x2(
              col_at);  //[a00,a00,a00,a00,a00,a00,a00,a00]
            float32x4x2 a1 = broadcast2float32x4x2(
              col_at + 1);  //[a10,a10,a10,a10,a10,a10,a10,a10]
            float32x4x2 a2 = broadcast2float32x4x2(
              col_at + 2);  //[a20,a20,a20,a20,a20,a20,a20,a20]
            float32x4x2 a3 = broadcast2float32x4x2(
              col_at + 3);  //[a30,a30,a30,a30,a30,a30,a30,a30]
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            float32x4x2 k0 = broadcast2float32x4x2(
              kernel_at);  //[k00,k00,k00,k00,k00,k00,k00,k00]
            float32x4x2 k1 = broadcast2float32x4x2(
              kernel_at + 1);  //[k01,k01,k01,k01,k01,k01,k01,k01]
            float32x4x2 k2 = broadcast2float32x4x2(
              kernel_at + 2);  //[k02,k02,k02,k02,k02,k02,k02,k02]
            float32x4x2 k3 = broadcast2float32x4x2(
              kernel_at + 3);  //[k03,k03,k03,k03,k03,k03,k03,k03]
