...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_conv2d_nchw_compute_run(
      const Tensor     &x,
      ...
      int  col_buffer_size = x_shape [1] * weight_shape [2] * weight_shape [3]
      ...
      for (int i = 0; i < number; i++) {
        ...
        // Tensor kernel_packed = stack.make(w.dtype(), w.sizes(),
        // MemoryDevice(CPU)); Conv2dAlgorithm<T>::kernel_pack8x8(w,
        // kernel_packed); packed_col = stack.make(x.dtype(), packed_shape,
        // MemoryDevice(CPU)); Conv2dAlgorithm<T>::col_pack8x8(col_buffer,
        // kernel_dims, conv_out_spatial_dim, packed_col.data<T>());
        // cpu::Conv2dAlgorithm<T>::gemm_pack8x8(weight_shape[0],
        // conv_out_spatial_dim, kernel_dims, kernel_packed.data<T>(),
        // packed_col.data<T>(), poutput); cpu::math<T,
        // T>::gemm(ts::blas::NoTrans,ts::blas::NoTrans, weight_shape[0],
        // conv_out_spatial_dim,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_conv2d_nchw_compute_run<TYPE>(                                         \
      x,                                                                       \
      padding,                                                                 \
      padding_value,                                                           \
      w,                                                                       \
