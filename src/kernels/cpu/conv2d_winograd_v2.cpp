...
namespace ts
{
  namespace cpu
  {
    ...
    void Conv2DWinogradV2::init() {
      ...
      m_op_conv2d_winograd = OperatorCreator::Create(
        context.computing_device.type(), name::layer::conv2d_winograd(), false);
      TS_CHECK_NQ(m_op_conv2d_winograd, nullptr)
        << "Can not find operator: " << name::layer::conv2d();

      m_op_conv2d_winograd->set(
        Bubble::RetentionParam::op,
        tensor::from(name::layer::conv2d_winograd()));
      m_op_conv2d_winograd->set(
      ...
      m_op_conv2d_winograd->set(
        name::kernel_winograd_transformed,
    ...
    int Conv2DWinogradV2::infer(
      Stack &stack, std::vector<ts::Tensor::Prototype> &output) {
