...
static float dmcn_im2col_bilinear(
...
static void modulated_deformable_im2col_gpu_kernel(
  const int    n,
  const float *data_im,
...
#  pragma omp parallel for num_threads(ts::openmp_threads())
  ...
  CPU_KERNEL_LOOP(index, n) {
    ...
    float *data_col_ptr = data_col
                        + ((b_col * num_channels * kernel_w * kernel_h + c_col)
                             * height_col
                           + h_col)
                            * width_col
                        + w_col;
    ...
    const float *data_offset_ptr = data_offset
                                 + (b_col * deformable_group
                                    + deformable_group_index)
                                     * 2 * kernel_h * kernel_w * height_col
                                     * width_col;

    const float *data_mask_ptr = data_mask
                               + (b_col * deformable_group
                                  + deformable_group_index)
                                   * kernel_h * kernel_w * height_col
                                   * width_col;
