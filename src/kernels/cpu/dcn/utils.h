  ...
  namespace dcn
  {
    namespace cpu
    {
      ...
      inline void adjustLdLevel3(
        ...
        // Note: leading dimensions generally are checked that they are > 0 and
        // at least as big the result requires (even if the value won't be
        // used).
      ...
      static void CBlas_SgemmBatched(
        char         transa,
        char         transb,
        int64_t      m,
        int64_t      n,
        ...
        if (
          ...
          TS_LOG_ERROR << "CBlas_SgemmBatched only supports m, n, k, lda, ldb, "
                          "ldc, batchCount"
                          "with the bound [val] <= "
                       << INT_MAX << eject;
        }
