...
namespace ts
{
  namespace cpu
  {
    ...
    inline static void depthwise_inner(
      ...
      for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
          ...
          int w_index = j + w_in;
          if (
            h_index >= 0 && h_index < input_h && w_index >= 0
            && w_index < input_w) {
        ...
        for (int c = 0; c < output_shape [1]; c++) {
          for (int h = 0; h < output_shape [2]; h++) {
            for (int w = 0; w < output_shape [3]; w++) {
              const T *pweight = pweight_base
                               + c * weight_shape [2] * weight_shape [3];
              T value = 0;
              for (int kh = 0; kh < weight_shape [2]; kh++) {
                for (int kw = 0; kw < weight_shape [3]; kw++) {
                  int h_in = -padding.top + h * stride.height
                           + kh * dilation.height;
                  int w_in = -padding.left + w * stride.width
                           + kw * dilation.width;
                  if (
                    (h_in >= 0) && (h_in < input_shape [2]) && (w_in >= 0)
                    && (w_in < input_shape [3])) {
                    int offset = ((n * output_shape [1] + c) * input_shape [2]
                                  + h_in)
                                 * input_shape [3]
                               + w_in;
    ...
    void DepthwiseConv2dAlgorithm<float>::depthwise_3x3_s1(
      ...
      // h_end = padding.bottom == 0 ? output_height : output_height -
      // ((padding.bottom - 1) / stride.height + 1) w_strst = padding.left == 0
      // ? 0 : (padding.left - 1) / stride.width + 1 w_end = padding.right == 0
      // ? output_width : output_width - ((padding.right - 1) / stride.width +
      // 1)
      int h_start = padding.top == 0 ? 0 : padding.top;
      int h_end   = padding.bottom == 0 ? output_height :
                                          output_height - padding.bottom;
      int w_start = padding.left == 0 ? 0 : padding.left;
      int w_end   = padding.right == 0 ? output_width :
                                         output_width - padding.right;
...
#  endif
#endif
        for (int c = 0; c < output_shape [1]; c++) {
          const float *input_at = pinput + n * input_num_offset
                                + c * input_channel_offset;
          const float *kernel_at = pkernel + c * 9;

          float *out_at = poutput + n * output_num_offset
                        + c * output_channel_offset;
          //#ifdef TS_USE_AVX
          ...
          for (h = h_start; h + 1 < h_end; h += 2) {
            ...
            for (w = w_start; w + 3 < w_end; w += 4) {
              ...
              const float *i0 = input_at + (h - padding.top) * input_width
                              + (w - padding.left);
    ...
    void DepthwiseConv2dAlgorithm<float>::depthwise_3x3_s2(
      ...
      int h_end   = padding.bottom == 0 ?
                      output_height :
                      output_height - ((padding.bottom - 1) / 2 + 1);
      int w_start = padding.left == 0 ? 0 : (padding.left - 1) / 2 + 1;
      int w_end   = padding.right == 0 ?
                      output_width :
                      output_width - ((padding.right - 1) / 2 + 1);
          ...
          for (h = h_start; h < h_end; h++) {
            ...
            for (w = w_start; w + 3 < w_end; w += 4) {
              ...
              const float *i0 = input_at
                              + (h * stride.height - padding.top) * input_width
                              + (w * stride.width - padding.left);
              const float *i1     = i0 + input_width;
              const float *i2     = i1 + input_width;
              // const float* i3 = i2 + input_width;
          ...
          for (h = h_start; h + 1 < h_end; h += 2) {
            ...
            for (w = w_start; w + 3 < w_end; w += 4) {
              ...
              float32x4 i20_x4 = inc_load(i2, 2),
                        i21_x4 = inc_load((i2 + 1), 2),
                        i22_x4 = inc_load((i2 + 2), 2);
              float32x4 i30_x4 = inc_load(i3, 2),
                        i31_x4 = inc_load((i3 + 1), 2),
                        i32_x4 = inc_load((i3 + 2), 2);
...
template class ts::cpu::DepthwiseConv2dAlgorithm<
  ts::dtype<ts::FLOAT32>::declare>;
template class ts::cpu::DepthwiseConv2dAlgorithm<
  ts::dtype<ts::FLOAT64>::declare>;