...
namespace ts
{
  namespace cpu
  {
    ...
    void DepthwiseConv2DV2::init() {
      ...
      m_op_conv2d = OperatorCreator::Create(
        context.computing_device.type(),
        name::layer::depthwise_conv2d(),
        false);

      TS_CHECK_NQ(m_op_conv2d, nullptr)
        << "Can not find operator: " << name::layer::depthwise_conv2d();

      m_op_conv2d->set(
        Bubble::RetentionParam::op,
        tensor::from(name::layer::depthwise_conv2d_v2()));
      m_op_conv2d->set(
      ...
      if (has(name::dilation))
        m_op_conv2d->set(name::dilation, get(name::dilation));
      if (has(name::typo::dialations))
        m_op_conv2d->set(name::typo::dialations, get(name::typo::dialations));
    }
