...
namespace ts
{
  namespace cpu
  {
    ...
    void caffe_set(const int N, const T alpha, T *X) {
      memset(X, CPU, N * sizeof(T), &alpha, CPU, sizeof(T));
    }
    ...
    class DetectionOutput : public Operator {
      public:
        ...
        DetectionOutput() {
          ...
          field(
            "variance_encoded_in_target", OPTIONAL, tensor::from<bool>(false));
        ...
        void init() final {
          ...
          variance_encoded_in_target_ = tensor::to_bool(
            get("variance_encoded_in_target"));
          ...
          TS_CHECK_GE(nms_threshold_, 0.)
            << "nms_threshold must be non negative." << eject;
          eta_ = tensor::to_float(get("nms_eta"));
          TS_AUTO_CHECK_GT(eta_, 0.);
          TS_AUTO_CHECK_LE(eta_, 1.);
          top_k_ = -1;
          if (has("nms_top_k")) { top_k_ = tensor::to_int(get("nms_top_k")); }
        }

        std::vector<int> infer_top_shape(
          Stack &stack, bool alloc_buffer = false) {
          auto &bottom = stack;
          ...
          // Since the number of bboxes to be kept is unknown before nms, we
          // manually set it to (fake) 1.
        ...
        int infer(Stack &stack, std::vector<Tensor::Prototype> &output) final {
          ...
          output [0] = Tensor::Prototype(
            stack [0].dtype(), infer_top_shape(stack, false));
          return 1;
        }
        ...
        int run(Stack &stack) final {
          ...
          GetConfidenceScores(
            conf_data, num, num_priors_, num_classes_, &all_conf_scores);

          // Retrieve all prior bboxes. It is same within a batch since we
          // assume all images in a batch are of same dimension.
          vector<NormalizedBBox> prior_bboxes;
          vector<vector<float>>  prior_variances;
          GetPriorBBoxes(
            prior_data, num_priors_, &prior_bboxes, &prior_variances);
          ...
          for (int i = 0; i < num; ++i) {
            ...
            for (int c = 0; c < num_classes_; ++c) {
              ...
              const vector<NormalizedBBox> &bboxes = decode_bboxes.find(label)
                                                       ->second;
              ApplyNMSFast(
                bboxes,
                scores,
          ...
          for (int i = 0; i < num; ++i) {
            ...
            for (map<int, vector<int>>::iterator it = all_indices [i].begin();
              ...
              if (decode_bboxes.find(loc_label) == decode_bboxes.end()) {
                // Something bad happened if there are no predictions for
                // current label.
                TS_LOG(LOG_FATAL) << "Could not find location predictions for "
                                  << loc_label << eject;
                continue;
              }
              const vector<NormalizedBBox>
                          &bboxes  = decode_bboxes.find(loc_label)->second;
