...
namespace ts
{
  namespace cpu
  {
    void Dimshuffle::dimshuffle(
      const Tensor &x, int dim, const std::vector<int> &shuffle, Tensor &out) {
      ...
      size_t preoffset = 1;
      for (int i = 0; i < dim; i++) { preoffset *= shape [i]; }
      ...
      for (size_t k = 0; k < preoffset; k++) {
        for (size_t i = 0; i < shuffle.size(); i++) {
          memcpy_handler(
            device_id,
            pdst + type_len * (k * newstride + i * backstride),
            device_id,
            psrc + type_len * (k * stride + shuffle [i] * backstride),
            ncpy);
        }
      }
    }
