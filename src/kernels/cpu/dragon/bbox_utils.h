...
#define ROUND(x) ((int)((x) + (T)0.5))
      ...
      inline void SortProposals(
        const int start, const int end, const int num_top, T *proposals) {
        ...
        if (start < right - 1)
          SortProposals(start, right - 1, num_top, proposals);
      ...
      inline void CollectRoIs(
        ...
        const T *roi = rois;
        for (int i = 0; i < num_rois; ++i) {
          int bin_idx = roi_level(
            min_level, max_level, canonical_level, canonical_scale, roi);
