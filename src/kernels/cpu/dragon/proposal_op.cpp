  ...
  namespace dragon
  {
      ...
      for (int n = 0; n < num_images; ++n) {
        ...
        if (strides.size() == 1) {
          ...
          CHECK_EQ(strides.size(), scales.size())
            << "\nGiven " << strides.size() << " strides and " << scales.size()
            << " scales" << eject;
          ...
          CHECK_EQ(acc_proposals, total_proposals)
            << "\nExcepted " << total_proposals
            << " proposals from the network, "
            << "but generated " << acc_proposals << " proposals." << eject;

          rcnn::GenerateProposals_v2<float, Context>(
            int(total_proposals),
            im_height,
            im_width,
            min_box_h,
          ...
          LOG(FATAL)
            << "There should be given at least one stride for proposals."
            << eject;
        }
        total_rois += num_rois;
        Ydata += (num_rois * 5);
    ...
    void ProposalOp<Context>::RunOnDevice() {
      ...
      if (XIsType(Input(-3), float)) {
        ...
        LOG(FATAL) << DTypeHelper(Input(0), { "float32", "float16" }) << eject;
    }
