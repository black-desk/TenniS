    ...
    class Expand : public Operator {
      ...
      public:
        ...
        bool expand(Shape &shape, size_t dims) {
          ...
          if (
            m_front > 0 && m_end > 0
            && (shape.size() + size_t(m_front) + size_t(m_end)) < dims) {
            return false;
          }
          if (m_inverse) {
            return expand_end(shape, dims, m_end)
                || expand_front(shape, dims, m_front);
          } else {
            return expand_front(shape, dims, m_front)
                || expand_end(shape, dims, m_end);
          }
        }
        ...
        std::vector<int32_t> expand(Stack &stack) {
          ...
          if (dims < 0 || ! expand(shape, size_t(dims))) {
            TS_LOG_ERROR << "Can not exapnd " << to_string(x.sizes())
                         << " to dims=" << dims << ", with front=" << m_front
                         << ", end=" << m_end << std::boolalpha
                         << ", inverse=" << m_inverse << "." << eject;
          }
