  ...
  namespace cpu
  {
    class Flatten2D : public OperatorOnAny<base::NewShape> {
      public:
        ...
        Flatten2D() { field("dim", OPTIONAL, tensor::from<int32_t>(1)); }

        void init() override { m_dim = tensor::to_int(get("dim")); }
        ...
        Shape newshape(const Tensor &x) final {
          ...
          if (fixed_dim <= 0) {
            ...
            return {
              std::accumulate(
                size.begin(),
                size.begin() + fixed_dim,
                1,
                std::multiplies<int>()),
              std::accumulate(
                size.begin() + fixed_dim, size.end(), 1, std::multiplies<int>())
            };
          }
        }
      private:
