...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    void cpu_force_gray_compute_run(
      const Tensor &x, const std::vector<float> &scale, Tensor &out) {
      auto &size   = x.sizes();
      auto  dims   = x.dims();
      auto  number = std::accumulate(
        size.begin(), size.end() - 1, 1, std::multiplies<int32_t>());
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_force_gray_compute_run<TYPE>(x, scale, out);                           \
    break;                                                                     \
  }
