...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_batch_norm_compute_run(
      const Tensor &x,
      const Tensor &mean,
      ...
      int          predims  = 1;
      int          backdims = 1;
      for (int i = 0; i < dim; i++) { predims *= shape [i]; }

      for (int i = dim + 1; i < shape.size(); i++) { backdims *= shape [i]; }
      ...
      for (int i = 0; i < predims; i++) {
        for (int k = 0; k < shape [dim]; k++) {
          ...
          for (int m = 0; m < backdims; m++) {
            *pdst_temp = (*psrc_temp - mean_val) * vec_val * scale_val
                       + bias_val;
            pdst_temp++;
            psrc_temp++;
          }
...
#ifdef TS_USE_SIMD
    template <>
    void cpu_batch_norm_compute_run<float>(
      const Tensor &x,
      const Tensor &mean,
      const Tensor &variance,
      const Tensor &scale,
    ...
    void FusedBatchNorm::batch_norm(
      const Tensor &x,
      const Tensor &mean,
      const Tensor &variance,
      const Tensor &scale,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_batch_norm_compute_run<TYPE>(                                          \
      x, mean, variance, scale, bias, dim, epsilon, out);                      \
    break;                                                                     \
  }
