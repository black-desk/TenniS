...
namespace ts
{
  ...
  {
    void Gather::gather(
      const Tensor &x, const Tensor &indices, int axis, Tensor &out) {
      ...
      auto  number  = std::accumulate(
        x_shape.begin(), x_shape.begin() + axis, 1, std::multiplies<int>());
      auto width = std::accumulate(
        x_shape.begin() + axis + 1, x_shape.end(), 1, std::multiplies<int>());
      ...
      for (int i = 0; i < norm_out_shape.shape(1); ++i) {
        ...
        for (int n = 0; n < norm_out_shape.shape(0); ++n) {
          ...
          norm_x_coord [0]   = n;
          auto src_ptr       = x_data
                       + norm_x_shape.to_index(norm_x_coord) * width_bytes;
          auto dst_ptr = out_data
                       + norm_out_shape.to_index(norm_out_coord) * width_bytes;
          memcpy_handler(
            device_id, dst_ptr, device_id, src_ptr, size_t(width_bytes));
        }
      }
    }
  }  // namespace cpu
