...
namespace ts
{
  ...
  void im2col_cpu(
    ...
    const int    kernel_w,
    const int    pad_h_top,
    const int    pad_h_bottom,
    const int    pad_w_left,
    const int    pad_w_right,
    const int    stride_h,
    const int    stride_w,
    ...
    const int output_h = int(std::floor(
      (height + pad_h_top + pad_h_bottom - (dilation_h * (kernel_h - 1) + 1))
        / stride_h
      + 1));

    const int output_w     = int(std::floor(
      (width + pad_w_left + pad_w_right - (dilation_w * (kernel_w - 1) + 1))
        / stride_w
      + 1));
...
#  endif
          ...
          for (int output_rows = output_h; output_rows; output_rows--) {
            if (! is_a_ge_zero_and_a_lt_b(input_row, height)) {
              ...
              for (int output_col = output_w; output_col; output_col--) {
                if (is_a_ge_zero_and_a_lt_b(input_col, width)) {
                  *(local_data_col++) =
                    local_data_im [input_row * width + input_col];
                } else {
                  *(local_data_col++) = padding_value;  // 0;
                }
                input_col += stride_w;
  ...
  template void im2col_cpu<int8_t>(
