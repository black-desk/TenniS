...
namespace ts
{
  ...
  {
    template <typename T>
    static void cpu_inner_prod_compute_run(
      const Tensor &lhs,
      const Tensor &rhs,
      bool          transpose,
      Tensor       &out,
      ...
      if (transpose && kernel_packed) {
        TS_LOG_ERROR << "What a Terrible Failure: dealing transpose weights "
                        "without transpose support, because supporting pack"
                     << eject;
      }

      auto no_trasposed_rhs = rhs;
      if (transpose) { no_trasposed_rhs = intime::transpose(rhs, { 1, 0 }); }
      ...
      cpu::math<T, T>::gemm(
        ...
        true,
        ! kernel_packed);
      // cpu::math<T, T>::gemm(blas::NoTrans, rhs_transpose, lhs_shape[0], N,
      // lhs_shape[1],
      //                    (T) 1, psrc, pdot, (T) 0, pdst);
#endif
    }

    void InnerProd::inner_prod(
      const Tensor &lhs,
      const Tensor &rhs,
      bool          transpose,
      Tensor       &out,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_inner_prod_compute_run<TYPE>(                                          \
      lhs, rhs, transpose, out, stack, kernel_packed);                         \
    break;                                                                     \
  }
