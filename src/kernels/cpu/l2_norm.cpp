  ...
  namespace cpu
  {
    class L2Norm : public OperatorOnCPU<base::L2Norm> {
      public:
        ...
        void normalize(
          const Tensor &x, int dim, float epsilon, Tensor &out) override;
    };
  }  // namespace cpu
}  // namespace ts
...
namespace ts
{
  ...
  {
    template <typename T>
    void cpu_l2_normalize_compute_run(
      const Tensor &x, int m_dim, float epsilon, Tensor &out) {
    ...
    template <typename T>
    void cpu_l2_norm_compute_run(
      const Tensor &x, int m_dim, float epsilon, Tensor &out) {
      return cpu_l2_normalize_compute_run<T>(x, m_dim, epsilon, out);
    }

    void L2Norm::normalize(
      const Tensor &x, int dim, float epsilon, Tensor &out) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_l2_norm_compute_run<TYPE>(x, dim, epsilon, out);                       \
    break;                                                                     \
  }
