...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_leay_relu_compute_run(
      const Tensor &x, float scale, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    void cpu_leay_relu_compute_run<float>(
      const Tensor &x, float scale, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int i = 0; i < count_4; ++i) {
        ...
        float32x4 output_data_x4 = max_float32x4(input_data_x4, const_num_x4)
                                 + casted_scale_x4
                                     * min_float32x4(
                                       input_data_x4, const_num_x4);
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_leay_relu_compute_run<TYPE>(x, scale, out);                            \
    break;                                                                     \
  }
