...
namespace ts
{
  namespace cpu
  {
    ...
    inline T_OUT inline_dot(
      int N, const T_IN *x, int incx, const T_IN *y, int incy) {
      T_OUT sum = 0;
...
#ifdef TS_USE_SIMD
    template <>
    inline float inline_dot<float, float>(
      ...
      float32x4 sumx4 = 0;
      for (; i < N - 3; i += 4) {
        sumx4 += float32x4(x [0], x [incx1], x [incx2], x [incx3])
               * float32x4(y [0], y [incy1], y [incy2], y [incy3]);
        x += incx4;
        y += incy4;
      }
    ...
    T_OUT math<T_IN, T_OUT>::dot(
      ...
      const auto count = range.second - range.first;
      parallel_sum [__parallel_id] += inline_dot<T_IN, T_OUT>(
        count, xx, incx, yy, incy);
      TS_PARALLEL_RANGE_END()
      T_OUT sum = 0;
    ...
    inline void inline_gemm_row_major(
      ...
      unsigned int condition = (TransA == blas::NoTrans ? 0U : 1U)
                             | ((TransB == blas::NoTrans ? 0U : 2U));
      switch (condition) {
        ...
        case 1:  // A: Trans, B: NoTrans
          ...
          for (int j = 0; j < N; ++j) {
            *C_anchor += alpha
                       * inline_dot<T_IN, T_OUT>(K, &A [i], lda, &B [j], ldb);
            C_anchor++;
          }
          TS_PARALLEL_FOR_END()
          break;
    ...
    void math<T_IN, T_OUT>::gemm(
      ...
      if (Order == blas::ColMajor) {
        inline_gemm_row_major<T_IN, T_OUT>(
    ...
    template <typename T_IN, typename T_OUT>
    void math<T_IN, T_OUT>::pack8_A(
      int row, int col, const T_IN *from, int lda, T_IN *to) {
      int out_loop = row >> 3;
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int n = remain; n < row; n++) {
        const T_IN *k0    = from + n * lda;
        T_IN       *to_at = to + n * col;
        for (int i = 0; i < col; i++) { *to_at++ = *k0++; }
      }
    }

    template <typename T_IN, typename T_OUT>
    inline void inline_pack8_B(
      int row, int col, const T_IN *from, int ldb, T_IN *to) {
      int out_loop = col >> 3;
      int remain   = out_loop << 3;
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    inline void inline_pack8_B<float, float>(
      int row, int col, const float *from, int ldb, float *to) {
      int out_loop = col >> 3;
      int remain   = out_loop << 3;
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T_IN, typename T_OUT>
    void math<T_IN, T_OUT>::pack8_B(
      int row, int col, const T_IN *from, int ldb, T_IN *to) {
      inline_pack8_B<T_IN, T_OUT>(row, col, from, ldb, to);
    }

    template <typename T_IN, typename T_OUT>
    inline void kernel_8x8(
      int         M,
      int         K,
      int         N,
      T_IN        alpha,
    ...
    template <>
    inline void kernel_8x8<float, float>(
      int          M,
      int          K,
      int          N,
      float        alpha,
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            ...
            float32x4x2 k0 = broadcast2float32x4x2(
              A_at);  //[k00,k00,k00,k00,k00,k00,k00,k00]
            float32x4x2 k1 = broadcast2float32x4x2(
              A_at + 1);  //[k10,k10,k10,k10,k10,k10,k10,k10]
            float32x4x2 k2 = broadcast2float32x4x2(
              A_at + 2);  //[k20,k20,k20,k20,k20,k20,k20,k20]
            float32x4x2 k3 = broadcast2float32x4x2(
              A_at + 3);  //[k30,k30,k30,k30,k30,k30,k30,k30]
            ...
            k0 = broadcast2float32x4x2(
              A_at + 4);  //[k40,k40,k40,k40,k40,k40,k40,k40]
            k1 = broadcast2float32x4x2(
              A_at + 5);  //[k50,k50,k50,k50,k50,k50,k50,k50]
            k2 = broadcast2float32x4x2(
              A_at + 6);  //[k60,k60,k60,k60,k60,k60,k60,k60]
            k3 = broadcast2float32x4x2(
              A_at + 7);  //[k70,k70,k70,k70,k70,k70,k70,k70]
            ...
            //=====================pack_gemm k==1=====================
            k0 = broadcast2float32x4x2(
              A_at + 8);  //[k01,k01,k01,k01,k01,k01,k01,k01]
            k1 = broadcast2float32x4x2(
              A_at + 9);  //[k11,k11,k11,k11,k11,k11,k11,k11]
            k2 = broadcast2float32x4x2(
              A_at + 10);  //[k21,k21,k21,k21,k21,k21,k21,k21]
            k3 = broadcast2float32x4x2(
              A_at + 11);  //[k31,k31,k31,k31,k31,k31,k31,k31]
            ...
            k0 = broadcast2float32x4x2(
              A_at + 12);  //[k41,k41,k41,k41,k41,k41,k41,k41]
            k1 = broadcast2float32x4x2(
              A_at + 13);  //[k51,k51,k51,k51,k51,k51,k51,k51]
            k2 = broadcast2float32x4x2(
              A_at + 14);  //[k61,k61,k61,k61,k61,k61,k61,k61]
            k3 = broadcast2float32x4x2(
              A_at + 15);  //[k71,k71,k71,k71,k71,k71,k71,k71]
            ...
            //=====================pack_gemm k==2=====================
            k0 = broadcast2float32x4x2(
              A_at + 16);  //[k02,k02,k02,k02,k02,k02,k02,k02]
            k1 = broadcast2float32x4x2(
              A_at + 17);  //[k12,k12,k12,k12,k12,k12,k12,k12]
            k2 = broadcast2float32x4x2(
              A_at + 18);  //[k22,k21,k21,k21,k21,k21,k21,k21]
            k3 = broadcast2float32x4x2(
              A_at + 19);  //[k32,k32,k32,k32,k32,k32,k32,k32]
            ...
            k0 = broadcast2float32x4x2(
              A_at + 20);  //[k42,k42,k42,k42,k42,k42,k42,k42]
            k1 = broadcast2float32x4x2(
              A_at + 21);  //[k52,k52,k52,k52,k52,k52,k52,k52]
            k2 = broadcast2float32x4x2(
              A_at + 22);  //[k62,k62,k62,k62,k62,k62,k62,k62]
            k3 = broadcast2float32x4x2(
              A_at + 23);  //[k72,k72,k72,k72,k72,k72,k72,k72]
            ...
            //=====================pack_gemm k==3=====================
            k0 = broadcast2float32x4x2(
              A_at + 24);  //[k03,k03,k03,k03,k03,k03,k03,k03]
            k1 = broadcast2float32x4x2(
              A_at + 25);  //[k13,k13,k13,k13,k13,k13,k13,k13]
            k2 = broadcast2float32x4x2(
              A_at + 26);  //[k23,k23,k23,k23,k23,k23,k23,k23]
            k3 = broadcast2float32x4x2(
              A_at + 27);  //[k33,k33,k33,k33,k33,k33,k33,k33]
            ...
            k0 = broadcast2float32x4x2(
              A_at + 28);  //[k43,k43,k43,k43,k43,k43,k43,k43]
            k1 = broadcast2float32x4x2(
              A_at + 29);  //[k53,k53,k53,k53,k53,k53,k53,k53]
            k2 = broadcast2float32x4x2(
              A_at + 30);  //[k63,k63,k63,k63,k63,k63,k63,k63]
            k3 = broadcast2float32x4x2(
              A_at + 31);  //[k73,k73,k73,k73,k73,k73,k73,k73]
        ...
        for (int n = n_remain; n < N; n++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            ...
            float32x4x2 a0 = broadcast2float32x4x2(
              B_at);  //[a00,a00,a00,a00,a00,a00,a00,a00]
            float32x4x2 a1 = broadcast2float32x4x2(
              B_at + 1);  //[a10,a10,a10,a10,a10,a10,a10,a10]
            float32x4x2 a2 = broadcast2float32x4x2(
              B_at + 2);  //[a20,a20,a20,a20,a20,a20,a20,a20]
            float32x4x2 a3 = broadcast2float32x4x2(
              B_at + 3);  //[a30,a30,a30,a30,a30,a30,a30,a30]
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int nn = 0; nn < n_loop; nn++) {
          ...
          for (int kk = 0; kk < k_loop; kk++) {
            float32x4x2 k0 = broadcast2float32x4x2(
              A_at);  //[k00,k00,k00,k00,k00,k00,k00,k00]
            float32x4x2 k1 = broadcast2float32x4x2(
              A_at + 1);  //[k01,k01,k01,k01,k01,k01,k01,k01]
            float32x4x2 k2 = broadcast2float32x4x2(
              A_at + 2);  //[k02,k02,k02,k02,k02,k02,k02,k02]
            float32x4x2 k3 = broadcast2float32x4x2(
              A_at + 3);  //[k03,k03,k03,k03,k03,k03,k03,k03]
    ...
    void math<T_IN, T_OUT>::gemm(
      ...
      if (! ts::near(alpha, T_IN(1)) || ! ts::near(beta, T_IN(0))) {
        TS_LOG_ERROR << "alpha should be one and beta should be zero now!"
                     << eject;
      }

      if (A_need_pack) { math<T_IN, T_OUT>::pack8_A(M, K, A, K, A_packed); }
      if (B_need_pack) { math<T_IN, T_OUT>::pack8_B(K, N, B, N, B_packed); }
    ...
    template <typename T_IN, typename T_OUT>
    void math<T_IN, T_OUT>::matrix_transpose(
      const T_IN *A, T_OUT *B, int m, int n) {
      int i, j;
      for (i = 0; i < n; i++) {
...
template class ts::cpu::
  math<ts::dtype<ts::FLOAT32>::declare, ts::dtype<ts::FLOAT32>::declare>;
template class ts::cpu::
  math<ts::dtype<ts::FLOAT64>::declare, ts::dtype<ts::FLOAT64>::declare>;
template class ts::cpu::
  math<ts::dtype<ts::INT8>::declare, ts::dtype<ts::INT32>::declare>;