...
namespace ts
{
  namespace cpu
  {
    ...
    static inline bool check_suppress(
      ...
      if (mode == 1) {
        ymin_i = std::min<T>(
          boxes [i * 4 + 0], boxes [i * 4 + 0] + boxes [i * 4 + 2]);
        xmin_i = std::min<T>(
          boxes [i * 4 + 1], boxes [i * 4 + 1] + boxes [i * 4 + 3]);
        ymax_i = std::max<T>(
          boxes [i * 4 + 0], boxes [i * 4 + 0] + boxes [i * 4 + 2]);
        xmax_i = std::max<T>(
          boxes [i * 4 + 1], boxes [i * 4 + 1] + boxes [i * 4 + 3]);
        ...
        ymin_j = std::min<T>(
          boxes [j * 4 + 0], boxes [j * 4 + 0] + boxes [j * 4 + 2]);
        xmin_j = std::min<T>(
          boxes [j * 4 + 1], boxes [j * 4 + 1] + boxes [j * 4 + 3]);
        ymax_j = std::max<T>(
          boxes [j * 4 + 0], boxes [j * 4 + 0] + boxes [j * 4 + 2]);
        xmax_j = std::max<T>(
          boxes [j * 4 + 1], boxes [j * 4 + 1] + boxes [j * 4 + 3]);
      ...
      const T intersection_area = std::max<T>(
    ...
    template <typename T>
    static void cpu_non_max_suppression_v3_compute_run(
      const Tensor      &x,
      const Tensor      &scores,
      int                max_output,
      float              iou_threshold,
      ...
      while (selected.size() < max_output
    ...
    void Non_Max_Suppression_V3::non_max_suppression_v3(
      const Tensor &x, const Tensor &scores, Tensor &out) {
      DTYPE dtype = x.dtype();
      switch (dtype) {
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_non_max_suppression_v3_compute_run<TYPE>(                              \
      x,                                                                       \
      scores,                                                                  \
      m_max_output_size,                                                       \
      m_iou_threshold,                                                         \
...
TS_REGISTER_OPERATOR(
  Non_Max_Suppression_V3, CPU, name::layer::non_max_suppression_v3())
