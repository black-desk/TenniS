...
namespace ts
{
  namespace cpu
  {
    ...
    static inline pad_menu get_pad_menu(
      int size, const std::array<int, 2> &padding) {
      int head = -padding [0];
      int tail = size + padding [1];

      if (head >= tail) { return { { 0, 0 }, { 0, 0, 0 }, { 0, 0 } }; }

      int      head_count = -head;
      set_menu menu_head  = head_count > 0 ? set_menu({ 0, head_count }) :
                                             set_menu({ 0, 0 });
      ...
      copy_menu menu_body  = body_count > 0 ?
                               copy_menu(
                                { body_left - head, body_left, body_count }) :
    ...
    static void pad2d(
      ...
      auto width = std::accumulate(
        shape.begin() + dim + 2, shape.end(), 1, std::multiplies<int>());
      ...
      auto memcpy_handler = HardConverter::Query(
        out.device().type(), x.device().type());
      ...
      for (int n = 0; n < number; ++n) {
        ...
        if (menu_h.head.count) {
          auto dst_index = dst_shape.to_index(
                             { n, menu_h.head.dst_index, 0, 0 })
                         * bytes;
          auto set_count = menu_h.head.count * dst_shape.shape(2)
                         * dst_shape.shape(3) * bytes;
          memset(
            out.data<char>() + dst_index,
            out.device(),
            set_count,
            padding_data.data(),
            padding_data.device(),
        ...
        if (menu_w.head.count) {
          for (int h = 0; h < menu_h.body.count; ++h) {
            auto dst_index = dst_shape.to_index({ n,
                                                  menu_h.body.dst_index + h,
                                                  menu_w.head.dst_index,
                                                  0 })
                           * bytes;
        ...
        if (menu_h.tail.count) {
          auto dst_index = dst_shape.to_index(
                             { n, menu_h.tail.dst_index, 0, 0 })
                         * bytes;
          auto set_count = menu_h.tail.count * dst_shape.shape(2)
                         * dst_shape.shape(3) * bytes;
          memset(
            out.data<char>() + dst_index,
            out.device(),
        ...
        if (menu_h.body.count && menu_w.body.count) {
          for (int h = 0; h < menu_h.body.count; ++h) {
            auto dst_index = dst_shape.to_index({ n,
                                                  menu_h.body.dst_index + h,
                                                  menu_w.body.dst_index,
                                                  0 })
                           * bytes;
            auto src_index = src_shape.to_index({ n,
                                                  menu_h.body.src_index + h,
                                                  menu_w.body.src_index,
                                                  0 })
                           * bytes;
    ...
    void Pad::pad(
      ...
      if (left >= padding.size()) {
        memcpy(
          out.data(),
          out.device(),
          size_t(out.count() * out.proto().type_bytes()),
          x.data(),
          x.device(),
          size_t(x.count() * x.proto().type_bytes()));
        return;
      }
