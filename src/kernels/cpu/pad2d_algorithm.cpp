...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    void PadAlgorithm<T>::pad_nchw_nhwc(
      const Tensor                          &x,
        ...
        for (int i = 0; i < dim1; ++i) {
          for (int j = 0; j < dim2; ++j) {
            ...
            if (tile < 12) {
              ...
              for (int l = 0; l < tile; ++l) { out_at [l] = input_at [l]; }
            } else {
              std::memcpy(
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < channel; c++) {
          ...
          for (; h < pad_top + src_shape [2]; h++) {
            int w = 0;
            for (; w < pad_left; w++) { dst_at [w] = value; }
            ...
            if (src_w < 12) {
              ...
              // memcpy(dst_at + pad_left, out_device, size_t(src_w *
              // out.proto().type_bytes()),
        ...
        for (int c = 0; c < channel; c++) {
          ...
          for (int h = 0; h < out_h; h++) {
            ...
            if (out_w < 12) {
              ...
              // memcpy(dst_at, out.device(), size_t(out_w *
              // out.proto().type_bytes()), cut_at, x.device(), size_t(out_w *
              // x.proto().type_bytes()));
