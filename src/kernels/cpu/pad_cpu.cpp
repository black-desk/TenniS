...
namespace ts
{
  namespace cpu
  {
     ...
     */
    static inline bool transpose_pad(
      const Shape                           &out_coord,
    ...
    template <typename T>
    static inline void cpu_pad_compute_run(
      const Tensor                          &x,
      const std::vector<std::array<int, 2>> &padding,
      float                                  padding_value,
      Tensor                                &out) {
      ...
      for (int i = 0; i < count; ++i) {
        bool not_overflow = transpose_pad(
          out_it.coordinate(), padding, x_shape, x_coord);
        if (not_overflow) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_pad_compute_run<TYPE>(x, padding, padding_value, out);                 \
    break;                                                                     \
  }
