...
namespace ts
{
  namespace cpu
  {
    ...
    using function =
      std::function<void(const Tensor &, Tensor &, const Padding2D &)>;

    template <typename T>
    static inline function get_pooling_kernel(
        ...
        for (int c = 0; c < output_shape [1]; c++) {
          for (int oh = 0; oh < output_shape [2]; oh++) {
            ...
            for (int ow = 0; ow < output_shape [3]; ow++) {
              ...
              if (count == 0) output_data [outIndex] = 0;
              else
              ...
              //	output_data[outIndex] = sumValue / count;
              // else
              //	output_data[outIndex] = (sumValue + (m_kernel_h * m_kernel_w -
              //count) * padding_value) / (m_kernel_h * m_kernel_w);
            }
          }
          input_data += input_channel_size;
          output_data += output_channel_size;
...
#define ENCODE(a, b) ((int(a) << 8) | int(b))
      ...
      static const auto BLACK_MAX = ENCODE(
        Padding2DType::BLACK, Pooling2DType::MAX);
      static const auto BLACK_AVG = ENCODE(
        Padding2DType::BLACK, Pooling2DType::AVG);
      static const auto WHITE_MAX = ENCODE(
        Padding2DType::WHITE, Pooling2DType::MAX);
      static const auto WHITE_AVG = ENCODE(
        Padding2DType::WHITE, Pooling2DType::AVG);
      ...
      if (code == BLACK_MAX) {
        ...
        TS_LOG_ERROR << "Pooling type only support MAX and AVG with BLACK and "
                        "WHITE padding"
                     << eject;
      }
#undef ENCODE
    ...
    void Pooling2DCore::pooling2d(
      ...
      if (
        padding_type != Padding2DType::BLACK
        && padding_type != Padding2DType::WHITE) {
        TS_LOG_ERROR << "Pooling2D only support black padding or white padding"
                     << eject;
      }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_pooling2d_compute_run<TYPE>(                                           \
      x, type, padding, padding_type, ksize, stride, format, out);             \
    break;                                                                     \
  }
...
#undef DECLARE_COMPUTE_RUN
        default: {
          TS_LOG_ERROR << "Pooling2D not support data type(" << dtype
