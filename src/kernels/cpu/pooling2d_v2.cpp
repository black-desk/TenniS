...
namespace ts
{
  namespace cpu
  {
    ...
    void Pooling2DV2::init() {
      ...
      m_op_pooling2d = OperatorCreator::Create(
        context.computing_device.type(), name::layer::pooling2d(), false);

      TS_CHECK_NQ(m_op_pooling2d, nullptr)
        << "Can not find operator: " << name::layer::pooling2d();

      m_op_pooling2d->set(
        Bubble::RetentionParam::op, tensor::from(name::layer::pooling2d_v2()));
      m_op_pooling2d->set(
    ...
    int Pooling2DV2::infer(
      ...
      if (updated) { m_op_pooling2d->init(); }
