...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    void PoolingAlgorithm<T>::max_pooling_k3s2(
      const Tensor &input, Tensor &out, const Padding2D &padding) {}

    template <>
    void PoolingAlgorithm<float>::max_pooling_k3s2(
      const Tensor &input, Tensor &out, const Padding2D &padding) {
      ...
      int real_width = 2 * output_width
                     + 1;  //(output_width - 1) * stride.w + kernel.w;
      int real_height = 2 * output_height
                      + 1;  //(output_height - 1) * stride.h + kernel.h;
      int pad_w_all = real_width - input_width;
      int pad_h_all = real_height - input_height;
      if (pad_w_all < 0) pad_w_all = 0;
      if (pad_h_all < 0) pad_h_all = 0;
      ...
      int width_blocks   = padding.left == 1 ? (output_width - 1) / 4 :
                                               output_width / 4;
      int input_w_remain = padding.left == 1 ?
                             input_width - output_width * 2 + 1 :
                             input_width - output_width * 2;

      for (int n = 0; n < input_shape [0]; ++n) {
        for (int c = 0; c < input_channel; ++c) {
          const float *input_at = input_ptr + n * in_num_offset
                                + c * input_channel_offset;
          const float *i0     = input_at;
          const float *i1     = i0 + input_width;
          const float *i2     = i1 + input_width;
          float       *out_at = output_ptr + n * out_num_offset
          ...
          if (padding.top == 1) {
            ...
            if (padding.left == 1) {
              *out_at = std::max(
                std::max(i0 [0], i0 [1]), std::max(i1 [0], i1 [1]));
              out_at++;
              i0++;
              i1++;
            }
            ...
            int remain_w_index = padding.left == 1 ? width_blocks * 4 + 1 :
                                                     width_blocks * 4;
          ...
          for (; h < output_height; ++h) {
            ...
            for (int w = 0; w < width_blocks; ++w) {
              ...
              i0_max_x4 = max_float32x4(
                max_float32x4(i0_max_x4, i1_max_x4), i2_max_x4);
    ...
    template <typename T>
    void PoolingAlgorithm<T>::max_pooling_k2s2(
      const Tensor &input, Tensor &out, const Padding2D &padding) {}

    template <>
    void PoolingAlgorithm<float>::max_pooling_k2s2(
      const Tensor &input, Tensor &out, const Padding2D &padding) {
      const float *input_ptr  = input.data<float>();
      float       *output_ptr = out.data<float>();
      ...
      int real_width =
        2 * output_width;  //(output_width - 1) * stride.w + kernel.w;
      int real_height =
        2 * output_height;  //(output_height - 1) * stride.h + kernel.h;
      int pad_w_all = real_width - input_width;
      int pad_h_all = real_height - input_height;
      if (pad_w_all < 0) pad_w_all = 0;
      if (pad_h_all < 0) pad_h_all = 0;
