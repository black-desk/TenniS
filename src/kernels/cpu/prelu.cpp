...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_prelu_compute_run(
      const Tensor &x, const Tensor &slope, int dim, Tensor &out) {
      ...
      int pre_dims = 1;
      for (int i = 0; i < dim; i++) { pre_dims *= output_shape [i]; }
      int last_dims = 1;
    ...
    template <>
    void cpu_prelu_compute_run<float>(
      const Tensor &x, const Tensor &slope, int dim, Tensor &out) {
      auto         output_shape = out.sizes();
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int j = 0; j < output_shape [dim]; j++) {
          ...
          for (int k = 0; k < last_dims - 3; k += 4) {
            ...
            float32x4 output_data_x4 = max_float32x4(input_data_x4, mul_const)
                                     + val_x4
                                         * min_float32x4(
                                           input_data_x4, mul_const);
            // float32x4 fabs_input_x4(input_data[index], input_data[index+1],
            // input_data[index+2], input_data[index+3]); float32x4
            // output_data_x4 = (fabs_input_x4 + input_data_x4) * mul_const +
            //     val_x4 * (input_data_x4 - fabs_input_x4) * mul_const;
            output_data_x4.store(&output_data [index]);
          }
          for (int k = last_dims / 4 * 4; k < last_dims; k++) {
            output_data [k + offset] = std::max(
                                         input_data [k + offset], float(0))
                                     + val
                                         * std::min(
                                           input_data [k + offset], float(0));
          }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_prelu_compute_run<TYPE>(x, slope, dim, out);                           \
    break;                                                                     \
  }
