      ...
      public:
        ...
        void init() final {
          ...
          for (size_t i = 0; i < tmp_aspect_ratio.size(); ++i) {
            ...
            if (! already_exist) {
              ...
              if (flip_) { aspect_ratios_.push_back(1. / ar); }
            }
          }
          ...
          if (has("img_h") || has("img_w")) {
            ...
            TS_CHECK_GT(img_size, 0)
              << "img_size should be larger than 0." << eject;
            img_h_ = img_size;
            img_w_ = img_size;
          } else {
            img_h_ = 0;
          ...
          if (has("step_h") || has("step_w")) {
            ...
            TS_CHECK_GT(step_h_, 0.)
              << "step_h should be larger than 0." << eject;
            step_w_ = tensor::to_int(get("step_w"));
            TS_CHECK_GT(step_w_, 0.)
              << "step_w should be larger than 0." << eject;
          } else if (has("step")) {
        ...
        int infer(Stack &stack, std::vector<Tensor::Prototype> &output) final {
          ...
          output [0] = Tensor::Prototype(
            stack [0].dtype(), infer_top_shape(stack [0]));
          return 1;
        }

        static int Offset(
          const Tensor &x, const int n, const int c = 0, const int h = 0) {
          auto &size = x.sizes();
        ...
        int run(Stack &stack) final {
          ...
          if (clip_) {
            for (int d = 0; d < dim; ++d) {
              top_data [d] = std::min<Dtype>(
                std::max<Dtype>(top_data [d], 0.), 1.);
            }
          }
          // set the variance.
          ...
          if (variance_.size() == 1) {
            ...
            memset(
              top_data, CPU, dim * sizeof(Dtype), &src, CPU, sizeof(Dtype));
          } else {
            int count = 0;
            for (int h = 0; h < layer_height; ++h) {
              for (int w = 0; w < layer_width; ++w) {
