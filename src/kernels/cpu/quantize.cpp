...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_quantize_compute_run(
      const Tensor &x, std::vector<float> quantize_scales, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        auto loop_count = int(
          std::ceil(static_cast<float>(count) / quantize_group));
        int index = 0;
    ...
    template <>
    void cpu_quantize_compute_run<float>(
      const Tensor &x, std::vector<float> quantize_scales, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int i = 0; i < count_4; i++) {
          ...
          *(output_data + ii + 7) = to_int8(output_int32x8 [7]);
          ...
          //*(output_data + ii + 1) = to_int8(*(((float*)&(output_x4x2.value)) +
          //1));
          //*(output_data + ii + 2) = to_int8(*(((float*)&(output_x4x2.value)) +
          //2));
          //*(output_data + ii + 3) = to_int8(*(((float*)&(output_x4x2.value)) +
          //3));
          //*(output_data + ii + 4) = to_int8(*(((float*)&(output_x4x2.value)) +
          //4));
          //*(output_data + ii + 5) = to_int8(*(((float*)&(output_x4x2.value)) +
          //5));
          //*(output_data + ii + 6) = to_int8(*(((float*)&(output_x4x2.value)) +
          //6));
          //*(output_data + ii + 7) = to_int8(*(((float*)&(output_x4x2.value)) +
          //7));
        }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_quantize_compute_run<TYPE>(x, quantize_scales, out);                   \
    break;                                                                     \
  }
...
#undef DECLARE_COMPUTE_RUN
        default: {
          TS_LOG_ERROR << this->op() << " not support this data type: " << dtype
                       << eject;
          break;
        }
      }
