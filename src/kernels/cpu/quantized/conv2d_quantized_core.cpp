...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < out_shape [1]; c++) {
          ...
          for (int i = 0; i < count_x8; i++) {
            int         ii         = i * 8;
            float32x4x2 input_x4x2 = intx4x2_to_float32x4x2(
              int32x4x2(&input_cur [ii]));
      ...
      // int loop_count = std::ceil(static_cast<float>(output_count) /
      // dequantize_group); int index = 0;
      //#ifdef TS_USE_OPENMP
      // #pragma omp parallel for num_threads(openmp_threads())
      //#endif
    ...
    void Conv2DQuantizedCore::conv2d(
      const Tensor      &x,
      const Padding2D   &padding,
      float              padding_value,
      const Tensor      &w,
...
#undef DECLARE_COMPUTE_RUN
        default: {
          TS_LOG_ERROR << "Conv2D_quantized not support this data type: "
