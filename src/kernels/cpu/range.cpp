...
namespace ts
{
  namespace cpu
  {
    ...
    void Range::init() { supper::init(); }

    template <typename T>
    static int cpu_range_infer(
      ...
      if (steps * delta + start < limit) { steps++; }
      return steps;
    }

    template <typename T>
    static void cpu_range_compute_run(
      const Tensor &start_tensor,
      const Tensor &limit_tensor,
      const Tensor &delta_tensor,
      Tensor       &out_tensor) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    steps = cpu_range_infer<TYPE>(start, limit, delta);                        \
    break;                                                                     \
  }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_range_compute_run<TYPE>(start, limit, delta, out);                     \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
