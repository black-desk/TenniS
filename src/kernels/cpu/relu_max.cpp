...
namespace ts
{
  ...
  {
    template <typename T>
    static void cpu_relu_max_compute_run(
      const Tensor &x, float max, Tensor &out) {
    ...
    template <>
    void cpu_relu_max_compute_run<float>(
      const Tensor &x, float max, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
      ...
      for (int i = 0; i < count_4; ++i) {
        ...
        float32x4 output_x4 = min_float32x4(
          max_float32x4(val_x4, const_num_x4), casted_max_x4);
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_relu_max_compute_run<TYPE>(x, max, out);                               \
    break;                                                                     \
  }
