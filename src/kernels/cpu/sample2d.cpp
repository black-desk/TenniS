...
namespace ts
{
  namespace cpu
  {
    ...
    void Sample2D::init() {
      ...
      if (m_scale < 1e-5) {
        TS_LOG_ERROR << "sample scale must greater than 1e-5, got" << m_scale
                     << eject;
      }

      auto &context = ctx::ref<DeviceContext>();

      m_sample_op = OperatorCreator::Create(
        context.computing_device.type(), name::layer::affine_sample2d(), false);

      TS_CHECK_NQ(m_sample_op, nullptr)
        << "Can not find operator: " << name::layer::affine_sample2d();
    ...
    int Sample2D::infer(Stack &stack, std::vector<Tensor::Prototype> &output) {
      ...
      if (fixed_dim < 0 || fixed_dim >= (max_dim - 1)) {
        TS_LOG_ERROR << "Sample2D dim must in [-" << max_dim << ", "
                     << max_dim - 1 << ")" << eject;
      }
    ...
    int Sample2D::run(Stack &stack) {
      ...
      m_sample_size.data<int32_t>(0) = int32_t(
        std::floor(shape [fixed_dim] * m_scale));
      m_sample_size.data<int32_t>(1) = int32_t(
        std::floor(shape [fixed_dim + 1] * m_scale));
