  ...
  namespace cpu
  {
    ...
    Sample2DV2::Sample2DV2() {
      field(
        name::type,
        OPTIONAL,
        tensor::from<int32_t>(int32_t(Resize2DType::HARD)));
    }
    ...
    void Sample2DV2::init() {
      ...
      m_op_resize2d = OperatorCreator::Create(
        context.computing_device.type(), name::layer::resize2d(), false);

      TS_CHECK_NQ(m_op_resize2d, nullptr)
        << "Can not find operator: " << name::layer::resize2d();

      m_op_resize2d->set(
        Bubble::RetentionParam::op,
        tensor::from(name::layer::resize_nearest_neighbor()));
      m_op_resize2d->set(
        Bubble::RetentionParam::name, tensor::from("_core" + name()));
    ...
    static Tensor GetSizeTensor(
      Operator *op, Stack &stack, const Tensor &x, const Tensor &scale) {
      auto shape       = x.sizes();
      auto scale_count = scale.count();
      if (shape.size() != size_t(scale_count)) {
        TS_LOG_ERROR
          << op->op() << ":" << op->name()
          << " scale must has same shape with input tensor, got input: "
      ...
      for (int32_t i = 0; i < scale_count; ++i) {
        size.data<int32_t>(i) = int32_t(
          std::floor(shape [i] * float_scale.data<float>(i)));
      }
      return size;
    }

    int Sample2DV2::infer(
