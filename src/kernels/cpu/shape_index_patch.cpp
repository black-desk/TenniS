...
namespace ts
{
  ...
  {
    template <typename T>
    static inline void cpu_sample_compute_run(
      const Tensor &x,
      const Tensor &pos,
      const Size2D &origin_patch,
      const Size2D &origin,
      Tensor       &out) {
      auto &feat_blob = x;
      auto &pos_blob  = pos;
      ...
      for (int i = 0; i < landmark_num; i++) {
        for (int n = 0; n < num; n++) {  // x1, y1, ..., xn, yn
          // coordinate of the first patch pixel, scale to the feature map
          // coordinate
          const int y = int(
            pos_data [pos_offset.to_index({ n, 2 * i + 1 })] * (feat_h - 1)
            - r_h + 0.5f);
          const int x = int(
            pos_data [pos_offset.to_index({ n, 2 * i })] * (feat_w - 1) - r_w
            + 0.5f);
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_sample_compute_run<TYPE>(x, pos, origin_patch, origin, out);           \
    break;                                                                     \
  }
