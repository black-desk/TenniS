...
namespace ts
{
  namespace cpu
  {
    ...
    template <>
    uint8_t neg(uint8_t a) {
      return a;
    }
    template <>
    uint16_t neg(uint16_t a) {
      return a;
    }
    template <>
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    //#ifdef TS_USE_SSE
    //        template<>
    //        static void cpu_sigmoid_compute_run<float>(const Tensor &x, Tensor
    //        &out) {
    ...
    //                float32x4 exp_val_x4(exp(-(input_data[i])),
    //                exp(-(input_data[i+1])), exp(-(input_data[i+2])),
    //                exp(-(input_data[i+3]))); float32x4 output_x4 = const_one
    //                / (const_one + exp_val_x4); output_x4.store(output_data);
    //                output_data += 4;
    //            }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_sigmoid_compute_run<TYPE>(x, out);                                     \
    break;                                                                     \
  }
