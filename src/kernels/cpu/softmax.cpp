    ...
    void cpu_softmax_compute_run(const Tensor &x, int m_dim, Tensor &out) {
      ...
      if (body_num == 1) {
        ...
        memset(
          output_data,
          out.device(),
          out.count() * out.proto().type_bytes(),
          &one,
          Device(CPU),
          sizeof(T));
        return;
      ...
      int head_num = 1;
      for (int i = 0; i < m_dim; i++) { head_num *= output_shape [i]; }
      int tail_num = 1;
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void cpu_smooth_softmax_compute_run(
      const Tensor &x, int m_dim, Tensor &out) {
      auto &output_shape = out.sizes();
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void cpu_softmax_compute_run(
      const Tensor &x, int m_dim, bool m_smooth, Tensor &out) {
      if (m_smooth) {
    ...
    // template<>
    // void cpu_softmax_compute_run<float>(const Tensor &x, int m_dim, bool
    // m_smooth, Tensor &out) {
    //     auto output_shape = out.sizes();
    //
    //     int pre_num = 1;
    ...
    //
    //     //memcpy(output_data, device_type, count * sizeof(float), input_data,
    //     device_type, count * sizeof(float));
    //
    ...
    //     Tensor denominator_tensor(Tensor::InFlow::HOST, out.dtype(),
    //     denominator_shape); float *denominator_data =
    //     denominator_tensor.data<float>();
    //
    //     for (int i = 0; i < pre_num; i++) {
    //         std::memset(denominator_data, 0, scale_data_size *
    //         sizeof(float)); int pre_offset = i * axis * inner_num; if
    //         (m_smooth) {
    //             //Caculate max value
    //             memcpy(scale_data, device_type, inner_num * sizeof(float),
    //             input_data + pre_offset,
    //                 device_type, inner_num * sizeof(float));
    //             //std::memcpy(scale_data,input_data + i * axis * inner_num,
    //             inner_num * sizeof(float)); for (int j = 0; j < axis; j++) {
    //                 int post_offset = j * inner_num;
    //                 for (int k = 0; k < inner_num - 3; k += 4) {
    //                     float *scale_temp = &scale_data[k];
    //                     float32x4 scale_data_x4(scale_temp);
    //                     float32x4 input_data_x4(&input_data[pre_offset +
    //                     post_offset + k]); scale_data_x4 =
    //                     max_float32x4(scale_data_x4, input_data_x4);
    ...
    //                         input_data[pre_offset + post_offset + k] -
    //                         scale_data[k]);
    //                     denominator_data[k] += output_data[pre_offset +
    //                     post_offset + k];
    //                 }
    //             }
    //         }
    ...
    //             int post_offset = j * inner_num;
    //             for (int k = 0; k < inner_num - 3; k += 4) {
    //                 float *output_temp = &output_data[pre_offset +
    //                 post_offset + k]; float32x4 output_data_x4(output_temp);
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_softmax_compute_run<TYPE>(x, dim, smooth, out);                        \
    break;                                                                     \
  }
