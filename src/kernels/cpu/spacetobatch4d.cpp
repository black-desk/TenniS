...
namespace ts
{
  namespace cpu
  {
    ...
    template <typename T>
    static void cpu_spacetobatch4d_compute_run(
      const Tensor &x,
      ...
      for (int n = 0; n < input_number; ++n) {
        for (int c = 0; c < input_channels; ++c) {
          ...
          for (int h = 0; h < input_height; ++h) {
            ...
            const int pre_compute = (h + padding_top) % block_height
                                  * block_width;
            for (int w = 0; w < input_width; ++w) {
              const int on = (pre_compute + (w + padding_left) % block_width)
                             * input_number
                           + n;
    ...
    void SpaceToBatch4D::spacetobatch4d_run(
      const Tensor &x,
      const int     padding_top,
      const int     padding_bottom,
      const int     padding_left,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_spacetobatch4d_compute_run<TYPE>(                                      \
      x,                                                                       \
      padding_top,                                                             \
      padding_bottom,                                                          \
      padding_left,                                                            \
