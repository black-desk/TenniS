...
namespace ts
{
  namespace cpu
  {
    void StackTensor::stack_tensor(
      const std::vector<ts::Tensor> &x, int axis, ts::Tensor &out) {
    ...
    void StackTensor::init() {
      ...
      m_op_concat = OperatorCreator::Create(
        context.computing_device.type(), name::layer::concat(), false);
