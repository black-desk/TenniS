...
namespace ts
{
  namespace cpu
  {
    ...
    static inline int to_mod_index(
      const HypeShape &hype, const Shape &coordinate) {
      auto temp = coordinate;
      for (size_t i = 0; i < temp.size(); ++i) { temp [i] %= hype.shape(i); }
      return hype.to_index(temp);
    }

    template <typename T>
    static inline void compute_run(
      ...
      for (int i = 0; i < ncount; i++) {
        ...
        reduce_operator(
          pout [i],
          plhs [to_mod_index(lhs_hype, tmpshape)],
          prhs [to_mod_index(rhs_hype, tmpshape)]);
        ++out_iterator;
      }
    }

    template <typename T>
    static inline void compute_run_scalar(
      const T *plhs, T scalar, T *pout, int count) {
      // this is CPU operator, so just using memcpy
      if (pout != plhs) std::memcpy(pout, plhs, count * sizeof(T));

      for (int i = 0; i < count; ++i) { reduce_operator(pout [i], scalar); }
    }

    template <typename T>
    static inline void compute_run_scalar_cross(
      T scalar, const T *prhs, T *pout, int count) {
      // this is CPU operator, so just using memcpy
    ...
    template <>
    inline void compute_run_scalar(
      const float *plhs, float scalar, float *pout, int count) {
      //#ifdef TS_USE_OPENMP
    ...
    template <>
    inline void compute_run_scalar_cross(
      float scalar, const float *prhs, float *pout, int count) {
      //#ifdef TS_USE_OPENMP
      //			//#pragma omp parallel for num_threads(1)
      //#pragma omp parallel for num_threads(openmp_threads(count))
      //#endif
    ...
    template <typename T>
    static inline void compute_run_same_shape(
      const T *plhs, const T *prhs, T *pout, int count) {
      // this is CPU operator, so just using memcpy
      if (pout != plhs) std::memcpy(pout, plhs, count * sizeof(T));

      for (int i = 0; i < count; ++i) { reduce_operator(pout [i], prhs [i]); }
    }

    template <>
    inline void compute_run_same_shape(
      const float *plhs, const float *prhs, float *pout, int count) {
      //#ifdef TS_USE_OPENMP
      ////#pragma omp parallel for num_threads(1)
      //#pragma omp parallel for num_threads(openmp_threads(count))
      //#endif
    ...
    template <typename T>
    static inline void compute_run_bias(
      const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
      auto plhs = lhs.data<T>();
      auto prhs = rhs.data<T>();
      auto pout = out.data<T>();
      ...
      auto number = std::accumulate(
        out_shape.begin(), out_shape.begin() + dim, 1, std::multiplies<int>());
      auto count = std::accumulate(
        out_shape.begin() + dim + 1,
        out_shape.end(),
        1,
        std::multiplies<int>());
    ...
    template <typename T>
    static inline void compute_run_bias_cross(
      const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
      auto plhs = lhs.data<T>();
      auto prhs = rhs.data<T>();
      auto pout = out.data<T>();
    ...
    void Sub::reduce_with_broadcast(
      const Tensor &lhs, const Tensor &rhs, Tensor &out) {
      // Notice: the all tensor' memory device are CPU, as given in
      // running_memory_device
      DTYPE dtype = out.dtype();
      switch (dtype) {
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run<TYPE>(lhs, rhs, out);                                          \
    break;                                                                     \
  }
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run_scalar<TYPE>(lhs, rhs, out);                                   \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run_bias<TYPE>(lhs, rhs, out, dim);                                \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
...
#undef DECLARE_COMPUTE_RUN
    ...
    void Sub::reduce_with_same_shape(
      const Tensor &lhs, const Tensor &rhs, Tensor &out) {
      // Notice: the all tensor' memory device are CPU, as given in
      // running_memory_device
      DTYPE dtype = out.dtype();
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run_same_shape<TYPE>(lhs, rhs, out);                               \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
...
#undef DECLARE_COMPUTE_RUN
    ...
    void Sub::reduce_with_bias_cross(
      const Tensor &lhs, const Tensor &rhs, Tensor &out, int dim) {
      // Notice: the all tensor' memory device are CPU, as given in
      // running_memory_device
      DTYPE dtype = out.dtype();
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run_bias_cross<TYPE>(lhs, rhs, out, dim);                          \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
...
#undef DECLARE_COMPUTE_RUN
    ...
    void Sub::reduce_with_scalar_cross(
      const Tensor &lhs, const Tensor &rhs, Tensor &out) {
      // Notice: the all tensor' memory device are CPU, as given in
      // running_memory_device
      DTYPE dtype = out.dtype();
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    compute_run_scalar_cross<TYPE>(lhs, rhs, out);                             \
    break;                                                                     \
  }
        DECLARE_COMPUTE_RUN(INT8, int8_t);
        DECLARE_COMPUTE_RUN(UINT8, uint8_t);
