...
namespace ts
{
  ...
  {
    template <typename T>
    static void cpu_tile_compute_run(
      const Tensor &x, const std::vector<int32_t> &repeats, Tensor &out) {
      ...
      auto r_count = std::accumulate(
        repeats.begin(), repeats.end(), 1, std::multiplies<int32_t>());
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_tile_compute_run<TYPE>(x, repeats, out);                               \
    break;                                                                     \
  }
