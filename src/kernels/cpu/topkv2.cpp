...
namespace ts
{
  namespace cpu
  {
    ...
    static void sort_heap(T *arr, int len, int *arr2) {
      for (int i = len / 2; i >= 0; i--) adjust_node(arr, i, len, arr2);
      int index;
      T   tmp;
    ...
    template <typename T>
    static void cpu_topkv2_sorted_compute_run(
      const Tensor &x, int K, Tensor &values, Tensor &indices) {
      auto N = std::accumulate(
        x.sizes().begin(), x.sizes().end() - 1, 1, std::multiplies<int32_t>());
      auto                 W = x.sizes().back();
    ...
    template <typename T>
    static void cpu_topkv2_compute_run(
      const Tensor &x, int K, bool sorted, Tensor &values, Tensor &indices) {
      if (sorted) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_topkv2_compute_run<TYPE>(x, K, sorted, values, indices);               \
    break;                                                                     \
  }
