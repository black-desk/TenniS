  ...
  namespace cpu
  {
    ...
    static void Transpose_transpose_run(
      ...
      for (int i = 0; i < len; i++) {
        ...
        index = hype_output_shape.to_index(
          tmpshape);  // to_index(reshape, tmpshape);

        // if(index < 0) {
        //     throw ts::Exception("transpose operator failed, index is
        //     invalid");
        // }
        pdst [index] = psrc [i];
        ++input_shape_it;
    ...
    template <typename T>
    static inline void NC3HWToNHWC3(
      const T *psrc, T *pdst, const Shape &input_shape) {
      int height = input_shape [2], width = input_shape [3],
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    inline void NC3HWToNHWC3<float>(
      const float *psrc, float *pdst, const Shape &input_shape) {
      int height = input_shape [2], width = input_shape [3],
          channel        = input_shape [1];
      int channel_offset = height * width;
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int h = 0; h < height; ++h) {
          ...
          int  w       = 0;
          for (; w + 3 < width; w += 4) {
            float32x4 i0(src_tmp), i1(src_tmp + channel_offset),
              i2(src_tmp + 2 * channel_offset);
          ...
          for (; w < width; ++w) {
            for (int c = 0; c < channel; ++c) {
              dst_at [h * out_h_offset + w * channel + c] =
                src_at [h * width + c * channel_offset + w];
            }
          }
        }
      }
    ...
    template <typename T>
    static inline void NHWC3ToNC3HW(
      const T *psrc, T *pdst, const Shape &input_shape) {
      int height = input_shape [1], width = input_shape [2],
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <>
    inline void NHWC3ToNC3HW<float>(
      const float *psrc, float *pdst, const Shape &input_shape) {
      int height = input_shape [1], width = input_shape [2],
          channel        = input_shape [3];
      int h_offset       = width * channel;
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int h = 0; h < height; ++h) {
          ...
          for (; w < width; ++w) {
            for (int c = 0; c < channel; ++c) {
              dst_at [h * width + c * channel_offset + w] =
                src_at [h * h_offset + w * channel + c];
            }
          }
        }
      }
    ...
    template <typename T>
    static inline void cpu_transpose_compute_run(
      const Tensor &x, const std::vector<int> &permute, Tensor &out) {
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_transpose_compute_run<TYPE>(x, permute, out);                          \
    break;                                                                     \
  }
