...
namespace ts
{
  ...
  {
    template <typename T>
    static void cpu_transpose_conv2d_nchw_compute_run(
      const Tensor     &x,
      ...
      int  output_number_offset = output_shape [1] * output_shape [2]
                               * output_shape [3];
      int input_number_offset = x_shape [1] * conv_out_spatial_dim;
      int col_buffer_size     = weight_shape [1] * weight_shape [2]
                          * weight_shape [3] * x_shape [2] * x_shape [3];
      ...
      auto transposed_weight = intime::transpose(
        w.reshape({ w.size(0), -1 }), { 1, 0 });
    ...
    void Conv2DTransposeCore::conv2d_transpose(
      const Tensor     &x,
      const Padding2D  &padding,
      float             padding_value,
      const Tensor     &w,
...
#define DECLARE_COMPUTE_RUN(DTYPE, TYPE)                                       \
  case DTYPE: {                                                                \
    cpu_transpose_conv2d_nchw_compute_run<TYPE>(                               \
      x,                                                                       \
      padding,                                                                 \
      padding_value,                                                           \
      w,                                                                       \
...
#undef DECLARE_COMPUTE_RUN
        default: {
          TS_LOG_ERROR << "Conv2DTransposeCore not support this data type: "
                       << "(" << dtype << ")" << type_str(dtype) << eject;
          break;
        }
      }
      return;
