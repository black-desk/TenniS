...
namespace ts
{
  namespace cpu
  {
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f23_transform_and_pack_kernel(
      const Tensor &kernel, int in_tile_size, Tensor &kernel_tm) {}

    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f23_transform_and_pack_kernel(
      const Tensor &kernel, int in_tile_size, Tensor &kernel_tm) {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int q = 0; q < input_channel; q++) {
          ...
          float tmp [3][4];
          for (int i = 0; i < 4; i++) {
            tmp [0][i] = k0 [0] * G [i][0] + k0 [1] * G [i][1]
                       + k0 [2] * G [i][2];
            tmp [1][i] = k1 [0] * G [i][0] + k1 [1] * G [i][1]
                       + k1 [2] * G [i][2];
            tmp [2][i] = k2 [0] * G [i][0] + k2 [1] * G [i][1]
                       + k2 [2] * G [i][2];
          }
          ...
          for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
              kernel_trans_at [(i * 4 + j) * stride] = tmp [0][j] * G [i][0]
                                                     + tmp [1][j] * G [i][1]
                                                     + tmp [2][j] * G [i][2];
            }
          }
      ...
      for (int i = 0; i < in_tile_size; ++i) {
        math<float, float>::pack8_A(
          out_channel, input_channel, from, input_channel, to);
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f23_transform_and_pack_input(
      const Tensor &x, int tile_count, Tensor &x_tm) {}

    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f23_transform_and_pack_input(
      const Tensor &x, int tile_count, Tensor &x_tm) {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < input_channel; ++c) {
          ...
          int          tile_index = 0;
          const float *input_cur  = input_ptr + n * input_num_offset
                                 + c * input_channel_offset;
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f23_transform_output(
      const Tensor &out_tm, int tile_count, Tensor &out) {}

    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f23_transform_output(
      const Tensor &out_tm, int tile_count, Tensor &out) {
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int c = 0; c < out_channel; ++c) {
          int          tile_offset = 0;
          const float *out_tm_cur  = out_tm_ptr + n * out_tm_num_offset
                                  + c * tile_count;
          float *out_cur = out_ptr + n * out_num_offset
                         + c * out_channel_offset;
    ...
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f23(
      const Tensor    &x,
    ...
    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f23(
      const Tensor    &x,
      const Padding2D &padding,
      float            padding_value,
      const Tensor    &kernel,
      ...
      Stride2D stride(1, 1);
      KSize2D  ksize(3, 3);
      KernelCommonFunc<float>::in_out_pad_and_fix_size(
        x,
        kernel_shape,
        out,
        tile_height,
      ...
      if (! kernel_transformed) {
        Shape  kernel_tm_shape = { in_tile_size,
                                  kernel_shape [0],
                                  kernel_shape [1] };
        Tensor kernel_tmp(
          Tensor::InFlow::HOST, kernel.dtype(), kernel_tm_shape);
        // std::memset(kernel_tmp.data<float>(), 0,
        // sizeof(float)*kernel_tmp.count());
        winograd_f23_transform_and_pack_kernel(
          kernel, in_tile_size, kernel_tmp);
        kernel_tm = kernel_tmp;
      }
      const float *kernel_ptr = kernel_tm.data<float>();

      // transform input
      Shape input_tm_shape = {
        num, in_tile_size, input_channel, tile_block_num
      };
      Tensor input_tm(
        Tensor::InFlow::HOST, input_padded.dtype(), input_tm_shape);
      winograd_f23_transform_and_pack_input(
        input_padded, tile_block_num, input_tm);
      const float *trans_input_ptr = input_tm.data<float>();

      // eltwise_gemm->gemm(O = U*V)
      Shape transform_out_shape = {
        num, in_tile_size, out_channel, tile_block_num
      };
      Tensor transform_out(
        Tensor::InFlow::HOST, out.dtype(), transform_out_shape);
      float *trans_out_ptr = transform_out.data<float>();

      int transform_kernel_tile_offset = out_channel * input_channel;
      int transform_input_tile_offset  = input_channel * tile_block_num;
      int transform_input_num_offset   = in_tile_size
                                     * transform_input_tile_offset;
      ...
      for (int n = 0; n < num; ++n) {
        const float *trans_input_cur = trans_input_ptr
                                     + n * transform_input_num_offset;
        float *trans_out_cur = trans_out_ptr + n * transform_out_num_offset;

        for (int i = 0; i < in_tile_size; ++i) {
          const float *trans_kernel_at = kernel_ptr
                                       + i * transform_kernel_tile_offset;
          const float *trans_input_at = trans_input_cur
                                      + i * transform_input_tile_offset;
          float *trans_out_at = trans_out_cur + i * transform_out_tile_offset;
          math<float, float>::gemm(
            out_channel,
            tile_block_num,
            input_channel,
            1.f,
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f63_transform_and_pack_kernel(
      const Tensor &kernel, int in_tile_size, Tensor &kernel_tm) {}

    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f63_transform_and_pack_kernel(
      const Tensor &kernel, int in_tile_size, Tensor &kernel_tm) {
      Shape kernel_shape  = kernel.sizes();
      int   out_channel   = kernel_shape [0];
      int   input_channel = kernel_shape [1];
...
#  pragma omp parallel for num_threads(openmp_threads())
        ...
        for (int q = 0; q < input_channel; ++q) {
          ...
          for (int i = 0; i < 8; ++i) {
            for (int j = 0; j < 8; ++j) {
              kernel_trans_at [(i * 8 + j) * stride] = tmp [0][j] * G [i][0]
                                                     + tmp [1][j] * G [i][1]
                                                     + tmp [2][j] * G [i][2];
            }
          }
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f63_transform_and_pack_input(
      const Tensor &x, int tile_count, Tensor &x_tm) {}

    // TODO:maybe simd optimize
    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f63_transform_and_pack_input(
      const Tensor &x, int tile_count, Tensor &x_tm) {
      Shape input_shape          = x.sizes();
      int   num                  = input_shape [0];
      int   input_channel        = input_shape [1];
...
#  pragma omp parallel for num_threads(openmp_threads())
     ...
     */
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f63_transform_output(
      const Tensor &out_tm, int tile_count, Tensor &out) {}

    // TODO:maybe simd optimize
    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f63_transform_output(
      const Tensor &out_tm, int tile_count, Tensor &out) {
      Shape out_shape          = out.sizes();
      Shape out_tm_shape       = out_tm.sizes();
      int   num                = out_tm_shape [0];
...
#  pragma omp parallel for num_threads(openmp_threads())
    ...
    template <typename T>
    void Conv2dWinogradAlgorithm<T>::winograd_f63(
      const Tensor    &x,
      const Padding2D &padding,
      float            padding_value,
      const Tensor    &kernel,
    ...
    template <>
    void Conv2dWinogradAlgorithm<float>::winograd_f63(
      const Tensor    &x,
      const Padding2D &padding,
      float            padding_value,
      const Tensor    &kernel,
      ...
      if (! kernel_transformed) {
        ...
        // std::memset(kernel_tmp.data<float>(), 0,
        // sizeof(float)*kernel_tmp.count());
        winograd_f63_transform_and_pack_kernel(
          kernel, in_tile_size, kernel_tmp);
        kernel_tm = kernel_tmp;
      }
      const float *kernel_ptr = kernel_tm.data<float>();
      ...
      Tensor input_tm(
        Tensor::InFlow::HOST, input_padded.dtype(), input_tm_shape);
      winograd_f63_transform_and_pack_input(
        input_padded, tile_block_num, input_tm);
      const float *trans_input_ptr = input_tm.data<float>();
