      ...
      public:
        ...
        int infer(
          ...
          for (int i = 1; i < num; ++i) {
            ...
            auto yolo     = tensor::cast(
              FLOAT32, features [0].view(MemoryDevice(CPU)));
        ...
        class detection_list {
          public:
            detection_list(size_t width, size_t capacity = 128) :
              ...
              m_data = std::make_shared<HardMemory>(
                MemoryDevice(CPU), m_capacity * m_width);
            }
            ...
            detection &operator[](size_t i) {
              return *reinterpret_cast<detection *>(data(i));
            }

            const detection &operator[](size_t i) const {
              return *reinterpret_cast<const detection *>(data(i));
            }

            detection &operator[](int i) {
              return *reinterpret_cast<detection *>(data(i));
            }

            const detection &operator[](int i) const {
              return *reinterpret_cast<const detection *>(data(i));
            }
          ...
          private:
            ...
            size_t index(int i) const {
              return i >= 0 ? size_t(i) : m_size - size_t(-i);
            }

            void *data(size_t i) { return m_data->data<char>() + i * m_width; }

            const void *data(size_t i) const {
              return m_data->data<char>() + i * m_width;
            }

            void *data(int i) {
              return m_data->data<char>() + index(i) * m_width;
            }
        ...
        static void get_yolo_detections(
          ...
          for (i = 0; i < l.w * l.h; ++i) {
            ...
            for (n = 0; n < l.n; ++n) {
              ...
              this_detection.bbox  = get_yolo_box(
                predictions,
                l.biases,
                l.mask [n],
                box_index,
        ...
        static void do_nms_sort(
          detection_list &dets, int total, int classes, float thresh) {
          int i, j, k;

          std::vector<int> sorted_template(total);
          for (i = 0; i < total; ++i) { sorted_template [i] = i; }
          ...
          for (k = 0; k < classes; ++k) {
            ...
            for (i = 0; i < total; ++i) {
              ...
              for (j = i + 1; j < total; ++j) {
                ...
                if (box_iou(a, b) > thresh) { dets [sorted_j].prob [k] = 0; }
              }
            }
          }
        }
        ...
        struct BindingBox {
            BindingBox() = default;
            BindingBox(
              float x, float y, float w, float h, float score, float label) :
              x(x), y(y), w(w), h(h), score(score), label(label) {}
            BindingBox(
              float x, float y, float w, float h, float score, int label) :
        ...
        int run(Stack &stack) override {
          ...
          for (int i = 1; i < num; ++i) {
            auto yolo = tensor::cast(
              FLOAT32, stack [i].field(0).view(MemoryDevice(CPU)));
          ...
          for (int n = 0; n < out_n; ++n) {
            ...
            for (int i = 1; i < num; ++i) {
              auto features = stack [i].unpack();
              auto yolo     = yolo_list [i - 1].slice(
                n, n + 1);  // got n-th feature map
            ...
            // to tensor
            Tensor field(
              Tensor::InFlow::HOST, FLOAT32, { int(boxes.size()), 6 });
            std::memcpy(
              field.data(),
              boxes.data(),
              field.count() * field.proto().type_bytes());
