...
namespace ts
{
  ...
  {
    inline Tensor empty(
      const Tensor::Prototype &proto, const MemoryDevice &device) {
    ...
    inline Tensor ones(
      ...
      memset(
        tensor.data(),
        tensor.device(),
        tensor.count() * tensor.proto().type_bytes(),
        one.data(),
        one.device(),
        one.count() * one.proto().type_bytes());
    ...
    inline Tensor empty(
      DTYPE dtype, const Shape &shape, const MemoryDevice &device) {
    ...
    inline void *CudaMalloc(
      const MemoryDevice &device, size_t size, SyncMemory &buffer) {
      buffer = RuntimeContext::FlowMemory()->alloc(device, size);
      return buffer.data();
    }

    inline void adjustLdLevel3(
      char     transa,
      char     transb,
      int64_t  m,
      int64_t  n,
    ...
    static void CudaBlas_SgemmStridedBatched(
      char         transa,
      char         transb,
      int64_t      m,
      int64_t      n,
      ...
      int64_t      batchCount) {
      if (
        (m >= INT_MAX) || (n >= INT_MAX) || (k >= INT_MAX) || (lda >= INT_MAX)
        || (ldb >= INT_MAX) || (ldc >= INT_MAX) || (batchCount >= INT_MAX)) {
        TS_LOG_ERROR << "Cublas_SgemmStridedBatched only supports m, n, k, "
                        "lda, ldb, ldc, batchCount"
                        "with the bound [val] <= "
                     << INT_MAX << eject;
      }
    ...
    static void CudaBlas_SgemmBatched(
      char         transa,
      char         transb,
      int64_t      m,
      int64_t      n,
      ...
      if (
        (m >= INT_MAX) || (n >= INT_MAX) || (k >= INT_MAX) || (lda >= INT_MAX)
        || (ldb >= INT_MAX) || (ldc >= INT_MAX) || (batchCount >= INT_MAX)) {
        TS_LOG_ERROR << "Cublas_SgemmBatched only supports m, n, k, lda, ldb, "
                        "ldc, batchCount"
                        "with the bound [val] <= "
                     << INT_MAX << eject;
      }
      ...
      const int64_t stridea = (transa == 'N' || transa == 'n') ? lda * k :
                                                                 lda * n;
      const int64_t strideb = (transb == 'N' || transb == 'n') ? ldb * n :
                                                                 ldb * k;
      const int64_t stridec = ldc * n;

      CudaBlas_SgemmStridedBatched(
        transa,
        transb,
        m,
        n,
