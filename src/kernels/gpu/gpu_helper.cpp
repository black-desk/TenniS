...
namespace ts
{
  namespace gpu
  {
    static inline std::shared_ptr<char> cpu_blocked(
      const std::vector<CpuBlock> &cpu) {
      int shift = 0;
      for (auto &pair: cpu) { shift += pair.second; }
      std::shared_ptr<char> cpu_memory(
        new char [shift], std::default_delete<char []>());
      shift = 0;
      for (auto &pair: cpu) {
    ...
    std::pair<SyncMemory, GpuHypeShape> MakeGPUHypeShape(
      const MemoryDevice &device, const Shape &shape) {
      HypeShape    hype_shape(shape);
      GpuHypeShape gpu_shape;
      gpu_shape.dims  = int(shape.size());
      auto gpu_memory = convert_block_to_gpu(
        RuntimeContext::FlowMemory(),
        device,
        {
          { (void *)(hype_shape.shape().data()),
            4 * int(hype_shape.shape().size()) },
          { (void *)(hype_shape.weight().data()),
            4 * int(hype_shape.weight().size()) },
        },
        { (void **)(&gpu_shape.shape), (void **)(&gpu_shape.weights) });
      return std::make_pair(std::move(gpu_memory), gpu_shape);
    }

    std::pair<SyncMemory, std::vector<GpuHypeShape>> MakeGPUHypeShape(
      const MemoryDevice &device, const std::vector<Shape> &shape) {
    ...
    std::pair<SyncMemory, std::vector<GpuHypeShape>> MakeGPUHypeShape(
      ...
      for (auto &item: shape) { hype_shape_array.emplace_back(item); }
      ...
      for (size_t i = 0; i < shape.size(); ++i) {
        ...
        cpu.push_back({ (void *)(hype_shape.shape().data()),
                        4 * int(hype_shape.shape().size()) });
        cpu.push_back({ (void *)(hype_shape.weight().data()),
                        4 * int(hype_shape.weight().size()) });
      ...
      auto gpu_memory = convert_block_to_gpu(
        RuntimeContext::FlowMemory(), device, cpu, gpu);
