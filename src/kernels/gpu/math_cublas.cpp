...
namespace ts
{
  namespace gpu
  {
    namespace cublas
    {
      ...
      template <typename T>
      inline bool cublas_dot(
        cublasHandle_t handle, int N, const T *x, const T *y, T *z) {
        return gpu::math<T>::dot(N, x, y, z);
      }

      template <>
      inline bool cublas_dot<float>(
      ...
      bool cblas_gemm(
        ...
        T             *C,
        int            ldc) {
        return gpu::math<T>::gemm(
          Order, TransA, TransB, M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
      }

      template <>
      bool cblas_gemm<half>(
        cublasHandle_t handle,
        Order          Order,
        Transpose      TransA,
        Transpose      TransB,
        ...
        half          *C,
        int            ldc) {
        cublasOperation_t cblas_TransA = TransA == ts::gpu::cublas::NoTrans ?
                                           CUBLAS_OP_N :
                                           CUBLAS_OP_T;
        cublasOperation_t cblas_TransB = TransB == ts::gpu::cublas::NoTrans ?
                                           CUBLAS_OP_N :
                                           CUBLAS_OP_T;
        if (Order == ColMajor) {
          if (
      ...
      template <>
      bool cublas_asum<float>(
        cublasHandle_t handle, int N, const float *x, float *out) {
        return CUBLAS_STATUS_SUCCESS == cublasSasum(handle, N, x, 1, out);
      }

      template <>
      bool cublas_asum<double>(
        cublasHandle_t handle, int N, const double *x, double *out) {
