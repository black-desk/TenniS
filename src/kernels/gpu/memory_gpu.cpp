...
namespace ts
{
  class CUDAException : public Exception {
    ...
    private:
      static std::string CUDAExceptionMessage(
        const std::string &api, cudaError_t cuda_error) {
        std::ostringstream oss;
        oss << "Call " << api << " failed. Error(" << cuda_error
            << "):" << cudaGetErrorString(cuda_error) << eject;
        return oss.str();
      }
  ...
  void *gpu_allocator(int id, size_t new_size, void *mem, size_t mem_size) {
    ...
    if (cuda_error != cudaSuccess) {
      TS_LOG_ERROR << "cudaSetDevice(" << id << ") failed. Error(" << cuda_error
                   << "):" << cudaGetErrorString(cuda_error) << eject;
    }
    ...
    if (new_size == 0) {
      ...
      if (cuda_error == cudaSuccess) {
        cuda_error = cudaMemcpy(
          new_mem, mem, std::min(new_size, mem_size), cudaMemcpyDeviceToDevice);
        cudaFree(mem);
      }
    } else {
      cuda_error = cudaMalloc(&new_mem, new_size);
    ...
    if (cuda_error != cudaSuccess) {
      TS_LOG_ERROR << "cudaMalloc(" << &new_mem << " " << new_size
                   << ") failed. Error(" << cuda_error
                   << "):" << cudaGetErrorString(cuda_error) << eject;
    }
    return new_mem;
  }

  void gpu2gpu_converter(
    int dst_id, void *dst, int src_id, const void *src, size_t size) {
    cudaStream_t cuda_stream = cudaStreamPerThread;
    auto         context     = ctx::get<DeviceContext>();
    if (context != nullptr) {
      CUDAContextHandle *handle = reinterpret_cast<CUDAContextHandle *>(
        context->handle);
  ...
  void cpu2gpu_converter(
    int dst_id, void *dst, int src_id, const void *src, size_t size) {
    auto cuda_error = cudaSetDevice(dst_id);
    if (cuda_error != cudaSuccess) {
      TS_LOG_ERROR << "cudaSetDevice(" << dst_id << ") failed. Error("
  ...
  void gpu2cpu_converter(
    int dst_id, void *dst, int src_id, const void *src, size_t size) {
    auto cuda_error = cudaSetDevice(src_id);
    if (cuda_error != cudaSuccess) {
      TS_LOG_ERROR << "cudaSetDevice(" << src_id << ") failed. Error("
                   << cuda_error << "):" << cudaGetErrorString(cuda_error)
                   << eject;
    }
...
TS_STATIC_ACTION(
  ts::HardConverter::Register, ts::GPU, ts::GPU, ts::gpu2gpu_converter)
TS_STATIC_ACTION(
  ts::HardConverter::Register, ts::GPU, ts::CPU, ts::cpu2gpu_converter)
TS_STATIC_ACTION(
  ts::HardConverter::Register, ts::CPU, ts::GPU, ts::gpu2cpu_converter)
