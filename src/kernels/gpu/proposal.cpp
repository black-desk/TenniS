...
#include "kernels/cpu/dragon/proposal_op.h"
...
namespace ts
{
  namespace gpu
  {
    class Proposal : public OperatorOnGPU<base::Proposal> {
      public:
        std::shared_ptr<dragon::ProposalOp<dragon::CUDAContext>> m_dragon;
        int                                                      m_output_size;

        void init() override {
          dragon::Workspace ws;
          m_dragon = std::make_shared<dragon::ProposalOp<dragon::CUDAContext>>(
            this, &ws);
...
using namespace ts;
using namespace gpu;
TS_REGISTER_OPERATOR(Proposal, GPU, name::layer::proposal())
