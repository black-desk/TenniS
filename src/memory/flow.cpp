...
namespace ts
{
  ...
  VatMemoryController::VatMemoryController(const MemoryDevice &device) {
    ...
    TS_CHECK(hard_allocator != nullptr)
      << "Can not found memory controller for " << device.type();
    using namespace std::placeholders;
    auto hard_free     = std::bind(hard_allocator, device.id(), 0, _1, 0);
    auto pot_allocator = [hard_allocator, device, hard_free](
                           size_t size) -> std::shared_ptr<void> {
      return std::shared_ptr<void>(
        hard_allocator(device.id(), size, nullptr, 0), hard_free);
    };

    m_impl->m_device = device;
    m_impl->m_vat    = std::make_shared<Vat>(pot_allocator);
    auto &vat        = m_impl->m_vat;
    m_impl->m_managed_allocator =
      [vat](int, size_t new_size, void *mem, size_t mem_size) -> void * {
  ...
  VatMemoryController::~VatMemoryController() { m_impl->m_vat->deprecated(); }

  Memory VatMemoryController::alloc(size_t size) {
    return Memory(std::make_shared<HardMemory>(
      m_impl->m_device, m_impl->m_managed_allocator, size));
  }
  ...
  Memory StackMemoryController::alloc(size_t size) { return Memory(); }
}  // namespace ts