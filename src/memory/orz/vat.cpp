...
namespace ts
{
  ...
  Vat::Vat() : self(nullptr) {}

  Vat::Vat(const Pot::allocator &ator) : m_allocator(ator) {
  ...
  static int binary_find(
    const std::vector<Pot> &heap, size_t size, int left, int right) {
    while (true) {
  ...
  void Vat::free(const void *ptr) {
    ...
    if (it == m_dict.end()) { throw Exception("Can not free this ptr"); }
  ...
  void Vat::reset() {
    ...
    m_dict.clear();
    std::sort(m_heap.begin(), m_heap.end(), [](const Pot &p1, const Pot &p2) {
  ...
  uint64_t Vat::summary() const {
    uint64_t sum = 0;
    for (auto &a: this->m_heap) { sum += a.capacity(); }
    for (auto &b: this->m_dict) { sum += b.second.capacity(); }
    return sum;
  }
}  // namespace ts
