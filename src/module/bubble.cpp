...
namespace ts
{
  ...
  static const std::unordered_set<std::string> EndPoints = { Bubble::Parameter,
                                                             Bubble::Const,
                                                             Bubble::Variable };
  ...
  Bubble::Bubble(const std::string &op) : m_op(op) {
  ...
  Tensor &Bubble::get(const std::string &param) {
    auto param_it = m_params.find(param);
    if (param_it == m_params.end()) {
      TS_LOG_ERROR << "Unidentified param \"" << param << "\", did you mean \""
                   << fuzzy_param_name(param) << "\"" << eject;
    }
  ...
  void Bubble::clear_params() {
    ...
    for (auto &param_tenosr_pair: m_params) {
      ...
      bool  is_retention_param = ! param.empty()
                             && param [0] == retention_param_sign;
  ...
  void Bubble::update_retention_params() {
    ...
    // set(RetentionParam::output_count, tensor::from(m_output_count)); // not
    // saving output count
    if (! m_shape.empty()) {
  ...
  static size_t write_param(
    StreamWriter &stream, const std::pair<std::string, Tensor> &param) {
  ...
  static size_t read_param(
    StreamReader &stream, std::pair<std::string, Tensor> &param) {
    auto    &name      = param.first;
    auto    &value     = param.second;
    size_t   read_size = 0;
  ...
  size_t Bubble::serialize(StreamWriter &stream) const {
    ...
    for (auto &param: m_params) { writen_size += write_param(stream, param); }
    return writen_size;
  }
  ...
  const Shape Bubble::shape() const { return m_shape; }
  ...
  Bubble::Bubble(const std::string &op, int output_count) : self(op) {
