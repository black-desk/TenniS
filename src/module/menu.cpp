...
namespace ts
{
  namespace bubble
  {
    ...
    Node data(
      const std::string &name, const Tensor &value, const DeviceType &device) {
      auto &g      = ctx::ref<Graph>();
      Node  result = g.make(Bubble::Const, name);
      result->set(name::value, value);
  ...
  size_t serialize_nodes(
    StreamWriter &stream, const std::vector<Node> &nodes, size_t base) {
    ...
    for (auto &node: nodes) {
      ...
      for (auto &input: node.inputs()) {
        ...
        if (input_it == map_node_index.end()) {
          TS_LOG_ERROR << "Can not link input " << input << " in node " << node
                       << eject;
        }
      ...
      writen_size += binio::write<uint32_t>(
        stream, uint32_t(node_input_index.size()));
