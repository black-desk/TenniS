...
namespace ts
{
  ...
  void Module::load(Graph g) {
    ...
    for (auto &node: nodes) {
      if (node.outputs().empty()) { outputs.push_back(node); }
    }
    load(g, outputs);
  }
  ...
  void Module::load(Graph g, const std::vector<std::string> &outputs) {
    ...
    for (auto &node: nodes) {
      ...
      if (name_it == map_name_found_node.end()) { continue; }
      if (name_it->second != empty_node) {
        TS_LOG_ERROR << "Found duplicate Node " << node.str() << ", with Node "
                     << name_it->second.str() << eject;
      }
      ++found_count;
      name_it->second = node;
    }
  ...
  std::vector<Node> Module::graph_walker(
    Graph g, const std::vector<Node> &outputs) {
    // calculate inputs and outputs
    auto                     nodes = g.nodes();
    std::unordered_set<Node> nodes_set;
    for (auto &node: nodes) { nodes_set.insert(node); }
    ...
    while (! walker.empty()) {
      ...
      if (op.op() == Bubble::Const) { continue; }
      if (node.inputs().empty()) {
        TS_LOG_ERROR << "Found not computable node: " << node.str() << eject;
      }
      for (auto &input: node.inputs()) { walker.push(input); }
    }
  ...
  void Module::sort_inputs(const std::vector<Node> &inputs) {
    ...
    for (auto &had_input: m_inputs) {
      if (sorted_input_set.find(had_input) == sorted_input_set.end()) {
        TS_LOG_ERROR << "The sorted inputs must content " << had_input.str()
                     << eject;
      }
    }
    m_inputs = inputs;
  ...
  void Module::sort_inputs(const std::vector<std::string> &input_names) {
    ...
    for (auto &input: m_inputs) {
      ...
      if (
        map_name_input_node.find(bubble.name()) != map_name_input_node.end()) {
        auto it = map_name_input_node.find(bubble.name());
        TS_LOG_ERROR << "Can not sort inputs with duplicate names: "
                     << input.str() << " and " << it->second.str() << eject;
      }
      map_name_input_node.insert(std::make_pair(bubble.name(), input));
    }
    // check inputs node
    std::unordered_set<Node>
      used_inputs;  // make sure all inputs must be used after used
  ...
  void Module::sort_inputs(
    const std::initializer_list<std::string> &input_names) {
    this->sort_inputs(
  ...
  std::vector<std::pair<Node, int>> Module::list_reference_nodes(
    ...
    std::vector<std::pair<Node, int>> computation_schedule(
      map_node_depth.begin(), map_node_depth.end());
    std::sort(
      computation_schedule.begin(),
      computation_schedule.end(),
  ...
  static size_t read_uint32_list(
    StreamReader &stream, std::vector<uint32_t> &list) {
    uint32_t size_buffer = 0;
    size_t   read_size   = 0;
    read_size += binio::read<uint32_t>(stream, size_buffer);
    list.resize(size_buffer);
    for (auto &elem: list) { read_size += binio::read<uint32_t>(stream, elem); }
    return read_size;
  }

  Module::shared Module::Load(
    StreamReader &stream, Module::SerializationFormat format) {
    TS_AUTO_CHECK(format == BINARY);
    ...
    const auto
      &nodes = g.nodes();  // TODO: Check if the read nodes is the given nodes
  ...
  Module::shared Module::Load(
    const std::string &filename, Module::SerializationFormat format) {
    TS_AUTO_CHECK(format == BINARY);
    FileStreamReader stream(filename);
    TS_CHECK(stream.is_open()) << "Can not access: " << filename << eject;
  ...
  Module::shared Module::Load(
    Graph g, const std::vector<std::string> &outputs) {
    auto module = std::make_shared<Module>();
    module->load(g, outputs);
    return std::move(module);
  }

  Module::shared Module::Translate(
    Module::shared         module,
  ...
  static Node clone_node(
    const Node &node, std::unordered_map<Node, Node> &cloned_nodes) {
  ...
  static std::vector<Node> clone_graph(
    const std::vector<Node>        &nodes,
    ...
    for (auto &node: nodes) {
      ...
      auto dolly_inputs = clone_graph(
        node.inputs(), cloned_nodes, linked_nodes);
  ...
  // static std::vector<Node> clone_graph(const std::vector<Node> &nodes,
  // std::unordered_map<Node, Node> &cloned_nodes) {
  //     std::unordered_map<Node, Node> linked_nodes;
  //     return clone_graph(nodes, cloned_nodes, linked_nodes);
  // }

  Module::shared Module::Fusion(
    const std::vector<shared> &submodules, const std::vector<Route> &routes) {
    ...
    for (auto &route: routes) {
      if (
        route.in < 0 || route.in >= int(submodules.size()) || route.out < 0
        || route.out >= int(submodules.size())
