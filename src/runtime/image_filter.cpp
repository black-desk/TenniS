...
namespace ts
{
  ...
  void ImageFilter::clear() {
    ...
    bubble::param(
      serial_name(), UINT8, { -1, -1, -1 });  // add input param to graph
      ...
      Shape before(const Shape &shape) {
        ...
        switch (shape.size()) {
          case 0:
            ...
            break;
          case 1: return { 1, shape [0], 1, 1 };
          case 2: return { 1, shape [0], shape [1], 1 };
          case 3: return { 1, shape [0], shape [1], shape [2] };
          case 4: return shape;
          default:
            return {
              shape [0],
              ...
              shape [2],
              std::accumulate(
                shape.begin() + 3, shape.end(), 1, std::multiplies<int>())
            };
        }
        return Shape();
      };
      Shape after(const Shape &shape) { return shape; }
    private:
      Shape m_base;
  };
  ...
  void ImageFilter::sub_mean(const std::vector<float> &mean) {
    auto mean_tensor = tensor::build(
      FLOAT32, { 1, 1, 1, int(mean.size()) }, mean);
    ctx::bind<Graph> _bind_graph(m_impl->m_graph.get());
    auto             lhs = m_impl->m_graph->nodes().back();
  ...
  void ImageFilter::div_std(const std::vector<float> &std) {
    ...
    for (int i = 0; i < count; ++i) { std_data [i] = 1.0f / std_data [i]; }
    ctx::bind<Graph> _bind_graph(m_impl->m_graph.get());
    auto             lhs = m_impl->m_graph->nodes().back();
  ...
  void ImageFilter::resize(int shot_size, ResizeMethod method) {
    ...
    auto             x    = m_impl->m_graph->nodes().back();
    auto             node = bubble::op(
      serial_name(), name::layer::nhwc_scale_resize2d(), { x });
  ...
  void ImageFilter::center_crop(int width, int height) {
    ...
    auto             x    = m_impl->m_graph->nodes().back();
    auto             node = bubble::op(
      serial_name(), name::layer::nhwc_center_crop2d(), { x });
    node->set(
      name::size,
      tensor::build(INT32, { width, height }));  // set resize method
    (void)(node);
    m_impl->m_compiled = false;
  }
  ...
  const Graph &ImageFilter::graph() const { return *m_impl->m_graph; }

  void ImageFilter::center_crop(int side) { center_crop(side, side); }
  ...
  Program::shared ImageFilter::program() const { return m_impl->m_program; }

  void ImageFilter::letterbox(
  ...
  void ImageFilter::force_gray(const std::vector<float> &scale) {
    auto scale_tensor = tensor::build(
      FLOAT32, { int32_t(scale.size()) }, scale);
    ctx::bind<Graph> _bind_graph(m_impl->m_graph.get());
    auto             top = m_impl->m_graph->nodes().back();
    auto node = bubble::op(serial_name(), name::layer::force_gray(), { top });
