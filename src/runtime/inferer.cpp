...
namespace ts
{
  ...
  void *FakeMemoryAllocator(int, size_t, void *, size_t) { return nullptr; }
  ...
  void infer_value(Node &node) {
    if (Bubble::IsEndPoint(node->op())) { return; }
  ...
  static TensorPrototype update_cache(
    Node                                      &node,
    std::unordered_map<Node, TensorPrototype> &cache,
    const TensorPrototype                     &proto) {
    cache.insert(std::make_pair(node, proto));
    if (node->op() != Bubble::Parameter) {
      auto N = proto.fields_count();
      for (size_t i = 0; i < N; ++i) {
        auto shape_key = Bubble::RetentionParam::shape
                       + (i ? "_" + std::to_string(i) : "");
        auto dtype_key = Bubble::RetentionParam::dtype
                       + (i ? "_" + std::to_string(i) : "");
        auto dtype_shape = proto.field(i);
        node->set(shape_key, tensor::build(INT32, dtype_shape.sizes()));
        node->set(
          dtype_key, tensor::build(INT32, int32_t(dtype_shape.dtype())));
      }
    }
    return proto;
  }

  TensorPrototype infer(
    Node &node, std::unordered_map<Node, TensorPrototype> &cache) {
...
#define RETURN_CACHE(proto) return update_cache(node, cache, proto)
    ...
    for (auto &i: node.inputs()) {
      ...
      if (input_proto.back().dtype() == VOID) { RETURN_CACHE(VOID); }
    }

    auto shape_infer = ShapeInferer::Query(node->op());
    if (shape_infer == nullptr) {
      TS_LOG_ERROR << "No method to infer " << node->op() << ":"
                   << node->name();
      RETURN_CACHE(VOID);
    }
    ...
    if (
      after_node_value.empty()
      || after_node_value.data() == before_node_value.data()) {
      // if node's #value not set or not updated, then do infer
      infer_value(node);
    }

    // if (node->op() != Bubble::Const)
    //     TS_LOG_INFO << node->op() << ":" << node->name() << " => " <<
    //     output_proto;
...
#undef RETURN_CACHE
  ...
  std::vector<TensorPrototype> infer(
    std::vector<Node>                         &nodes,
    std::unordered_map<Node, TensorPrototype> &cache) {
    std::vector<TensorPrototype> shapes;
    shapes.reserve(nodes.size());
    for (auto &node: nodes) { shapes.push_back(infer(node, cache)); }
    return shapes;
  }
