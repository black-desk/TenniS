...
namespace ts
{
  namespace infer_factory
  {
    static Tensor get_value(const Node &node) {
      if (node->op() == Bubble::Const) { return node->get("value"); }
    ...
    static Tensor try_run(const Node &node, const std::vector<Tensor> &inputs) {
      ...
      if (Bubble::IsEndPoint(node->op())) { return Tensor(); }
      ...
      auto op = OperatorCreator::CreateNoException(
        memory_device.type(), node->op());
      if (op == nullptr) return Tensor();

      Tensor output;

      try {
        for (const auto &it: node->params()) { op->set(it.first, it.second); }
        op->init();
        for (auto &t: inputs) { stack.push(t); }
        ...
        if (out == 1) {
          ...
          output = Tensor::Pack(
            std::vector<Tensor>(stack.begin(), stack.end()));
        }
      } catch (...) { return Tensor(); }
    ...
    static TensorPrototype _resize2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x          = inputs [0];
    ...
    static TensorPrototype _transpose(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto                 x = inputs [0];
      ...
      if (node->has("permute")) {
        ...
        for (size_t i = 0; i < x.dims(); ++i) { tmp.push_back(int32_t(i)); }
    ...
    static TensorPrototype _copy(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      return inputs [0];
    }

    static TensorPrototype to_float(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
    ...
    static TensorPrototype crop_nd(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x          = inputs [0];
      auto size_value = get_value(node.input(1));
      if (size_value.empty()) return VOID;
    ...
    static int32_t conv2d_forward(
      ...
      int32_t stride) {
      return int32_t(
        std::floor((x + padding - (dilation * (kernel - 1) + 1)) / stride + 1));
    }

    static TensorPrototype conv2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
    ...
    static int32_t pooling2d_forward(
      int32_t x, int32_t padding, int32_t kernel, int32_t stride) {
      return int32_t(std::ceil((x + padding - kernel) / float(stride) + 1));
    }

    static TensorPrototype pooling2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    static TensorPrototype _eltwise(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
    ...
    static TensorPrototype flatten(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      if (need_size < x_size) {
        ...
        shape.back() = std::accumulate(
          size.begin() + dim, size.end(), 1, std::multiplies<int>());
    ...
    static TensorPrototype flatten2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      if (dim <= 0) {
        ...
        y_shape = { std::accumulate(
                      size.begin(), size.end(), 1, std::multiplies<int>()),
                    1 };
      } else {
        y_shape = {
          std::accumulate(
            size.begin(), size.begin() + dim, 1, std::multiplies<int>()),
          std::accumulate(
            size.begin() + dim, size.end(), 1, std::multiplies<int>())
        };
      }
    ...
    static TensorPrototype inner_prod(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      auto &w = inputs [1];

      bool transpose = false;
      if (node->has("transpose")) { transpose = node->get_bool("transpose"); }
    ...
    static TensorPrototype _reshape(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x     = inputs [0];
    ...
    static TensorPrototype dynamic_padding(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      ...
      if (param0_shape [channel_dim] < 0) { return VOID; }
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_onnx_pooling2d_padding", dynamic_padding)
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_dragon_pooling2d_padding", dynamic_padding)
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_mx_pooling2d_padding", dynamic_padding)
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_tf_conv2d_padding", dynamic_padding)
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_tf_pooling2d_padding", dynamic_padding)
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_dragon_conv2d_padding", dynamic_padding)

    static TensorPrototype pooling2d_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
...

      {
        auto &update_node = *const_cast<Node *>(&node);
        update_node->set(
          "#padding", tensor::build<int32_t>(INT32, { 4, 2 }, padding));
      }
    ...
    static TensorPrototype conv2d_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    static TensorPrototype global_pooling2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "global_pooling2d", global_pooling2d)

    TS_STATIC_ACTION(ShapeInferer::Register, "sigmoid", _copy)

    static TensorPrototype _dims(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      {
        auto &update_node = *const_cast<Node *>(&node);
        auto  dims        = inputs [0].dims();
        update_node->set(
          "#value", tensor::build<int32_t>(INT32, int32_t(dims)));
      }
    ...
    static TensorPrototype _expand(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x = inputs [0];
      ...
      if (! inverse) {
        while (y.size() < dims && front > 0) { y.insert(y.begin(), 1); }
        while (y.size() < dims && end > 0) { y.insert(y.end(), 1); }
      } else {
        while (y.size() < dims && end > 0) { y.insert(y.end(), 1); }
        while (y.size() < dims && front > 0) { y.insert(y.begin(), 1); }
      }
    ...
    static TensorPrototype _dimshuffle(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x = inputs [0];
    ...
    static TensorPrototype _nhwc_center_crop2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x    = inputs [0];
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_nhwc_center_crop2d", _nhwc_center_crop2d)

    static TensorPrototype _nhwc_letterbox(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x    = inputs [0];
      auto size = node->get_int_list("size");
    ...
    static TensorPrototype _nhwc_scale_resize2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto x    = inputs [0];
      auto size = node->get_int_list("size");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "_nhwc_scale_resize2d", _nhwc_scale_resize2d)

    static TensorPrototype _reshape_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype affine_sample2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype batch_to_space4d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      output_shape [0] = input_shape [0] < 0 ?
                         -1 :
                         input_shape [0] / (block_height * block_width);
      output_shape [2] = input_shape [2] < 0 ? -1 :
                                               input_shape [2] * block_height
                                                 - crop_top - crop_bottom;
      output_shape [3] = input_shape [3] < 0 ?
                         -1 :
                         input_shape [3] * block_width - crop_left - crop_right;
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "batch_to_space4d", batch_to_space4d)

    static TensorPrototype space_to_batch4d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      output_shape [0] = input_shape [0] < 0 ?
                         -1 :
                         input_shape [0] * block_height * block_width;
      output_shape [2] = input_shape [2] < 0 ?
                         -1 :
                         (input_shape [2] + padding_top + padding_bottom)
                             / block_height;
      output_shape [3] = input_shape [3] < 0 ?
                         -1 :
                         (input_shape [3] + padding_left + padding_right)
                             / block_width;
      output_shape [1] = input_shape [1] < 0 ? -1 : input_shape [1];
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "space_to_batch4d", space_to_batch4d)

    static TensorPrototype _field(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype _pack(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      std::vector<Tensor::Prototype> output;
      for (auto &i: inputs) { output.emplace_back(i.dtype(), i.sizes()); }
    ...
    static TensorPrototype quantize(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      return { INT8, x.sizes() };
    }
    ...
    static TensorPrototype broadcast(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto dtype = inputs [0].dtype();
    ...
    static TensorPrototype conv2d_quantized(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "conv2d_quantized", conv2d_quantized)

    static TensorPrototype conv2d_winograd(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    static TensorPrototype conv2d_winograd_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "conv2d_winograd_v2", conv2d_winograd_v2)

    static TensorPrototype dcn_v2_forward(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    static TensorPrototype depthwise_conv2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "depthwise_conv2d", depthwise_conv2d)

    static TensorPrototype depthwise_conv2d_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "depthwise_conv2d_v2", depthwise_conv2d_v2)

    static TensorPrototype divided(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      while (size.size() < x.dims()) { size.insert(size.begin(), 1); }

      auto y_shape = x.sizes();

      for (size_t i = 0; i < x.dims(); ++i) {
        if (size [i] == 1) continue;
        y_shape [i] = int32_t(std::ceil(float(y_shape [i]) / size [i]))
                    * size [i];
      }
    ...
    static TensorPrototype force_color(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype force_gray(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      if (x.dims() == 0) return VOID;
    ...
    static TensorPrototype gatherv2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x       = inputs [0];
      ...
      auto  input_shape   = x.sizes();
      if (indices_shape [indices_shape.size() - 1] > input_shape.size())
        return VOID;

      output_shape.insert(
        output_shape.end(),
        input_shape.begin() + indices_shape [indices_shape.size() - 1],
        input_shape.end());
    ...
    class ReductionOp {
      public:
        ...
        ReductionOp(
          const std::string &axes_attr,
          const std::string &keep_axes_attr) TS_NOEXCEPT :
          axes_attr_name(axes_attr),
          keep_axes_attr_name(keep_axes_attr) {}

        TensorPrototype operator()(
          const Node &node, const std::vector<TensorPrototype> &inputs) {
          auto &x    = inputs [0];
          auto  dims = int32_t(x.dims());
          ...
          std::sort(
            axes.begin(), axes.end(), [](int a, int b) { return a > b; });

          auto y_shape = x.sizes();
          for (auto axis: axes) {
            y_shape.erase(y_shape.begin() + axis);
            if (keep_axes) { y_shape.insert(y_shape.begin() + axis, 1); }
          }
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "max", ReductionOp("dim", "keep_dims"))

    TS_STATIC_ACTION(ShapeInferer::Register, "maximum", _eltwise)

    static TensorPrototype non_max_suppression_v3(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      // auto &x = inputs[0];
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "non_max_suppression_v3", non_max_suppression_v3)

    static int32_t conv2d_backward(
      int32_t y,
      int32_t padding,
      int32_t dilation,
      int32_t kernel,
    ...
    static TensorPrototype transpose_conv2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto format = node->get_string("format");
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "transpose_conv2d", transpose_conv2d)

    static TensorPrototype winograd_transform_kernel(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register,
      "winograd_transform_kernel",
      winograd_transform_kernel)

    static TensorPrototype pad(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
...
#define TYR_GET_NODE_INT_ATTR(attr, value)                                     \
  ...
  if (node->has(#attr)) attr = node->get_int(#attr);
    ...
    static TensorPrototype proposal(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype range(
      ...
      auto output = try_run(
        node,
        {
          get_value(node.input(0)),
          get_value(node.input(1)),
          get_value(node.input(2)),
        });
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "reduce_mean", ReductionOp("dims", "keep_dims"))
    TS_STATIC_ACTION(
      ShapeInferer::Register, "reduce_sum", ReductionOp("dims", "keep_dims"))
...
#define TRY_GET_NODE_INT_LIST_ATTR(attr)                                       \
  ...
  if (node->has(#attr)) attr = node->get_int_list(#attr);
...
#define FIX_DIM_ADD_1(dim, x)                                                  \
  ...
  if (dim < 0 || dim + 1 >= int32_t(x.dims())) return VOID;
    ...
    static TensorPrototype resize_nearest_neighbor(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype roi_align(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
    ...
    static TensorPrototype sample2d(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      ...
      auto y_shape = x.sizes();
      if (y_shape [dim] > 0) y_shape [dim] = int32_t(x.sizes() [dim] * scale);
      if (y_shape [dim + 1] > 0)
    ...
    static TensorPrototype shape_index_patch(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.size() != 1) return VOID;
      auto &x   = inputs [0];
      auto &pos = inputs [1];
      ...
      auto x_patch_h = int(
        origin_patch [0] * height / float(origin [0]) + 0.5f);
      auto x_patch_w = int(origin_patch [1] * width / float(origin [1]) + 0.5f);

      Shape y_shape = {
        number, channels, x_patch_h, landmarkx2 / 2, x_patch_w
      };
    ...
    TS_STATIC_ACTION(
      ShapeInferer::Register, "shape_index_patch", shape_index_patch)

    static TensorPrototype squeeze(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
    ...
    static TensorPrototype unsqueeze(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      auto &x = inputs [0];
      ...
      for (auto axis: axes) {
        ...
        if (axis > max_axis || axis < -max_axis) { return VOID; }
        if (axis >= 0) {
    ...
    static Shape infer_tile(Shape &x, Shape &repeats) {
      if (x.size() == repeats.size()) {
        ...
        do { x.insert(x.begin(), 1); } while (x.size() < repeats.size());
      }
      Shape y(x.size());
    ...
    static TensorPrototype yolo_poster(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.empty()) return VOID;
      ...
      std::vector<Tensor::Prototype> outputs(
        N, Tensor::Prototype(FLOAT32, { -1, 6 }));
    ...
    static int infer_output(
      int x, int &begin, int &end, int stride, bool begin_flag, bool end_flag) {
      // begin \in [0, x)
      // end \in [-1, x]
      if (x >= 0) {
        if (begin_flag) {
    ...
    static TensorPrototype slice_v3(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.empty()) return VOID;
      ...
      for (size_t i = 0; i < N; ++i) {
        ...
        y_shape [dim] = infer_output(
          y_shape [dim], starts [i], ends [i], steps [i], false, false);
      }
    ...
    class SliceDim {
      public:
        ...
        SliceDim(
          ...
          bool new_axis_flag    = false,
          bool shrink_axis_flag = false) {
          this->begin            = begin;
          this->end              = end;
          this->stride           = stride;
    ...
    static TensorPrototype strided_slice(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.empty()) return VOID;
        ...
        if (ellipsis_count) {
          for (auto it = slice_dims.begin(); it != slice_dims.end(); ++it) {
            if (it->flag.ellipsis) {
              ...
              std::vector<SliceDim> full_select_dims(
                x.dims() - sliced_count, SliceDim(0, 0, 1, true, true));
              slice_dims.insert(
                it, full_select_dims.begin(), full_select_dims.end());
              break;
            }
          }
          ...
          std::vector<SliceDim> full_select_dims(
            x.dims() - sliced_count, SliceDim(0, 0, 1, true, true));
          slice_dims.insert(
            slice_dims.end(), full_select_dims.begin(), full_select_dims.end());
        }
      }
      ...
      for (size_t i = 0; i < slice_dims.size(); ++i) {
        auto &s     = slice_dims [i];
        y_shape [i] = infer_output(
          y_shape [i], s.begin, s.end, s.stride, s.flag.begin, s.flag.end);
      }
      ...
      slice_dims                  = have_new_axes_slice_dims;
      {
        for (size_t i = 0; i < slice_dims.size(); ++i) {
          auto &s = slice_dims [i];
          if (s.flag.new_axis) { y_shape.insert(y_shape.begin() + i, 1); }
        }
      }
      // do shrink axis
      {
        for (int i = int(slice_dims.size()) - 1; i >= 0; --i) {
          auto &s = slice_dims [i];
          if (s.flag.shrink_axis) { y_shape.erase(y_shape.begin() + i); }
        }
      }
    ...
    static TensorPrototype sample2d_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.size() != 2) return VOID;
      auto &x = inputs [0];
    ...
    static TensorPrototype slice_v2(
      const Node &node, const std::vector<TensorPrototype> &inputs) {
      if (inputs.empty()) return VOID;
