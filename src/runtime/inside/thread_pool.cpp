...
namespace ts
{
  ...
  Thread::Thread() : is_running(true), task(nullptr), after_task(nullptr) {
  ...
  void Thread::run(
    int                            signet,
    const Thread::task_type       &task,
    const Thread::after_task_type &after_task) {
  ...
  ThreadPool::ThreadPool(size_t pool_size) : thread_pool(pool_size) {
  ...
  Thread *ThreadPool::run(const Thread::task_type &task) {
    if (thread_pool.size() == 0) {
      ...
      cart->run(
        signet,
        task,
        Thread::after_task_type(std::bind(
          &ThreadPool::recycling_thread, this, std::placeholders::_1)));
      return cart;
    }
  }

  Thread *ThreadPool::run(
    const Thread::task_type &task, const Thread::after_task_type &after_task) {
    if (thread_pool.size() == 0) {
      task(0);
      return nullptr;
    } else {
  ...
  void ThreadPool::join() {
    std::unique_lock<std::mutex> locker(running_core_mutex);
    while (this->running_core.size() != this->thread_pool.size())
      running_core_cond.wait(locker);
  }
  ...
  size_t ThreadPool::size() const { return thread_pool.size(); }
