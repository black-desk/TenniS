...
namespace ts
{
  ...
  OperatorInstruction::OperatorInstruction(
    const Operator::shared &func, int nargs, int nresults) :
  ...
  Later profiler_run(const Operator::shared &op) {
    ...
    oss << "op(%04d):" << tensor::to_string(op->get(Bubble::RetentionParam::op))
  ...
  void OperatorInstruction::run(Workbench &workbench) {
    ...
    if (return_size != m_nresults) {
      TS_LOG_ERROR << "Operator " << op()->name() << "<" << op()->op()
                   << "> expected " << m_nresults << " outputs, got "
                   << return_size << eject;
    }  // must return given sizes
    TS_AUTO_CHECK(
      stack.size()
      >= static_cast<size_t>(return_size));  // must have enough returns
  ...
  OperatorInstruction::shared OperatorInstruction::clone() const {
    ...
    auto op = m_creator();
    for (auto &param: m_func->params()) { op->set(param.first, param.second); }
    op->init();
    auto dolly = std::make_shared<OperatorInstruction>(
      op, m_nargs, m_nresults, m_description);
  ...
  LambdaInstruction::LambdaInstruction(
    const LambdaInstruction::Lambda &lambda) :
    m_lambda(lambda) {}

  void LambdaInstruction::run(Workbench &workbench) { m_lambda(workbench); }

  LambdaInstruction::LambdaInstruction(
    const LambdaInstruction::Lambda &lambda, const std::string &description) :
  ...
  std::string Instruction::repr() const { return this->str(); }
