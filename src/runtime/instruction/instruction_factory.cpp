...
namespace ts
{
  ...
  static std::map<std::string, InstructionCreator::function>
    &MapOpInstructionCreator() {
    static std::map<std::string, InstructionCreator::function> map_op_creator;
    return map_op_creator;
  };

  InstructionCreator::function InstructionCreator::Query(const std::string &op)
    TS_NOEXCEPT {
    auto &map_op_creator = MapOpInstructionCreator();
    auto  op_creator     = map_op_creator.find(op);
    if (op_creator != map_op_creator.end()) { return op_creator->second; }
    return function(nullptr);
  }

  void InstructionCreator::Register(
    const std::string                  &op,
