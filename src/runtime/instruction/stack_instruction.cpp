  ...
  namespace instruction
  {
    Instruction::shared Stack::push(int i) {
      return std::make_shared<LambdaInstruction>(
        [=](Workbench &workbench) { workbench.stack().push(i); },
        "push(" + std::to_string(i) + ")");
    }

    Instruction::shared Stack::clone(int i) {
      return std::make_shared<LambdaInstruction>(
        [=](Workbench &workbench) { workbench.stack().clone(i); },
        "clone(" + std::to_string(i) + ")");
    }

    Instruction::shared Stack::erase(int i) {
      return std::make_shared<LambdaInstruction>(
        [=](Workbench &workbench) { workbench.stack().erase(i); },
    ...
    Instruction::shared Stack::erase(int beg, int end) {
      return std::make_shared<LambdaInstruction>(
        [=](Workbench &workbench) { workbench.stack().erase(beg, end); },
