...
namespace ts
{
  ...
  void Operator::set(const std::string &param, const Tensor &value) {
    ...
    if (
      ! is_retention_param && ! is_in_fields(param)
      && m_param_checking_mode == ParamCheckingMode::STRICT) {
  ...
  void Operator::clear_fields() {
    ...
    for (auto &param: this->m_required_fields) { this->m_params.erase(param); }
    this->m_required_fields.clear();
  }

  void Operator::field(
    const std::string  &param,
    Operator::FieldAttr attr,
  ...
  void Operator::field(const std::string &param, Operator::FieldAttr attr) {
    switch (attr) {
      default: break;
      case OPTIONAL: this->m_optional_fields.insert(param); break;
      case REQUIRED:
  ...
  void Operator::init() {
    if (! this->check_params()) {
      ...
      oss << "Operator " << op << " \"" << name
          << "\" has unsatisfied fields: ";
  ...
  int Operator::output_count() const { return 1; }
  ...
  std::vector<std::string> Operator::list_all_fields() const {
    ...
    for (auto &f: m_required_fields) { result.emplace_back(f); }
    for (auto &f: m_optional_fields) { result.emplace_back(f); }
    return result;
  }
