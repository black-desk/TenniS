...
namespace ts
{
  static std::string fuzzy_name(
    const Program::map<std::string, int> &map_name_slot,
  ...
  Program::shared Program::Compile(
    ...
    for (auto &inst: block.instructions) {
      auto data_sagment_inst = dynamic_cast<DataSegmentInstruction *>(
        inst.get());
      if (data_sagment_inst == nullptr) continue;
      inst = std::make_shared<DataSegmentInstruction>(
        data_sagment_inst->data_index() + data_sagment_base);
    }
    ...
    for (auto &input: module_inputs) {
      program->m_map_input_slots.insert(
        std::make_pair(input.bubble().name(), slot_i++));
      program->m_input_names.emplace_back(input.bubble().name());
    }
    slot_i = 0;
    for (auto &output: module_outputs) {
      program->m_map_output_slots.insert(
        std::make_pair(output.bubble().name(), slot_i++));
    ...
    for (size_t i = 0; i < module_inputs.size(); ++i) {
      ...
      program->m_input_dtypes [i] = DTYPE(
        tensor::to_int(input->get(Bubble::RetentionParam::dtype)));
    }
    for (size_t i = 0; i < module_outputs.size(); ++i) {
      auto &output = module_outputs [i];
      if (! output->has(Bubble::RetentionParam::dtype)) continue;
      program->m_output_dtypes [i] = DTYPE(
        tensor::to_int(output->get(Bubble::RetentionParam::dtype)));
    }
  ...
  Program::Program(
    const ComputingDevice &device, const std::shared_ptr<std::mutex> &mutex) :
    m_device(device), m_mutex(mutex) {
    auto memory_device = ComputingMemory::Query(m_device);

    this->m_data_segment = std::make_shared<Stack>(
      memory_device, DynamicSyncMemoryController::Make(memory_device, true));
  }
  ...
  Program::shared Program::input_filter(int slot) const {
    if (slot < 0 || slot >= input_count()) {
      TS_LOG_ERROR << "Input index out of range[0, " << input_count()
                   << "). with index=" << slot << eject;
    }
  ...
  int Program::input_slot(const std::string &name) const {
    ...
    if (slot_it == m_map_input_slots.end()) {
      TS_LOG_ERROR << "Can not identify the name \"" << name
                   << "\", did you mean: "
  ...
  int Program::input_count() const { return int(m_input_dtypes.size()); }

  int Program::output_count() const { return int(m_output_dtypes.size()); }

  size_t Program::length() const { return m_program.size(); }
  ...
  void Program::bind_filter(int slot, Program::shared filter) {
    ...
    if (filter != nullptr) {
      if (filter->input_count() != 1 || filter->output_count() != 1) {
        TS_LOG_ERROR << "Filter's input count and output count must both be 1."
                     << eject;
      }
    }
  ...
  void Program::set_operator_param(
  ...
  const Stack &Program::data_segment() const { return *m_data_segment; }
