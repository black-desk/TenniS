...
namespace ts
{
  RuntimeContext::RuntimeContext() { set_computing_thread_number(4); }
  RuntimeContext::RuntimeContext(const MemoryDevice &device) : self() {
    this->m_flow = HypeSyncMemoryController<FlowMemoryController>::Make(
      device, false);
    this->m_dynamic = DynamicSyncMemoryController::Make(device, false);
  }

  void RuntimeContext::set_computing_thread_number(
    int computing_thread_number) {
...
#  endif
  ...
  RuntimeContext::self RuntimeContext::clone() const {
    ...
    if (m_thread_pool) {
      doly.m_thread_pool = std::make_shared<ThreadPool>(
        this->m_thread_pool->size());
    }
    if (this->m_dynamic) { doly.m_dynamic = this->m_dynamic->clone(); }
    if (this->m_flow) { doly.m_flow = this->m_flow->clone(); }
  ...
  RuntimeContext::self &RuntimeContext::operator=(
  ...
  ThreadPool &RuntimeContext::thread_pool() { return *this->m_thread_pool; }
  ...
  SyncMemoryController::shared RuntimeContext::flow() const { return m_flow; }
