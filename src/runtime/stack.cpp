...
#define relative2absolute(i) ((i) >= 0 ? m_base + (i) : m_stack.size() + (i))
  ...
  Tensor *Stack::index(int i) {
    return &this->m_stack.at(relative2absolute(i));
  }

  const Tensor *Stack::index(int i) const {
    return &this->m_stack.at(relative2absolute(i));
  }
  ...
  HardConverter::function Stack::converter() const {
    if (this->m_converter == nullptr) {
      this->m_converter = HardConverter::Query(
        m_device.type(), m_device.type());
