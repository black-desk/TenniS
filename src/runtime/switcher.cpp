...
namespace ts
{
  ...
  typedef ts_device_context *(*ts_initial_device_context)(
    const ts_Device *device);
  typedef void (*free_device_context)(ts_device_context *);

  typedef void (*ts_bind_context)(ts_device_context *);

  static inline bool check_cpu_features(
    const std::initializer_list<CPUFeature> &features) {
  ...
  static inline std::string cut_path_tail(
    const std::string &path, std::string &tail) {
  ...
  static inline std::string cut_name_ext(
    const std::string &name_ext, std::string &ext) {
    ...
    if (fp != NULL) {
      while (! feof(fp)) {
        ...
        if (pSymbol >= (void *)start && pSymbol < (void *)end) {
          ...
          if (
            name == model_name || name == libname || name == model_named
            || name == libnamed) {
            fclose(fp);
            return ret;
          }
  ...
  class TS_DEBUG_API Switcher {
    public:
      ...
      Switcher() { m_importer = std::make_shared<Importor>(); };
      ~Switcher() { free(); };
      ...
      bool auto_switch(const ComputingDevice &device) {
        ...
        if (check_cpu_features({ AVX, FMA })) {
          ...
          TS_LOG_INFO << "Load dll:" << path
                      << " to support AVX and FMA instruction.";
          m_is_loaded = m_importer->load(path);
          if (! m_is_loaded) {
            m_is_loaded = m_importer->load(tennis_avx_fma_dll);
            if (! m_is_loaded)
              TS_LOG_ERROR << "Load dll failed,The current machine does not "
                              "support the AVX or FMA instruction set"
                           << eject;
          }
          ...
          if (! m_is_loaded) {
            ...
            if (! m_is_loaded)
              TS_LOG_ERROR << "Load dll failed,The current machine does not "
                              "support the AVX instruction set"
                           << eject;
          }
          ...
          TS_LOG_ERROR
            << "Minimum support for SSE instruction,Otherwise you need to "
               "compile a version that does not support any instruction set"
            << eject;
        }

        m_pre_creator_map.reset(
          ts_plugin_get_creator_map(), ts_plugin_free_creator_map);

        get_creator_map creator_map_fuc = (get_creator_map)
                                            m_importer->get_fuc_address(
                                              "ts_plugin_get_creator_map");
        free_creator_map free_creator_map_fuc =
          (free_creator_map)m_importer->get_fuc_address(
            "ts_plugin_free_creator_map");
      ...
      bool is_load_dll() { return m_is_loaded; }

      Importor::shared importor() { return m_importer; }
    private:
      void free() {
  ...
  bool SwitchControll::is_load_dll() { return m_is_loaded; }

  void SwitchControll::init_context(const ComputingDevice &device) {
    if (! is_load_dll()) {
      TS_LOG_ERROR
        << "Dynamic library not loaded, please call auto_switch first" << eject;
    }

    auto                     &switcher = get_switcher();
    ts_initial_device_context initial_device_context_fuc =
      (ts_initial_device_context)switcher.importor()->get_fuc_address(
        "ts_plugin_initial_device_context");
    free_device_context free_device_context_fuc =
      (free_device_context)switcher.importor()->get_fuc_address(
        "ts_plugin_free_device_context");
    ts_Device ts_device;
    ts_device.id   = device.id();
    ts_device.type = device.type().c_str();
    m_device_context.reset(
      initial_device_context_fuc(&ts_device), free_device_context_fuc);
  }
  ...
  void SwitchControll::bind_context() {
    ...
    if (! m_device_context) {
      TS_LOG_ERROR << "DeviceContext is nullptr, please call init_context first"
                   << eject;
    }
    auto           &switcher            = get_switcher();
    ts_bind_context ts_bind_context_fuc = (ts_bind_context)switcher.importor()
                                            ->get_fuc_address(
                                              "ts_plugin_bind_device_context");
