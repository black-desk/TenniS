...
namespace ts
{
  class BindWorkbenchRuntime {
    public:
      ...
      ~BindWorkbenchRuntime() { DeviceContext::Switch(m_pre_device_context); }
    private:
  ...
  static std::string feature_log(const std::vector<CPUFeature> &features) {
    ...
    for (size_t i = 0; i < features.size(); ++i) {
      ...
      oss << cpu_feature_str(feature) << "="
          << (check_cpu_feature(feature) ? "ok" : "failed");
    }
    oss << "}";
    return oss.str();
  }
  ...
  static bool check_cpu_features() {
    ...
    for (auto &fea: features) {
      ...
      if (! flag) {
        /*TS_LOG_ERROR << "The processor does not support the current
           instruction set: " << feature_log(features)
                     << eject;*/
        return false;
      }
    }
  ...
  Workbench::Workbench(const ComputingDevice &device) {
    ...
    this->m_static_memory = DynamicSyncMemoryController::Make(
      memory_device, true);
    // TODO: Make real flow memory controller
    this->m_flow_memory = HypeSyncMemoryController<FlowMemoryController>::Make(
      memory_device, false);
    this->m_dynamic_memory = DynamicSyncMemoryController::Make(
      memory_device, false);
    this->m_stack = std::make_shared<Stack>(memory_device, this->m_flow_memory);
    // bind flow and dynamic memory, so you can use it to alloc memory in any
    // where
    ...
    if (! check_cpu_features()) { m_switch_controller->auto_switch(device); }
  }

  Workbench::Workbench(
    const ComputingDevice &device, int computing_thread_number) :
    self(device) {
    this->m_runtime_context.set_computing_thread_number(
      computing_thread_number);
  }

  Workbench::Workbench(
    const ComputingDevice &device, CpuEnable::CpuPowerMode cpu_mode) :
    self(device) {
  ...
  static inline std::unique_ptr<ctx::bind<Profiler>> bind_profiler(
    bool _do, Profiler &profiler) {
    if (! _do) return nullptr;
    return std::unique_ptr<ctx::bind<Profiler>>(
      new ctx::bind<Profiler>(profiler));
  }
  ...
  Workbench::shared Workbench::clone() const {
    Workbench::shared dolly(
      new Workbench(this->m_device_context.computing_device));
    ...
    if (this->m_desktop) { dolly->m_desktop = this->m_desktop->clone(); }

    return std::move(dolly);
  }

  Workbench::shared Workbench::Load(
    const Module::shared &module, const ComputingDevice &device) {
  ...
  int Workbench::input_count() const { return int(m_inputs.size()); }

  int Workbench::output_count() const { return int(m_outputs.size()); }
  ...
  void Workbench::bind_filter(
    const std::string &name, ImageFilter::shared filter) {
    if (m_desktop == nullptr) {
      TS_LOG_ERROR << "Can not run workbench with no program setup" << eject;
    }
  ...
  const Tensor &Workbench::output(const std::string &name) const {
    {
      ...
      if (it != m_hooked_tensor.end()) { return it->second; }
    }
    if (m_desktop == nullptr) {
      TS_LOG_ERROR << "Can not run workbench with no program setup" << eject;
    }
  ...
  Operator::shared Workbench::offline_create(
    const Bubble &bubble, bool strict) {
    BindWorkbenchRuntime _bind_runtime(*this);

    auto built_op = OperatorCreator::Create(
      m_device_context.computing_device.type(), bubble.op(), strict);
  ...
  void Workbench::offline_run(
    ...
    for (auto &tensor: input) { stack.push(tensor); }
    ...
    for (int i = 0; i < output_count; ++i) { output [i] = stack [i]; }
  }

  void Workbench::offline_infer(
    Operator::shared                op,
    const std::vector<Tensor>      &input,
  ...
  void Workbench::set_operator_param(
  ...
  void Workbench::cast_tensor(DTYPE dtype) {
    if (m_cast_op == nullptr) {
      m_cast_op = OperatorCreator::Create(
        m_device_context.computing_device.type(), name::layer::cast(), false);
    }
  ...
  int Workbench::online_run(
    Operator::shared op, const std::vector<Tensor> &input) {
    // m_stack->push_base(m_stack->size()); // empty base
    // need pop_base(&Stack::pop_base, m_stack.get());
    for (auto &tensor: input) { m_stack->push(tensor); }
  ...
  void Workbench::online_run(
    Instruction::shared inst, const std::vector<Tensor> &input) {
    // m_stack->push_base(m_stack->size()); // empty base
    // need pop_base(&Stack::pop_base, m_stack.get());
    for (auto &tensor: input) { m_stack->push(tensor); }
    online_run(inst);
  ...
  int Workbench::online_run(
    const Bubble &bubble, const std::vector<Tensor> &input, bool strict) {
  ...
  static Tensor adjust_nhwc(const Tensor &tensor) {
    if (tensor.dims() < 2 || tensor.dims() > 4) {
      TS_LOG_ERROR << "Can not filter input with shape: "
                   << to_string(tensor.sizes()) << eject;
    }
    if (tensor.dims() == 2) {
    ...
    if (tensor.dims() == 3) {
      return tensor.reshape(
        { 1, tensor.size(0), tensor.size(1), tensor.size(2) });
    }
    return tensor;
  }
  ...
  int Workbench::launch_online(Program::shared program, int nargs) {
    ...
    if (program->input_count() != nargs) {
      TS_LOG_ERROR << "nargs must be " << program->input_count() << " vs. "
                   << nargs << " got." << eject;
    }
    if (m_stack->size() < nargs) {
      TS_LOG_ERROR << "stack must have arguments at less " << nargs << " vs. "
                   << m_stack->size() << " got." << eject;
    }
    if (program->device() != this->device().computing_device) {
      TS_LOG_ERROR << "Running " << program->device() << " on "
                   << this->device().computing_device;
    }
     ...
     */
    auto _bind_profiler = bind_profiler(
      m_do_profile && ctx::get<Profiler>() != &m_profiler, m_profiler);
    ...
    for (int i = 0; i < nargs; ++i) {
      ...
      if (arg.device() == this->device().memory_device) {
        ...
        this->m_stack->clone_push(
          arg);  // in case of sync memory out of flow memory
      }
    ...
    if (this->m_stack->size() != program->output_count()) {
      TS_LOG_ERROR << "Got unexpected output number want "
                   << program->output_count() << " vs. "
  ...
  std::vector<Tensor> Workbench::launch_offline(
    Program::shared program, const std::vector<Tensor> &args) {
    /**
     * save stack
     */
    ...
    for (auto &arg: args) { this->m_stack->push(arg); }
  ...
  std::vector<Tensor> Workbench::launch_offline(
    Program::shared program, const std::map<std::string, Tensor> &args) {
  ...
  Program::shared Workbench::compile(
    const Module::shared &module, const std::string &options) {
    BindWorkbenchRuntime _bind_runtime(*this);
  ...
  void Workbench::run_hook(const std::vector<std::string> &node_names) {
    ...
    auto controller = std::make_shared<DynamicMemoryController>(
      MemoryDevice(CPU));
    ...
    for (auto &node_name: node_names) { node_name_set.insert(node_name); }
  ...
  bool Workbench::set_cpu_power_mode(CpuEnable::CpuPowerMode cpu_mode) {
    ...
    if (flag) {
      int fixed_thread_num = this->m_runtime_context
                               .get_computing_thread_number();
      switch (cpu_mode) {
