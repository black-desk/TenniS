...
// NOTE:only support androi and linux now
    ...
    while (std::getline(fread, line)) {
      ...
      if (std::regex_match(line, result, pattern)) count++;
    }
    fread.close();
    if (count < 1) return 1;
    return count;
  ...
  static int get_cpu_max_freq(int cpu_id) {
    ...
    int           max_cpu_freq = 0;
    std::string   path         = "/sys/devices/system/cpu/cpu" + id_str
    ...
    path = "/sys/devices/system/cpu/cpu" + id_str
  ...
  static void split_cpu_big_little(
    std::vector<int> &cpu_sorted_ids,
    std::vector<int> &cpu_big_ids,
    std::vector<int> &cpu_little_ids) {
...
#  endif
    ...
    cpu_set_t mask;
    CPU_ZERO(&mask);
    for (int i = 0; i < cpu_ids.size(); i++) { CPU_SET(cpu_ids [i], &mask); }
  ...
  int CpuEnable::get_cpu_num() { return g_cpu_num; }

  int CpuEnable::get_cpu_big_num() {
    if (g_set_cpu_set_power_mode_flag) return int(cpu_big_ids.size());
    return -1;
  }

  int CpuEnable::get_cpu_little_num() {
    if (g_set_cpu_set_power_mode_flag) return int(cpu_little_ids.size());
    return -1;
  }
  ...
  bool CpuEnable::set_power_mode(CpuPowerMode mode) {
    ...
    if (
      (cpu_little_ids.empty() || cpu_little_ids.size() == 0)
      && mode != BALANCE) {
