...
int main() {
  ...
  Tensor tensor = tensor::build(
    UINT8, { 1, cvimage.rows, cvimage.cols, cvimage.channels() }, cvimage.data);
  ...
  for (int i = 0; i < output_count; ++i) {
    ...
    for (int n = 0; n < N; ++n) {
      ...
      cv::rectangle(
        cvimage, cv::Rect(box.x, box.y, box.w, box.h), CV_RGB(r, g, b), 2);
      ...
      if (label.empty()) {
        ...
        oss << label [int(box.label)] << ": "
            << int(std::round(box.scale * 100)) << "%";
      }
