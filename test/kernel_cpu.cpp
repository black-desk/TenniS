...
void test_blas(ts::Random &rand, float *ratio = nullptr) {
  ...
  T                     alpha  = rand.u() * 2;
  T                     beta   = rand.u() * 2;
  ts::blas::Order       order  = rand.u() > 0.5 ? ts::blas::RowMajor :
                                                  ts::blas::ColMajor;
  ts::blas::Transpose   TransA = rand.u() > 0.5 ? ts::blas::NoTrans :
                                                  ts::blas::Trans;
  ts::blas::Transpose   TransB = rand.u() > 0.5 ? ts::blas::NoTrans :
                                                  ts::blas::Trans;
  ...
  try {
    ts::cpu::math<T, T>::gemm(
      ...
      C.get(),
      ldc);
  } catch (const ts::Exception &e) { return; }
  ...
  std::cout << sum << " " << spent << "ms vs. " << spent2 << "ms, "
            << spent2 / spent << std::endl;
...
int main() {
  ...
  //
  // ts::cpu::math<double>::gemm(blas::ColMajor, blas::NoTrans, blas::NoTrans,
  // 3, 3, 2, 1, A, 3, B, 2, 1, C, 3);
  ts::Random rand(4481);
  int        N         = 100;
  float      ratio_sum = 0;
  for (int i = 0; i < N; ++i) {
