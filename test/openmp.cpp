...
float dot7(const float *x, const float *y, int N) {
  float         sum   = 0;
  ts::float32x4 sumx4 = 0;
  //#pragma omp parallel for reduction(+:sumx4) num_threads(ts::openmp_threads(N
  /// 4))
  ...
  for (int i = N / 4 * 4; i < N; ++i) { sum += x [i] * y [i]; }
...
#pragma omp parallel for num_threads(1)
  for (int i = 0; i < 10; ++i) {
    printf(
      "Top task: %2d, Top ID: %d, Bottom task: %2d, Bottom ID: %d\n",
      top,
      top_id,
      i,
      omp_get_thread_num());
...
void        test_loop_top() {
#pragma omp parallel for num_threads(1)
  for (int i = 0; i < 10; ++i) { test_loop_bottom(i, omp_get_thread_num()); }
}
...
void print_avg_time(
  ...
  for (int i = 0; i < times; ++i) { sum += func(a, b, N); }
  ...
  std::cout << title << ": sum=" << sum << ", spent=" << spent << "ms"
            << std::endl;
}
