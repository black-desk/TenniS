...
#define TS_LOG_CHECKING(condition)                                             \
  TS_LOG_INFO("Case [")((condition) ? "PASSED" : "FAILED")("]: ")(#condition)
...
std::string p(void *p) {
  ...
  oss << "0x" << std::hex << std::setw(sizeof(p) * 2) << std::setfill('0')
      << uint64_t(p);
  return oss.str();
}
...
void *fake_allocator(int id, size_t new_size, void *mem, size_t mem_size) {
  ...
  if (new_size == 0) {
    ...
    TS_LOG_INFO << std::hex << "realloc(" << p(mem) << ", " << std::oct
                << new_size << ") -> " << std::hex << "0x" << new_mem;
  } else {
    new_mem = std::malloc(new_size);
    TS_LOG_INFO << "malloc(" << new_size << ") -> " << std::hex << p(new_mem);
  }
  if (new_mem == nullptr)
    throw OutOfMemoryException(MemoryDevice(fake_device_type, id), new_size);
  return new_mem;
}
