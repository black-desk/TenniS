...
void test_speed(
  ...
  auto pad_param      = tensor::build(
    FLOAT32, padding_shape, { 0, 0, 0, 0, 0, 0, 0, 0 });
  ...
  std::cout << "====device: " << device << " comment: " << comment
            << "====" << std::endl;
...
void print_avg_time(
  ...
  if (f23_kernel_flag)
    cpu::Conv2dAlgorithm<float>::conv3x3_winograd23_transform_kernel(
      kernel, kernel_trans);
  else
    cpu::Conv2dAlgorithm<float>::conv3x3_winograd63_transform_kernel(
      kernel, kernel_trans);

  Tensor x(FLOAT32, input_shape);
  float *x_ptr = x.data<float>();
  for (int i = 0; i < x.count(); i++) { x_ptr [i] = i; }

  Shape out_shape = {
    input_shape [0], kernel_shape [0], input_shape [2] - 2, input_shape [3] - 2
  };
  ...
  for (int i = 0; i < times; ++i) { func(x, kernel_trans, out); }

  auto end = system_clock::now();
  duration += duration_cast<microseconds>(end - start);
  double spent = 1.0 * duration.count() / 1000;
  std::cout << title << ", spent=" << spent << "ms"
            << " avg time = " << spent / times << "ms" << std::endl;
}
...
int main() {
  ...
  // Tensor kernel = tensor::build(FLOAT32, kernel_shape,
  ...
  print_avg_time(
    ...
    x_shape,
    kernel_shape,
    cpu::Conv2dAlgorithm<float>::conv3x3_winograd23,
    true);
  print_avg_time(
    "conv3x3_winograd63",
    times,
    x_shape,
    kernel_shape,
    cpu::Conv2dAlgorithm<float>::conv3x3_winograd63,
    false);
  // test_speed("cpu", times, x_shape, kernel_shape, "input:{ 1,64,21,21
  // },kernel:{ 2,64,3,3 }");
  ...
  // print_avg_time("conv3x3_winograd23", times, x_shape, kernel_shape,
  // opt::Conv2dAlgorithm<float>::conv3x3_winograd23, true);
  // print_avg_time("conv3x3_winograd63", times, x_shape, kernel_shape,
  // opt::Conv2dAlgorithm<float>::conv3x3_winograd63, false); test_speed("cpu",
  // times, x_shape, kernel_shape, "input:{ 1,64,21,21 },kernel:{ 64,64,3,3 }");
  // test_speed("opt", times, x_shape, kernel_shape, "input:{ 1,64,21,21
  // },kernel:{ 64,64,3,3 }");
  ...
  // print_avg_time("conv3x3_winograd63", times, x_shape, kernel_shape,
  // opt::Conv2dAlgorithm<float>::conv3x3_winograd63, false); test_speed("cpu",
  // times, x_shape, kernel_shape, "input:{ 1,64,8,8 },kernel:{ 64,64,3,3 }");
  // test_speed("opt", times, x_shape, kernel_shape, "input:{ 1,64,8,8
  // },kernel:{ 64,64,3,3 }");
  ...
  // print_avg_time("conv3x3_winograd63", times, x_shape, kernel_shape,
  // opt::Conv2dAlgorithm<float>::conv3x3_winograd63, false); test_speed("cpu",
  // times, x_shape, kernel_shape, "input:{ 1,64,50,50 },kernel:{ 64,64,3,3 }");
  // test_speed("opt", times, x_shape, kernel_shape, "input:{ 1,64,50,50
  // },kernel:{ 64,64,3,3 }");
  ...
  // print_avg_time("conv3x3_winograd63", times, x_shape, kernel_shape,
  // opt::Conv2dAlgorithm<float>::conv3x3_winograd63, false); test_speed("cpu",
  // times, x_shape, kernel_shape, "input:{ 1,64,100,100 },kernel:{ 64,64,3,3
  // }"); test_speed("opt", times, x_shape, kernel_shape, "input:{ 1,64,100,1000
  // },kernel:{ 64,64,3,3 }");
