  ...
  class DataBase {
    public:
      ...
      explicit DataBase(int count_num) :
        m_index(0), m_count_num(count_num), m_avg_pass_time(0) {
      ...
      void index_inch() { m_index++; }

      void add(const DataS &data) { m_times [m_index].emplace_back(data); }
      ...
      void analyze() {
        ...
        std::vector<std::pair<std::string, OpDataS>> vt(
          time_op.begin(), time_op.end());
        std::sort(
          vt.begin(),
          vt.end(),
          [](
            const std::pair<std::string, OpDataS> &a,
            const std::pair<std::string, OpDataS> &b) {
            return a.second.avg_time >= b.second.avg_time;
          });
        time_op_vec = vt;
      }
      ...
      void log(std::ostream &out) {
        ...
        out << "[loop_count : " << m_count_num << " ][layer_num : " << layer_num
            << " ]" << std::endl;
        int  index = 0;
        auto data  = m_times [0];
        ...
        for (int i = 0; i < layer_num; i++) {
          ...
          out << "[index:" << index << ",op:" << op << ",name:" << name
  ...
  class BenchMark {
    ...
    public:
      BenchMark(const Option &op, bool need_statistical = false) :
        option(op), need_statistical(need_statistical) {
      ...
      BenchMark(const BenchMark &other) :
        option(other.option), need_statistical(other.need_statistical) {
      ...
      void benchmark(
        ...
        const std::initializer_list<int> &input_shape) {
        srand(time(NULL));
        ...
        std::cout << "Net: " << name << " ,input:[ " << shape [0] << ","
                  << shape [1] << "," << shape [2] << "," << shape [3] << "]"
                  << " ,max: " << max_time << "ms"
                  << " ,min: " << min_time
                  << "ms"
                     " ,avg: "
                  << count_time / option.loop_counts << "ms" << std::endl;
