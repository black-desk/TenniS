...
static bool get_node_info(const std::string &name, node_info &info) {
  std::unordered_map<std::string, node_info>::iterator iter = g_node_infos.find(
    name);
...
/*
static void ReplaceAll(std::string& strSource, const std::string& strOld, const
std::string& strNew)
{
    size_t nPos = 0;
...
static void print_tensor(
  ...
  int                layer,
  std::string       &res) {
  if (g_retentions.find(name) != g_retentions.end()) { return; }
  ...
  for (int i = 0; i < nlen; i++) {
    ...
    switch (tensor.dtype()) {
      ...
      case INT32:
        ...
        if (
          (op == name::layer::pooling2d() || op == name::layer::pooling2d_v2())
          && name == name::type) {
      ...
      case UINT64:
        ...
        break;
      default:
        std::cout << "unknow data type:" << type_str(tensor.dtype())
                  << std::endl;
        break;
    }
  }
...
static void print_param(
  const std::string                             &op,
  const std::unordered_map<std::string, Tensor> &params,
  int                                            layer,
  std::string                                   &res) {
  ...
  for (iter = params.begin(); iter != params.end(); ++iter) {
    tmp_params.insert(
      std::map<std::string, Tensor>::value_type(iter->first, iter->second));
  }
...
static void print_node(
  const Node &node, int layer, bool isinput, int nprint, bool hype = false) {
  std::string name;
  ...
  if (! hype) str2 = convert_html(str2);

  if (str2.length() < 1) { str2 = "&nbsp;&nbsp;"; }
  g_node_stream
    << name
    << " [label=< <table border=\"0\" ><tr><td><table border=\"0\"><tr><td>";
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
    if (nprint != 2 && inputs [i].bubble().op() == "<const>") { continue; }
    ...
    std::string tmpname = get_bubble_name(
      inputs [i].bubble().name(), inputs [i].bubble().name().c_str());
...
void usage() {
  ...
  ;
  std::cout << "draw net without parameters and const inputs: draw_net *.tsm"
            << std::endl;
  std::cout << "draw net with parameters: draw_net [-p] *.tsm" << std::endl;
  std::cout << "draw net with parameters and const inputs: draw_net [-a] *.tsm"
            << std::endl;
...
int main(int argc, char **argv) {
...

  {
    ...
    for (size_t i = 0; i < m->inputs().size(); ++i) {
      ...
      if (input->has("#shape")) {
        ...
        oss << ", "
            << "<font color=\"blue\">" << type_str(dtype)
            << to_string(input->get_int_list("#shape")) << "</font>";
      }
      oss << "<br/>";
    }
  ...
  for (auto &node: m->outputs()) {
    std::string tmpname = get_bubble_name(
      node.bubble().name(), node.bubble().name().c_str());
    if (g_set.find(tmpname) == g_set.end()) {
      g_set.insert(tmpname);
  ...
  for (auto &node: m->inputs()) { print_node(node, 1, true, nprint); }
