...
int main(int argc, const char *argv []) {
  ...
  if (argc > 3) { id = int(std::strtol(argv [3], nullptr, 10)); }
  ...
  for (auto &subdir: subdirs) {
    ...
    TestCase tc;
    try {
      if (! tc.load(case_root, case_filenames)) { continue; }
    } catch (const Exception &e) { std::cerr << e.what() << std::endl; }
    // try infer
    ...
    switch (status) {
      case TestCase::Status::OK: {
        ok_count++;
        std::cout << "[OK]: " << subdir << " on " << computing_device
                  << std::endl;
        // std::cout << tc.log();
        break;
      }
      case TestCase::Status::SKIP: {
        skip_count++;
        std::cout << "[SKIP]: " << subdir << " on " << computing_device
                  << std::endl;
        // std::cout << tc.log();
        break;
      }
      case TestCase::Status::WARNING: {
        ...
        std::cout << "---------------------------------------------------------"
                     "----------------"
                  << std::endl;
        std::cout << "[WARNING]: " << subdir << " on " << computing_device
                  << std::endl;
        std::cout << tc.log();
        std::cout << "---------------------------------------------------------"
                     "----------------"
      ...
      case TestCase::Status::FAILED: {
        ...
        std::cout << "---------------------------------------------------------"
                     "----------------"
                  << std::endl;
        std::cout << "[FAILED]: " << subdir << " on " << computing_device
                  << std::endl;
        std::cout << tc.log();
        std::cout << "---------------------------------------------------------"
                     "----------------"
  ...
  TS_LOG_INFO << "[OK]: " << ok_count << ", [SKIP]: " << skip_count
